...
#include "hu_stuff.h" // ZarroTsu
#include "i_system.h" // ZarroTsu 
...
static void P_DoSpring(mobj_t *spring, mobj_t *object)
{
	...
	if (object->player && object->player->spectator)
		return;
#ifdef RIDERS
	if (riderslevel && object->player && object->player->charexgear != 1)
		return;
#endif
...
static boolean PIT_CheckThing(mobj_t *thing)
{
	...
	//|| tmthing->type == MT_TBLACKSHELLSHIELD || tmthing->type == MT_TBLACKSHELLSHIELD2 || tmthing->type == MT_TBLACKSHELLSHIELD3)
	{
		...
		if (thing->type == MT_BANANAITEM && (tmthing->z > thing->z + thing->height*4))
			return true; // Banana height fix

		if (tmthing->target && tmthing->target->type == thing->type && thing == tmthing->target)
			return true;

		if (!(thing->type == MT_SHELLITEM || thing->type == MT_EXPLOSION || thing->type == MT_REDSHELLITEM
		|| thing->type == MT_REDSHELLITEM2 || thing->type == MT_BOMBSHIELD || tmthing->type == MT_BOMBITEM
		|| thing->type == MT_BANANAITEM || thing->type == MT_BOMBSHIELD2 || thing->type == MT_BOMBSHIELD3 
		|| thing->type == MT_BLACKSHELLSHIELD
		|| thing->type == MT_TBLACKSHELLSHIELD || thing->type == MT_TBLACKSHELLSHIELD2 || thing->type == MT_TBLACKSHELLSHIELD3

		|| thing->type == MT_BANANASHIELD || thing->type == MT_BANANASHIELD2 || thing->type == MT_BANANASHIELD3
		//|| thing->type == MT_REDSHELLSHIELD || thing->type == MT_BLACKSHELLSHIELD || thing->type == MT_SHELLSHIELD
		...
		|| thing->type == MT_PLAYER	|| thing->type == MT_BANANAITEM  || (thing->type == MT_BLACKSHELLITEM && thing->health > 0)))
			return false;

		if (thing->type == MT_PLAYER)
			P_DamageMobj(thing, tmthing, tmthing->target, 1);
		else if (thing->type == MT_SHELLITEM || thing->type == MT_REDSHELLITEM
		...
		|| thing->type == MT_REDSHELLITEM2 || thing->type == MT_BANANAITEM)
		{
			S_StartSound(thing, thing->info->deathsound);
			P_SetMobjState(thing, S_DISS);
		}
		else if (thing->type == MT_BLACKSHELLITEM && thing->health > 0)
		{
			P_SetMobjState(thing, S_RINGEXPLODE);
			S_StartSound(tmthing, tmthing->info->deathsound);
			P_SetMobjState(tmthing, S_DISS);
		}
		if (thing != tmthing->target)
			tmthing->threshold = 1;
	}
	if (thing->type == MT_SHELLSHIELD || thing->type == MT_REDSHELLSHIELD
	|| thing->type == MT_TSHELLSHIELD || thing->type == MT_TSHELLSHIELD2 || thing->type == MT_TSHELLSHIELD3
	|| thing->type == MT_TREDSHELLSHIELD || thing->type == MT_TREDSHELLSHIELD2 || thing->type == MT_TREDSHELLSHIELD3)
	//|| thing->type == MT_TBLACKSHELLSHIELD || thing->type == MT_TBLACKSHELLSHIELD2 || thing->type == MT_TBLACKSHELLSHIELD3)
	{
		...
		if (thing->z + thing->height < tmthing->z)
			return true; // underneath
		if (tmthing->type == MT_BANANAITEM && (thing->z > tmthing->z + tmthing->height*4))
			return true; // Banana height fix

		if (thing->target && thing->target->type == tmthing->type && tmthing == thing->target)
			return true;

		if (!(tmthing->type == MT_SHELLITEM || tmthing->type == MT_EXPLOSION || tmthing->type == MT_REDSHELLITEM
		|| tmthing->type == MT_REDSHELLITEM2 || tmthing->type == MT_BOMBSHIELD || tmthing->type == MT_BOMBITEM
		|| tmthing->type == MT_BANANAITEM || tmthing->type == MT_BOMBSHIELD2 || tmthing->type == MT_BOMBSHIELD3 
		|| tmthing->type == MT_BLACKSHELLSHIELD
		|| tmthing->type == MT_TBLACKSHELLSHIELD || tmthing->type == MT_TBLACKSHELLSHIELD2 || tmthing->type == MT_TBLACKSHELLSHIELD3

		|| tmthing->type == MT_BANANASHIELD || tmthing->type == MT_BANANASHIELD2 || tmthing->type == MT_BANANASHIELD3
		//|| tmthing->type == MT_REDSHELLSHIELD || tmthing->type == MT_BLACKSHELLSHIELD || tmthing->type == MT_SHELLSHIELD
		//|| tmthing->type == MT_TSHELLSHIELD || tmthing->type == MT_TSHELLSHIELD2 || tmthing->type == MT_TSHELLSHIELD3
		//|| tmthing->type == MT_TREDSHELLSHIELD || tmthing->type == MT_TREDSHELLSHIELD2 || tmthing->type == MT_TREDSHELLSHIELD3

		|| tmthing->type == MT_PLAYER || tmthing->type == MT_BANANAITEM || (tmthing->type == MT_BLACKSHELLITEM && tmthing->health > 0)))
			return false;

		if (tmthing->type == MT_PLAYER)
			P_DamageMobj(tmthing, thing, thing->target, 1);
		else if (tmthing->type == MT_SHELLITEM || tmthing->type == MT_REDSHELLITEM
		...
		|| tmthing->type == MT_REDSHELLITEM2 || tmthing->type == MT_BANANAITEM)
		{
			S_StartSound(tmthing, tmthing->info->deathsound);
			P_SetMobjState(tmthing, S_DISS);
		}
		else if (tmthing->type == MT_BLACKSHELLITEM && tmthing->health > 0)
		{
			P_SetMobjState(tmthing, S_RINGEXPLODE);
			S_StartSound(thing, thing->info->deathsound);
			P_SetMobjState(thing, S_DISS);
		}
		if (tmthing != thing->target)
			thing->threshold = 1;
	}
	if (tmthing->type == MT_SHELLITEM || tmthing->type == MT_REDSHELLITEM || tmthing->type == MT_REDSHELLITEM2)
	{
		...
		if (tmthing->z + tmthing->height < thing->z)
			return true; // underneath
		// Don't hurt your owner.
		if (tmthing->target && tmthing->target->type == thing->type && thing == tmthing->target)
			return true;

		if (thing->type == MT_SPINFIRE)
			return false;

		if (thing->type == MT_SHELLITEM || thing->type == MT_REDSHELLITEM
		|| thing->type == MT_REDSHELLITEM2 || thing->type == MT_BOMBITEM
		|| thing->type == MT_BANANASHIELD || thing->type == MT_BANANASHIELD2 || thing->type == MT_BANANASHIELD3
		|| thing->type == MT_BOMBSHIELD || thing->type == MT_BOMBSHIELD2 || thing->type == MT_BOMBSHIELD3
		|| thing->type == MT_REDSHELLSHIELD || thing->type == MT_BLACKSHELLSHIELD || thing->type == MT_SHELLSHIELD
		|| thing->type == MT_TBLACKSHELLSHIELD || thing->type == MT_TBLACKSHELLSHIELD2 || thing->type == MT_TBLACKSHELLSHIELD3
		|| thing->type == MT_TSHELLSHIELD || thing->type == MT_TSHELLSHIELD2 || thing->type == MT_TSHELLSHIELD3
		|| thing->type == MT_TREDSHELLSHIELD || thing->type == MT_TREDSHELLSHIELD2 || thing->type == MT_TREDSHELLSHIELD3
		...
		if (thing->type == MT_BLACKSHELLITEM && thing->health > 0)
		{
			P_SetMobjState(thing, S_RINGEXPLODE);
			S_StartSound(tmthing, tmthing->info->deathsound);
			P_SetMobjState(tmthing, S_DISS);
	...
	if (thing->type == MT_SHELLITEM || thing->type == MT_REDSHELLITEM || thing->type == MT_REDSHELLITEM2)
	{
		// see if it went over / under
		if (thing->z > tmthing->z + tmthing->height)
			return true; // overhead
		...
		if (tmthing->type == MT_SPINFIRE)
			return false;

		if (tmthing->type == MT_SHELLITEM || tmthing->type == MT_REDSHELLITEM
		|| tmthing->type == MT_REDSHELLITEM2 || tmthing->type == MT_BOMBITEM
		|| tmthing->type == MT_BANANASHIELD || tmthing->type == MT_BANANASHIELD2 || tmthing->type == MT_BANANASHIELD3
		|| tmthing->type == MT_BOMBSHIELD || tmthing->type == MT_BOMBSHIELD2 || tmthing->type == MT_BOMBSHIELD3
		|| tmthing->type == MT_REDSHELLSHIELD || tmthing->type == MT_BLACKSHELLSHIELD || tmthing->type == MT_SHELLSHIELD
		|| tmthing->type == MT_TBLACKSHELLSHIELD || tmthing->type == MT_TBLACKSHELLSHIELD2 || tmthing->type == MT_TBLACKSHELLSHIELD3
		|| tmthing->type == MT_TSHELLSHIELD || tmthing->type == MT_TSHELLSHIELD2 || tmthing->type == MT_TSHELLSHIELD3
		|| tmthing->type == MT_TREDSHELLSHIELD || tmthing->type == MT_TREDSHELLSHIELD2 || tmthing->type == MT_TREDSHELLSHIELD3
		|| tmthing->type == MT_BANANAITEM)
		{
			P_SetMobjState(thing, S_DISS);
			P_SetMobjState(tmthing, S_DISS);
		}
		if (tmthing->type == MT_BOMBSHIELD)
			P_SetMobjState(thing, S_DISS);
		if (tmthing->type == MT_PLAYER)
		{
			P_DamageMobj(tmthing, thing, thing->target, 1);
		...
		if (tmthing->type == MT_BLACKSHELLITEM && tmthing->health > 0)
		{
			P_SetMobjState(tmthing, S_RINGEXPLODE);
			S_StartSound(thing, thing->info->deathsound);
			P_SetMobjState(thing, S_DISS);
	...
	if (tmthing->type == MT_KITCHENSINK)
	{
		// see if it went over / under
		if (tmthing->z > thing->z + thing->height)
			return true; // overhead
	...
	if (tmthing->type == MT_EXPLOSION)
	{
		...
		if (thing->type == MT_PLAYER && !thing->player->powers[pw_introcam]
		&& !thing->player->powers[pw_invulnerability] && thing->player->powers[pw_shrink] >= 0)
		{
			if (thing->player->powers[pw_jumpshield] || thing->player->powers[pw_zringshield]
			|| thing->player->powers[pw_watershield] || thing->player->powers[pw_bombshield]
			|| thing->player->powers[pw_forceshield] == 1)
			{
				thing->player->powers[pw_bananacam] = 175;
				thing->player->powers[pw_introcam] = 3;

				S_StartSound (thing->player->mo, sfx_shldls);
				thing->player->powers[pw_jumpshield] = 0;
				thing->player->powers[pw_zringshield] = 0;
				thing->player->powers[pw_watershield] = 0;
				thing->player->powers[pw_bombshield] = 0;
				thing->player->powers[pw_forceshield] = 0;
				thing->player->powers[pw_psychic] = 48;
			}
			else if (thing->player->powers[pw_forceshield] == 2)
			{
				S_StartSound(thing->player->mo, sfx_s3k_14);
				thing->player->powers[pw_forceshield] = 1;
			}
			else
			{
				thing->player->powers[pw_bananacam] = 275;
				thing->player->powers[pw_introcam] = 3;
			}
		}
	}
	if (thing->type == MT_EXPLOSION)
	{
		// see if it went over / under
		if (thing->z > tmthing->z + tmthing->height)
			return true; // overhead
		...
		if (tmthing->type == MT_PLAYER && !tmthing->player->powers[pw_introcam]
		&& !tmthing->player->powers[pw_invulnerability] && tmthing->player->powers[pw_shrink] >= 0)
		{
			if (tmthing->player->powers[pw_jumpshield] || tmthing->player->powers[pw_zringshield]
			|| tmthing->player->powers[pw_watershield] || tmthing->player->powers[pw_bombshield]
			|| tmthing->player->powers[pw_forceshield] == 1)
			{
				tmthing->player->powers[pw_bananacam] = 175;
				tmthing->player->powers[pw_introcam] = 3;

				S_StartSound (tmthing->player->mo, sfx_shldls);
				tmthing->player->powers[pw_jumpshield] = 0;
				tmthing->player->powers[pw_zringshield] = 0;
				tmthing->player->powers[pw_watershield] = 0;
				tmthing->player->powers[pw_bombshield] = 0;
				tmthing->player->powers[pw_forceshield] = 0;
				tmthing->player->powers[pw_psychic] = 48;
			}
			else if (tmthing->player->powers[pw_forceshield] == 2)
			{
				S_StartSound(tmthing->player->mo, sfx_s3k_14);
				tmthing->player->powers[pw_forceshield] = 1;
			}
			else
			{
				tmthing->player->powers[pw_bananacam] = 275;
				tmthing->player->powers[pw_introcam] = 3;
			}
		}
	}

	if (tmthing->type == MT_FIREBALL)
	{
		// see if it went over / under
		if (tmthing->z > thing->z + thing->height)
			return true; // overhead
		...
		if (!(thing->type == MT_PLAYER || thing->type == MT_QUESTIONBOX2))
			return false;
	...
	if (thing->type == MT_FIREBALL)
	{
		...
		if (!(tmthing->type == MT_PLAYER || tmthing->type == MT_QUESTIONBOX2))
			return false;
	...
	if (tmthing->type == MT_BANANASHIELD || tmthing->type == MT_BANANASHIELD2 || tmthing->type == MT_BANANASHIELD3)
	{
		// see if it went over / under
		if (tmthing->z > thing->z + thing->height)
			return true; // overhead
		...
		if (!(thing->type == MT_SHELLITEM || thing->type == MT_EXPLOSION || thing->type == MT_REDSHELLITEM
		|| thing->type == MT_REDSHELLITEM2 || thing->type == MT_BOMBSHIELD || thing->type == MT_BOMBITEM
		|| thing->type == MT_BANANAITEM || thing->type == MT_BOMBSHIELD2 || thing->type == MT_BOMBSHIELD3 
		|| thing->type == MT_BLACKSHELLSHIELD
		|| thing->type == MT_TBLACKSHELLSHIELD || thing->type == MT_TBLACKSHELLSHIELD2 || thing->type == MT_TBLACKSHELLSHIELD3
		|| thing->type == MT_TSHELLSHIELD || thing->type == MT_TSHELLSHIELD2 || thing->type == MT_TSHELLSHIELD3
		...
		&& !thing->player->powers[pw_invulnerability] && thing->player->powers[pw_shrink] >= 0)
		{
			//if (((thing->player->turning == 1 || thing->player->turning == 0) && !thing->player->powers[pw_drift])
			//	|| thing->player->powers[pw_drift] >= 35791394)
			//{
			//	thing->player->powers[pw_bananacam] = -5;
			//	thing->player->powers[pw_introcam] = 2;
			//}
			//if ((thing->player->turning == -1 && !thing->player->powers[pw_drift])
			//	|| thing->player->powers[pw_drift] <= -35791394)
			//{
				if (thing->player->powers[pw_jumpshield] || thing->player->powers[pw_zringshield]
				 || thing->player->powers[pw_watershield] || thing->player->powers[pw_bombshield]
		...
		|| thing->type == MT_REDSHELLITEM2)
			P_SetMobjState(thing, S_DISS);
		if (thing != tmthing->target)
			tmthing->threshold = 1;
	}
	if (thing->type == MT_BANANASHIELD || thing->type == MT_BANANASHIELD2 || thing->type == MT_BANANASHIELD3)
	{
		// see if it went over / under
		if (thing->z > tmthing->z + tmthing->height)
			return true; // overhead
		...
		&& !tmthing->player->powers[pw_invulnerability] && tmthing->player->powers[pw_shrink] >= 0)
		{
			//if (((tmthing->player->turning == 1 || tmthing->player->turning == 0) && !tmthing->player->powers[pw_drift])
			//	|| tmthing->player->powers[pw_drift] >= 35791394)
			//{
			//	tmthing->player->powers[pw_bananacam] = -5;
			//	tmthing->player->powers[pw_introcam] = 2;
			//}
			//if ((tmthing->player->turning == -1 && !tmthing->player->powers[pw_drift])
			//	|| tmthing->player->powers[pw_drift] <= -35791394)
			//{
				if (tmthing->player->powers[pw_jumpshield] || tmthing->player->powers[pw_zringshield]
				 || tmthing->player->powers[pw_watershield] || tmthing->player->powers[pw_bombshield]
		...
		|| tmthing->type == MT_REDSHELLITEM2)
			P_SetMobjState(tmthing, S_DISS);
		if (tmthing != thing->target)
			thing->threshold = 1;
	}	
	if (tmthing->type == MT_BANANAITEM)
	{
		// see if it went over / under
		if (tmthing->z > thing->z + thing->height)
			return true; // overhead
		if (tmthing->z + tmthing->height*4 < thing->z)
			return true; // underneath
	...
	if (thing->type == MT_BANANAITEM)
	{
		// see if it went over / under
		if (thing->z > tmthing->z + tmthing->height)
			return true; // overhead
		if (thing->z + thing->height*4 < tmthing->z)
			return true; // underneath
	...
	if (tmthing->type == MT_BOMBSHIELD || tmthing->type == MT_BOMBSHIELD2 || tmthing->type == MT_BOMBSHIELD3
	 || tmthing->type == MT_TBLACKSHELLSHIELD || tmthing->type == MT_TBLACKSHELLSHIELD2 || tmthing->type == MT_TBLACKSHELLSHIELD3
	 || tmthing->type == MT_BLACKSHELLSHIELD)
	{
		// see if it went over / under
	...
	if (thing->type == MT_BOMBSHIELD || thing->type == MT_BOMBSHIELD2 || thing->type == MT_BOMBSHIELD3
	 || thing->type == MT_TBLACKSHELLSHIELD || thing->type == MT_TBLACKSHELLSHIELD2 || thing->type == MT_TBLACKSHELLSHIELD3
	 || thing->type == MT_BLACKSHELLSHIELD)
	{
		// see if it went over / under
...
#ifdef RIDERS
	if (thing->flags & MF_SPECIAL && thing->type != MT_POKEY)
#else
	if (thing->flags & MF_SPECIAL)
#endif
	{
...
#ifdef RIDERS
	if (tmthing->flags & MF_SPECIAL && tmthing->type != MT_POKEY)
#else
	if (tmthing->flags & MF_SPECIAL)
#endif
	{
	...
	if (!(tmthing->flags & MF_SPRING) && tmthing->z <= (thing->z + thing->height + FRACUNIT)
		&& (tmthing->z + tmthing->height) >= thing->z)
	{
		if (thing->flags & MF_SPRING)
		{
			...
			//if (!(tmthing->player) && !(tmthing->flags & MF_SPRING))
			{
		...
		&& !(thing->z + thing->height < tmthing->z || thing->z > tmthing->z + tmthing->height))
	{
			...
			|| (gametype == GT_RACE))
		{
			...
			if (tmthing->player->powers[pw_shrink] < -1 && thing->player->powers[pw_shrink] >= 0)
			{
				thing->player->mo->flags |= MF_NOCLIP;
				if (thing->player->powers[pw_squished] < (100-(63 -(thing->player->accelstart/40)*21)) && !thing->player->powers[pw_invulnerability])
					thing->player->powers[pw_squished] = (210-((thing->player->accelstart/40)*35-105));
			}
			else if (thing->player->powers[pw_shrink] < -1 && tmthing->player->powers[pw_shrink] >= 0)
			{
				tmthing->player->mo->flags |= MF_NOCLIP;
				if (tmthing->player->powers[pw_squished] < (100-(63 -(tmthing->player->accelstart/40)*21)) && !tmthing->player->powers[pw_invulnerability])
					tmthing->player->powers[pw_squished] = (210-((tmthing->player->accelstart/40)*35-105));
			}
#endif
...
#ifdef RIDERS
					|| (tmthing->player->charability == CA_FLY && (tmthing->state == &states[S_PLAY_FLY1] || tmthing->state == &states[S_PLAY_FLY2] || tmthing->state == &states[S_PLAY_FLY3] || tmthing->state == &states[S_PLAY_FLY4])))
#else
...
#endif
			{
					...
					&& thing->momz >= 0))
				{
					if (gametype == GT_MATCH
						...
						|| (gametype == GT_RACE && (!(tmthing->player->pflags & PF_TAGIT) != !(thing->player->pflags & PF_TAGIT)))
...
#ifdef RIDERS
			if (thing->z + thing->height <= tmthing->z + tmthing->height && !(thing->state == &states[S_PLAY_PAIN] || thing->state == &states[S_PLAY_KARTPAIN] || thing->state == &states[S_PLAY_KARTSPAIN] || thing->state == &states[S_PLAY_RPAIN])) // Stuff where da player don't gotta move
#else
...
#ifdef RIDERS
			if (thing->z >= tmthing->z && !(thing->state == &states[S_PLAY_PAIN] || thing->state == &states[S_PLAY_KARTPAIN] || thing->state == &states[S_PLAY_KARTSPAIN] || thing->state == &states[S_PLAY_RPAIN])) // Stuff where da player don't gotta move
#else
	...
	{
#ifdef RIDERS
		if (tmthing->z >= thing->z && !(tmthing->state == &states[S_PLAY_PAIN] || tmthing->state == &states[S_PLAY_KARTPAIN] || tmthing->state == &states[S_PLAY_KARTSPAIN] || tmthing->state == &states[S_PLAY_RPAIN]))
#else
...
#ifdef RIDERS
				&& ((riderslevel || kartmode) || (tmthing->player->pflags & PF_JUMPED) || (tmthing->player->pflags & PF_SPINNING)))
#else
...
#ifdef RIDERS
		// Jump Pad.
		if (thing->player && thing->player->powers[pw_ramp] > 0
		&& thing->momz < 0
		&& (kartmode || (riderslevel && !(thing->player->mo->state >= &states[S_PLAY_JUMP]
			&& thing->player->mo->state <= &states[S_PLAY_ABL8]))))
		{
			thing->momz = thing->player->powers[pw_rampheight2]*FRACUNIT;
			thing->player->mo->angle = thing->player->powers[pw_rampangle];

			if (thing->player == &players[consoleplayer])
				localangle = thing->player->mo->angle;
			else if (splitscreen && thing->player == &players[secondarydisplayplayer])
				localangle2 = thing->player->mo->angle;

			P_InstaThrust(thing->player->mo, thing->player->mo->angle, thing->player->powers[pw_rampspeed2]*FRACUNIT);
			P_ResetPlayer(thing->player);
			if (riderslevel)
			{
				P_SetPlayerMobjState(thing->player->mo, S_PLAY_PLG1);
				thing->player->powers[pw_jmpcharge] = 5;
				thing->player->powers[pw_sounds] = 3;
			}
		}
#endif
...
retry:
	...
	if (mo->player)
	{
		...
		mo->player->powers[pw_mushroom] = 0;
		if (mo->player->powers[pw_exspeed] != 0 || mo->player->powers[pw_drift] != 0)
		{
			mo->player->powers[pw_exspeed] = 0;
			mo->player->powers[pw_drift] = 0;
			mo->player->powers[pw_driftcharge] = 0;
		}
	}
	else
	{
...
#ifdef RIDERS
	if (realcrush && crushchange && !kartmode)
#else
