...
void P_DoSpring(mobj_t* spring, mobj_t* object)
{
	switch(spring->type)
	{
		...
		case MT_REDDIAG: // Red diagonal spring (pointing up from ground)
		    ...
		    P_InstaThrust(object, spring->angle, spring->info->damage);
...
// Snowballs can hit other things Tails 12-12-2001
    ...
    #ifdef SPMOVES
	if(tmthing->target && tmthing->target->player && cv_gametype.value == GT_COOP && !cv_ringslinger.value && thing->player)
		return false;
	#endif
	...
	{ // Push thing! Tails 02-02-2002
		...
		#ifdef SPMOVES
		if (mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE && tmthing->player
			&& !tmthing->player->helditem && !tmthing->player->ride && !tmthing->player->lightdashallowed && !tmthing->player->lightdashdown)
			{
			tmthing->player->pickupitem = thing;
			//tmthing->player->lightdashdown = true;
			}
		#endif
	}

	#ifdef SPMOVES
	if(thing->flags2 & MF2_STICKY && tmthing->z >= thing->z + thing->height - FRACUNIT && tmthing->z <= thing->z + thing->height + FRACUNIT && tmthing->player && !tmthing->player->ride)
	    tmthing->player->ride = thing;
	else if(tmthing->flags2 & MF2_STICKY && thing->z >= tmthing->z + tmthing->height - FRACUNIT && thing->z <= tmthing->z + tmthing->height + FRACUNIT && thing->player && !thing->player->ride)
	    thing->player->ride = tmthing;
    #endif

	if(tmthing->type == MT_SPIKEBALL)
		P_TouchSpecialThing(tmthing, thing, true);
	else if(thing->type == MT_SPIKEBALL)
    ...
    if (thing->flags & MF_SPECIAL)
    {
        ...
        if (tmthing->player || tmthing->flags2 & MF2_CHAOATTACK)
        {
            // can remove thing
            P_TouchSpecialThing (thing, tmthing, true);
        }
    ...
    if(tmthing->flags & MF_SPECIAL)
    {
        ...
        if (thing->player || thing->flags2 & MF2_CHAOATTACK)
        {
            // can remove thing
            P_TouchSpecialThing (tmthing, thing, true);
        }
	...
	if(cv_tailspickup.value && (tmthing->player && thing->player) && !(mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE))
	{
	...
	if(thing->player)
	{
			...
			&& thing->z == thing->floorz)
		{
			...
			{
				if(thing != tmthing->target && (tmthing->momx != 0 || tmthing->momy != 0) && !(tmthing->flags & MF_ENEMY || tmthing->flags & MF_BOSS))
		...
		#ifdef SPMOVES
		// Adventure battle system thing.
		if(cv_gametype.value == GT_MATCH && tmthing->player && mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE && (((thing->z <= tmthing->z + tmthing->height) && (thing->z + thing->height >= tmthing->z)) || (tmthing->z == thing->z + thing->height + FRACUNIT)))
		{
			if(!(tmthing->player->powers[pw_flashing] || tmthing->player->mfjumped || tmthing->player->powers[pw_invulnerability] || tmthing->player->mfspinning)
			&& (thing->player->mfjumped || thing->player->mfspinning || thing->player->powers[pw_invulnerability]))
			{
				P_DamageMobj(tmthing, thing, thing, 1);
				if(thing->player && thing->player->mfjumped && thing->momz < 0)
					thing->momz = -thing->momz;
			}
			if(!(thing->player->powers[pw_flashing] || thing->player->mfjumped || thing->player->powers[pw_invulnerability] || thing->player->mfspinning)
			&& (tmthing->player->mfjumped || tmthing->player->mfspinning || tmthing->player->powers[pw_invulnerability]))
			{
				P_DamageMobj(thing, tmthing, tmthing, 1);
				if(tmthing->player && tmthing->player->mfjumped && tmthing->momz < 0)
					tmthing->momz = -tmthing->momz;
			}
		}
		#endif
		...
		if(((thing->z <= tmthing->z + tmthing->height) && (thing->z + thing->height >= tmthing->z)) || (tmthing->z == thing->z + thing->height + FRACUNIT)) // Are you touching the side of it?
		{
			if((tmthing->player->mfjumped == 1) || (tmthing->player->mfspinning == 1) || (tmthing->player->powers[pw_invulnerability]) || (tmthing->player->powers[pw_super])) // Do you possess the ability to subdue the object?
			{
				if(thing->flags & MF_MONITOR)
				{
					...
					else*/ // *coughredundantstupiditycough* JTE
					{
						if(tmthing->momz < 0)
							tmthing->momz = -tmthing->momz;
						P_DamageMobj(thing, tmthing, tmthing, 1);
                    ...
                    boolean       allowdropoff)
{
...
#endif
    {
		...
		if(thing->player && ((thing->player->specialsector == 992
		  || R_PointInSubsector (x,y)->sector->special == 992)
		  || (thing->player->specialsector == 960
		  || R_PointInSubsector (x,y)->sector->special == 960))
...
boolean P_IsClimbingValid(player_t* player)
{
	...
	if(glidesector->sector != player->mo->subsector->sector)
	{
		...
		if(floorclimb == false && glidesector->sector->floorheight < player->mo->z + 16*FRACUNIT
  ...
  isblocking:
	{
		...
		{
		    ticcmd_t* cmd = &slidemo->player->cmd;
			if((slidemo->player->gliding && slidemo->player->charability == 2) || slidemo->player->climbing
			   #ifdef SPMOVES
               || (slidemo->player->charability == 6 && slidemo->player->gliding && slidemo->player->thokked)
			   || (slidemo->player->charability == 8 && slidemo->player->mfjumped)
			   || (slidemo->player->charability == 12 && slidemo->player->mfjumped && (slidemo->player->rpLevel >= 3 && slidemo->player->rpLevel < 5))
			   || (slidemo->player->charability == 12 && slidemo->player->gliding && !slidemo->player->thokked)
               || (slidemo->player->charability == 15 && slidemo->player->mfjumped /*&& cmd->buttons & BT_JUMP && !slidemo->player->jumpdown*/)
               #endif
               )
			{
				angle_t climbline;
...
boolean PTR_ShootTraverse (intercept_t* in)
{
	...
	|| li->frontsector->ceilingpic == sky3flatnum
	|| li->frontsector->ceilingpic == sky4flatnum
	|| li->frontsector->ceilingpic == sky5flatnum
	|| li->frontsector->ceilingpic == sky6flatnum
	|| li->frontsector->ceilingpic == sky7flatnum
	|| li->frontsector->ceilingpic == sky8flatnum
	|| li->frontsector->ceilingpic == sky9flatnum)
        {
            // don't shoot the sky!
		 ...
		 || li->backsector->ceilingpic == sky3flatnum
		 || li->backsector->ceilingpic == sky4flatnum
		 || li->backsector->ceilingpic == sky5flatnum
		 || li->backsector->ceilingpic == sky6flatnum
		 || li->backsector->ceilingpic == sky7flatnum
		 || li->backsector->ceilingpic == sky8flatnum
		 || li->backsector->ceilingpic == sky9flatnum)))
              return false;
        }
