...
#ifdef SRB2PLUS // tailspickup and solid players at the same time, you can also pickup opponents in match/tag/ctf/race
    if (tmthing->player && thing->player)
    {
                ...
                || (tmthing->player->charability == CA_FLY && (tmthing->state == &states[S_PLAY_SPC1] || tmthing->state == &states[S_PLAY_SPC2] || tmthing->state == &states[S_PLAY_SPC3] || tmthing->state == &states[S_PLAY_SPC4])))
        {
            if (thing->player->pflags & PF_NIGHTSMODE)
                ;
            else if ((thing->eflags & MFE_VERTICALFLIP) && !(tmthing->eflags & MFE_VERTICALFLIP))
                ;
            else if (!(thing->eflags & MFE_VERTICALFLIP) && (tmthing->eflags & MFE_VERTICALFLIP))
                ;
            else if ((!(tmthing->eflags & MFE_VERTICALFLIP) && (tmthing->z <= thing->z + thing->height + FRACUNIT)
                ...
                && thing->momz >= 0))
            {
                if (netgame && (tmthing->player->spectator || thing->player->spectator))
...
#ifndef SRB2PLUS // remove autoaim
