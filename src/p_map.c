...
#if defined(JTEBOTS) && defined(SUCKYBOTWAYPOINTS)
	if(gametype == GT_RACE)
		JB_CreateWaypoint(spring->x, spring->y, spring->z, true);
#endif
...
#ifdef SCALEGRAVITY
	// Scale spring power too?
	if(object->scale != 100)
		object->momz = FIXEDSCALE(object->momz,object->scale);
#endif
...
#ifdef UNIVERSE
	if(tmthing->type == MT_BARREL
	&& tmthing->z + tmthing->height == thing->z - 1*FRACUNIT)
	{
		if(tmthing->movecount == 0)
		{
			tmthing->movecount = 1;
		...
		else if(tmthing->movecount == 1)
			;
		else
		{
			// Todo: Damage for jumping into barrel full of slime/lava/whatever
		}
	}
#endif
...
#ifdef JTEFLYCHAINS
					|| tmthing->player->carried) // If Tails is flying or carried...
					&& (!tmthing->player->carrying || tmthing->target == thing) // And Tails is not carrying anything or they're carrying you...
					&& !thing->player->carried // And you're not being carried, so they're not stealing you...
#else
					)
#endif
					)
...
#ifdef JTEBOTS
					// Bots can't fly you if you don't want them to.
					|| (tmthing->player->bot && !cv_botcanflyme.value)
#endif
					)
					{
						thing->player->carried = false;
#if defined(JTEFLYCHAINS) || defined(JTEBOTS)
						thing->player->carrying = false;
#endif
					}
					else
					{
...
#ifdef SPMOVES
				|| tmthing->player->powers[pw_amydash]
#endif
#ifdef UNIVERSE
				|| tmthing->player->powers[pw_sword]
#endif
				))
			{
	...
	if (newsubsec->sector->ffloors)
	{
		...
		for (rover = newsubsec->sector->ffloors; rover; rover = rover->next)
		{
			...
			if (!(rover->flags & FF_SOLID))
			{
				...
				&& !thing->player->homing
#endif
				&& (((thing->player->charflags & SF_RUNONWATER) || thing->player->powers[pw_super]) && thing->ceilingz-*rover->topheight >= thing->height)
...
#ifdef JTEMISC // Noclipping player camera noclips too!!
	if((thiscam == &camera && players[displayplayer].cheats & CF_NOCLIP)
	|| (thiscam == &camera2 && players[secondarydisplayplayer].cheats & CF_NOCLIP))
	{
		floatok = true;
		oldx = thiscam->x;
		oldy = thiscam->y;
...
boolean P_TryMove(mobj_t *thing, fixed_t x, fixed_t y, boolean allowdropoff)
{
	...
	if (!(thing->flags & MF_NOCLIP))
	{
		...
		fixed_t maxstep;
		if(thing->scale != 100)
			maxstep = FIXEDSCALE(MAXSTEPMOVE, thing->scale);
		else
			maxstep = MAXSTEPMOVE;
#else
