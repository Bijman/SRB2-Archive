...
#include "JTE/SPM_Command.h" // SPMOVES
#include "JTE/SR_Info.h" // RUSSIA
#include "JTE/JTEMisc.h" // SCALEVAR
...
#ifdef HOLIDAYS
byte holiday = H_NONE, serverholiday = H_NONE, localholiday = H_NONE;
#else
...
#ifdef ZELDA
boolean zelda = false; // Zelda mode
#endif
...
static void D_TimeWad(void)
{
...
#ifdef HOLIDAYS
	if(M_CheckParm("-noholiday") || M_CheckParm("-noxmas"))
		holiday = H_NONE;
	else if(M_CheckParm("-holiday"))
		holiday = atoi(M_GetNextParm());
	else
	{
#endif
	t1 = time(NULL);
	if (t1 != (time_t)-1)
	{
		...
		if (tptr)
		{
			...
			else if (tptr->tm_mon == 9)
				holiday = H_WEEN;
			else if (tptr->tm_mon == 10)
				holiday = H_THXG;
			else if (tptr->tm_mon == 11)
			{
				holiday = H_XMAS;
				if(tptr->tm_mday > 29)
					holiday = H_NEWY;
	...
	localholiday = serverholiday = holiday;
#else
...
void D_SRB2Main(void)
{
	...
	{
#ifndef JTEMISC // -skill and -warp work regardless. They set modifiedgame, no cheating.
		if (modifiedgame || netgame)
#endif
		{
			gameskill = M_GetNextParm()[0]-'0';
			autostart = true;
#ifdef JTEMISC // Set modifiedgame, no cheating.
			if(!netgame)
				modifiedgame = true;
#endif
		}
...
#ifdef DARKWORLD
	DW_Init(); // Init the Dark World mobj
#endif
#ifdef UNIVERSE
	SU_Init(); // Init the SRB2 Universe stuff
#endif
#ifdef SACITY
	SC_Init(); // Init the SA City stuff
#endif
#ifdef RUSSIA
	SR_Init(); // Init the Soviet Russia stuff
#endif
#ifdef SPRITEOFFS
	JTE_InitSpriteOffs(); // Init the sprite offset stuff
#endif
#ifdef HOLIDAYS
	JTE_InitHolidaySprites(); // Because I've no better way to do it yet... >_x
#endif
...
#ifdef HOLIDAYS
	// R_Init() must be called before this.
	JTE_LoadHolidayCfg();
#endif
