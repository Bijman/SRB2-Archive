...
boolean         nomonsters;     // checkparm of -nomonsters // NOT ANYMORE! Now we use this unsuspecting variable for HAXX0R mode!
...
#if defined(__WIN32__) && !defined(R_OK)
...
void IdentifyVersion (void)
{
   ...
   I_LocateWad();
	...
	{
        CONS_Printf("Doom2Wad = %s\n", doom2wad);
		if ( !access (doom2wad,R_OK) )
		{
			D_AddFile (doom2wad);
		}
		else if ( !access ("srb2.srb",R_OK) )
		{
			D_AddFile ("srb2.srb");
		}
		...
		{
			I_Error ("%s not found! Omfg, nooooo!1!!11!1\n", doom2wad); // Tails 12-23-2001
		}
		CONS_Printf("Omg, yay!\n");
	}
...
//    nomonsters = M_CheckParm ("-noenemies");
    ...
    #ifdef SPMOVES
    strcpy (title,"Sonic Robo Blast 2: SPMoves Edition"); // W00t!
    #else
    ...
    #ifdef SPMOVES
    sprintf(legacy,"Sonic Robo Blast 2: SPMoves Edition"); // W00t!
    #else
    strcpy (legacy,"Sonic Robo Blast 2"); // Tails 03-26-2001
    #endif
   ...
   snprintf( pmData->title, sizeof( pmData->title), 
	...
	if( (t1 = time( (time_t *) 0 )) != (time_t) -1 )
	{
		tptr = localtime(&t1);
		#ifdef SPMOVES
		if(!M_CheckParm("-noseasons"))
		{
			if((M_CheckParm ("-season") && M_CheckParm ("spring")) || (!M_CheckParm ("-season") && tptr->tm_mon > 2 && tptr->tm_mon < 6)) // Spring time!
			{
				D_AddFile("SPMoves-Spring.wad");
				springmode = true;
			}
			if((M_CheckParm ("-season") && M_CheckParm ("summer")) || (!M_CheckParm ("-season") && tptr->tm_mon >= 6 && tptr->tm_mon < 8)) // Summer time!
			{
				D_AddFile("SPMoves-Summer.wad");
				summermode = true;
			}
			if((M_CheckParm ("-season") && M_CheckParm ("fall")) || (!M_CheckParm ("-season") && tptr->tm_mon >= 8 && tptr->tm_mon < 11)) // Fall time!
			{
				D_AddFile("SPMoves-Fall.wad");
				fallmode = true;
			}
			if((M_CheckParm ("-season") && M_CheckParm ("winter")) || (!M_CheckParm ("-season") && tptr->tm_mon == 11 || tptr->tm_mon < 3)) // Winter time!
			{
				D_AddFile("SPMoves-Winter.wad");
				xmasmode = true;
				xmasoverride = true;
			}
		}
		if(!M_CheckParm("-noholidays"))
		{
			if((M_CheckParm ("-holiday") && M_CheckParm ("xmas")) || (!M_CheckParm ("-holiday") && tptr->tm_mon < 1 && tptr->tm_mday < 6) || (!M_CheckParm ("-holiday") && tptr->tm_mon == 11 && tptr->tm_mday > 24)) // Christmas to Epiphany
			{
				D_AddFile("SPMoves-Xmas.wad");
				xmasmode = true;
				xmasoverride = true;
			}
			if((M_CheckParm ("-holiday") && M_CheckParm ("halloween")) || (!M_CheckParm ("-holiday") && tptr->tm_mon == 9)) // Halloween for all of october!
			{
				D_AddFile("SPMoves-Halloween.wad");
				fallmode = true;
			}
			if((M_CheckParm ("-holiday") && M_CheckParm ("easter")) || (!M_CheckParm ("-holiday") && tptr->tm_mon == 3)) // Easter changes every year, so just have it for all of April
			{
				D_AddFile("SPMoves-Easter.wad");
				eastermode = true;
			}
		}
		#else
		if((tptr->tm_mon < 1 && tptr->tm_mday < 6) || (tptr->tm_mon == 11 && tptr->tm_mday > 24))
		{
...
// Sudden death setting Tails 11-18-2002
    ...
    nomonsters = M_CheckParm ("-statcopy");
    p = 0;
    if (p && p<myargc-1)
    {
        I_Error("Sorry but statcopy isn't supported at this time\n");
	// I left it in so it shows up in place in the exe...
	//HOWEVER, p is always assigned 0 so it never happens!
	//Can you say "EEEEEEEEEEEVVVVIIIIIIILLLLLLLLLL!!!!!"?
