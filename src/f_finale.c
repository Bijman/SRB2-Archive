...
#ifdef RIDERS
static patch_t *ttmenubg;
static patch_t *ttmenu1;
static patch_t *ttmenu2;
static patch_t *ttmenu3;
static patch_t *ttmenu4;
static patch_t *ttmenu5;
static patch_t *ttmenu6;
static patch_t *ttmenu7;
static patch_t *ttmenu8;
static patch_t *ttmenu9;
static patch_t *ttmenu10;
static patch_t *ttmenu11;
static patch_t *ttmenu12;
static patch_t *ttmenu13;
static patch_t *ttmenu14;
static patch_t *ttmenu15;
static patch_t *ttmenu16;
static patch_t *ttmenu17;
static patch_t *ttmenu18;

static patch_t *ttoptbg;
static patch_t *ttopt1;
static patch_t *ttopt2;
static patch_t *ttopt3;
static patch_t *ttopt4;
static patch_t *ttopt5;
static patch_t *ttopt6;
static patch_t *ttopt7;
static patch_t *ttopt8;
static patch_t *ttopt9;
static patch_t *ttopt10;
static patch_t *ttopt11;
static patch_t *ttopt12;
static patch_t *ttopt13;
static patch_t *ttopt14;
static patch_t *ttopt15;
static patch_t *ttopt16;
static patch_t *ttopt17;
static patch_t *ttopt18;

static void F_SkyScroll(INT32 menu);
#else
...
#ifdef RIDERS
void F_StartMenuScreen(void)
{
	G_SetGamestate(GS_MENUSCREEN);
	CON_ClearHUD();

	// IWAD dependent stuff.

	S_ChangeMusic(mus_rmenu, true);

	//finalecount = 0;
	//finalestage = 0;
	//animtimer = 0;

	ttmenubg = W_CachePatchName("TTMENUBG", PU_LEVEL);
	ttmenu1 = W_CachePatchName("TTMENU1", PU_LEVEL);
	ttmenu2 = W_CachePatchName("TTMENU2", PU_LEVEL);
	ttmenu3 = W_CachePatchName("TTMENU3", PU_LEVEL);
	ttmenu4 = W_CachePatchName("TTMENU4", PU_LEVEL);
	ttmenu5 = W_CachePatchName("TTMENU5", PU_LEVEL);
	ttmenu6 = W_CachePatchName("TTMENU6", PU_LEVEL);
	ttmenu7 = W_CachePatchName("TTMENU7", PU_LEVEL);
	ttmenu8 = W_CachePatchName("TTMENU8", PU_LEVEL);
	ttmenu9 = W_CachePatchName("TTMENU9", PU_LEVEL);
	ttmenu10 = W_CachePatchName("TTMENU10", PU_LEVEL);
	ttmenu11 = W_CachePatchName("TTMENU11", PU_LEVEL);
	ttmenu12 = W_CachePatchName("TTMENU12", PU_LEVEL);
	ttmenu13 = W_CachePatchName("TTMENU13", PU_LEVEL);
	ttmenu14 = W_CachePatchName("TTMENU14", PU_LEVEL);
	ttmenu15 = W_CachePatchName("TTMENU15", PU_LEVEL);
	ttmenu16 = W_CachePatchName("TTMENU16", PU_LEVEL);
	ttmenu17 = W_CachePatchName("TTMENU17", PU_LEVEL);
	ttmenu18 = W_CachePatchName("TTMENU18", PU_LEVEL);
}

void F_StartKartScreen(void)
{
	G_SetGamestate(GS_KARTSCREEN);
	CON_ClearHUD();

	// IWAD dependent stuff.

	S_ChangeMusic(mus_mmenu, true);

	//finalecount = 0;
	//finalestage = 0;
	//animtimer = 0;
}
void F_StartOptionScreen(void)
{
	G_SetGamestate(GS_OPTIONSCREEN);
	CON_ClearHUD();

	// IWAD dependent stuff.

	S_ChangeMusic(mus_opmenu, looptitle);

	//finalecount = 0;
	//finalestage = 0;
	//animtimer = 0;

	ttoptbg = W_CachePatchName("TTOPTBG", PU_LEVEL);
	ttopt1 = W_CachePatchName("TTOPT1", PU_LEVEL);
	ttopt2 = W_CachePatchName("TTOPT2", PU_LEVEL);
	ttopt3 = W_CachePatchName("TTOPT3", PU_LEVEL);
	ttopt4 = W_CachePatchName("TTOPT4", PU_LEVEL);
	ttopt5 = W_CachePatchName("TTOPT5", PU_LEVEL);
	ttopt6 = W_CachePatchName("TTOPT6", PU_LEVEL);
	ttopt7 = W_CachePatchName("TTOPT7", PU_LEVEL);
	ttopt8 = W_CachePatchName("TTOPT8", PU_LEVEL);
	ttopt9 = W_CachePatchName("TTOPT9", PU_LEVEL);
	ttopt10 = W_CachePatchName("TTOPT10", PU_LEVEL);
	ttopt11 = W_CachePatchName("TTOPT11", PU_LEVEL);
	ttopt12 = W_CachePatchName("TTOPT12", PU_LEVEL);
	ttopt13 = W_CachePatchName("TTOPT13", PU_LEVEL);
	ttopt14 = W_CachePatchName("TTOPT14", PU_LEVEL);
	ttopt15 = W_CachePatchName("TTOPT15", PU_LEVEL);
	ttopt16 = W_CachePatchName("TTOPT16", PU_LEVEL);
	ttopt17 = W_CachePatchName("TTOPT17", PU_LEVEL);
	ttopt18 = W_CachePatchName("TTOPT18", PU_LEVEL);
}
#endif
...
#ifdef RIDERS
	INT32 newscrollspeed = 10;

	if (menu == 1)
	{
		pat = W_CachePatchName("TTKART", PU_CACHE);
		animtimer = ((finalecount*((gamestate == GS_INTRO || gamestate == GS_INTRO2) ? newscrollspeed*4 : newscrollspeed))/16) % SHORT(pat->width);
	}
	else
	{
#endif
...
#ifdef RIDERS
void F_MenuScreenDrawer(void)
{

	V_DrawScaledPatch(0, 0, 0, ttmenubg);

	if (finalecount < 57)
	{
		if (finalecount == 1 || finalecount == 2)
			V_DrawScaledPatch(179, 40, 0, ttmenu1);
		else if (finalecount == 3 || finalecount == 4)
			V_DrawScaledPatch(179, 40, 0, ttmenu2);
		else if (finalecount == 5 || finalecount == 6)
			V_DrawScaledPatch(179, 40, 0, ttmenu3);

		else if (finalecount == 7 || finalecount == 8)
			V_DrawScaledPatch(179, 40, 0, ttmenu4);
		else if (finalecount == 9 || finalecount == 10)
			V_DrawScaledPatch(179, 40, 0, ttmenu5);
		else if (finalecount == 11 || finalecount == 12)
			V_DrawScaledPatch(179, 40, 0, ttmenu6);

		else if (finalecount == 13 || finalecount == 14)
			V_DrawScaledPatch(179, 40, 0, ttmenu7);
		else if (finalecount == 15 || finalecount == 16)
			V_DrawScaledPatch(179, 40, 0, ttmenu8);
		else if (finalecount == 17 || finalecount == 18)
			V_DrawScaledPatch(179, 40, 0, ttmenu9);

		else if (finalecount == 19 || finalecount == 20)
			V_DrawScaledPatch(179, 40, 0, ttmenu10);
		else if (finalecount == 21 || finalecount == 22)
			V_DrawScaledPatch(179, 40, 0, ttmenu11);
		else if (finalecount == 23 || finalecount == 24)
			V_DrawScaledPatch(179, 40, 0, ttmenu12);

		else if (finalecount == 25 || finalecount == 26)
			V_DrawScaledPatch(179, 40, 0, ttmenu13);
		else if (finalecount == 27 || finalecount == 28)
			V_DrawScaledPatch(179, 40, 0, ttmenu14);
		else if (finalecount == 29 || finalecount == 30)
			V_DrawScaledPatch(179, 40, 0, ttmenu15);

		else if (finalecount == 31 || finalecount == 32)
			V_DrawScaledPatch(179, 40, 0, ttmenu16);
		else if (finalecount == 33 || finalecount == 34)
			V_DrawScaledPatch(179, 40, 0, ttmenu17);
		else if (finalecount == 35 || finalecount == 36)
			V_DrawScaledPatch(179, 40, 0, ttmenu18);
		else if (finalecount >= 37)
			V_DrawScaledPatch(179, 40, 0, ttmenu1);
...
void F_MarioKartScreenDrawer(void)
{
	// That's it? Yeah, I'm a riot :3
	F_SkyScroll(1);
}

void F_OptionScreenDrawer(void)
{

	V_DrawScaledPatch(0, 0, 0, ttoptbg);

	if (finalecount < 57)
	{
		if (finalecount == 1 || finalecount == 2)
			V_DrawScaledPatch(179, 40, 0, ttopt1);
		else if (finalecount == 3 || finalecount == 4)
			V_DrawScaledPatch(179, 40, 0, ttopt2);
		else if (finalecount == 5 || finalecount == 6)
			V_DrawScaledPatch(179, 40, 0, ttopt3);

		else if (finalecount == 7 || finalecount == 8)
			V_DrawScaledPatch(179, 40, 0, ttopt4);
		else if (finalecount == 9 || finalecount == 10)
			V_DrawScaledPatch(179, 40, 0, ttopt5);
		else if (finalecount == 11 || finalecount == 12)
			V_DrawScaledPatch(179, 40, 0, ttopt6);

		else if (finalecount == 13 || finalecount == 14)
			V_DrawScaledPatch(179, 40, 0, ttopt7);
		else if (finalecount == 15 || finalecount == 16)
			V_DrawScaledPatch(179, 40, 0, ttopt8);
		else if (finalecount == 17 || finalecount == 18)
			V_DrawScaledPatch(179, 40, 0, ttopt9);

		else if (finalecount == 19 || finalecount == 20)
			V_DrawScaledPatch(179, 40, 0, ttopt10);
		else if (finalecount == 21 || finalecount == 22)
			V_DrawScaledPatch(179, 40, 0, ttopt11);
		else if (finalecount == 23 || finalecount == 24)
			V_DrawScaledPatch(179, 40, 0, ttopt12);

		else if (finalecount == 25 || finalecount == 26)
			V_DrawScaledPatch(179, 40, 0, ttopt13);
		else if (finalecount == 27 || finalecount == 28)
			V_DrawScaledPatch(179, 40, 0, ttopt14);
		else if (finalecount == 29 || finalecount == 30)
			V_DrawScaledPatch(179, 40, 0, ttopt15);

		else if (finalecount == 31 || finalecount == 32)
			V_DrawScaledPatch(179, 40, 0, ttopt16);
		else if (finalecount == 33 || finalecount == 34)
			V_DrawScaledPatch(179, 40, 0, ttopt17);
		else if (finalecount == 35 || finalecount == 36)
			V_DrawScaledPatch(179, 40, 0, ttopt18);
		else if (finalecount >= 37)
			V_DrawScaledPatch(179, 40, 0, ttopt1);
