...
#include "JTE/SV_Virus.h" // SUPERVIRUS3
...
#ifdef SRB2JTE
	strcpy(credits[i].header, "SRB2JTE\n");
	strcpy(credits[i].fakenames[0], "A kickass mod by\n");
	strcpy(credits[i].fakenames[1], "Jason the Echidna\n");
	strcpy(credits[i].realnames[0], "A kickass mod by\n");
	strcpy(credits[i].realnames[1], "John J. Muniz\n");
	credits[i].numnames = 2;
	i++;

	strcpy(credits[i].header, "Featuring ideas from\n");
	strcpy(credits[i].fakenames[0], "Jason the Echidna\n");
	strcpy(credits[i].fakenames[1], "Fox Unit\n");
	strcpy(credits[i].fakenames[2], "Blaze Hedgehog\n");
	strcpy(credits[i].realnames[0], "John J. Muniz\n");
	strcpy(credits[i].realnames[1], "Alex Strout\n");
	strcpy(credits[i].realnames[2], "Ryan Bloom\n");
	credits[i].numnames = 3;
	i++;

	strcpy(credits[i].header, "Extra thanks to\n");
	strcpy(credits[i].fakenames[0], "Kroze\n");
	strcpy(credits[i].fakenames[1], "Alam_GBC\n");
	strcpy(credits[i].realnames[0], "That sexy dude\n");
	strcpy(credits[i].realnames[1], "Alam Arias\n");
	credits[i].numnames = 2;
	i++;
#endif
}
...
#ifdef SUPERVIRUS3
	// I can't use P_Random() or M_Random() here, it seems...
	// Next best thing, eh? We're offline now, so it dun matter...
	if(!(totalplaytime%16))
	{
		Vir_StartIntro();
		return;
...
#ifdef HOLIDAYS
	const char *holidaystr = NULL;
	if(holidaynicks[holiday])
		holidaystr = va("%sSKY",holidaynicks[holiday]);
	if(!holidaystr || W_CheckNumForName(holidaystr) == -1)
		holidaystr = "TITLESKY";
	pat = W_CachePatchName(holidaystr, PU_CACHE);
#else
