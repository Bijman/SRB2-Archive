...
// Initalize the credits table
	...
	i++;
	#ifdef SPMOVES
	strcpy(credits[i].header, "SPMoves staff:\n");
	strcpy(credits[i].fakenames[0], "Jason the Echidna: Project Founder, Programmer\n");
	strcpy(credits[i].fakenames[1], "OmegaHedgehog: Lead Producer, Animator\n");
	strcpy(credits[i].fakenames[2], "Mecha Knuckles: Suggestions and Testing\n");
	strcpy(credits[i].fakenames[3], "Shadow: Misc\n");
	strcpy(credits[i].realnames[0], "John Muniz: Project Founder, Programmer\n");
	strcpy(credits[i].realnames[1], "Mike Meredith: Lead Producer, Animator\n");
	strcpy(credits[i].realnames[2], "Dan P.: Suggestions and Testing\n");
	strcpy(credits[i].realnames[3], "Shadow: Misc\n");
	credits[i].numnames = 4;
	i++;
	#endif
}
...
void F_IntroTextWrite (void)
{
	...
	#ifdef SPMOVES
	|| (finaletext == JTETEXT && !nomonsters)
	#endif
	)
		nobg = true;
	#ifdef SPMOVES
	else if(finaletext == JTETEXT)
	{
		background = W_CachePatchName ("EVILLOGO", PU_CACHE);
		//highres = true;
	}
	#endif
	...
	#ifdef SPMOVES
	if(finaletext == JTETEXT)
	{
	    if(nomonsters)
	    {
	         V_DrawCreditString(160-(V_CreditStringWidth("SPMOVES")/2), 0, 0, "SPMOVES");
	         V_DrawCreditString(160-(V_CreditStringWidth("EVIL EDITION!")/2), 16, 0, "EVIL EDITION!");
	         V_DrawCreditString(160-(V_CreditStringWidth("CREATED BY:")/2), 168, 0, "CREATED BY:");
	         V_DrawCreditString(160-(V_CreditStringWidth("JASON THE ECHIDNA")/2), 184, 0, "JASON THE ECHIDNA");
        }
	    else
	    {
	         V_DrawCreditString(160-(V_CreditStringWidth("JASON THE ECHIDNA")/2), 80, 0, "JASON THE ECHIDNA");
	         V_DrawCreditString(160-(V_CreditStringWidth("ALONG WITH")/2), 96, 0, "ALONG WITH");
		}
	}
	else
	#endif
	if(finaletext == E0TEXT)
	{
		...
		#ifdef SPMOVES
		V_DrawCreditString(160-(V_CreditStringWidth("PRESENT")/2), 96, 0, "PRESENT");
		#else
	...
	else if (finaletext == C6TEXT)
	{
		...
		else
		{
			if(timetonext == 7)
			{
				...
				#ifdef SPMOVES
				F_Scroll(16, 20, W_CachePatchName ("TTLSKY2", PU_CACHE), W_CachePatchName ("TTLSKY2", PU_CACHE), 1);
				#else
			...
			{
			    #ifdef SPMOVES
				F_Scroll(16, 20, W_CachePatchName ("TTLSKY2", PU_CACHE), W_CachePatchName ("TTLSKY2", PU_CACHE), 42);
				#else
...
void F_SkyScroll (void)
{
    ...
    #ifdef SPMOVES
    p1 = W_CachePatchName ("TTLSKY1", PU_CACHE);
    p2 = W_CachePatchName ("TTLSKY1", PU_CACHE);
    #else
	...
	animtimer = finalecount % 32;

    scrolled = 320 - (animtimer)*10;
    if (scrolled > 320)
        scrolled = 320;
    if (scrolled < 0)
        scrolled = 0;
...
#define HANDY 111
	...
	switch(finalestage)
	{
		...
		case 4:
			V_DrawString(8, 152, 0, "3) Stop by the #srb2 chatroom\nat irc.accessirc.net.");
		#ifdef SPMOVES
		case 3:
			V_DrawString(8, 120, 0, "2) Poke around the wads\nsection of the website.");
		case 2:
			V_DrawString(8, 80, 0, "1) Visit JTE's website at\nhttp://JTE.rose-manor.net/\n for news and updates.");
		case 1:
			V_DrawString(8, 4, 0, "Thanks for playing this SRB2\nSPMoves demo. It'll be a freakin long time\nbefore the final SPMoves comes\nout. Here are several things\nyou you can do while\nyou wait:");
		#else
		case 3:
...
timetonext += INTERVAL;
	...
	if(finalecount >= 5*TICRATE)
	{
		...
		if(grade & 8)
			V_DrawString(8, 32, 0, "Mario's Koopa Blast");

		if(grade & 16)
			V_DrawString(8, 40, 0, "Sonic Into Dreams");

		if(grade & 32)
			V_DrawString(8, 48, 0, "Christmas Hunt");

		if(grade & 64)
			V_DrawString(8, 56, 0, "Adventure Example");
		...
		if(grade & 1024)
			V_DrawString(8, 96, 0, "Sonic Golf");

		if(netgame)
			V_DrawString(8, 104, V_WHITEMAP, "Prizes only\nawarded in\nsingle player!");
		else if(modifiedgame)
			V_DrawString(8, 104, V_WHITEMAP, "Prizes not\nawarded in\nmodified games!");
	}
}
...
void F_IntroDrawer (void)
{
	...
	{
		#ifdef SPMOVES
		if(finaletext == JTETEXT && !nomonsters)
			finaletext = E0TEXT;
		else if(finaletext == JTETEXT)
		{
		    D_StartTitle();
