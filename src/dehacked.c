...
#ifdef RIDERS
			else if (!strcmp(word, "AUTOMAP"))
			{
				DEH_WriteUndoline(word, va("%d", mapheaderinfo[num-1].automap), UNDO_NONE);
				mapheaderinfo[num-1].automap = i;
			}
			else if (!strcmp(word, "SKYMOUNT"))
			{
				DEH_WriteUndoline(word, va("%d", mapheaderinfo[num-1].skymount), UNDO_NONE);
				mapheaderinfo[num-1].skymount = i;
			}
#endif
			else
...
static actionpointer_t actionpointers[] =
{
	...
	{{A_RecyclePowersW},       "A_RECYCLEPOWERSW"},
...
#else
#ifdef DELFILE
	if (stricmp(s, "srb2riders.dat") == 0 && !disableundo) return false;
#else
	if (stricmp(s, "srb2riders.dat") == 0) return false;
#endif
#endif
