...
#include "JTE/JTEMisc.h" // A_TORNADOPARTICLE HOLIDAYS
...
#ifdef DARKWORLD
			else if (!strcmp(word, "DARKWORLD"))           mapheaderinfo[num-1].darkworld = i;
#endif
...
static actionpointer_t actionpointers[] =
{
	...
	{{A_TornadoParticle},		"A_TORNADOPARTICLE"},
#endif
...
#ifdef HOLIDAYS
				else if (!strcmp(word, "REPLACE"))
				{
					// Cut the stupid junk off the end of word2
					for(p = word2; *p; p++)
						if(*p == '\n' || *p == ' ' || *p == '#')
							*p = '\0';

					// Enumerate word2 if you can, warn if you can't.
					if(!strcmp(word2, "FLATS"))
						i = 0;
					else if(!strcmp(word2, "MUSIC"))
						i = 1;
					else if(!strcmp(word2, "SPRITES"))
						i = 2;
					else if(!strcmp(word2, "SFX"))
						i = 3;
					else if(!isdigit(*word2))
						deh_warning("Unknown replace section '%s'\n", word2);

					// Read if the section number is correctly set.
					if(i < 4)
						JTE_ReadReplace(f,i);
					else
						deh_warning("Replace number %d out of range\n", i);
				}
#endif
				else
