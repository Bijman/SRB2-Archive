...
#ifdef SRB2PLUS // 2 seconds default flashing time
INT32 flashingtics = 2*TICRATE;
#else
...
void G_BuildTiccmd(ticcmd_t *cmd, INT32 realtics)
{
		...
		(gamepadjoystickmove && axis > 0))
	{
		...
		localaiming = 0;
#ifndef SRB2PLUS // remove allowmlook
...
void G_PlayerReborn(INT32 player)
{
	...
	tic_t jointime;
	UINT8 spectator;
#ifdef SRB2PLUS // new player vars on player spawn
    INT16 lastinflictor = -1;
#endif
...
#ifdef SRB2PLUS // set new player vars on player spawn
    p->lastinflictor = lastinflictor;
#endif
