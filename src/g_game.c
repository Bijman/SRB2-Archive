...
void G_PlayerReborn(INT32 player)
{
	...
	// If you start a new game of teams, we'll set for you a random team :P ~Manimi.
	
	if (p->ctfteam == 0 && G_GametypeHasTeams())
	{
		p->ctfteam = M_RandomRange(1,2);
	}
}
...
void G_DoReborn(INT32 playernum)
{
	...
	if (countdowntimeup || (!multiplayer && gametype == GT_COOP))
	{
		...
		if (!countdowntimeup && (mapheaderinfo[gamemap-1]->levelflags & LF_NORELOAD))
		{
			...
			//if (botingame)
			//{ // Bots respawn next to their master- No.
				//players[playernum].playerstate = PST_REBORN;
				//G_SpawnPlayer(playernum, false);
			//}
		}
		else
#ifdef HAVE_BLUA
...
void G_ExitLevel(void)
{
	if (gamestate == GS_LEVEL)
	{
		...
		if (gametype != GT_COOP)
			CONS_Printf(M_GetText("Time up! Good game!\n"));
...
{
	return (gametype == GT_TEAMMATCH || gametype == GT_CTF || (gametype == GT_COMPETITION && cv_teams.value == 1));
}
