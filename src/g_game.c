...
boolean G_Responder(event_t *ev)
{
	...
	if (gamestate == GS_LEVEL && ev->type == ev_keydown && ev->data1 == KEY_F12)
	{
		...
		else
		{
			...
			while (true)
			{
#ifdef RABBIT // CSTODO: Merge to be compatible
				csdisplayplayer ++;

				if (csdisplayplayer >= MAXPLAYERS)
					csdisplayplayer = 0;

                if (csdisplayplayer != self_id)
                {
                    if (! virtualplayers[csdisplayplayer].in_game || virtualplayers[csdisplayplayer].spectator)
                        continue;

                    if ((gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value)) && players[0].ctfteam &&
                        players[0].ctfteam != virtualplayers[csdisplayplayer].ctfteam)
                        continue;

                    if ((gametype == GT_MATCH && ! cv_matchtype.value) && ! players[0].spectator)
                        continue;
                }
#else
				displayplayer++;
...
#else
            CONS_Printf(M_GetText("Viewpoint: %s\n"), csdisplayplayer == self_id ? player_names[0] : virtualplayers[csdisplayplayer].name);
#endif
			return true;
		}
	}
...
#ifdef RABBIT
			CS_Tick(); // Intermission tick.
			SendMessages(); // CSTODO: Needs to be fixed, segmentation fault in INTERMISSION!
#endif
			Y_Ticker();
			HU_Ticker();
			break;
...
void G_PlayerReborn(INT32 player)
{
	...
	tic_t jointime;
	UINT8 spectator;
#ifdef RABBIT // CSTODO: Bots
	UINT8 cpu_player; // LXShadow
#endif
...
#ifdef RABBIT
	cpu_player = players[player].cpu_player; // LXShadow
#endif
...
#ifdef RABBIT
	p->cpu_player = cpu_player; // LXShadow
#endif
...
#else
                int player_count = 0, count_id = 0;

                for (i = 0; i < MAXPLAYERS; i ++) // Lol reusing i.
                {
                    if (! virtualplayers[i].in_game && i != self_id)
                        continue;

                    if (i == self_id)
                        count_id = player_count;

                    player_count ++;
                }

                if (player_count) // We want to be random, but we don't want to spawn in another player's position.
                {                     // So this is how it's done. Instead of being completely random, each player "chooses" from a set of slots assigned to them only.
                    int min = numdmstarts * count_id / player_count;
                    int max = numdmstarts * (count_id + 1) / player_count;

                    i = (min + (P_Random() % (max - min))) % numdmstarts; // % numdmstarts just in case...
                }
                else
                    i = P_Random() % numdmstarts;
#endif
...
void G_CoopSpawnPlayer(INT32 playernum, boolean starpost)
{
	...
	{
#ifdef RABBIT
	    int redplayercount = 0, blueplayercount = 0, team_id = 0; // team_id is like self_id, but relative to your team.

        for (i = 0; i < MAXPLAYERS; i ++)
        {
            if (! virtualplayers[i].in_game && i != self_id) continue;

            if (i != self_id && virtualplayers[i].ctfteam == 1) redplayercount ++;
            if (i != self_id && virtualplayers[i].ctfteam == 2) blueplayercount ++;

            if (i == self_id && players[0].ctfteam == 1) team_id = redplayercount ++;
            if (i == self_id && players[0].ctfteam == 2) team_id = blueplayercount ++;
        }
#endif
		...
		switch (players[playernum].ctfteam)
		{
			case 1: // Red Team
				...
				for (j = 0; j < 32; j++)
				{
                    ...
                    if (redplayercount)
                    {
                        int min = numredctfstarts * team_id / redplayercount;
                        int max = numredctfstarts * (team_id + 1) / redplayercount - 1; // Needs to be <numdmstarts
                        i = min + (P_Random() % (max - min));
                    }
                    else
                        i = P_Random() % numredctfstarts;
#endif
			...
			case 2: // Blue Team
				...
				for (j = 0; j < 32; j++)
				{
                    ...
                    if (blueplayercount)
                    {
                        int min = numbluectfstarts * team_id / blueplayercount;
                        int max = numbluectfstarts * (team_id + 1) / blueplayercount - 1; // Needs to be <numdmstarts
                        i = min + (P_Random() % (max - min));
                    }
                    else
                        i = P_Random() % numbluectfstarts;
...
#ifdef RABBIT
        if (CS_host) // The map change at the end of intermission is now server-side.
        {
            int i;
            INT32 tempmap = gamemap;
            INT32 temptype = gametype;
            gamemap = nextmap+1;
