...
void G_PlayerReborn(int player)
{
	...
	byte bowmode;
	byte swimming;
	byte gettingiteminfo;
	byte fairy;
	unsigned short equips;
#endif

#if defined(UNIVERSE) || defined(JTEBOTS)
	int i;
	unsigned int health;
#endif

#ifdef UNIVERSE
	unsigned char sulevel;
	unsigned int  suexp;

	unsigned int  sumaxhp;
	unsigned int  sumaxtp;

	unsigned int  sutp;
	unsigned int  sumoney;
	unsigned char psuitems[NUMSUITEMS];

	unsigned long equipweapon;
	unsigned long equipbarrier;
	unsigned long equiparmor;
	unsigned long equipslot[4];
	unsigned long equipmag;
#endif
	...
	sulevel = players[player].sulevel;
	suexp = players[player].suexp;

	sumaxhp = players[player].sumaxhp;
	sumaxtp = players[player].sumaxtp;

	sutp = players[player].sutp;
	sumoney = players[player].sumoney;
	memcpy(psuitems,players[player].suitems,sizeof(psuitems));

	equipweapon = players[player].equipweapon;
	equipbarrier = players[player].equipbarrier;
	equiparmor = players[player].equiparmor;
	for(i = 0; i < 4; i++)
		equipslot[i] = players[player].equipslot[i];
	equipmag = players[player].equipmag;
#endif
	...
	if(bot && gametype == GT_COOP)
	{
		...
		for(i = 0; i < NUMPOWERS; i++)
			powers[i] = players[player].powers[i];
		powers[pw_tailsfly] = powers[pw_underwater] = powers[pw_spacetime] =
		powers[pw_flashing] = powers[pw_extralife] = 0;
	#ifdef SPMOVES
		powers[pw_amydash] = 0;
	#endif
	#ifdef UNIVERSE
...
#ifdef ZELDA
	maxhearts = players[player].maxhearts;
	equipped = players[player].equipped;
	equips = players[player].equips;
	bombsout = players[player].bombsout;
	swording = players[player].swording;
	bombtics = players[player].bombtics;
	bowmode = players[player].bowmode;
	swimming = players[player].swimming;
	gettingiteminfo = players[player].gettingiteminfo;
	fairy = players[player].fairy;
#endif
	...
	p->sutp = sutp;
	p->sumoney = sumoney;
	memcpy(p->suitems,psuitems,sizeof(psuitems));

	p->equipweapon = equipweapon;
	p->equipbarrier = equipbarrier;
	p->equiparmor = equiparmor;
	for(i = 0; i < 4; i++)
		p->equipslot[i] = equipslot[i];
	p->equipmag = equipmag;
#endif
...
#ifdef ZELDA
	if(gametype == GT_COOP && maxhearts >= 3)
...
#ifdef ZELDA
	if(zelda)
		p->health = HEARTS(3); // 3 hearts, no matter what maxhearts is.
	else
#endif
#ifdef JTEBOTS
	if(bot && gametype == GT_COOP && health > 1)
		p->health = health;
	else
#endif
	p->health = 1; // 0 rings
...
void G_DoCompleted(void)
{
	...
	if(universe)
	{
		...
		if(nextmap < 1100-1 // If you're not going to the credits or anything like that...
		&& stricmp(mapheaderinfo[nextmap].lvlttl, // And the next map's level title...
		mapheaderinfo[gamemap-1].lvlttl)) // Does not match this map's level title...
			nextmap = SUHUBMAP-1; // Then go back to the Pioneer 2!
	}
	else
#endif
...
skipit:
	...
	if (skipstats
#ifdef UNIVERSE
	|| universe // No intermission in SRB2 Universe!
#endif
	)
		G_AfterIntermission();
	else
...
void G_DoLoadGame(unsigned int slot)
{
	...
	save_p = savebuffer = NULL;
#ifdef JTEBOTS
	multiplayer = (playeringame[1] && !players[1].bot);
...
void G_InitNew(skill_t skill, const char *mapname, boolean resetplayer, boolean skipprecutscene)
{
	...
	if (resetplayer)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
			...
			if(universe)
			{
				players[i].health = players[i].sumaxhp;
				players[i].sutp = players[i].sumaxtp;
			}
#endif
		}
