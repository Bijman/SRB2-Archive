...
void JB_Init(void)
{
	...
	strcpy(description[3].picname, "SRB2BACK"); // SRB2CBTODO: Should use a CB graphic
	...
	// Sonic & Tails char select menu option
	strcpy(description[4].info, "             Slowest\n                 Fly/Swin\n             Tails & Sonic\na new combination,\nbut Tails takes the\nlead this time.");
	strcpy(description[4].picname, "SRB2BACK");
	strcpy(description[4].skinname, "TAILS");
	strcpy(charselbots[4], "SONIC");
	PlayerMenu[4].status = IT_CALL|IT_STRING;
	PlayerMenu[4].text = "Tails & Sonic";
}
...
static void Got_NameAndColor(byte **cp, int playernum)
{
	...
	{
#ifndef NORMALSRB2
		long previousskin = players[playernum].skin;
#endif
		SetPlayerSkin(playernum, str);
#ifndef NORMALSRB2
		// If a player changes skin while alive and not a spectator, make them respawn // SRB2CBTODO: Not die, respawn
		if ((netgame || multiplayer) && players[playernum].mo && !players[playernum].spectator && players[playernum].skin != previousskin)
		{
			P_ResetPlayer(&players[playernum]);
			P_DamageMobj(players[playernum].mo, NULL, NULL, 10001); // SRB2CBTODO: Respawn, not kill!
        }
#endif
	}
}
...
// SRB2CBTODO: No, the game can't JUST search for things to unload,
// the game must store a list of files to change/added to restore for later
  */
static void Command_Delfile(void)
{
