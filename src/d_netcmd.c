...
#if 0
#ifdef RABBIT
// LXShadow (next two functions)
void Add_CPU();
int maxjumpheight = 0;
static void Command_AddBot_f(void)
{
	PF_LevelInitialise();
	Add_CPU();
}
...
void AI_Call();
static void Command_CallBot_f(void)
{
	AI_Call();
}
...
#ifdef RABBIT
	// Lawl, LXShadow goes here. (Console command registering.)
#if 0
	COM_AddCommand("csaddbot", Command_AddBot_f); // Commands changed to csaddbot and cscallbot for SRB2CB compatability
	COM_AddCommand("cscallbot", Command_CallBot_f);
#endif
#endif
...
#ifdef BUNNIES
    if (newgametype != GT_CTF && newgametype != GT_MATCH && newgametype != GT_RACE)
    {
        CONS_Printf("CS warning: CS mode is not compatible with this gametype. Switching to Match...\n");
...
#ifdef RABBIT // CSTODO: Make sure this is ok, if yes, make it default :P
	// LXShadow added this. Instant map change.
    if (CS_host)
    {
        int i;

        // A bit of a hack - try to tell the other players that we're changing maps, without being too dangerous about it.
        INT32 tempmap = gamemap;
        INT32 temptype = gametype;
        gamemap = newmapnum;
        gametype = newgametype;
        mapswitches ++; // This is what makes the map change instant, including restarts.
...
static void Command_Version_f(void)
{
	CONS_Printf(M_GetText("SRB2%s (%s %s)\n"), VERSIONSTRING, compdate, comptime);
//	CONS_Printf(M_GetText("SRB2%s (%s %s %s)\n"), VERSIONSTRING, compdate, comptime, comprevision); // CSTODO: use if build environment supports
}
...
#else
    if (! CS_client && (multiplayer || netgame)) // We don't want the point/time limits to be changed; let ReceiveServerPacket do that.
#endif
	{
