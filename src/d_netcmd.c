...
void Command_Recorddemo_f (void);
...
// PSO Stuffs!
#ifdef SPMOVES
void Command_Equip_f(void);
void Command_Inventory_f(void);
#endif
...
void D_RegisterClientCommands (void)
{
  ...
  //
    ...
    COM_AddCommand ("recorddemo", Command_Recorddemo_f);
...
#ifdef ACBOT
    CV_RegisterVar (&cv_predictingmonsters);
#endif
...
//	CV_RegisterVar (&cv_fishcake);
    ...
    // PSO!
    #ifdef SPMOVES
    COM_AddCommand("equip", Command_Equip_f);
    COM_AddCommand("inventory", Command_Inventory_f);
    #endif
...
void SendNameAndColor(void)
{
    ...
    *(p-1) = 0; // finish teh string;
	...
	if(!(server || admin || nomonsters) && cv_forceskin.value)
	{
...
void Command_Teleport_f (void)
{
	...
	if(!(cv_debug || devparm))
	{
		CONS_Printf("Sorry, this command is only for level development.\n");
		return;
	}
    ...
    if(netgame)
    {
        CONS_Printf ("You can't teleport while in a netgame, even though that would be bloody cool!\n");
        return;
    }
	...
	p->mo->z = p->mo->subsector->sector->floorheight;
...
{
    #ifdef RECORDDEMO
    if (COM_Argc() != 2 || playeringame[0])
    {
        CONS_Printf ("recorddemo <demoname> : record a demo (must be in a game)\n");
        return;
    }
    ...
    G_RecordDemo(COM_Argv(1));
    #else
    CONS_Printf ("Sorry, in-game demo recording currently doesn't do anything but crash. :(\n");
    #endif
}
...
/*
	...
	if(COM_CheckParm("stfu") == 0)
	{
		if(!netgame)
		{
			if(!modifiedgame && !nomonsters)
			{
    ...
    /*if((i = COM_CheckParm("-monsters")) != 0
        && (COM_CheckParm("stfu") != 0 || netgame || multiplayer || modifiedgame || nomonsters))
    ...
    if(COM_CheckParm("-noresetplayers")
        && (COM_CheckParm("stfu") || netgame || multiplayer || modifiedgame || nomonsters))
        buf[1]|=2;
...
void Command_Pause(void)
{
	...
	if(cv_pause.value != 1 && (server || admin || nomonsters))
...
{
    #ifdef SPMOVES
    CONS_Printf ("SRB2 v1.08 SPMoves ("
                __TIME__" "__DATE__")\n");
    #else
...
void ObjectPlace_OnChange(void)
{
	...
	if((netgame || multiplayer) && cv_objectplace.value != 0 && !(server || admin || nomonsters)) // You spoon!
	{
		cv_objectplace.value = 0;
		CV_SetValue(&cv_objectplace, 0);
		//CONS_Printf("No, dummy, you can't use this in multiplayer!\n");
		CONS_Printf("You're not teh server! No objectplace for j00!\n");
		return;
	}
#endif
...
void GameType_OnChange(void) // Tails 03-13-2001
{
    ...
    if((server || admin || nomonsters) && (multiplayer || netgame))
    {
	...
	else if (!(multiplayer || netgame || nomonsters) && cv_gametype.value != GT_COOP)
	{
...
void Command_ExitLevel_f(void)
{
	if(!netgame && !cv_debug && !nomonsters)
	{
...
void Command_Tunes_f(void)
{
    ...
    //players[consoleplayer].tunes = COM_Argv(1);
}
...
void Command_Equip_f(void)
{
	...
	{
		CONS_Printf("This command is only for PSO levels, sorry.\n");
		return;
	}
	...
	if(COM_Argc() < 2)
	{
		...
		if(players[consoleplayer].equipweapon)
		        CONS_Printf("Weapon: %s\n",psoitems[players[consoleplayer].equipweapon].name);
		if(players[consoleplayer].equipbarrier)
		        CONS_Printf("Barrier: %s\n",psoitems[players[consoleplayer].equipbarrier].name);
		if(players[consoleplayer].equiparmor)
		        CONS_Printf("Armor: %s\n",psoitems[players[consoleplayer].equiparmor].name);
		if(players[consoleplayer].equipmag)
		        CONS_Printf("Mag: %d\n",players[consoleplayer].equipmag->type);
		if(!players[consoleplayer].equipweapon
	...
	for(i = 0; i < NUMPSOITEMS; i++)
		if(!strcmp(psoitems[i].name, COM_Args()))
		{
			itemnum = i;
			switch(psoitems[i].type)
			{
			   case PSOI_SABER:
			   case PSOI_SWORD:
			   case PSOI_DAGGER:
	...
	if(!players[consoleplayer].psoitems[itemnum] && !cv_debug)
	{
		CONS_Printf("You don't have that item!\n");
		return;
	}
	...
	if(itemtype == 1)
	{
		players[consoleplayer].equipweapon = itemnum;
		CONS_Printf("Weapon equipped.\n");
	}
	else if(itemtype == 2)
	{
		players[consoleplayer].equipbarrier = itemnum;
		CONS_Printf("Barrier equipped.\n");
	}
	else if(itemtype == 3)
	{
		players[consoleplayer].equiparmor = itemnum;
		CONS_Printf("Armor equipped.\n");
	}
	/*
	else if(itemtype == 4 && itemnum < **first item goes here**)
	{
	    if(!players[consoleplayer].equipslot1)
   		     players[consoleplayer].equipslot1 = itemnum;
	    else if(!players[consoleplayer].equipslot2)
   		     players[consoleplayer].equipslot2 = itemnum;
	    else if(!players[consoleplayer].equipslot3)
   		     players[consoleplayer].equipslot3 = itemnum;
	    else if(!players[consoleplayer].equipslot4)
   		     players[consoleplayer].equipslot4 = itemnum;
        else
        {
	...
	else if(itemtype == 4 && itemnum > **last item goes here**)
	{
		players[consoleplayer].equipmag = P_SpawnMobj(players[consoleplayer].mo->x, players[consoleplayer].mo->y, players[consoleplayer].mo->z, psomags[itemnum - **last item goes here**]);
...
void Command_Inventory_f(void)
{
	...
	if(players[consoleplayer].psomoney)
		CONS_Printf("Meseta: %d\n",players[consoleplayer].psomoney);

	for(i = 0; i < NUMPSOITEMS; i++)
		if(players[consoleplayer].psoitems[i] && psoitems[i].name)
			CONS_Printf("%s: %d\n",psoitems[i].name,players[consoleplayer].psoitems[i]);
}
#endif
