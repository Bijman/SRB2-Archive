...
static void Command_Teleport_f(void)
{
	...
	CONS_Printf("Teleporting to %d, %d, %d...", intx, inty, intz/FRACUNIT);
...
static void Command_Map_f(void)
{
	...
	if (i)
	{
		...
		{
#ifdef RUSSIA
			if (!strcasecmp("Russia", COM_Argv(i+1)))
				russia = true;
			else
#endif
...
#ifndef JTEIGNOREWAD // In SRB2JTE, you may add wads during a netgame...
	if (netgame && !(server || consoleplayer == adminplayer))
	{
		CONS_Printf("Sorry, only the server can do this.\n");
		return;
...
#endif
		if(!(netgame || multiplayer)
#ifdef BUGFIXES // Only announce modifiedgame ONCE.
		&& !modifiedgame
#endif
		)
			CONS_Printf("WARNING: Game must be restarted to record statistics.\n");
...
void ObjectPlace_OnChange(void)
{
...
#ifdef RUSSIA
	if (russia && cv_objectplace.value) // You spoon!
	{
		CV_StealthSetValue(&cv_objectplace, 0);
		CONS_Printf("In Soviet Russia Blast 2, objectplace turns on YOU!\n");
		return;
	}
#endif
	...
	if(cv_objectplace.value)
	{
		...
		if(players[0].playerstate != PST_LIVE)
			return;
	...
	else if(players[0].mo)
	{
		...
		if(!players[0].nightsmode)
		{
			...
			{
				P_RemoveMobj(players[0].mo->target);
				players[0].mo->target = NULL;
			}
#else
			if (players[0].mo->target)
...
#ifdef JTEMISC // Allow loading for server.
	if(!(server || (adminplayer == consoleplayer)))
	{
		CONS_Printf("Only server can load during a netgame\n");
		return;
	}
#else
	if (netgame)
...
#ifdef JTEMISC // Allow saving for server.
	if(!(server || (adminplayer == consoleplayer)))
	{
		CONS_Printf("Only server can save a netgame\n");
		return;
	}
#else
	if (netgame)
...
static void Skin2_OnChange(void)
{
	if (
		...
		!universe &&
#endif
		!P_PlayerMoving(secondarydisplayplayer)
	)
		SendNameAndColor2();
	else
		CV_StealthSet(&cv_skin2, skins[players[secondarydisplayplayer].skin].name);
