...
typedef union
{
	struct
	{
		...
		INT32 score, total; // fake score, total
		UINT32 tics; // time
...
void Y_IntermissionDrawer(void)
{
	...
	else if (intertype == int_classicrace && cv_teams.value == 0)
	{
		INT32 j = 0;
		INT32 x = 4;
		INT32 y = 48;
	...
	else if (intertype == int_classicrace && cv_teams.value == 1)
	{
		INT32 x = 4, y = 0;
	...
	else if (intertype == int_match)
	{
		...
		for (i = 0; i < data.competition.numplayers; i++)
		{
			...
			pscore = (data.competition.scores[i] != 0);
		...
		if (cv_dummyscramble.value == 0)
		{
		V_DrawRightAlignedThinString(x+288, y, V_MONOSPACE|((data.competition.scores[i]) ? V_YELLOWMAP : 0), va("%u", pscore));
		}
				// Final Points
	...
	if (timer)
		V_DrawCenteredString(BASEVIDWIDTH/2, 188, V_YELLOWMAP,
			va("get ready, start in %d seconds", timer/TICRATE));
...
void Y_Ticker(void)
{
	...
	else if (intertype == int_classicrace || intertype == int_ctf || intertype == int_teammatch) // match
	{
		if (!intertic) // first time only
	...
	else if (intertype == int_race || intertype == int_match) // race
	{
		if (!intertic) // first time only
			S_ChangeMusicInternal("racent", true); // loop it
...
static void Y_CalculateCompetitionWinners(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		scores[i]   = min((INT32)players[i].score, 99999990);
