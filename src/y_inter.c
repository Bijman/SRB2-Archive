...
#ifdef HOLIDAYS
			memset(holidaystr,0,sizeof(holidaystr));
			if(holidaynicks[holiday])
			{
				holidaystr[0] = va("%sINTR",holidaynicks[holiday]);
				holidaystr[1] = va("%sINTW",holidaynicks[holiday]);
			}
			else
				holidaystr[1] = "INTERSCW";
			if(!holidaystr[0] || W_CheckNumForName(holidaystr[0]) == -1)
				holidaystr[0] = "INTERSCR";
			if(!holidaystr[1] || W_CheckNumForName(holidaystr[1]) == -1)
				holidaystr[1] = holidaystr[0];
			widebgpatch = W_CachePatchName(holidaystr[1], PU_STATIC);
			bgpatch = W_CachePatchName(holidaystr[0], PU_STATIC);
#else
...
#ifdef RUSSIA
			// In Soviet Russia, act passed YOU!
			if(russia)
			{
				strcpy(data.coop.passed2, skins[players[consoleplayer].skin].name);
				data.coop.passedx2 = 62 + (176 - V_LevelNameWidth(data.coop.passed2))/2;
				if (mapheaderinfo[gamemap-1].actnum)
				{
					strcpy(data.coop.passed1, "ACT PASSED");
					data.coop.passedx1 = 62 + (176 - V_LevelNameWidth(data.coop.passed1))/2;
				}
				else
				{
					strcpy(data.coop.passed1, "THE ACT PASSED");
					data.coop.passedx1 = 62 + (240 - V_LevelNameWidth(data.coop.passed1))/2;
				}
			}
			else
#endif
