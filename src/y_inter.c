...
#ifdef RABBIT // CSTODO: Move to ClientServer folder
#include "CS_Global.h"
#endif
		...
		for (i = 0; i < data.match.numplayers; i++)
		{
...
#ifdef RABBIT
			if (data.match.num[i] == self_id || (virtualplayers[data.match.num[i]].in_game && data.match.num[i] != self_id))
#else
...
#endif
			{
				...
				else
				{
					...
					{
#ifdef RABBIT
                        boolean dead = (players[0].lives <= 0);

                        if (data.match.num[i] != self_id)
                            dead = virtualplayers[data.match.num[i]].permadead;
#endif
#ifndef RABBIT
...
#else
			if (! (data.match.spectator[i])) // LXShadow: During Y_CalculateMatchWinners, isn't the playeringame check already done?
#endif
			{
				UINT8 *colormap = R_GetTranslationColormap(*data.match.character[i], *data.match.color[i], GTC_CACHE);
...
void Y_Ticker(void)
{
	...
	{
#ifdef RABBIT
        UINT8 i, numplayers = 0; // Whoops -- numplayers = 0!
        for (i = 0; i < MAXPLAYERS; i ++) {if (virtualplayers[i].in_game || i == self_id) numplayers ++;}
#endif
		if (!intertic) // first time only
...
static void Y_CalculateMatchWinners(void)
{
...
#ifdef RABBIT // BETODO: Support this for all the other modes
	for (j = 0; j < MAXPLAYERS; j++)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
            UINT32 score = players[0].score;
            INT32* colour = &players[0].skincolor;
            char* name = player_names[0];
            INT32* skin = &players[0].skin;
            boolean spectator = players[0].spectator;
            ...
            if (i != self_id)
            {
                ...
                colour = &virtualplayers[i].colour;
                name = virtualplayers[i].name;
                skin = &virtualplayers[i].skin;
                spectator = virtualplayers[i].spectator;
            }

			if (gametype == GT_TAG)
			{
				if (score >= data.match.scores[data.match.numplayers] && completed[i] == false)
				{
					data.match.scores[data.match.numplayers] = score;
					data.match.color[data.match.numplayers] = colour;
					data.match.character[data.match.numplayers] = skin;
					data.match.name[data.match.numplayers] = name;
					data.match.spectator[data.match.numplayers] = spectator;
					data.match.num[data.match.numplayers] = i;
				}
			}
			else if (score >= data.match.scores[data.match.numplayers] && completed[i] == false)
			{
                data.match.scores[data.match.numplayers] = score;
                data.match.color[data.match.numplayers] = colour;
                data.match.character[data.match.numplayers] = skin;
...
static void Y_CalculateTimeRaceWinners(void)
{
	...
	for (j = 0; j < MAXPLAYERS; j++)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
...
#else
            UINT32 score = players[0].realtime;
            INT32* colour = &players[0].skincolor;
            INT32* skin = &players[0].skin;
            char* name = player_names[0];
            ...
            if (i != self_id)
            {
                score = virtualplayers[i].real_time;
                colour = &virtualplayers[i].colour;
                skin = &virtualplayers[i].skin;
                name = virtualplayers[i].name;
            }
