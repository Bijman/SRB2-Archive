...
void R_InitSprites (char** namelist)
{
    ...
    /*for (i=0; i<numsprites; i++)
         if (sprites[i].numframes<1)
             I_Error ("Sprite %s has no frames at all!\n", sprnames[i]);*/
}
...
void R_AddSprites (sector_t* sec, int lightlevel)
{
	...
	// PSO Stages have a sprite draw distance too due to massive mobjs!
	// It's not nearly as many as NiGHTs, though, so a more forgiving distance is possable.
	#ifdef SPMOVES
	else if(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO || mapheaderinfo[gamemap-1].typeoflevel & TOL_CHATROOM)
	{
		// Special function for precipitation Tails 08-18-2002
		for (thing = sec->thinglist ; thing ; thing = thing->snext)
		{
...
void SetPlayerSkin (int playernum, char *skinname)
{
    ...
    for(i=0;i<numskins;i++)
    {
        ...
        if (stricmp(skins[i].name,skinname)==0)
        {
			...
			players[playernum].oldnormalspeed = atoi(skins[i].normalspeed)*NEWTICRATERATIO; // Tails
			players[playernum].oldthrustfactor = atoi(skins[i].thrustfactor)*NEWTICRATERATIO; // Tails
			players[playernum].oldaccelstart = atoi(skins[i].accelstart)*NEWTICRATERATIO; // Tails
			players[playernum].oldacceleration = atoi(skins[i].acceleration)*NEWTICRATERATIO; // Tails

			// Cheat checks!
			if(players[playernum].oldnormalspeed > 36*NEWTICRATERATIO)
				players[playernum].oldnormalspeed = 36*NEWTICRATERATIO;
			if(players[playernum].oldthrustfactor > 5*NEWTICRATERATIO)
				players[playernum].oldthrustfactor = 5*NEWTICRATERATIO;
			if(players[playernum].oldaccelstart > 192*NEWTICRATERATIO)
				players[playernum].oldaccelstart = 192*NEWTICRATERATIO;
			if(players[playernum].oldacceleration > 50*NEWTICRATERATIO)
				players[playernum].oldacceleration = 50*NEWTICRATERATIO;
			...
			if(players[playernum].jumpfactor > 900)
				players[playernum].jumpfactor = 900;
			...
			#ifdef SPMOVES
			players[playernum].customchao = atoi(skins[i].customchao);
			#endif
...
void SetPlayerSkinByNum (int playernum, int skinnum)
{
	if(skinnum < numskins) // Make sure it exists!
	{
		...
		// Set the "old" versions directly Graue 12-22-2003
		players[playernum].oldnormalspeed = atoi(skins[skinnum].normalspeed)*NEWTICRATERATIO; // Tails
		players[playernum].oldthrustfactor = atoi(skins[skinnum].thrustfactor)*NEWTICRATERATIO; // Tails
		players[playernum].oldaccelstart = atoi(skins[skinnum].accelstart)*NEWTICRATERATIO; // Tails
		players[playernum].oldacceleration = atoi(skins[skinnum].acceleration)*NEWTICRATERATIO; // Tails
		...
		#ifdef SPMOVES
		players[playernum].customchao = atoi(skins[skinnum].customchao);
		#endif
...
// start character type identification Tails 03-01-2000
            ...
            {
                strncpy (skins[numskins].ability, value, 2);
...
// End Custom translation table Tails 06-07-2002
			...
			if (!strcmp(token, "chao")) // Custom SPMoves Chao!
			{
					strncpy (skins[numskins].customchao, value, 2);
					strupr (skins[numskins].customchao);
			}
		#endif
            else
            {
                ...
                if(!found)
                    CONS_Printf("Unknown keyword '%s' in S_SKIN\n",token);
            }
next_token:
            token = strtok (NULL,"\r\n= ");
        }
	...
	#ifdef SPMOVES
	    char* spm_name = strupr(skins[numskins].name);
		if(!strcmp(spm_name, "SHADOW"))
			spm_shadow = numskins;
		if(!strcmp(spm_name, "MECHA_KNUCKLES"))
			spm_mechaknux = numskins;
		if(!strcmp(spm_name, "EGGMAN"))
			spm_eggman = numskins;
		if(!strcmp(spm_name, "JASON"))
			spm_jte = numskins;
		if(!strcmp(spm_name, "JAKE"))
			spm_jte2 = numskins;
		if(!strcmp(spm_name, "SS005"))
			spm_ss005 = numskins;
		if(!strcmp(spm_name, "CYAN"))
			spm_cyan = numskins;
		if(!strcmp(spm_name, "FOX"))
			spm_fox = numskins;
		if(!strcmp(spm_name, "HIKARU"))
			spm_hikaru = numskins;
		if(!strcmp(spm_name, "CHOMPY"))
			spm_chompy = numskins;
		if(!strcmp(spm_name, "ZIM"))
			spm_zim = numskins;
	#endif
#ifdef SKINVALUES
