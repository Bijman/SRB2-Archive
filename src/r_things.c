...
void SetPlayerSkinByNum(INT32 playernum, INT32 skinnum)
{
...
#ifdef SRB2PLUS // skin attribute hacks this is a really dumb place to put this code... todo
    // Sonic
    strcpy(skins[0].acceleration, "30"); // slower acceleration (eff 150 from 200)
    strcpy(skins[0].normalspeed, "40"); // top speed buff as consolation prize for accel nerf
    strcpy(skins[0].actionspd, "10"); // chop thok speed, but can still be used to accelerate to MAXMOVE
    // Tails
    strcpy(skins[1].acceleration, "40"); // way better accel (eff 200 from 150)
    strcpy(skins[1].thrustfactor, "5"); // standardize thrustfactor
    strcpy(skins[1].normalspeed, "30"); // slightly less top speed
    strcpy(skins[1].jumpfactor, "110"); // jump higher
    // Knuckles
    strcpy(skins[2].acceleration, "35"); // slightly worse accel (eff 175 from 180)
    strcpy(skins[2].thrustfactor, "5"); // standardize thrustfactor
    strcpy(skins[2].normalspeed, "35"); // slight top speed increase
    strcpy(skins[2].actionspd, "30"); // somewhat faster gliding
#endif
	...
	if (skinnum >= 0 && skinnum < numskins) // Make sure it exists!
	{
		...
		// Cheat checks!
#ifdef SRB2PLUS // a little more speed please :)
		if (players[playernum].normalspeed > 40)
			players[playernum].normalspeed = 40;
#else
...
#ifdef SRB2PLUS // a little more height please :)
		if (players[playernum].jumpfactor > 110)
			players[playernum].jumpfactor = 110;
#else
