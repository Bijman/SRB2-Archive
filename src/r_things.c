...
#ifdef RIDERS
static spriteframe_t sprtemp[84];
#else
...
#ifdef RIDERS
			if (frame >= 84 || rotation > 8) // Give an actual NAME error -_-...
#else
...
#ifdef RIDERS
	strcpy(skin->super, "0");
	strcpy(skin->superanims, "0");
	strcpy(skin->superspin, "0");
	strcpy(skin->iconprefix, "SONICICN");
	strcpy(skin->thokcolor, "0");
	strcpy(skin->prefthokcolor, "0");
	strcpy(skin->runframe, "0");
	strcpy(skin->exgear, "0");
	strcpy(skin->starttranscolor, "224");
#else
	strcpy(skin->super, "1");
...
#ifdef RIDERS
	skins[0].spritedef.numframes = sprites[SPR_RIDR].numframes;
	skins[0].spritedef.spriteframes = sprites[SPR_RIDR].spriteframes;
#else
...
#ifdef RIDERS
	ST_LoadIconGraphics(skins[0].iconprefix, 0);
#endif
}
...
void SetPlayerSkinByNum(INT32 playernum, INT32 skinnum)
{
	...
	if (skinnum >= 0 && skinnum < numskins) // Make sure it exists!
	{
...
#ifdef RIDERS
		players[playernum].charexgear = atoi(skins[skinnum].exgear);

		players[playernum].thokcolor = atoi(skins[skinnum].thokcolor);
		players[playernum].prefthokcolor = atoi(skins[skinnum].prefthokcolor);

		players[playernum].runframe = atoi(skins[skinnum].runframe);
#endif
		...
		// Cheat checks!
		if (players[playernum].normalspeed > 39)//36)
			players[playernum].normalspeed = 39;//36;
		if (players[playernum].normalspeed < 31)//36)
			players[playernum].normalspeed = 31;//36;
		...
		if (players[playernum].accelstart < 48)
			players[playernum].accelstart = 48;
		if (players[playernum].acceleration > 50)
			players[playernum].acceleration = 50;
		if (players[playernum].acceleration < 34)
			players[playernum].acceleration = 34;
...
#ifdef RIDERS
			GETSKINATTRIB("faceicon", iconprefix)
			GETSKINATTRIB_(exgear)
			GETSKINATTRIB_(thokcolor)
			GETSKINATTRIB_(prefthokcolor)
			GETSKINATTRIB_(runframe)
#endif
...
#ifdef RIDERS
		ST_LoadIconGraphics(skins[numskins].iconprefix, numskins);
#endif
