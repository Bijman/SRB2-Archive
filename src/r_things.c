...
static boolean R_AddSingleSpriteDef(const char *sprname, spritedef_t *spritedef, int wadnum, int startlump, int endlump)
{
    ...
    for (l = startlump; l < endlump; l++)
    {
        if (*(int *)lumpinfo[l].name == intname)
        {
			...
			{
				char name[9];
				strncpy(name, lumpinfo[l].name, 8);
				name[8] = '\0';
				if(frame >= 64 || rotation > 8)
				{
					CONS_Printf("WARNING! Bad sprite name: %s", name);
					continue;
				}
			}
#endif
...
static void R_ProjectSprite(mobj_t *thing)
{
	...
	{
		CONS_Printf("Warning: Mobj of type %d with invalid sprite data detected and removed.\n", thing->type);
		if(!thing->player)
...
#ifdef MOBJSCALE
	if(thing->scale > 400)
		tx -= FIXEDSCALE(spriteoffset[lump],400);
	else if(thing->scale != 100)
		tx -= FIXEDSCALE(spriteoffset[lump],thing->scale);
	else
#endif
...
#ifdef MOBJSCALE
	if(thing->scale > 400)
		tx += FIXEDSCALE(spritewidth[lump],400);
	else if(thing->scale != 100)
		tx += FIXEDSCALE(spritewidth[lump],thing->scale);
	else
#endif
	if (thing->flags & MF_HIRES)
...
#ifdef MOBJSCALE
	if(thing->scale > 400)
		gzt = thing->z + FIXEDSCALE(spritetopoffset[lump],400);
	else if(thing->scale != 100)
		gzt = thing->z + FIXEDSCALE(spritetopoffset[lump],thing->scale);
	else
#endif
	if (thing->flags & MF_HIRES)
...
#ifdef MOBJSCALE
    if (thing->scale > 400)
       vis->gz = gzt - FIXEDSCALE(spriteheight[lump],400);
    else if (thing->scale != 100)
       vis->gz = gzt - FIXEDSCALE(spriteheight[lump],thing->scale);
	else
#endif
    if (thing->flags & MF_HIRES)
...
void SetPlayerSkinByNum(int playernum, int skinnum)
{
	if (skinnum >= 0 && skinnum < numskins) // Make sure it exists!
	{
...
#ifdef SPMOVES
		{
			...
			const unsigned short spmoves = skins[skinnum].spmoves;
			int *charflags = &players[playernum].charflags;

			if(spmoves & SPM_CHAOSCONTROL)
				*charflags |= SF_CHAOSCONTROL;
			if(spmoves & SPM_METALSONIC)
				*charflags |= SF_METAL;
			if(spmoves & SPM_AMYDASH)
				*charflags |= SF_AMYDASH;
			if(spmoves & SPM_HMS123)
				*charflags |= SF_HMS123;
			if(spmoves & SPM_FLAME)
				*charflags |= SF_FLAME;
			if(spmoves & SPM_NODROWN)
				*charflags |= SF_NODROWN;
			if(spmoves & SPM_GRABOBJ)
				*charflags |= SF_GRABOBJ;
	}
#endif
		...
		// Cheat checks!
#ifndef SPMOVES // Handled elsewhere
...
#ifdef UNIVERSE
		// In SRB2 Universe, walk.
		if(universe)
			players[playernum].normalspeed /= 2;
#endif
