...
#ifdef RABBIT
extern char server_ips[MAXSERVERLIST][25];
#endif
...
#ifdef RABBIT
	if (serverlist[choice-FIRSTSERVERLINE + serverlistpage * SERVERS_PER_PAGE].info.netmode == NETMODE_STANDARD)
#endif
		COM_BufAddText(va("connect node %d\n", serverlist[choice-FIRSTSERVERLINE + serverlistpage * SERVERS_PER_PAGE].node));
#ifdef RABBIT
    else
    {
        COM_BufAddText(va("csconnect %s", server_ips[serverlist[choice-FIRSTSERVERLINE + serverlistpage * SERVERS_PER_PAGE].node]));
    }
#endif
...
void M_Drawer(void)
{
	...
	if (customversionstring[0] != '\0')
		...
		V_DrawString(0, BASEVIDHEIGHT - 8, V_TRANSLUCENT, va("%s", VERSIONSTRING));
//		V_DrawString(0, BASEVIDHEIGHT - 8, V_TRANSLUCENT, va("%s (%s)", VERSIONSTRING, comprevision)); // CSTODO: use if build environment supports.
}
