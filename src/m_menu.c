...
#ifdef JTEMISC // String height is really 8
...
#ifdef CENTEREDMENU
#ifdef UNIVERSE
	{IT_CALL    | IT_STRING, NULL, "secrets",       M_SecretsMenu,    0},
	{IT_SUBMENU | IT_STRING, NULL, "SRB2 Universe",&SUDef,            8},
#else
	{IT_CALL    | IT_STRING, NULL, "secrets",       M_SecretsMenu,    8},
#endif
	{IT_SUBMENU | IT_STRING, NULL, "1 player",     &SinglePlayerDef, 16},
	{IT_SUBMENU | IT_STRING, NULL, "multiplayer",  &MultiPlayerDef,  24},
	{IT_CALL    | IT_STRING, NULL, "options",       M_OptionsMenu,   32},
	{IT_CALL    | IT_STRING, NULL, "quit game",     M_QuitSRB2,      40},
#else
...
static menuitem_t NewGameMenu[] =
{
	// Tails
#ifdef RUSSIA
	{IT_CALL | IT_STRING, NULL, "Soviet Russia",  M_ChooseSkill, 80},
#endif
...
#ifdef JTEBOTS
	if(charselbots[choice][0] != '\0')
	{
		strlwr(charselbots[choice]);
		JB_BotWaitAdd(R_SkinAvailable(charselbots[choice]));
	}
#endif
...
{
#ifdef RUSSIA
	skillnum = choice; // Tails 03-02-2002
#else
...
static void M_DrawReadThis1(void)
{
	...
	holidaystr = va("%sHELP",holidaynicks[holiday]);
	if(W_CheckNumForName(holidaystr) == -1)
		holidaystr = "HELP";
	V_DrawScaledPatch (0,0,0,W_CachePatchName(holidaystr,PU_CACHE));
#else
...
void M_QuitResponse(int ch)
{
	...
	{
#ifdef SRB2JTE
		if (quitsounds2[(gametic>>2)&2]) S_StartSound(NULL, quitsounds2[(gametic>>2)&2]);
#else
...
#ifdef SUPERVIRUS
				else if(virus && !players[consoleplayer].infected)
					return false;
#endif
				else
					M_StartControlPanel();
