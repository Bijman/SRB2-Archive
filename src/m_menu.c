...
#define  IT_MSGHANDLER       12     // same as key but with event and sometime can handle y/n key (special for messages)
...
{
    #ifdef SPMOVES
	{IT_CALL   | IT_STRING,0, "    extras", M_SecretsMenu, 84},
	#else
...
menuitem_t MultiPlayerMenu[] =
{
    ...
    {IT_CALL | IT_STRING,0,"JOIN GAME (Search Master Server)",M_ConnectMenu         , 20},
...
menuitem_t SetupMultiPlayerMenu[] =
{
    ...
    {IT_CVAR | IT_STRING | IT_CV_NOPRINT,0,"Your color",&cv_playercolor         ,MAXSKINCOLORS},
...
{
	{IT_CALL | IT_STRING,0,"SONIC",            M_ChoosePlayer, 0},
	{IT_CALL | IT_STRING,0,"TAILS",            M_ChoosePlayer, 8},
	{IT_CALL | IT_STRING,0,"KNUCKLES",         M_ChoosePlayer, 16},
	{IT_DISABLED,        0,"UNKNOWN",          M_ChoosePlayer, 24},
	{IT_DISABLED,        0,"UNKNOWN",          M_ChoosePlayer, 24},
	{IT_DISABLED,        0,"UNKNOWN",          M_ChoosePlayer, 24},
	{IT_DISABLED,        0,"UNKNOWN",          M_ChoosePlayer, 24},
	{IT_DISABLED,        0,"UNKNOWN",          M_ChoosePlayer, 24},
...
// activate setup for player 2
    if (cv_splitscreen.value || nomonsters)
...
void M_DrawSetupChoosePlayerMenu(void)
{
    ...
    JTE_InitCharData();
    if (nomonsters)
        for(p = Player1; p < player_end; p++)
                PlayerMenu[p].status = IT_CALL | IT_STRING;
    #endif
	...
	speed = atoi(skins[itemOn].normalspeed)*NEWTICRATERATIO;
	sprintf (txtspeed,"Speed: %d",speed);

	V_DrawString(mx-16, my+80, 0, txtspeed);
	V_DrawString(mx-16, my+80, V_WHITEMAP, "Speed:\nAbility:\nNotes:");

	V_DrawScaledPatch(mx+160,my,0,patch);
	V_DrawString(mx-16, my+92, 0, description[itemOn].info);
}
...
void M_NewGame(int choice)
{
	...
	if(veryhardcleared || nomonsters) // Ultimate skill level is unlockable Tails 05-19-2003
...
void M_ChooseSkill(int choice)
{
	...
	#ifdef SPMOVES
	if(spm_nocharsel && spm_skin)
	   M_ChoosePlayer(spm_skin);
	spm_nocharsel = false;
	#endif
...
#define NUMCHECKLIST 9
	...
	checklist[7].name = "SRB2 Universe";
	checklist[7].requirement = "Finish\nUltimate Mode";
	checklist[7].unlocked = (grade & 1024);

	checklist[8].name = "Chao World SPM";
	checklist[8].requirement = "Find all\nMultiplayer Eggs";
	checklist[8].unlocked = (grade & 2048);

	for(i=0; i<NUMCHECKLIST; i++)
	{
		V_DrawString(8, 8+(16*i), V_RETURN8, checklist[i].name);
		V_DrawString(160, 8+(16*i), V_RETURN8, checklist[i].requirement);

		if(checklist[i].unlocked)
			V_DrawString(308, 8+(16*i), V_WHITEMAP, "Y");
		else
			V_DrawString(308, 8+(16*i), V_WHITEMAP, "N");
	}
}
...
menuitem_t RewardMenu[]=
{
	...
	{IT_STRING | IT_CALL,0,"Mario Koopa Blast", M_MarioGame,     40},
	{IT_STRING | IT_CALL,0,"Adventure Example", M_AdventureGame, 50},
	{IT_STRING | IT_CALL,0,"Christmas Hunt",    M_ChristmasGame, 60},
	{IT_STRING | IT_CALL,0,"Sonic Golf",        M_GolfGame,     70}
	#ifdef SPMOVES
	,{IT_STRING | IT_CALL,0,"SRB2 Universe", M_PSOGame, 70},
	{IT_STRING | IT_CALL,0,"Rock Skull Mountain", M_JTEGame, 80}
	#endif
};
...
void M_Reward (int choice)
{
	...
	if(grade & 1024 || nomonsters)
		RewardMenu[universe].status = IT_STRING | IT_CALL;
	else
		RewardMenu[universe].status |= IT_DISABLED;

	//if(<insert condition under which to give JTE's level here>)
		RewardMenu[jtelevel].status = IT_STRING | IT_CALL;
	/*else
		RewardMenu[jtelevel].status |= IT_DISABLED;*/
	#endif
...
#ifdef ACBOT
	{IT_STRING | IT_CVAR,0,"Predicting Monsters" ,&cv_predictingmonsters ,130},       //added by AC for predmonsters
#endif
};
...
void M_SetupNextMenu(menu_t *menudef)
{
	...
	if(currentMenu->menuitems[itemOn].status==IT_DISABLED && !nomonsters)
	{
		for(i=0; i<currentMenu->numitems; i++)
		{
...
void M_DrawQuestMenu()
{
    ...
    M_DrawGenericMenu();
	//V_DrawString(mx-16, my+92, 0, questDescription[itemOn]);
}
...
menuitem_t QuestMenu[]= {
    {IT_CALL|IT_STRING,0,"Quest 1",JTE_PSOQuest1,0},
    {IT_CALL|IT_STRING,0,"Quest 2",JTE_PSOQuest2,10},
    {IT_CALL|IT_STRING,0,"Quest 3",JTE_PSOQuest3,20},
    {IT_CALL|IT_STRING,0,"Quest 4",JTE_PSOQuest4,30},
    {IT_CALL|IT_STRING,0,"Quest 5",JTE_PSOQuest5,40},
    {IT_CALL|IT_STRING,0,"Quest 6",JTE_PSOQuest6,50},
    {IT_CALL|IT_STRING,0,"Quest 7",JTE_PSOQuest7,60},
    {IT_CALL|IT_STRING,0,"Quest 8",JTE_PSOQuest8,70},
    {IT_CALL|IT_STRING,0,"Quest 9",JTE_PSOQuest9,80},
    {IT_CALL|IT_STRING,0,"Quest 10",JTE_PSOQuest10,90},
    {IT_CALL|IT_STRING,0,"Quest 11",JTE_PSOQuest11,100},
    {IT_CALL|IT_STRING,0,"Quest 12",JTE_PSOQuest12,110},
    {IT_CALL|IT_STRING,0,"Quest 13",JTE_PSOQuest13,120},
    {IT_CALL|IT_STRING,0,"Quest 14",JTE_PSOQuest14,130},
    {IT_CALL|IT_STRING,0,"Quest 15",JTE_PSOQuest15,140},
    {IT_CALL|IT_STRING,0,"Quest 16",JTE_PSOQuest16,150},
    {IT_CALL|IT_STRING,0,"Quest 17",JTE_PSOQuest17,160},
    {IT_CALL|IT_STRING,0,"Quest 18",JTE_PSOQuest18,170},
    {IT_CALL|IT_STRING,0,"Quest 19",JTE_PSOQuest19,180},
    {IT_CALL|IT_STRING,0,"Quest 20",JTE_PSOQuest20,190},
};
...
menu_t QuestDef =
{
    "PSOQUEST",
    "Select a Quest",
    sizeof(QuestMenu)/sizeof(menuitem_t),
    &QuestDef,
    QuestMenu,
    M_DrawQuestMenu,
    24,0,
