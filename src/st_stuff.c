...
#include "JTE/Universe/SU_HUD.h" // UNIVERSE
#include "JTE/Universe/SU_Command.h" // UNIVERSE
#include "JTE/SV_Virus.h" // SUPERVIRUS
#include "JTE/SC_HUD.h" // SACITY
#include "JTE/JTEMisc.h" // CCPAL
...
void ST_doPaletteStuff(void)
{
...
#ifdef SUPERVIRUS
	if(virus && cv_renderview.value)
		palette = 2;
	else
#endif
#if defined(CCPAL) && defined(SPMOVES)
	if(stplyr->chaoscontrol)
		palette = 1;
	else
#endif
		palette = 0;
	...
	if (palette != st_palette)
	{
		...
		if (rendermode != render_none)
		{
			...
			{
#ifdef HOLIDAYS
				if(holidaycmaps[holiday])
					colormaps = holidaycmaps[holiday];
				else
					colormaps = holidaycmaps[H_NONE];
#endif
#ifdef CCPAL
				if(palette == 1)
					colormaps = neoncolormaps;
				else
					colormaps = realcolormaps;
#endif
#if defined(HOLIDAYS) || defined(CCPAL)
				JTE_ReinitLightTables();
#endif
...
static void ST_drawDebugInfo(void)
{
	...
	sprintf(sendcolor, "%d", atoi(skins[stplyr->skin].endtranscolor));
#else
...
#ifdef SPMOVES
		else if(stplyr->powers[pw_super])
		{
			const int num = ((stplyr->mo->flags & MF_TRANSLATION)>>MF_TRANSSHIFT);
			const byte* colormap = (byte*)translationtables[stplyr->skin] - 256 + ((num)<<8);
