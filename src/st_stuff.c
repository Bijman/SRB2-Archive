...
void ST_initData(void)
{
    ...
    if (singledemo || cv_gametype.value == GT_RACE
    #ifdef CIRCUITMODE
...
//		return;
    ...
    #ifdef SPMOVES
	st_borderpatchnum = W_GetNumForName ("MENUBACK"); // Tails 10-26-99
	#else
...
//    colon     = W_CachePatchName ("WICOLON", PU_STATIC); // Tails 03-14-2000
	...
	#ifdef SPMOVES
	psohud = W_CachePatchName ("PSOHUD1", PU_STATIC);
	#endif
}
...
void ST_overlayDrawer ()
{
	...
	#ifdef SPMOVES
    && !(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO)
    #endif
    )
	{
	...
	#ifdef SPMOVES
	&& !(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO || mapheaderinfo[gamemap-1].typeoflevel & TOL_CHATROOM)
	#endif
	)
	{
		...
		#endif
		{
			...
			if(plyr->skincolor==0)
				colormap = colormaps;
			else if(!plyr->powers[pw_super])
				colormap = (byte *) translationtables[plyr->skin] - 256 + ((plyr->skincolor)<<8);
			else
				colormap = (byte *) translationtables[plyr->skin] - 256 + ((16)<<8);
	...
	#ifdef SPMOVES
	if(plyr == &players[consoleplayer] && hu_showscores && showmultiegghud && !cv_objectplace.value)
	{
		char eggsfound[3];
		int found = 0;
		...
		sprintf(eggsfound, "Multiplayer\nEggs Found:\n    %i/%i", found, NUMMULTIEGGS);
		if(found < NUMMULTIEGGS)
			V_DrawString(0, STRINGY(0), 0, eggsfound);
		else if(!(grade & 2048))
		  grade |= 2048;
	}
	...
	if(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO)
	{
		...
		V_DrawScaledPatch (SCX(8), SCY(16), FG | V_NOSCALESTART, psohud);
		ST_drawOverlayNum (SCX(118), SCY(8)+11*vid.fdupy, plyr->health, tallnum, NULL); // HP
		ST_drawOverlayNum (SCX(108), SCY(27)+11*vid.fdupy, plyr->psomagic, tallnum, NULL); // TP
		//ST_drawOverlayNum (SCX(56), SCY(18)+11*vid.fdupy, plyr->psophotonblast, tallnum, NULL); // PB
		ST_drawOverlayNum (SCX(58), SCY(44)+11*vid.fdupy, plyr->psolevel, tallnum, NULL); // Level

		// HP Bar
		origamount = ((plyr->psolevel) * 100) / 30;
		amount = (origamount - plyr->mo->health);
		amount = amount * ((float)length/origamount);
		if(amount > 0 && amount < length)
		  		V_DrawFill (69, STRINGY(7)+24, amount, 7, 103);
		else if(amount >= length)
		  		V_DrawFill (69, STRINGY(7)+24, length, 7, 103);

		// TP Bar
		origamount = ((plyr->psolevel) * 100) / 50;
		amount = (origamount - plyr->psomagic);
		amount = amount * ((float)length/origamount);
		if(amount > 0)
		  		V_DrawFill (61, STRINGY(7)+42, amount, 7, 103);
	}
	else
	#endif
	...
	#ifdef SPMOVES
	if (!(hu_showscores && showmultiegghud))
	#endif
	{
		if(cv_splitscreen.value)
		{
...
*/
    ...
    if(cv_debug && (netgame || multiplayer))
        cv_debug = false;

	if(cv_debug && !hu_showscores)
	{
	...
	if(cv_objectplace.value && plyr->mo && plyr->mo->target)
	{
		...
		if(!multiplayer && !netgame && !plyr->nightsmode && !hu_showscores)
		{
	...
	if(!(netgame || multiplayer) && eastermode && !modifiedgame && !cv_objectplace.value && !cv_debug)
	{
		char eggsfound[3];
		int found = 0;
	...
	if(hu_showscores && cv_gametype.value == GT_COOP /*&& !cv_objectplace.value*/)
	{
	...
	if(plyr->nightstime > 0)
	{
		...
		if(plyr->nightstime < 10)
			...
			ST_drawNightsOverlayNum(SCX(160) + numbersize, SCY(32), plyr->nightstime, nightsnum, 15); // Yellow
	}
	...
	// SPMoves Pickup item hud thingie!
	#ifdef SPMOVES
	else if(plyr->pickupitem)
	{
		V_DrawFill(224, 6, 72, 12, 119);
		V_DrawCenteredString(292-32, 8, 0, "ACTIVATE");
		V_DrawCenteredString(292-32, 20, 0, "PICK UP");
	}
	else if(plyr->helditem && !plyr->mfjumped)
	{
		V_DrawFill(224, 6, 72, 12, 119);
		V_DrawCenteredString(292-32, 8, 0, "ACTIVATE");
		V_DrawCenteredString(292-32, 20, 0, "DROP");
	}
	else if(plyr->helditem && plyr->mfjumped)
	{
		V_DrawFill(224, 6, 72, 12, 119);
		V_DrawCenteredString(292-32, 8, 0, "ACTIVATE");
		V_DrawCenteredString(292-32, 20, 0, "TOSS");
	}
	#endif
...
#define ZONE "ZONE"
		...
		if(*lvlttl == '\0'
		#ifdef CIRCUITMODE
		|| (cv_gametype.value == GT_CIRCUIT && circintrodone)
		#endif
		)
