...
static void ST_overlayDrawer(void)
{
	...
	if (!hu_showscores && (netgame || splitscreen))
	{
		...
		{
#ifdef SRB2PLUS // enforce respawn time so that there is a minimum breather between fights
			// todo cleaner
			INT16 rtime = stplyr->deadtimer/TICRATE - 54;
			if (rtime > 0 && !stplyr->spectator)
				V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(132), V_TRANSLUCENT, va("Respawn in %d seconds.", rtime));
			else
#endif
