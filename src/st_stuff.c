...
static void ST_drawMatchHUD(void)
{
    ...
    {
        patch_t* weapon_patches[7] = {normring, autoring, bouncering, scatterring, grenadering, explosionring, railring};
        int emerald_x[7] = {28, 40, 40, 28, 16, 16, 28};
        int emerald_y[7] = {136, 144, 160, 168, 160, 144, 152};
        int i;

        // Seven weapons.
        for (i = 0; i < 7; i ++)
        {
            V_DrawScaledPatch(88 + (i * 20), STRINGY(162), V_SNAPTOLEFT, weapon_patches[i]);

            if (virtualplayers[csdisplayplayer].current_weapon == i)
                V_DrawScaledPatch(86 + (i * 20), STRINGY(162 - (splitscreen ? 4 : 2)), V_SNAPTOLEFT, curweapon);
        }

        // Seven emeralds.
        for (i = 0; i < 7; i ++)
        {
            if (virtualplayers[csdisplayplayer].powers[pw_emeralds] & 1 << i)
                V_DrawScaledPatch(emerald_x[i], STRINGY(emerald_y[i]), V_SNAPTOLEFT, tinyemeraldpics[i]);
        }
...
static void ST_drawCTFHUD(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
        ...
        if (flagholder[1] == i)
		{
			team = 2; // Dude, if they're holding the RED flag then they need to be BLUE.
			whichflag = MF_REDFLAG; // ?
			break;
		}
#endif
	}
...
static void ST_overlayDrawer(void)
{
	...
	{
#ifdef RABBIT
	    boolean viewing_spectator = gametype != GT_CTF ? players[0].spectator : (! players[0].ctfteam);
...
#else
		if ((gametype == GT_MATCH || gametype == GT_TAG || gametype == GT_CTF) && ! viewing_spectator)
#endif
			ST_drawMatchHUD();
	...
	if (!hu_showscores) // again, hide the following if TAB is held
	{
		...
		if (csdisplayplayer != self_id)
		{
			...
			strlcpy(name, virtualplayers[csdisplayplayer].name, 13);
		...
		if ((stplyr == &players[consoleplayer] && !cv_chasecam.value)
			...
			|| (csdisplayplayer != self_id && ! cv_chasecam.value))
		{
