...
static inline void ST_drawCompetitionHUD(void)
{
	if (leveltime > TICRATE && leveltime <= 2*TICRATE)
		V_DrawScaledPatch(SCX((BASEVIDWIDTH - SHORT(race3->width))/2), (INT32)(SCY(BASEVIDHEIGHT/2)), V_NOSCALESTART, race3);
	else if (leveltime > 2*TICRATE && leveltime <= 3*TICRATE)
	...
	{
		    if (cv_timelimit.value && timelimitintics >= leveltime)
			sprintf(pstime, "%d", (timelimitintics-leveltime+1*TICRATE)/TICRATE);
	           if (cv_timelimit.value)
				sprintf(pstext, "%s", M_GetText("TIME REMAINING:"));
			   else //Since having no hud message in tag is not characteristic:
				sprintf(pstext, "%s", M_GetText("NO TIME LIMIT"));
	...
	if (pstext[0])
	{
	 if (leveltime < (timelimitintics - 5*TICRATE))
	 {
		if (splitscreen)
			V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(5), 0, pstext);
		else
			V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(15), 0, pstext);
	 }
	}
	if (pstime[0])
	{
	 ...
	 {
		if (splitscreen)
			V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(25), 0, pstime);
		else
			V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(25), 0, pstime);
	 }
	}
...
static inline void ST_drawTeamName(void)
{
	if (stplyr->ctfteam == 1)
		...
		V_DrawString(244, (splitscreen) ? STRINGY(184) : STRINGY(192), V_HUDTRANSHALF, "CHOOSE TEAM");
}
...
static void ST_overlayDrawer(void)
{
	...
	if (!hu_showscores || hu_showscores) // hide the following if TAB is held
	{
		...
		if (gametype == GT_COMPETITION && cv_overtime.value
			&& (leveltime > (timelimitintics + TICRATE/2)) && cv_timelimit.value && (leveltime/TICRATE % 2 == 0) && cv_timeup.value == 0)
		{
			if (splitscreen)
				V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(25), 0, M_GetText("SUDDEN DEATH!"));
			else
				V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(25), 0, M_GetText("SUDDEN DEATH!"));
		}
		...
		//wew new competition HUD for SSB
		else if (gametype == GT_COMPETITION)
			ST_drawCompetitionHUD();
