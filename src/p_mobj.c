        ...
        if (!P_TryMove (mo, ptryx, ptryy, true)) //SoM: 4/10/2000
        {
			...
			else if (mo->flags & MF_MISSILE)
			{
				...
				if (ceilingline &&
					...
					ceilingline->backsector->ceilingpic == sky3flatnum ||
					ceilingline->backsector->ceilingpic == sky4flatnum ||
					ceilingline->backsector->ceilingpic == sky5flatnum ||
					ceilingline->backsector->ceilingpic == sky6flatnum ||
					ceilingline->backsector->ceilingpic == sky7flatnum ||
					ceilingline->backsector->ceilingpic == sky8flatnum ||
					ceilingline->backsector->ceilingpic == sky9flatnum) &&
					ceilingline->frontsector &&
					(ceilingline->frontsector->ceilingpic == skyflatnum ||
					ceilingline->frontsector->ceilingpic == sky2flatnum ||
					ceilingline->frontsector->ceilingpic == sky3flatnum ||
					ceilingline->frontsector->ceilingpic == sky4flatnum ||
					ceilingline->frontsector->ceilingpic == sky5flatnum ||
					ceilingline->frontsector->ceilingpic == sky6flatnum ||
					ceilingline->frontsector->ceilingpic == sky7flatnum ||
					ceilingline->frontsector->ceilingpic == sky8flatnum ||
					ceilingline->frontsector->ceilingpic == sky9flatnum) &&
...
// Tails
            ...
            if ((mo->z <= mo->floorz) && (!(tmfloorthing) || (((tmfloorthing->flags & MF_PUSHABLE) || (tmfloorthing->flags2 & MF2_STANDONME) || (tmfloorthing->flags & MF_SOLID && !(tmfloorthing->flags & MF_MONITOR || tmfloorthing->flags & MF_ENEMY || tmfloorthing->flags & MF_BOSS))) || (tmfloorthing->type == MT_PLAYER && !(mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE && cv_gametype.value == GT_MATCH)) || tmfloorthing->type == MT_FLOORSPIKE))) // Tails 9-15-99 Spin Attack
			{
...
// end Tails
            ...
            else if (!(tmfloorthing) || (((tmfloorthing->flags & MF_PUSHABLE) || (tmfloorthing->flags2 & MF2_STANDONME) || (tmfloorthing->flags & MF_SOLID && !(tmfloorthing->flags & MF_MONITOR || tmfloorthing->flags & MF_ENEMY || tmfloorthing->flags & MF_BOSS))) || (tmfloorthing->type == MT_PLAYER && !(mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE && cv_gametype.value == GT_MATCH)) || tmfloorthing->type == MT_FLOORSPIKE))
			{
              mo->momz = 0;
			}
        }
    ...
    if (mo->z + mo->height > mo->ceilingz)
    {
		...
		if (mo->type == MT_FIREBALL)
        {
	       ...
	       mo->subsector->sector->ceilingpic == sky3flatnum ||
	       mo->subsector->sector->ceilingpic == sky4flatnum ||
	       mo->subsector->sector->ceilingpic == sky5flatnum ||
	       mo->subsector->sector->ceilingpic == sky6flatnum ||
	       mo->subsector->sector->ceilingpic == sky7flatnum ||
	       mo->subsector->sector->ceilingpic == sky8flatnum ||
	       mo->subsector->sector->ceilingpic == sky9flatnum) &&
               mo->subsector->sector->ceilingheight == mo->ceilingz)
            {
                P_SetMobjState(mo, S_DISS);
                return;
...
//                P_RemoveMobj(mo);
    ...
    if((mo->flags & MF_PUSHABLE) && (mo->subsector->sector->special == 961 || (mo->subsector->sector->special == 962 && mo->z == mo->floorz)) && mo->target->player && !mo->player)
    {
	int masterline, specialtype = 96;

	// Support multiple trigger types (96-98) Graue 12-19-2003
	while((masterline = P_FindSpecialLineFromTag(specialtype, mo->subsector->sector->tag)) < 0)
	{
		specialtype++;
...
// Tails
			...
			{
				if((tmfloorthing->flags & MF_MONITOR) || (tmfloorthing->flags & MF_PUSHABLE)
					|| (tmfloorthing->flags2 & MF2_STANDONME) || (tmfloorthing->flags & MF_SOLID && !(tmfloorthing->flags & MF_MONITOR || tmfloorthing->flags & MF_ENEMY || tmfloorthing->flags & MF_BOSS)))
				{
					if(mo->player)
					{
...
// end Tails
            ...
            if (!(tmfloorthing) || (((tmfloorthing->flags & MF_PUSHABLE) || (tmfloorthing->flags2 & MF2_STANDONME) || (tmfloorthing->flags & MF_SOLID && !(tmfloorthing->flags & MF_MONITOR || tmfloorthing->flags & MF_ENEMY || tmfloorthing->flags & MF_BOSS))) || (tmfloorthing->type == MT_PLAYER && !(mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE && cv_gametype.value == GT_MATCH)) || tmfloorthing->type == MT_FLOORSPIKE))
			{
              mo->momz = 0;
			}
        }
...
playergravity:
		if ((mo->player->charability == 1
		#ifdef SPMOVES
		|| mo->player->charability == 9
		#endif
		) && ((mo->player->powers[pw_tailsfly]) || (mo->player->mo->state == &states[S_PLAY_SPC1]) || (mo->player->mo->state == &states[S_PLAY_SPC2]) || (mo->player->mo->state == &states[S_PLAY_SPC3]) || (mo->player->mo->state == &states[S_PLAY_SPC4])))
			gravityadd = gravityadd/3; // less gravity while flying
		if(mo->player->gliding && mo->player->charability != 16)
...
void P_SnowThinker(precipmobj_t* mobj)
{
	...
	if(mobj->subsector->sector->ffloors)
	{
	  ...
	  for(rover = mobj->subsector->sector->ffloors; rover; rover = rover->next)
	  {
		...
		if(mobj->subsector->sector->ceilingheight <= mobj->floorz)
		{
			P_RemovePrecipMobj(mobj);
		}
...
void P_Boss1Thinker(mobj_t* mobj)
{
    ...
    if (!mobj->target
        || !(mobj->target->flags&MF_SHOOTABLE) && !mobj->flags2 & MF2_BOSSFLEE)
    {
		if(mobj->health <= 0)
		{
		...
		switch(mobj->type)
		{
			...
			case MT_SIGN: // Start Level end sign stuff Tails 01-14-2001
				if(players[consoleplayer].exiting)
				{
					...
					#ifdef SPMOVES
					&& players[consoleplayer].skin != spm_eggman
					#endif
					)
					{
...
switch(mobj->type)
{
	...
	case MT_REDRING:
		...
		if(eastermode)
		    mobj->flags =  (mobj->flags & ~MF_TRANSLATION) | ((leveltime % MAXSKINCOLORS)<<MF_TRANSSHIFT);
		break;
	...
	case MT_JETFUME1:
		{
			...
			#ifdef SPMOVES
			if((mobj->target->health <= 0 && mobj->target->player)
			|| !mobj->target)
			{
				if(mobj->target->player)
					mobj->target->player->fly1 = 0;
				P_SetMobjState(mobj, S_DISS);
			}
			else if(mobj->target->player
			&& !(mobj->target->player->skin == spm_eggman
			|| mobj->target->player->skin == spm_mechaknux))
			{
				mobj->target->player->fly1 = 0;
				P_SetMobjState(mobj, S_DISS);
		    }
		    else if(mobj->target->player
		    && mobj->target->player->skin == spm_mechaknux && (mobj->fuse != 56
		    || !mobj->target->player->gliding))
				P_SetMobjState(mobj, S_DISS);
            #endif
			mobj->fuse++;
		}
	...
	case MT_APPLE:
...

		{
			...
			mobj->fuse = 2*TICRATE; // Makes it disappear and respawn at its first spawnpoint later on
		}
	default:
		#ifdef SPMOVES
		if(JTE_SPMovesMobjProg(mobj))
			break;
		#endif
		// check mobj against possible water content, before movement code
		P_MobjCheckWater (mobj);
                      ...
                      mobjtype_t    type )
{
    ...
    #ifdef SPMOVES
    if(mobj->type == MT_BIRD || mobj->type == MT_SQRL || mobj->type == MT_MOUSE || mobj->type == MT_CWBALL || (mobj->flags & MF_ENEMY && eastermode))
    {
        //mobj->fuse = 300 + (P_Random() % 50);
        if(!xmasmode)
                mobj->flags = (mobj->flags & ~MF_TRANSLATION) | (((P_Random() % MAXSKINCOLORS)+1)<<MF_TRANSSHIFT);
	    else
        {
            if(P_Random() & 1)
	            mobj->flags = (mobj->flags & ~MF_TRANSLATION) | ((6)<<MF_TRANSSHIFT); // Red
            else
	            mobj->flags = (mobj->flags & ~MF_TRANSLATION) | ((12)<<MF_TRANSSHIFT); // Green
	    }
    } // SPMoves special coloring ^.^
...
void P_SpawnPrecipitation()
{
		...
		for(i=0; i < (1048576/cv_numsnow.value); i++)
		{
			...
			if(mapheaderinfo[gamemap-1].typeoflevel & TOL_NIGHTS) // Spawn snow from normal ceilings
			{
				if(snowsector->sector->floorheight < snowsector->sector->ceilingheight)
				{
					...
					{
						if (z < 144)
							P_SetPrecipMobjState(P_SpawnSnowMobj(x, y, height, MT_LEAF), S_LEAF2);
						else
							P_SpawnSnowMobj(x, y, height, MT_LEAF);
					}
					#endif
				}
			}
			else
			{
				...
				|| snowsector->sector->ceilingpic == sky2flatnum
				|| snowsector->sector->ceilingpic == sky3flatnum
				|| snowsector->sector->ceilingpic == sky4flatnum
				|| snowsector->sector->ceilingpic == sky5flatnum
				|| snowsector->sector->ceilingpic == sky6flatnum
				|| snowsector->sector->ceilingpic == sky7flatnum
				|| snowsector->sector->ceilingpic == sky8flatnum
				|| snowsector->sector->ceilingpic == sky9flatnum)
				&& snowsector->sector->floorheight < snowsector->sector->ceilingheight)
				{
					while(!(height < snowsector->sector->ceilingheight &&
						height > snowsector->sector->floorheight))
						height = ((rand() * 65535) - 32768) << FRACBITS;
	...
	else if(cv_storm.value && !netgame)
	{
		...
		for(i=0; i < (1048576/cv_raindensity.value); i++)
		{
			...
			if((rainsector->sector->ceilingpic == skyflatnum
			|| rainsector->sector->ceilingpic == sky2flatnum
			|| rainsector->sector->ceilingpic == sky3flatnum
			|| rainsector->sector->ceilingpic == sky4flatnum
			|| rainsector->sector->ceilingpic == sky5flatnum
			|| rainsector->sector->ceilingpic == sky6flatnum
			|| rainsector->sector->ceilingpic == sky7flatnum
			|| rainsector->sector->ceilingpic == sky8flatnum
			|| rainsector->sector->ceilingpic == sky9flatnum)
			&& rainsector->sector->floorheight < rainsector->sector->ceilingheight)
			{
...
void P_RespawnSpecials (void)
{
	...
	if(cv_storm.value)
	{
		...
		|| players[displayplayer].mo->subsector->sector->ceilingpic == sky2flatnum
		|| players[displayplayer].mo->subsector->sector->ceilingpic == sky3flatnum
		|| players[displayplayer].mo->subsector->sector->ceilingpic == sky4flatnum
		|| players[displayplayer].mo->subsector->sector->ceilingpic == sky5flatnum
		|| players[displayplayer].mo->subsector->sector->ceilingpic == sky6flatnum
		|| players[displayplayer].mo->subsector->sector->ceilingpic == sky7flatnum
		|| players[displayplayer].mo->subsector->sector->ceilingpic == sky8flatnum
		|| players[displayplayer].mo->subsector->sector->ceilingpic == sky9flatnum);
		else
		{
			...
			for (y=yl ; y<=yh ; y += FRACUNIT*64)
				for (x=xl ; x<=xh ; x += FRACUNIT*64)
				{
					...
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky2flatnum
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky3flatnum
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky4flatnum
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky5flatnum
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky6flatnum
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky7flatnum
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky8flatnum
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky9flatnum) // Found the outdoors!
					{
		...
		if(P_Random() < 2)
		{
			...
			for(i=0; i<numsectors; i++, ss++)
			{
				...
				|| ss->ceilingpic == sky2flatnum
				|| ss->ceilingpic == sky3flatnum
				|| ss->ceilingpic == sky4flatnum
				|| ss->ceilingpic == sky5flatnum
				|| ss->ceilingpic == sky6flatnum
				|| ss->ceilingpic == sky7flatnum
				|| ss->ceilingpic == sky8flatnum
				|| ss->ceilingpic == sky9flatnum) // Only for the sky.
...
void P_SpawnPlayer (mapthing_t* mthing, int playernum)
{
	...
	// Copy in the PSO stuff from the dead body...
	#ifdef SPMOVES
	for(i=0; i<NUMPSOITEMS;i++)
		psoitems[i]=p->psoitems[i];
	equiparmor = p->equiparmor;
	equipslot1 = p->equipslot1;
	equipslot2 = p->equipslot2;
	equipslot3 = p->equipslot3;
	equipslot4 = p->equipslot4;
	equipbarrier = p->equipbarrier;
	equipmag = p->equipmag;
	psolevel = p->psolevel;
	psoscore = p->psoscore/2; // Onos, teh "divide experience by 2 for death" penalty!
	#endif
	...
	// Write out the PSO stuff copied from the dead body...
	#ifdef SPMOVES
	for(i=0; i<NUMPSOITEMS;i++)
		p->psoitems[i]=psoitems[i];
	p->equiparmor = equiparmor;
	p->equipslot1 = equipslot1;
	p->equipslot2 = equipslot2;
	p->equipslot3 = equipslot3;
	p->equipslot4 = equipslot4;
	p->equipbarrier = equipbarrier;
	p->equipmag = equipmag;
	p->psolevel = psolevel;
	p->psoscore = psoscore;
	#endif
...
//        P_AddAmbientSfx(mthing->type-1200);
	...
	if(!(cv_gametype.value == GT_MATCH || cv_gametype.value == GT_CTF || cv_gametype.value == GT_TAG)
		...
		#ifdef CIRCUITMODE
		&& !(cv_gametype.value == GT_CIRCUIT && cv_circuit_ringthrow.value)
		#endif
	...
	#ifdef SPMOVES
	else if(i == MT_PAYMACHINE || i == MT_PSOTELLER)
	{
		if(mthing->options & 16) // Green shield / Quest
			mobj->fuse = 2;
		else if(mthing->options & 32) // Red shield / Bank
			mobj->fuse = 3;
		else if(mthing->options & 64) // Black shield / Shop
			mobj->fuse = 4;
		else if(mthing->options & 128) // Yellow shield
			mobj->fuse = 5;
		else if(mthing->options & 256) // Item #6
			mobj->fuse = 6;
		else if(mthing->options & 512) // Item #7
			mobj->fuse = 7;
		else if(mthing->options & 1024) // Item #8
			mobj->fuse = 8;
		else if(mthing->options & 2048) // Item #9
			mobj->fuse = 9;
		else if(mthing->options & 4096) // Item #10
			mobj->fuse = 10;
		else // Blue shield / None
			mobj->fuse = 1;
	}
	#endif
...
*/
			...
			if(xmasmode && (i&1))
				mobj->flags =  (mobj->flags & ~MF_TRANSLATION) | (15<<MF_TRANSSHIFT); // Yellow

            #ifdef SPMOVES
			if(fallmode && (i&1))
				mobj->flags =  (mobj->flags & ~MF_TRANSLATION) | (14<<MF_TRANSSHIFT); // Black
			if(fallmode && !(i&1))
				mobj->flags =  (mobj->flags & ~MF_TRANSLATION) | (5<<MF_TRANSSHIFT); // Orange
			#endif
						 ...
						 fixed_t z)
{
...
#ifdef ACBOT
        if (cv_predictingmonsters.value)
        {
                boolean canHit;
                fixed_t             px, py, pz;
                int time, t;
    ...
    // chaos controlling player
    if (dest->flags2 & MF2_DONTDRAW && dest->player && !dest->player->powers[pw_flashing])
    {
                         ...
                         mobjtype_t    type )
{
	...
	if(source->type == MT_TURRET) // More accurate!
		an = R_PointToAngle2 (source->x, source->y, dest->x+(dest->momx*(fixed_t)(3*gameskill)), dest->y+(dest->momy*(fixed_t)(3*gameskill)));
	else
