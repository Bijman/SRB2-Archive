...
void P_CameraThinker(player_t *player, camera_t *thiscam)
{
	...
	if (thiscam->momx || thiscam->momy)
	{
		...
		do
		{
			// Camera is a little less restricted here
#ifdef THINGSCALING
			...
			{
				if (!P_CheckSight(&dummy, player->mo)) // SRB2CBTODO: "P_CheckCameraSight" or "P_CameraCheckSight" instead.
	...
	if (thiscam->momz)
	{
		...
		// clip movement
		if (thiscam->z <= thiscam->floorz+thiscam->height) // hit the floor
		{
#ifdef ESLOPE
			...
			// When a camera is up on a steep slope, the camera needs to not reset
#ifdef ESLOPE
			if (!(thiscam->subsector->sector->f_slope || thiscam->subsector->sector->c_slope))
#endif
			{
		...
		if (thiscam->z + thiscam->height > thiscam->ceilingz)
		{
			...
			if (thiscam->z + thiscam->height < player->mo->z)
				P_ResetCamera(player,&camera); // SRB2CBTODO: ESLOPE: Support ceiling slopes
		}
	...
	else switch (mobj->type)
	{
		...
		case MT_BIGAIRMINE:
		{
								   ...
								   mobj->tracer->z - mobj->z) <= mobj->info->radius * (16 + (mobj->spawnpoint->angle/(ANG45/45))))
			{
				...
				P_HomingAttack(mobj, mobj->tracer, false);
