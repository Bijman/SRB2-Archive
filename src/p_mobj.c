...
#ifndef SRB2PLUS // in the old system flashingtics did not decrement until your painstate ended
...
#ifdef SRB2PLUS // cap z momentum to avoid wonky colissions
	if (mo->momz > MAXMOVE)
		mo->momz = MAXMOVE;
	else if (mo->momz < -MAXMOVE)
		mo->momz = -MAXMOVE;
#endif
}
...
/*	if (mo->pmomz && mo->z > mo->floorz && !(mo->player->pflags & PF_JUMPED))
	...
	if (P_IsObjectOnGround(mo))
	{
		...
		// Get up if you fell. // SRB2PLUS todo
...
#ifdef SRB2PLUS // give flamejets their source so you get knocked away from the flame source
                    flame->target = mobj;
#endif
