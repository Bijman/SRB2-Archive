...
#ifdef RABBIT
#include "CS_Global.h" // LXShadow: My header file is SPEZIALE.
#endif
...
void P_EmeraldManager(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		if (!players[i].mo)
			continue;
#else
        if (i != self_id && (! virtualplayers[i].in_game || ! virtualplayers[i].mo))
            continue;

        if (i == self_id && (! playeringame[0] || ! players[0].mo))
            continue;

        if (i != self_id) powers = virtualplayers[i].powers;
        else powers = players[0].powers;
#endif
...
{
#ifdef RABBIT // 60*FRACUNIT just isn't enough!
	if (mo->type == MT_REDRING || mo->type == MT_THROWNAUTOMATIC || mo->type == MT_THROWNBOUNCE ||
        mo->type == MT_THROWNEXPLOSION || mo->type == MT_THROWNSCATTER)
        return;
#endif
	if (mo->momx > MAXMOVE)
		mo->momx = MAXMOVE;
	...
	switch (mo->type)
	{
		...
		case MT_VERTICALFLAMEJET:
			if (!(mo->flags & MF_BOUNCE))
				return;
			break;
//#ifdef RABBIT // LXShadow: This is a general bugfix - not just netcode.
        case MT_REDFLAG:
        case MT_BLUEFLAG:
        	if (((mo->z <= mo->subsector->sector->floorheight && !(mo->eflags & MFE_VERTICALFLIP))
            || (mo->z + mo->height >= mo->subsector->sector->ceilingheight && (mo->eflags & MFE_VERTICALFLIP)))
...
static boolean P_ShieldLook(mobj_t *thing, powertype_t power)
{
    ...
    for (i = 0; i < MAXPLAYERS; i ++)
    {
        if (virtualplayers[i].mo == thing->target)
            player = &virtualplayers[i];
    }
#endif
    ...
    {
        if (! thing->target || ! player->in_game || ! player->powers[power] || player->powers[pw_invulnerability] ||
            player->powers[pw_super] || player->spectator)
        {
            P_SetMobjState(thing, S_DISS);
            player->shield = NULL;
...
void P_MobjThinker(mobj_t *mobj)
{
	...
	if (mobj->flags & MF_SCENERY)
	{
		switch (mobj->type)
		{
			...
			case MT_YELLOWORB:
			case MT_BLUEORB:
				if (!P_AddShield(mobj)) //CSTODO: code is completely different here, not sure if broken or not.
					return;
				break;
			case MT_WATERDROP:
				if (mobj->z <= mobj->floorz
				...
				case MT_REDFLAG:
					if (mobj->spawnpoint)
					{
						...
						else // MT_BLUEFLAG
						{
							...
							blueflag = NULL;
#ifdef RABBIT
							flagstate[2] = FLAGSTATE_BASE; // LXShadow added this (flag returns).
#endif
						}
					}
					P_SetMobjState(mobj, S_DISS);
...
#ifdef RABBIT
// CSTODO: May need to be updated for what I really want.
