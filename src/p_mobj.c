...
#include "JTE/Universe/SU_Info.h" // UNIVERSE
...
#include "JTE/JTEMisc.h" // MOBJSCALE

#ifdef UNIVERSE
boolean su_bossmap = false;
#endif
...
#ifdef SPMOVES
		if(mobj->player
			&& !(mobj->player->charflags & SF_HMS123)
			&& mobj->player->chaoscontrol
			&& state >= S_PLAY_ATK1
			&& state <= S_PLAY_ATK4)
		{
			const statenum_t temp = state - S_PLAY_ATK1 + S_PLAY_SPD1;
			mobj->sprite = states[temp].sprite;
			mobj->frame = states[temp].frame;

#ifdef CCPAL
			// NO TRANSPARENT PLAYER IN THE CHAOS CONTROL PALETTE!
			if(colormaps != neoncolormaps)
			// Since I don't want to bother adding v_video.h just for this little bit,
			// I'll just check for which palette's on by using the colormap pointer...
			// YAY FOR EXTREME LAZYNESS!
#endif
			mobj->frame |= (tr_transfir<<FF_TRANSSHIFT);
		}
#endif
...
#ifdef DARKWORLD
	DW_Shadow(mobj); // Update your shadow! >.>
#endif
...
#ifdef HOLIDAYS
		// Replace sprites and frames. :)
		// Damn this is a sucky way of doing it, but I don't want
		// "replace" changes to be as permanent as a normal SOC...
		for(replace = holidayreplace, last = NULL; replace; replace = replace->next)
		{
			if(replace->type == 2 && !strcmp(sprnames[mobj->sprite],replace->original))
			{
				snprintf(replace->original,8,"%d",mobj->sprite);
				replace->original[8] = '\0';
				for(sprite = 0; sprite < NUMSPRITES; sprite++)
					if(!strncmp(sprnames[sprite],replace->replace,4))
					{
						mobj->sprite = sprite;
						break;
					}
				...
				{
					CONS_Printf("You screwed up, dude. The sprites %s aren't on the list.\n",replace->replace);
					if(last)
						last->next = replace->next;
					else
					{
						holidayreplace = replace->next;
						last = holidayreplace;
					}
					Z_Free(replace);
					replace = last;
				...
				else if(replace->replace[4] != '\0')
				{
					mobj->frame = replace->replace[4] - 'A';
					snprintf(replace->replace,8,"%d",mobj->frame+mobj->sprite*64);
					replace->type = 3;
				}
				else
				{
					snprintf(replace->replace,8,"%d",mobj->sprite);
					replace->type = 4;
				}
				replace->replace[8] = '\0';
			}
			else if(replace->type == 3 && mobj->sprite == (unsigned)atoi(replace->original))
			{
				mobj->sprite = atoi(replace->replace)/64;
				mobj->frame = atoi(replace->replace)%64;
			}
			else if(replace->type == 4 && mobj->sprite == (unsigned)atoi(replace->original))
				mobj->sprite = atoi(replace->replace);
			last = replace;
		}
#endif
...
#ifdef JTEMISC
	// Much cleaner: Let the deton do it's fancey stuff here, but still use P_KillMobj.
	// This way, all the special stuff that normally happens when an object is killed
	// will even happen to missiles when they hit something. (Detons have animals too!)
	// Only thing left to be desired: Getting points from an enemy for it...
	if (mo->type == MT_DETON)
	{
...
#ifdef SPMOVES
	// Friction in the air for flying Super Metal Sonic
	&& (!mo->player || !(mo->player->charflags & SF_METAL)
	|| !mo->player->powers[pw_super] || mo->state-states == S_PLAY_PLG1)
#endif
	)
...
#ifdef SCALEGRAVITY
		if(mo->scale != 100)
			gravityadd = FIXEDSCALE(gravityadd,mo->scale);
#endif
...
static void P_PlayerZMovement(mobj_t *mo)
{
	...
	if (mo->subsector->sector->ffloors)
	{
		...
		for (rover = mo->subsector->sector->ffloors; rover; rover = rover->next)
		{
			...
			if ((!(rover->flags & FF_SOLID || rover->flags & FF_QUICKSAND) && !(mo->player && !mo->player->nightsmode
#ifdef HOMINGFIX
			&& !mo->player->homing
#endif
			&& (((mo->player->charflags & SF_RUNONWATER) || mo->player->powers[pw_super]) && mo->ceilingz-*rover->topheight >= mo->height) && (rover->flags & FF_SWIMMABLE) && !mo->player->mfspinning && mo->player->speed > mo->player->runspeed && /*mo->ceilingz - *rover->topheight >= mo->height && */mo->z < *rover->topheight + 30*FRACUNIT && mo->z > *rover->topheight - 30*FRACUNIT)))
				continue;
...
#ifdef UNIVERSE
		&& (!universe || mo->health > 0)
#endif
		)
			P_SetPlayerMobjState(mo, S_PLAY_STND);
#ifdef UNIVERSE
		else if(universe && mo->health <= 0)
...
#ifdef SCALESIZE
				if(mo->scale != 100)
					mo->player->deltaviewheight = FIXEDSCALE(mo->player->deltaviewheight,mo->scale);
#endif
			}
...
playergravity:
		...
		{
			const int temp = SPM_MetalSonicFly(mo->player);
			if(temp == 1)
				gravityadd = 0;
			else if(temp == 2)
				gravityadd = gravity;
...
void P_MobjCheckWater(mobj_t *mobj)
{
	...
	if (sector->ffloors) // 3D water
	{
		...
		for (rover = sector->ffloors; rover; rover = rover->next)
		{
			if (!(rover->flags && FF_EXISTS) || !(rover->flags & FF_SWIMMABLE)
				...
				continue;
#ifdef SCALESIZE
			if (*rover->topheight < mobj->z || *rover->bottomheight > (mobj->z + FIXEDSCALE(mobj->info->height/2,mobj->scale)))
#else
...
#ifdef SCALESIZE
			if (mobj->z + FIXEDSCALE(mobj->info->height,mobj->scale) > *rover->topheight)
#else
...
#ifdef SCALESIZE
			if (mobj->z + FIXEDSCALE(mobj->info->height/2,mobj->scale) < *rover->topheight)
#else
...
#endif
			{
...
#ifdef SPMOVES
						&& !(mobj->player->charflags & SF_NODROWN)
#endif
						)
					{
				...
				{
					// First of all, kick the rider out...
					if(mobj->movecount == 1)
					{
						if(mobj->target // If the barrel is holding someone...
						&& mobj->target->health) // and they're alive...
							SU_BarrelPop(mobj->target); // Pop them out of the barrel!
					}

					// Barrel is now filled with lava!
					if(rover->master->frontsector->special == 7
					|| rover->master->frontsector->special == 519)
						mobj->movecount = 3;
					// Barrel is now filled with slime!
					else if(rover->master->frontsector->special == 983
						|| rover->master->frontsector->special == 984)
						mobj->movecount = 4;
					// Barrel is now filled with just regular water...
					else
						mobj->movecount = 2;
				}
#endif
		...
		&& ((mobj->eflags & MF_UNDERWATER) != wasinwater))
	{
...
#ifdef SCALESIZE
		if (mobj->watertop-mobj->floorz <= FIXEDSCALE(mobj->info->height,mobj->scale)>>1)
#else
		...
		{
#ifdef SCALESIZE
			if (mobj->z+(FIXEDSCALE(mobj->info->height,mobj->scale)>>1)-mobj->momz >= mobj->watertop)
#else
...
static void P_SceneryCheckWater(mobj_t *mobj)
{
	...
	if (sector->ffloors)
	{
		...
		for (rover = sector->ffloors; rover; rover = rover->next)
		{
...
#ifdef SCALESIZE
				|| *rover->bottomheight > (mobj->z + (FIXEDSCALE(mobj->info->height,mobj->scale) >> 1)))
#else
			...
			mobj->waterbottom = *rover->bottomheight;
#ifdef SCALESIZE
			if (mobj->z + (FIXEDSCALE(mobj->info->height,mobj->scale) >> 1) < *rover->topheight)
#else
...
void P_Attract(mobj_t *source, mobj_t *enemy, boolean nightsgrab) // Home in on your target
{
	...
	else if(russia && !enemy->player->powers[pw_ringshield])
		speedmul = 4*FRACUNIT;
#endif
	else
...
static boolean P_ShieldLook(mobj_t *thing, powertype_t power)
{
	...
	if (!thing->target || thing->target->health <= 0 || !thing->target->player
		|| !thing->target->player->powers[power]
		|| thing->target->player->powers[pw_super]
		|| thing->target->player->powers[pw_invulnerability] > 1
#ifdef SPMOVES
		|| thing->target->player->chaoscontrol // No shield on Chaos Controlling players!
#endif
		)
	{
...
void P_MobjThinker(mobj_t *mobj)
{
...
#ifdef RUSSIA
	if(mobj->player && mobj->player->ring)
		return;
#endif

#ifdef DARKWORLD
	if(mobj->type == MT_DARKSHADOW) // If you're a dark world shadow...
	{
		// And you're invalid because you're object is gone or something...
		if(!mobj->darkshadow || !mobj->darkshadow->darkshadow)
		{
			if(mobj->player)
				P_SetPlayerMobjState(mobj, S_DISS);
			else
				P_SetMobjState(mobj, S_DISS); // Then disappear as well!
		}
		return;
	}
	if(!(mobj->flags & MF_SCENERY)) // And if you're not a scenery object...
		DW_Shadow(mobj); // Update your shadow! >.>
#endif
	...
	{
		unsigned int speed = (abs(mobj->scale - mobj->destscale)>>8);
		if(speed > 1)
			speed *= mobj->scalespeed;
		else
			speed = mobj->scalespeed;

		if((unsigned)abs(mobj->scale - mobj->destscale) < speed)
			JTE_SetScale(mobj,mobj->destscale);
		else if(mobj->scale > mobj->destscale)
			JTE_SetScale(mobj,(unsigned short)(mobj->scale - speed));
		else if(mobj->scale < mobj->destscale)
			JTE_SetScale(mobj,(unsigned short)(mobj->scale + speed));
	}
#endif
...
#ifdef THOKTARGET
		case MT_THOK:
			// Thok object takes the sprite of it's target!
			if(mobj->target
#ifndef SCALEMISC
			&& !(mobj->target->player && mobj->target->player->mfstartdash)
#endif
			)
			{
				mobj->angle = mobj->target->angle;
				mobj->sprite = mobj->target->sprite;
				mobj->frame = mobj->target->frame;
				mobj->frame &= ~FF_TRANSMASK;
				mobj->frame |= tr_transfir<<FF_TRANSSHIFT;
				mobj->skin = mobj->target->skin;
				//mobj->player = mobj->target->player; // Unstable... :x Dun wanna bother fixing...
#ifdef SCALEMISC
				// Scale to the target's size, then start scaling down.
				JTE_SetScale(mobj,(unsigned short)(4*mobj->target->scale/3));
				mobj->destscale = (unsigned short)(mobj->target->scale/2);
				mobj->tics = -1;
#endif
			}
			mobj->target = NULL;
			if(mobj->scale == mobj->destscale && mobj->tics < 0)
				P_SetMobjState(mobj, S_DISS);
			break;
#endif
#ifdef SCALEMISC
			case MT_HYPERSPARK:
				JTE_SetScale(mobj,mobj->target->scale);
...
#endif
#ifdef SPRITEOFFS
		case MT_EGGMOBILE_FG:
		case MT_EGGMOBILE_BG:
			if(!mobj->target)
			{
				P_SetMobjState(mobj, S_DISS);
				break;
...
#if defined(MULTIHPENEMIES) || defined(SU)
	if(mobj->flags & MF_ENEMY && mobj->type != MT_CRAWLACOMMANDER)
	{
		if(mobj->fuse)
		{
			if(mobj->fuse & 1)
...
#ifdef RUSSIA
	// In Soviet Russia, mine drops YOU!
	else if(!mobj->player && mobj->flags2 & MF2_DEBRIS && !mobj->fuse && mobj->z <= mobj->floorz)
	{
		mobj->flags = mobj->info->flags;
		mobj->flags2 &= ~MF2_DEBRIS;
	}
	// In Soviet Russia, zoom attack launches YOU!
	else if (mobj->player && mobj->flags2 & MF2_SKULLFLY)
	{
		mobj_t *spawnmobj;
		spawnmobj = P_SpawnMobj(mobj->x, mobj->y, mobj->z, MT_THOK);
		spawnmobj->target = mobj;
		spawnmobj->flags = (spawnmobj->flags & ~MF_TRANSLATION) | (1<<MF_TRANSSHIFT);
	}
#endif
...
#ifdef SPMOVES
				if(mobj->target->player // If you're chasing a player...
				&& mobj->target->player->charflags & SF_METAL // And the player is Metal Sonic
				&& (mobj->target->player->jet != mobj // And the player doesn't have a jet or the player's jet is not you...
				|| mobj->target->player->playerstate != PST_LIVE)) // Or the player is not alive...
				{ // Then go away! I dun need you!
					P_SetMobjState(mobj, S_DISS);
					return;
				}
#endif
...
#ifdef SPMOVES
				else if(mobj->fuse == 59) // Metal Sonic's jet
				{
					jetx = mobj->target->x + P_ReturnThrustX(mobj->target, mobj->target->angle, -16*FRACUNIT);
					jety = mobj->target->y + P_ReturnThrustY(mobj->target, mobj->target->angle, -16*FRACUNIT);
					P_UnsetThingPosition(mobj);
					mobj->x = jetx;
					mobj->y = jety;
					mobj->z = mobj->target->z + 16*FRACUNIT;
					mobj->floorz = mobj->z;
					mobj->ceilingz = mobj->z+mobj->height;
					P_SetThingPosition(mobj);
				}
		...
		if (!mobj->fuse)
		{
			...
			switch (mobj->type)
			{
				...
				case MT_EGGMOBILE:
				{
					...
					// (Game modified/infected while playing?)
					if(modifiedgame || virus || netgame || gamemap != 3)
						break;
					// You were defeated?
					if(!mobj->health)
						break;
					// Somebody died? (Death does not part us...)
					for(i = 0; i < MAXPLAYERS; i++)
						if(playeringame[i] && !players[i].health)
							break;
					if(i != MAXPLAYERS)
						break;
					...
					// Eggmobile is no longer needed, make it just a scenery object.
					P_UnsetThingPosition(mobj);
					mobj->flags = MF_NOTHINK|MF_NOBLOCKMAP|MF_NOCLIP|MF_NOGRAVITY|MF_SCENERY;
					P_SetThingPosition(mobj);
				}
				break;
#endif
...
mobj_t *P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
...
#ifdef UNIVERSE
	if(universe && mobj->flags & MF_ENEMY)
		mobj->health *= 15;
	else if(universe && mobj->flags & MF_BOSS)
		mobj->health *= 8;
#endif
	...
	// Depending on the type of object, they may scale up/down faster.
	switch(type)
	{
		case MT_THOK:
			mobj->scalespeed = 12;
	...
	switch (mobj->type)
	{
		...
		case MT_REDRING: // Make MT_REDRING red by default
			...
			break;
#ifndef JTEMISC // Why only if the game is not modified? Try always MF_NOBLOCKMAP unless SOCed!
		case MT_BUSH:
		case MT_BERRYBUSH:
		case MT_THZPLANT:
		case MT_GFZFLOWER1:
		...
		default:
#ifndef JTEMISC // Don't check this here.
...
#ifdef JTEMISC // Any object can be a monitor. No hardcoding! :E
	if(maptol & TOL_ADVENTURE && mobj->flags & MF_MONITOR)
		mobj->flags &= ~MF_SOLID;
#endif
...
void P_RemoveMobj(mobj_t *mobj)
{
	...
	if(mobj->darkshadow)
	{
		mobj->darkshadow->darkshadow = NULL;
		P_RemoveMobj(mobj->darkshadow);
	}
#endif
...
#ifdef SCALEVAR
	// Send your scale again when you spawn.
	if(p-players == consoleplayer
	&& cv_myscale.value != 100)
		JTE_SendMyScale();
#endif

/*#ifdef JTEMISC // Save bonustime, you needs it. ... I think.
...
void P_SpawnMapThing(mapthing_t *mthing)
{
...
#ifdef UNIVERSE
		if(universe && mthing->type == 1 && gamemap != SULOBBYMAP && gamemap != SUHUBMAP && !su_bossmap)
		{
			// Spawn the start teleport behind player 1.
			x = mthing->x << FRACBITS;
			y = mthing->y << FRACBITS;
			ss = R_PointInSubsector(x, y);
			if(mthing->options >> 5)
			...
			mobj = P_SpawnMobj(x, y, z, MT_STARTTELEPORT);
...

			{
				...
				while(dist > 0)
				{
					x = mobj->x + P_ReturnThrustX(mobj, mthing->angle, -dist*FRACUNIT);
					y = mobj->y + P_ReturnThrustX(mobj, mthing->angle, -dist*FRACUNIT);

					if(R_PointInSubsector(x,y)->sector != R_PointInSubsector(mobj->x,mobj->y)->sector
						|| R_PointInSubsector(x-mobj->radius,y-mobj->radius)->sector != R_PointInSubsector(mobj->x,mobj->y)->sector
						|| R_PointInSubsector(x+mobj->radius,y+mobj->radius)->sector != R_PointInSubsector(mobj->x,mobj->y)->sector)
					{
						dist--;
					}
	...
	else if (i == MT_EMERHUNT)
	{
		...
		mthing->z = (short)((ss->sector->floorheight/FRACUNIT) + (mthing->options >> 4));
...
#ifdef UNIVERSE
	// Place the end teleport where the end of level sign would be...
	if(universe && i == MT_SIGN)
	{
		i = mthing->type = MT_ENDTELEPORT;
	}

	// Wait... This is a boss map?! Crap, remove the start teleport!
	if(universe && !su_bossmap && mobjinfo[i].flags & MF_BOSS)
	{
		thinker_t* think;
		mobj_t* mo;

		// Scan the thinkers for the start teleport...
		for(think = thinkercap.next; think != &thinkercap; think = think->next)
		{
			// This isn't a mobj...
			if(think->function.acp1 != (actionf_p1)P_MobjThinker)
			...
			// Now then... Is it a start teleport?
			mo = (mobj_t*)think;
			if(mo && mo->type == MT_STARTTELEPORT)
			{
				// Found it! Remove it and take 5.
				P_RemoveMobj(mo);
				break;
			}
			// Didn't find it? Then loop... It's in here somewhere...
		}
		// Either way, mark this as a boss map.
		su_bossmap = true;
	}
#endif
	...
	if(universe)
	{
		// No starposts in SRB2 Universe.
		if(i == MT_STARPOST)
			return;

		// Rare versions of mobjs randomly swapped for the regular ones.
		{
			int loop = 0;
			sumobjcheck:
			loop++;
			if(sumobjinfo[i] && loop < 10)
			{
				if(sumobjinfo[i]->raretype != MT_DISS
				&& sumobjinfo[i]->raretype != i)
				{
					if(sumobjinfo[i]->raretyperarity == 255)
					{
						// This is a rare monster that was placed on the map.
						// Swap it for it's normal version and check against that instead.
						i = sumobjinfo[i]->raretype;
						goto sumobjcheck;
					}
					// P_Random()/2.55f returns a number between 0 and 100
					// raretyperarity should not be greater then 100 unless it is 255, as handled above.
					// If you wish to swap this division by a float for multiplication, use
					//else if(P_Random()*40 < sumobjinfo[i]->raretyperarity*102)
					// instead, as this will have the same effect.
					else if(P_Random()/2.55f < sumobjinfo[i]->raretyperarity)
					{
					// Otherwise, lets swap it at random.
					// Lower rarity value = More common 'rare' versions.
						i = sumobjinfo[i]->raretype;
					}
				}
			}
...
#ifdef UNIVERSE
	else if(mobj->type == MT_PAYMACHINE)
	{
		mobj->health = mthing->angle;
		mobj->fuse = mthing->options;
		z = ONFLOORZ;
	}
#endif
...
void P_SpawnHoopsAndRings(mapthing_t *mthing)
{
...
#ifdef UNIVERSE
	if(universe) // No rings or wings in SRB2 Universe!
		return;
#endif
	...
	if (mthing->type == 57) // NiGHTS hoop!
	{
		...
		hoopcenter->movecount = AngleFixed(closestangle)>>FRACBITS;
...
#ifdef HOLIDAYS
			else if(holiday == H_WEEN)
			{
				if(i&1)
					mobj->flags = (mobj->flags & ~MF_TRANSLATION)
						 | (1<<MF_TRANSSHIFT); // Grey
				else
					mobj->flags = (mobj->flags & ~MF_TRANSLATION)
						 | (5<<MF_TRANSSHIFT); // Orange
			}
#endif
...
#ifdef RUSSIA
	// In Soviet Russia, missile shoots YOU!
	if(russia)
	{
		...
		source->angle = an;
		an >>= ANGLETOFINESHIFT;
		source->momx = FixedMul(speed, finecosine[an]);
		source->momy = FixedMul(speed, finesine[an]);
	}
	else
	{
#endif
...
#ifdef RUSSIA
	// In Soviet Russia, missile shoots YOU!
	if(russia && type == MT_TURRETLASER)
		source->momz = (dest->z + (dest->momz*gsf) - z) / dist;
	else if(russia)
		source->momz = (dest->z - z) / dist;
	else
#endif
...
mobj_t *P_SPMAngle(mobj_t *source, mobjtype_t type, angle_t angle)
{
	...
	if
		...
		(source->player->bot ||
#endif
		(source->player->autoaim_toggle && cv_allowautoaim.value
