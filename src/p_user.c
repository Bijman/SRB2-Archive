 ...
 */
	...
	if (player->charability == CA_HOMINGTHOK)
	{
		...
		if (player->homing && player->mo->tracer)
		{
			...
			P_HomingAttack(player->mo, player->mo->tracer, true);
			...
			if (player->mo->tracer->health <= 0 || (player->mo->tracer->flags2 & MF2_FRET))
			{
				if (player->mo->eflags & MFE_UNDERWATER)
				{
					P_SetObjectMomZ(player->mo, FixedDiv(457*FRACUNIT,72*FRACUNIT), false, false);
				}
				else
					P_SetObjectMomZ(player->mo, 10*FRACUNIT, false, false);
...
// Home in on your target, option to change the angle of the source to point to target
void P_HomingAttack(mobj_t *source, mobj_t *enemy, boolean changeangle)
{
	fixed_t dist;
	...
	{
		const fixed_t ns = source->player->actionspd; // SRB2CBTODO: *FRACUNIT?
...
void P_MoveChaseCamera(player_t *player, camera_t *thiscam, boolean netcalled)
{
	...
	if (twodlevel || (mo->flags2 & MF2_TWOD))
		angle = ANG90;
	else if ((twodlevel || (mo->flags2 & MF2_TWOD)) && player->twodcamangle) // SRB2CBTODO:
		angle = player->twodcamangle;
	else if (camstill)
		angle = thiscam->angle;
	...
	if (twodlevel || (mo->flags2 & MF2_TWOD))
	{
		if (player->twodcamangle)
			dist = (FIXEDSCALE(480 + player->twodcamdist, mo->scale)) << FRACBITS;
		else
			dist = (FIXEDSCALE(480, mo->scale)) << FRACBITS;
	}
	else
	{
		dist = camdist;
	...
	if (playerslope) // VPHYSICS: Use real vectors to find diff in camera height
	{
		if (mo->player)
		{
...
*/
	...
	{
		if (P_AproxDistance(mo->x - thiscam->x, mo->y - thiscam->y) > checkdist * 6
			|| abs(mo->z - thiscam->z) > checkdist * 5)
			P_ResetCamera(player, thiscam);
	}
