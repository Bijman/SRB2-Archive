...
/*					if (!ultimatemode && players[i].marescore > 50000
	...
	if (gametype == GT_TEAMMATCH || (gametype == GT_COMPETITION && cv_teams.value == 1))
	{
		if (player->ctfteam == 1)
			redscore += amount;
...
void P_DoJump(player_t *player, boolean soundandstate)
{
	...
	if (soundandstate)
	{
		if (!player->spectator)
			...
			if (player->charability != CA_DOUBLEJUMP)
			{
		    S_StartSound(NULL, sfx_jump); // Play jump sound!
			}
			
			if (player->charability == CA_DOUBLEJUMP && onground)
			{
		    S_StartSound(NULL, sfx_jump); // Play jump sound!
			}
			...
			switch (player->charability)
			{
				...
				case CA_DOUBLEJUMP: // Double-Jump
					if (!(player->pflags & PF_THOKKED))
					{
						...
						S_StartSound(NULL, player->mo->info->attacksound); // Play the THOK sound
...
static void P_DeathThink(player_t *player)
{
	...
	else if (player->lives > 0 && !G_IsSpecialStage(gamemap)) // Don't allow "click to respawn" in special stages!
	{
		...
		if ((cmd->buttons & BT_JUMP) && player->deadtimer > TICRATE && (gametype == GT_COOP))
			player->playerstate = PST_REBORN;
		
		//FUCK YOU
		if ((cmd->buttons & BT_JUMP) && player->deadtimer > 2*TICRATE+20 && (gametype == GT_COMPETITION))
			player->playerstate = PST_REBORN;
