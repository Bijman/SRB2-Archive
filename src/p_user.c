...
{
#ifdef RABBIT
    return; // Disable race player exit.
#endif
	if (player->exiting)
		return;
...
void P_DoJump(player_t *player, boolean soundandstate)
{
	...
	if (soundandstate)
	{
#ifdef RABBIT // This is cool, event based stuff
		if ((!player->spectator) && soundandstate)
			CS_AddEvent(1 | (sfx_jump << 5));
#endif
...
#ifdef RABBIT
	CS_AddEvent(EVENT_SOUNDPLAY | (2 << 5));
#endif
}
...
#ifdef RABBIT // CSTODO: add a flag name here?
								CS_AddEvent(3);
#endif
							}
						}
...
static void P_MovePlayer(player_t *player)
{
		...
		((gametype == GT_MATCH && cv_matchtype.value) || gametype == GT_CTF))
	{
		if (cv_allowteamchange.value)
		{
            ...
            for (i = 0; i < 32; i ++)
            {
                if (i == self_id || ! virtualplayers[i].in_game) continue;

                if (virtualplayers[i].ctfteam == 1) red ++;
                if (virtualplayers[i].ctfteam == 2) blue ++;
            }
#endif
...
#ifdef RABBIT // Still here for reference, LXShadow
		//        CS_AddEvent(4);
#endif
	}
...
#ifdef RABBIT // CSTODO: Make sure all the weapon rings get a correct network sync
			CS_SendPlayerFire((cmd->buttons & BT_FIRENORMAL) ? 0 : player->currentweapon, 0);
#endif
...
#ifdef RABBIT
			CS_SendPlayerFire(WEP_GRENADE, player->tossstrength);
#endif
...
#else // Race Fix #1
            for (i = 0; i < MAXPLAYERS; i ++)
                if ((i == self_id && players[0].laps < (unsigned) cv_numlaps.value && players[0].lives > 0) ||
                    (i != self_id && virtualplayers[i].in_game && virtualplayers[i].laps < (unsigned) cv_numlaps.value &&
                     ! virtualplayers[i].permadead))
                     break;
#endif
			if (i == MAXPLAYERS)
			{
...
#ifdef RABBIT
    if (csdisplayplayer != self_id && virtualplayers[csdisplayplayer].mo && virtualplayers[csdisplayplayer].in_game)
    {
        x = virtualplayers[csdisplayplayer].mo->x;
        y = virtualplayers[csdisplayplayer].mo->y;
        z = virtualplayers[csdisplayplayer].mo->z + (cv_viewheight.value << FRACBITS);
    }
#endif
...
#ifdef RABBIT
    if (csdisplayplayer != self_id && virtualplayers[csdisplayplayer].mo && virtualplayers[csdisplayplayer].in_game)
        thiscam->angle = virtualplayers[csdisplayplayer].mo->angle;
    else
#endif
...
void P_MoveChaseCamera(player_t *player, camera_t *thiscam, boolean netcalled)
{
	...
	netcalled = false; // CSTODO: kill warning
...
#ifdef RABBIT
    if (csdisplayplayer != self_id && virtualplayers[csdisplayplayer].mo && virtualplayers[csdisplayplayer].in_game)
        mo = virtualplayers[csdisplayplayer].mo;
#endif
...
#else
	{
		angle_t playeraiming = player->aiming;

		if (csdisplayplayer != self_id)
			playeraiming = virtualplayers[csdisplayplayer].aiming;

		if (player->mo->eflags & MFE_VERTICALFLIP)
		// oh jeez, what a headache!
			angle = R_PointToAngle2(0, thiscam->z, dist,mo->z + (FixedDiv(FixedMul(mo->info->height,3),4) >> 1)
				- ((mo->height != mo->info->height) ? (mo->info->height - mo->height) >> 1 : 0)
				+ (FINESINE((playeraiming>>ANGLETOFINESHIFT) & FINEMASK) * 64));
		else
			angle = R_PointToAngle2(0, thiscam->z, dist,mo->z + (P_GetPlayerHeight(player) >> 1)
				+ (FINESINE((playeraiming>>ANGLETOFINESHIFT) & FINEMASK) * 64));
	}
...
#ifdef RABBIT
        if (csdisplayplayer != self_id && virtualplayers[csdisplayplayer].powers[pw_gravityboots])
            *type = postimg_flip;
#endif
