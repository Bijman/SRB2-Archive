...
void P_DoPlayerPain(player_t *player, mobj_t *source, mobj_t *inflictor)
{
...
#ifdef SRB2PLUS // some weapons get extra vertical knockback
    fixed_t verticalspeed;
#endif
	player->mo->z++;
#ifndef SRB2PLUS // move this further down
...
#endif
#ifdef SRB2PLUS // necessary because some sources of damage might not have a proper source
    if (!source && inflictor && inflictor->flags & MF_MISSILE)
        CONS_Printf("No damage source!\n"); // so we can maybe go find it later so that it knocks back correctly too :)
#endif
	if (inflictor)
	{
...
#ifdef SRB2PLUS // scale weapon knockback based on distance/falloff, explosions based on blast angle
        if (source && inflictor->flags & MF_MISSILE)
        {
			fixed_t dist = P_AproxDistance(P_AproxDistance(source->x-player->mo->x, source->y-player->mo->y), source->z-player->mo->z);
			fixed_t inflictordist = P_AproxDistance(P_AproxDistance(inflictor->x-player->mo->x, inflictor->y-player->mo->y), inflictor->z-player->mo->z);

            if (inflictor->flags2 & MF2_AUTOMATIC)
            {
                 fixed_t automagicforce = 4*FRACUNIT - dist/64; // 256 (128)

                if (automagicforce < 2*FRACUNIT)
                    automagicforce = 2*FRACUNIT;

                fallbackspeed = automagicforce;
                verticalspeed = 0;
            }
            else if (inflictor->flags2 & MF2_BOUNCERING)
            {
                fixed_t bounceforce = 8*FRACUNIT - dist/128; // 1024 (512)

                if (bounceforce < 4*FRACUNIT || inflictor->threshold) // extra bounces do minimum knockback
                    bounceforce = 4*FRACUNIT;

                fallbackspeed = bounceforce;
                verticalspeed = 0;
            }
            else if (inflictor->flags2 & MF2_SCATTER)
            {
                fixed_t scatterforce = 64*FRACUNIT - dist/8; // 512 (480)

                if (scatterforce < 4*FRACUNIT)
                    scatterforce = 4*FRACUNIT;

                fallbackspeed = scatterforce;
                verticalspeed = 0;
            }
            else if (inflictor->flags2 & MF2_GRENADE)
            {
                fixed_t grenadeforce = 20*FRACUNIT - inflictordist/8; // 160 (96/128)
                fixed_t verticalforce = 10*FRACUNIT - abs(inflictor->z - player->mo->z)/8;

                if (grenadeforce < 8*FRACUNIT)
                    grenadeforce = 8*FRACUNIT;
                if (verticalforce < 4*FRACUNIT)
                    verticalforce = 4*FRACUNIT;

                fallbackspeed = grenadeforce;
                verticalspeed = verticalforce;
            }
            else if (inflictor->flags2 & MF2_EXPLOSION)
            {
                fixed_t grenadeforce = 20*FRACUNIT - inflictordist/8; // 160 (96/196)
                fixed_t verticalforce = 10*FRACUNIT - abs(inflictor->z - player->mo->z)/8;
            ...
            {
                fixed_t railforce = 24*FRACUNIT - dist/128; // 3072 (2816)

                if (railforce < 2*FRACUNIT)
                    railforce = 2*FRACUNIT;

                fallbackspeed = railforce;
                verticalspeed = 0;
            }
            else // red ring and enemy/boss projectiles?
            {
                fixed_t ringforce = 8*FRACUNIT - dist/128; // 1024 (512)

                if (ringforce < 4*FRACUNIT)
                    ringforce = 4*FRACUNIT;

                fallbackspeed = ringforce;
                verticalspeed = 0;
            }
        }
        else
        {
             // the usual amount of force
	...
	else
	{
		...
		fallbackspeed = 4*FRACUNIT;
#ifdef SRB2PLUS // cover default knockback
        verticalspeed = 0;
#endif
	}
...
#ifdef SRB2PLUS // apply either vertical momentum from explosions or default vertical momentum
    if (verticalspeed)
    {
        if (player->mo->eflags & MFE_UNDERWATER)
        {
            // approximately about 1.6% off...
             verticalspeed *= 3;
             verticalspeed /= 5;
        }
        if (player->mo->eflags & MFE_VERTICALFLIP) // handle this outside P_SetObjectMomZ as silly as it looks
            verticalspeed = -verticalspeed;
        if (inflictor->z > player->mo->z)
            P_SetObjectMomZ(player->mo, -verticalspeed, false);
        else
            P_SetObjectMomZ(player->mo, verticalspeed, false);
    }
    else
    {
        // default vertical knockback
        if (player->mo->eflags & MFE_UNDERWATER)
            P_SetObjectMomZ(player->mo, FixedDiv(10511*FRACUNIT,2600*FRACUNIT), false);
        else
            P_SetObjectMomZ(player->mo, FixedDiv(69*FRACUNIT,10*FRACUNIT), false);
...
//    CONS_Printf("H: %d, V: %d\n", fallbackspeed/FRACUNIT, verticalspeed/FRACUNIT);
#endif
	P_InstaThrust(player->mo, ang, fallbackspeed);

	if ((player->pflags & PF_ROPEHANG) || (player->pflags & PF_MINECART))
		P_SetTarget(&player->mo->tracer, NULL);
//#ifndef SRB2PLUS // penalty no longer required for tag as everything is based on kills now
...
//#ifndef SRB2PLUS // This is required for shield damage and stuff
...
#ifdef SRB2PLUS // losscount decrement on ring pickup
    // Decrement losscount for every fresh ring picked up. Losscount causes the fuse for dropped ring to be reduced
    // by one second per losscount to a minimum of one second. Old system losscount is refreshed upon picking up a
    // fresh ring or by having more than one ring, with fuse going down to a minimum of two seconds.
	if (!flingring)
	{
        if (player->losscount > 0)
            player->losscount -= num_rings;
        if (player->losscount < 0)
            player->losscount = 0;
...
#ifdef SRB2PLUS // no points on hit
    // big hack, but will implement better later
    // Do not award points for hits, only for kills.
    // No penalty for suicide, we will award points to the last inflictor instead
    if (amount == 50)
        return;
#endif
...
#ifdef SRB2PLUS // set jumpangle (used to calc thok) based on movement angle of this jump
    player->jumpangle = R_PointToAngle2(player->mo->x, player->mo->y, player->mo->x + player->mo->momx, player->mo->y + player->mo->momy);
#endif
	player->pflags |= PF_JUMPED;
					...
					if (player->pflags & PF_JUMPED)
					{
						...
						else // Otherwise, THOK!
						{
							if (!(player->pflags & PF_THOKKED) || (player->charability2 == CA2_MULTIABILITY))
							{
								mobj_t *item;
								// Catapult the player
#ifdef SRB2PLUS // thok takes previous thok force and previous thok angle into account
                                // Sonic can still turn on a dime, just not instantly at MAXMOVE both directions
                                fixed_t thokforce = player->speed<<FRACBITS;
                                INT32 anglediff = abs((signed)player->mo->angle - (signed)player->jumpangle);

                                if (anglediff < ANG105) // under 210 cone
                                    thokforce += player->actionspd<<FRACBITS;
                                else // 210 and beyond
                                    thokforce /= 2;

                                // enforce min and max thok force
                                if (thokforce > MAXMOVE)
                                    thokforce = MAXMOVE;
                                else if (thokforce < player->actionspd<<FRACBITS)
                                    thokforce = player->actionspd<<FRACBITS;

								if ((player->mo->eflags & MFE_UNDERWATER))
									P_InstaThrust(player->mo, player->mo->angle, thokforce*3/5);
								else
									P_InstaThrust(player->mo, player->mo->angle, thokforce);

                                player->jumpangle = player->mo->angle; // probably overkill
#else
								if ((player->mo->eflags & MFE_UNDERWATER))
...
//////////////////
	...
	{
		fixed_t leeway;
#ifdef SRB2PLUS // make gliding in gravity flip not broken
        if (player->mo->eflags & MFE_VERTICALFLIP)
        {
...
///////////////////////////
	...
	if ((player->charability == CA_FLY || player->charability == CA_SWIM) && !(player->pflags & PF_STASIS || player->powers[pw_nocontrol]))
	{
		...
		if (player->powers[pw_tailsfly])
		{
			...
			{
#ifdef SRB2PLUS // double rate of Tails' descent
				if (player->mo->eflags & MFE_VERTICALFLIP)
				{
					if (player->mo->momz < (10*actionspd)/NEWTICRATERATIO)
						player->mo->momz += (2*actionspd)/NEWTICRATERATIO;
				}
				else if (player->mo->momz > (-10*actionspd)/NEWTICRATERATIO)
					player->mo->momz -= (2*actionspd)/NEWTICRATERATIO;
#else
				if (player->mo->eflags & MFE_VERTICALFLIP)
				{
...
#ifndef SRB2PLUS // no more knuckles weapon buff
...
#endif
#ifdef SRB2PLUS // expend ammo before firing the shot to ensure proper ammo/ring usage and to avoid zombie bug
					player->powers[pw_explosionring]--;
					player->mo->health--;
					player->health--;
#endif
					mo = P_SpawnPlayerMissile(player->mo, MT_THROWNEXPLOSION, MF2_EXPLOSION, false);
#ifndef SRB2PLUS // move this before launch, to avoid firing without expending ammo
					player->powers[pw_explosionring]--;
					player->mo->health--;
					player->health--;
#endif
...
#ifdef SRB2PLUS // self damage exception for bomb shield, do not nuke yourself
        if (mo == player->mo)
            continue;
#endif
...
static void P_DeathThink(player_t *player)
{
	...
	{
#ifdef SRB2PLUS // enforce respawntime
        if (player->deadtimer < 55*TICRATE && !player->spectator)
#endif
		player->playerstate = PST_REBORN;
	}
...
//					P_SpawnMobj(mo->x, mo->y, mo->z, MT_SPARK);
...
#ifdef SRB2PLUS // always decrement flashing tics
    if (player->powers[pw_flashing] && !player->startflashingtics)
        player->startflashingtics = player->powers[pw_flashing];
	if (player->powers[pw_flashing] > 0)
        player->powers[pw_flashing]--;
    else
        player->startflashingtics = 0;
#else
	...
	if (player->taunttimer)
		player->taunttimer--;
#ifdef SRB2PLUS // decrement lastinflictor
    if (player->lastinflictortime)
        player->lastinflictortime--;
    else
        player->lastinflictor = -1;
#endif
	// Flash player after being hit.
	if (!(player->pflags & PF_NIGHTSMODE))
	{
#ifdef SRB2PLUS // always flash if flashing
		if ((player->powers[pw_flashing] > TICRATE && (leveltime & 1)) ||
			(player->powers[pw_flashing] < TICRATE && player->powers[pw_flashing] > 0 && (leveltime % 6) < 3)) // flash slower for last second as indication of flashing about to wear off
			player->mo->flags2 |= MF2_DONTDRAW;
#else
...
#if defined (SRB2PLUS) && (PARANOIA) // use the right function so PARANOIA is satisfied
