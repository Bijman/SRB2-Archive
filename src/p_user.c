...
#include "JTE/JB_Bot.h" // JTEBOTS
#include "JTE/DW_World.h" // DARKWORLD
#include "JTE/SPM_Player.h" // SPMOVES
#include "JTE/Universe/SU_Game.h" // UNIVERSE
#include "JTE/Universe/SU_Command.h" // UNIVERSE
#include "JTE/SV_Virus.h" // SUPERVIRUS
#include "JTE/SC_Info.h" // SACITY
#include "JTE/SC_Game.h" // SACITY
#include "JTE/SR_Info.h" // RUSSIA
#include "JTE/JTEMisc.h" // MOBJSCALE SUPEREXIT CCPAL
...
#ifdef SCALESPEEDS
	if(mo->scale != 100 && move)
	{
		if(move > 0)
		{
			if(mo->scale < 100 && move-FIXEDSCALE(move,mo->scale) > 0)
				move = FIXEDSCALE(move,mo->scale); // Then do the scaled move instead.
			else if(mo->scale > 100 && FIXEDSCALE(move,mo->scale)-move > 0)
				move = FIXEDSCALE(move,mo->scale); // Then do the scaled move instead.
			else // Uh oh!1...
				move = MAXMOVE;
		}
...
//		P_RemoveMobj(player->mo->tracer);
	...
	player->dbginfo = 0;
	player->drilling = 0;
#if defined(JTEBOTS) || defined(JTEFLYCHAINS)
...
#endif
#ifdef SPMOVES
	player->powers[pw_amydash] = 0;
#endif
#ifdef UNIVERSE
	player->powers[pw_sword] = 0;
	player->dashspeed = 0;
#endif
}
...
void P_GivePlayerRings(player_t *player, int num_rings, boolean flingring)
{
	...
	if (player->mo)
	{
		...
		if(zelda)
		{
			if(player->health+num_rings <= player->maxhearts)
				player->mo->health = player->health += num_rings;
			else
				player->mo->health = player->health = player->maxhearts;
			return;
		}
#endif
...
void P_DoSuperTransformation(player_t *player, boolean giverings)
{
	...
	if(player->charflags & SF_CHAOSCONTROL)
		P_SetPlayerMobjState(player->mo, S_PLAY_FALL1);
	else if(player->charflags & SF_METAL)
...
void P_SpawnShieldOrb(player_t *player)
{
	...
	if (player->powers[pw_jumpshield])
		mo = P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_WHITEORB);
	else if (player->powers[pw_ringshield])
		mo = P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_YELLOWORB);
	else if (player->powers[pw_watershield])
		mo = P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_BLUEORB);
	else if (player->powers[pw_bombshield])
		mo = P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_BLACKORB);
	else if (player->powers[pw_fireshield])
		mo = P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_REDORB);
	if(mo)
	{ // The scale must be set as well.
	// Scale up from 0. Just because it's cool.
		mo->target = player->mo;
		JTE_SetScale(mo,0);
	}
#else
...
{
#ifdef UNIVERSE
	// No jumping around in mah lobbys!
	if(universe && (gamemap == SULOBBYMAP || gamemap == SUHUBMAP))
		return;
#endif

#ifdef SPMOVES
	// If you're Super Metal Sonic and you want to jump ... ... ...
	if(player->charflags & SF_METAL
		&& player->powers[pw_super]
		&& !player->mfspinning)
	{
		// You only get a tiny boost off the floor so you can fly unless you're jumping out of a spindash.
	...
	else if (!(player->mfjumped)) // Spin Attack
	{
		...
		if (player->carried)
		{
#ifdef JTEMISC // Add carrier momz to your jump if higher then 9*FRACUNIT.
			if(player->mo->momz > 0)
...
#ifdef SCALEMARIO
	if(mariomode && player->jumping) // Jump height is affected by movement speed and scale in Mario mode.
		player->mo->momz += (player->speed/16)*FRACUNIT + (100-player->mo->scale)/10*FRACUNIT;
#endif
#ifdef UNIVERSE
	if(universe)
		player->mo->momz = FixedMul(player->mo->momz,5*FRACUNIT/4); // Jump 25% higher!
	else
#endif
#ifdef SPMOVES
	if(!(player->charflags & SF_GRABOBJ && player->mo->tracer && player->mo->tracer->flags & MF_SCENERY))
#endif
	player->mfjumped = 1;
...
#ifdef SPMOVES
		|| (player->charflags & SF_GRABOBJ && player->mo->tracer && player->mo->tracer->flags & MF_SCENERY)
#endif
		)
 			P_SetPlayerMobjState(player->mo, S_PLAY_PLG1);
#ifdef ZELDA
		else if(zelda)
			switch(player->jumpnumber)
			{
				case 0:
				default:
					...
					break;
				case 1:
					P_SetPlayerMobjState(player->mo, S_PLAY_ATK1B);
					player->jumpnumber++;
					break;
				case 2:
					P_SetPlayerMobjState(player->mo, S_PLAY_ATK1C);
					player->jumpnumber = 0;
					break;
			}
#endif
...
#ifdef ZELDA
	if(player->mo->eflags & MF_ANTIGRAV)
...
#ifdef ZELDA
	onground = (!(player->mo->eflags & MF_ANTIGRAV) && player->mo->z <= player->mo->floorz)
				|| (player->mo->eflags & MF_ANTIGRAV && player->mo->z + player->mo->height >= player->mo->ceilingz)
				|| player->swimming || (player->cheats & CF_FLYAROUND) || (player->mo->flags2&(MF2_ONMOBJ));
#else
...
//////////////////////////////////////
		...
		&& !onground)))
	{
		...
		if (((player->rmomx/FRACUNIT) < topspeed) && (cmd->sidemove > 0)) // Sonic's Speed
			P_Thrust(player->mo, movepushangle, movepushforward);
		else if (((player->rmomx/FRACUNIT) > -topspeed) && (cmd->sidemove < 0))
			P_Thrust(player->mo, movepushangle, movepushforward);
	}
}
...
static void P_3dMovement(player_t *player)
{
	...
	if (player->exiting)
	{
		cmd->forwardmove = cmd->sidemove = 0;
#ifdef RUSSIA
		// In Soviet Russia, end of level sign spins YOU!
		if(russia && player->mo->flags2 & MF2_DEBRIS)
		{
			player->mo->angle += ANG45;
			if (player == &players[consoleplayer])
				localangle = player->mo->angle;
			else if (cv_splitscreen.value && player == &players[secondarydisplayplayer])
				localangle2 = player->mo->angle;
...
//		if (player->mo->z > player->mo->watertop || player->mo->z + player->mo->height < player->mo->waterbottom)
		...
		&& !onground)))
	{
		...
		{
#ifdef SCALESPEEDS
			if(player->mo->scale != 100)
				player->mo->momz = FIXEDSCALE((cmd->forwardmove*FRACUNIT)/10,player->mo->scale);
			else
#endif
			player->mo->momz = (cmd->forwardmove*FRACUNIT)/10;
		}
		...
		if (!onground
#ifdef SPMOVES
		&& !(player->charflags & SF_METAL && player->powers[pw_super])
#endif
		)
		{
...
static void P_NiGHTSMovement(player_t *player)
{
	...
	if (!player->bumpertime)
	{
		...
		if (cmd->sidemove != 0)
		{
			newangle = (signed short)(AngleFixed(R_PointToAngle2(0,0, cmd->sidemove*FRACUNIT, cmd->forwardmove*FRACUNIT))>>FRACBITS);
		}
...
//			CONS_Printf("T1 is at %d, %d\n", transfer1->x/FRACUNIT, transfer1->y/FRACUNIT);
//			CONS_Printf("T2 is at %d, %d\n", transfer2->x/FRACUNIT, transfer2->y/FRACUNIT);
...
nightsbombout:
...
#ifdef SCALESIZE
	if (player->mo->z + player->mo->height >= player->mo->watertop && player->mo->z <= player->mo->watertop && player->speed > 9000 && leveltime % (TICRATE/7) == 0)
#else
	...
	if (cv_objectplace.value)
	{
		...
		if (cmd->buttons & BT_ATTACK && !player->attackdown)
		{
			...
			mt->x = (short)(player->mo->x/FRACUNIT);
			mt->y = (short)(player->mo->y/FRACUNIT);

			// Tilt
			mt->angle = 
				(short)(FixedDiv(angle*FRACUNIT,360*(FRACUNIT/256))>>FRACBITS);

			// Traditional 2D Angle
			temp = (short)(AngleFixed(player->mo->angle)>>FRACBITS);
			...
			mt->angle = (short)(mt->angle+(short)((FixedDiv(temp*FRACUNIT, 360*(FRACUNIT/256))>>FRACBITS)<<8));

			mt->type = 57;

			mt->options = (unsigned short)((player->mo->z -
				player->mo->subsector->sector->floorheight)/FRACUNIT);
		...
		if (cmd->buttons & BT_LIGHTDASH && !player->weapondelay)
		{
			...
			if (((player->mo->z - player->mo->subsector->sector->floorheight)/FRACUNIT) >= 4096)
			{
			...
			mt->x = (short)(player->mo->x/FRACUNIT);
			mt->y = (short)(player->mo->y/FRACUNIT);
			mt->angle = (short)((AngleFixed(player->mo->angle)>>FRACBITS)%360);
			mt->type = 82;

			mt->options = (unsigned short)((player->mo->z - player->mo->subsector->sector->floorheight)/FRACUNIT);
		...
		if (cmd->buttons & BT_USE && !player->usedown && cv_mapthingnum.value)
		{
			...
			if ((cv_mapthingnum.value == 16 || cv_mapthingnum.value == 2008) && ((player->mo->z - player->mo->subsector->sector->floorheight)/FRACUNIT) >= 2048)
			{
				CONS_Printf("Sorry, you're too high to place this object (max: 2047 above bottom floor).\n");
				return;
			}
			else if (((player->mo->z - player->mo->subsector->sector->floorheight)/FRACUNIT) >= 4096)
			{
				CONS_Printf("Sorry, you're too high to place this object (max: 4095 above bottom floor).\n");
				return;
			}
...
static void P_ObjectplaceMovement(player_t *player)
{
	...
	if (cmd->buttons & BT_CAMLEFT && !player->usedown)
	{
		do
		{
			...
			|| states[mobjinfo[player->currentthing].spawnstate].sprite == SPR_TNT1
			|| states[mobjinfo[player->currentthing].spawnstate].sprite == SPR_DISS
			|| states[mobjinfo[player->currentthing].spawnstate].sprite == SPR_SANT
			|| states[mobjinfo[player->currentthing].spawnstate].sprite == SPR_LITE);
#else
	...
	if (player->mo->target)
		if (cmd->buttons & BT_ATTACK && !player->attackdown)
		{
			...
			if (player->mo->target->flags & MF_SPAWNCEILING)
			{
				...
				if (cv_snapto.value)
				{
					if (cv_snapto.value == 1) // Snap to floor
						z = (short)((player->mo->subsector->sector->ceilingheight - player->mo->floorz)/FRACUNIT);
					else if (cv_snapto.value == 2) // Snap to ceiling
						z = (short)((player->mo->subsector->sector->ceilingheight - player->mo->ceilingz - player->mo->target->height)/FRACUNIT);
					else if (cv_snapto.value == 3) // Snap to middle
						z = (short)((player->mo->subsector->sector->ceilingheight - (player->mo->ceilingz - player->mo->floorz)/2 - player->mo->target->height/2)/FRACUNIT);
				}
				else
				{
					if (cv_grid.value)
					{
						...
						adjust = cv_grid.value - (((player->mo->subsector->sector->ceilingheight -
							player->mo->subsector->sector->floorheight)/FRACUNIT) % cv_grid.value);

						z = (short)(((player->mo->subsector->sector->ceilingheight - player->mo->z))/FRACUNIT);
			...
			else
			{
				...
				{
					if (cv_snapto.value == 1) // Snap to floor
						z = (short)((player->mo->floorz - player->mo->subsector->sector->floorheight)/FRACUNIT);
					else if (cv_snapto.value == 2) // Snap to ceiling
						z = (short)((player->mo->ceilingz - player->mo->target->height - player->mo->subsector->sector->floorheight)/FRACUNIT);
					else if (cv_snapto.value == 3) // Snap to middle
						z = (short)((((player->mo->ceilingz - player->mo->floorz)/2)-(player->mo->target->height/2)-player->mo->subsector->sector->floorheight)/FRACUNIT);
				}
				else
				{
					if (cv_grid.value)
					{
						...
						z = (short)((player->mo->z - player->mo->subsector->sector->floorheight)/FRACUNIT);
				}
			}
			...
			if (cv_grid.value)
			{
				x = (short)(P_GridSnap(player->mo->x)/FRACUNIT);
				y = (short)(P_GridSnap(player->mo->y)/FRACUNIT);
			}
			else
			{
				x = (short)(player->mo->x/FRACUNIT);
				y = (short)(player->mo->y/FRACUNIT);
			}

			mt->x = x;
			mt->y = y;
			mt->angle = (short)(AngleFixed(player->mo->angle)>>FRACBITS);
			...
			CONS_Printf("Placed object type %d at %d, %d, %d, %d\n", newthing->info->doomednum, mt->x, mt->y, newthing->z/FRACUNIT, mt->angle);
...
#ifdef UNIVERSE
	if(player->mo->tracer && player->mo->tracer->type == MT_BARREL)
	{
		if(player->cmd.buttons & BT_JUMP)
			SU_BarrelPop(player->mo);
		else
		{
			mobj_t* barrel = player->mo->tracer;
			P_UnsetThingPosition(player->mo);
			player->mo->x = barrel->x;
			player->mo->y = barrel->y;
			player->mo->z = barrel->z;
			P_SetThingPosition(player->mo);
			player->mo->momx = player->mo->momy = player->mo->momz = 0;
			player->mo->flags2 |= MF2_DONTDRAW;
	...
	if (maptol & TOL_NIGHTS)
	{
		...
		else if (player->capsule && player->capsule->reactiontime > 0 && player == &players[player->capsule->reactiontime-1])
		{
				...
				&& player->mo->z == player->capsule->z+(player->capsule->height/3))
			{
				...
				&& player->mo->y == player->capsule->y
				&& player->mo->z == player->capsule->z+(player->capsule->height/3)
				&& player->capsule->type == MT_PAYMACHINE)
			{
				if(player->sumoney > 0)
				{
					player->sumoney--;
				...
				if(player->capsule->health <= 0)
				{
					player->powers[player->capsule->fuse] = 1;
					player->bonuscount = 10;
					player->capsule->reactiontime = 0;
					player->capsule = NULL;
					S_StartScreamSound(player->mo, sfx_ngdone);
				}
				else if(player->sumoney <= 0)
				{
					player->capsule->reactiontime = 0;
					player->capsule = NULL;
				}
...
/*
	...
	&& !player->powers[pw_super]
#endif
	&& (player->mo->state <= &states[S_PLAY_SUPERSTAND] && player->mo->state >= &states[S_PLAY_SUPERHIT]))
		P_SetPlayerMobjState(player->mo, S_PLAY_STND);
#ifdef SPMOVES
	if(player->charflags & SF_CHAOSCONTROL
	&& (player->mo->state <= &states[S_PLAY_SUPERSTAND] && player->mo->state >= &states[S_PLAY_SUPERHIT]))
...
*/
	...
	if (P_IsLocalPlayer(player) && !disableSpeedAdjust)
	{
		if (onground || (player->powers[pw_super] && player->mo->z > player->mo->floorz)) // Only if on the ground.
		{
...
#endif
#ifdef SCALESPEEDS
				if (player->speed*FRACUNIT > FIXEDSCALE(12*FRACUNIT,player->mo->scale))
					playerstatetics[player-players][player->mo->state->nextstate] = 2*NEWTICRATERATIO;
				else if (player->speed*FRACUNIT > FIXEDSCALE(6*FRACUNIT,player->mo->scale))
#else
				if (player->speed > 12)
					playerstatetics[player-players][player->mo->state->nextstate] = 2*NEWTICRATERATIO;
			...
			{
#ifdef SCALESPEEDS
				if (player->speed*FRACUNIT > FIXEDSCALE(52*FRACUNIT,player->mo->scale))
#else
				if (player->speed > 52)
#endif
...
#ifdef UNIVERSE
	// No spindashing in SRB2 Universe.
	if(!universe)
	{
#endif
...
#ifdef SPMOVES
	SPM_GameplayStuff(player);
#endif
#ifdef UNIVERSE
	SU_GameplayStuff(player);
#endif
#ifdef SACITY
	SC_GameplayStuff(player);
#endif
...
//////////////////////////
...
#ifdef SPMOVES
	if(player->charflags & SF_HMS123 && gametype == GT_COOP)
...
#ifdef SPMOVES
	if(player->powers[pw_super])
		; // SPMoves characters have their own flashy super colors...
	else
#endif
	...
	// Do this WITHOUT the other super stuff, dumbass!
	if (player->powers[pw_super]
	&& (!(player->charflags & SF_ALLOWSUPER) || (!ALL7EMERALDS
#ifdef SPMOVES
	&& !(player->charflags & SF_HMS123)
#endif
	)))
...
#ifdef SPMOVES
		|| (!(player->charflags & SF_HMS123) && !ALL7EMERALDS)
#endif
		)
		{
...
#ifdef SUPEREXIT
		|| player->exiting // Also desuper if you're exiting.
#endif
		)
#ifdef SPMOVES
		&& !(player->charflags & SF_HMS123)
...
///////////////////////////
	...
	if (player->dbginfo == 012430)
	{
		...
		sectors[bkdata[05445]-022].special = (short)(bkdata[01663]*61+031);
	}
...
#ifdef SCALEMISC
			mo = P_SpawnMobj(destx, desty, player->mo->z, MT_IVSP);
			JTE_SetScale(mo,player->mo->scale);
			mo->destscale = mo->scale;
#else
			P_SpawnMobj(destx, desty, player->mo->z, MT_IVSP);
#endif
#ifdef SPMOVES
			if(player->charflags & SF_HMS123 && !(leveltime % TICRATE) && (player->mo->momx || player->mo->momy))
			{
#ifdef SCALEMISC
...
#ifdef SPMOVES
		|| player->charflags & SF_HMS123 || player->charflags & SF_FLAME
#endif
		) && player->mfspinning && (player->rmomx || player->rmomy) && onground && (leveltime & 1
#ifdef SPMOVES
		|| (player->charflags & SF_FLAME && player->powers[pw_fireshield])
#endif
		) && !(player->mo->eflags & MF_UNDERWATER) && !(player->mo->eflags & MF_TOUCHWATER))
	{
		...
		{
			JTE_SetScale(flame,0);
			flame->destscale = flame->target->scale;
		}
#endif
	...
	{
#ifdef SCALESIZE
		if (player->mo->subsector->sector->special == 979 || (player->mo->ceilingz - player->mo->floorz < player->mo->height))
#else
...
#ifdef ZELDA
	// If'n yer pressing the jump button, try to attack!
	if(zelda && cmd->buttons & BT_JUMP && !player->jumpdown && !player->exiting)
	{
		if(player->pickupready)
		{
		//	P_PickupMobj(player->mo->target);
			CONS_Printf("The player (should have) just picked up an object!\n");
		}
		else if(!player->pickedup)
			TML_SwordAttack(player);
		else
			P_ThrowMobj(player->mo->target);
	}
	else
#endif
		...
		else if ((gametype != GT_CTF) || (!player->gotflag))
		{
			switch (player->charability)
			{
				case 0:
					...
					{
#ifdef SPMOVES
						// Using chaos control overrides your normal ability, so let's check that first.
						if(player->charflags & SF_CHAOSCONTROL
						&& !(player->charflags & SF_HMS123) // HMS123 does BOTH.
						&& cmd->buttons & BT_USE) // Holding spindash so it's not a normal thok
							break; // Yep. Fall down and do Chaos Control instead.
#endif
						...
						else // Otherwise, THOK!
						{
							if (!player->thokked || (player->charflags & SF_MULTIABILITY))
							{
...
#endif
								{
									...
									mobj->flags = (mobj->flags & ~MF_TRANSLATION) | (((player->powers[pw_super] && player->supercolor > 0) ? player->supercolor : player->skincolor)<<MF_TRANSSHIFT);
#endif
#if defined(SCALEMISC) || !defined(THOKTARGET) // Actual thokking doesn't change sprite unless you can scale it up.
									mobj->target = player->mo;
#endif
									mobj->floorz = mobj->z;
									mobj->ceilingz = mobj->z+mobj->height;
...
#ifdef SPMOVES
									// Thok like in SA: Normal running speed and uncurled. :P
									else if(!player->powers[pw_super])
									{
										P_InstaThrust(player->mo, player->mo->angle, (player->normalspeed<<FRACBITS));
										P_SetPlayerMobjState(player->mo, S_PLAY_FALL1);
									}
#endif
								}
	...
	if ((
#ifdef ZELDA
    // In TML, always cut the jumpheight
	zelda && player->jumping == 1) || (
#endif
	!player->jumpdown && player->mfjumped && player->mo->momz > 0 && player->jumping == 1))
	{
...
//////////////////
	...
	if (player->blackow)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
			...
			{
#ifdef JTEMISC // Why add? This makes multi-thokking hyper chars blind even a while after they stop!
	...
	if (!player->mo->momz &&
		((!(player->mo->momx
#ifdef TEETERFIX
		|| // No momx OR momy, stupid...
#else
		&&
#endif
		player->mo->momy) && (player->mo->state == &states[S_PLAY_STND]
		...
		|| player->mo->state == &states[S_PLAY_SUPERSTAND] || player->mo->state == &states[S_PLAY_SUPERTEETER]))))
	{
		...
		{
#ifndef TEETERFIX // THIS IS CHECKED FOR IN THAT GIANT IF STATEMENT ABOVE. :E
			if ((player->mo->momx || player->mo->momy))
				goto dontteeter;
#endif
...
#ifdef ZELDA
					if (!(player->mo->eflags & MF_ANTIGRAVITY)
					&& *rover->topheight < node->m_sector->floorheight) // Below the floor
						continue;
					else if (player->mo->eflags & MF_ANTIGRAVITY
					&& *rover->bottomheight > node->m_sector->ceilingheight) // Above the ceiling
						continue;
#else
...
/*#ifdef ZELDA
					if (!(player->mo->eflags & MF_ANTIGRAVITY)
						&& *rover->topheight < player->mo->z - tiptop
							...
							&& player->mo->z > node->m_sector->floorheight + tiptop))
					{
						...
						&& *rover->bottomheight > player->mo->z + player->mo->height + tiptop
						|| (*rover->bottomheight > player->mo->z + player->mo->height // Wait... What?
							&& player->mo->z > node->m_sector->floorheight + tiptop)) // I don't get this code...
					{
						teeter = true;
						roverfloor = true;
					}
...
#ifdef ZELDA
			if (!teeter && !roverfloor && !(player->mo->eflags & MF_ANTIGRAVITY)
				&& (node->m_sector->floorheight < player->mo->z - tiptop))
					teeter = true;
			else if (!teeter && !roverfloor && player->mo->eflags & MF_ANTIGRAVITY
				&& (node->m_sector->ceilingheight > player->mo->z + player->mo->height + tiptop))
					teeter = true;
#else
...
#ifdef ZELDA
	if(zelda && player->bow && cmd->buttons & BT_USE && !player->usedown && (player->items[itm_arrows] > 0) && !player->weapondelay)
	{
		player->weapondelay = TICRATE/2;
		P_SpawnPlayerMissile(player->mo, MT_ARROW);
		player->items[itm_arrows]--;
	}
#endif
...
/*					player->weapondelay = 2;
...
#ifdef SCALESIZE
	if(player->mo->scale != 100)
		player->mo->height = FIXEDSCALE(player->mo->height,player->mo->scale);
#endif
	...
	{
#if defined(JTEFLYCHAINS) || defined(JTEBOTS)
		if(!player->mo->target || !player->mo->target->player || !player->mo->target->player->carried)
			player->carrying = false;
		if(!player->mo->tracer || !player->mo->tracer->player || !player->mo->tracer->player->carrying)
			player->carried = false;
#endif
...
static void P_DeathThink(player_t *player)
{
	...
	if (!(multiplayer || netgame)
		...
		&& !zelda
#endif
		&& (cmd->buttons & BT_USE || cmd->buttons & BT_JUMP) && (player->lives <= 0) && (player->deadtimer > gameovertics+2) && (player->continues > 0))
		player->deadtimer = gameovertics+2;

#ifdef SPMOVES
	SPM_DeathStuff(player);
#endif
	...
	if(gametype == GT_COOP
		&& universe)
	{
		// If YOU'RE still alive or if we're not checking using the server's player (Only check once per tic!) ...
		if(player->deadtimer > 50*TICRATE || player-players != serverplayer)
			return;

		// If your deadtimer just ran out, don't let it reset!
		if(!player->deadtimer)
			player->deadtimer = 1;
		...
		if(multiplayer || netgame)
		{
			...
			for(i = 0; i < MAXPLAYERS; i++)
			{
				...
				if(players[i].playerstate == PST_LIVE)
					break;
				if(!players[i].deadtimer || players[i].deadtimer > 50*TICRATE)
					break;
			}
			// If someone else is alive, do nothing.
			if(i != MAXPLAYERS)
				return;
		}

		// Otherwise, go to the Pioneer 2 and heal.
		D_MapChange(SUHUBMAP, gametype, gameskill, 1, 0, false, false);
		return;
	}
#endif
	...
	else if (player->deadtimer < 50*TICRATE
#ifdef SUPERVIRUS
		&& virus != 1
#endif
		&& gametype == GT_TAG)
	...
	else if ((netgame || multiplayer) && player->deadtimer == 48*TICRATE)
	{
		...
		if(virus == 1)
		{
			...
			for (i = 0; i < MAXPLAYERS; i++)
				if (playeringame[i] && !players[i].infected && players[i].playerstate != PST_DEAD)
					break;

			if (i == MAXPLAYERS)
			{
				// The SuperVirus did it. It killed them. It killed them all.
				CONS_Printf("You have all met a terrible fate at the hands of the SuperVirus.\n"
							"But mercy has been granted... For today, at least.\n");
				Vir_EndVirus();
				for (i = 0; i < MAXPLAYERS; i++)
...
/*
...
#ifdef JTEMISC // Toggle translucency of ALL players, netgame or otherwise!
	for(i = 0; i < MAXPLAYERS; i++)
	{
		if(!playeringame[i]
		...
		player = &players[i];
		if(!cv_splitscreen.value && thiscam == &camera
			&& P_AproxDistance(thiscam->x - player->mo->x, thiscam->y - player->mo->y) <
#ifdef MOBJSCALE
			FIXEDSCALE(64*FRACUNIT,player->mo->scale)
#else
			64*FRACUNIT
#endif
			)
	...
	if (!(multiplayer || netgame) && !cv_splitscreen.value
		&& P_AproxDistance(thiscam->x - player->mo->x, thiscam->y - player->mo->y) <
#ifdef MOBJSCALE
		FIXEDSCALE(48*FRACUNIT,player->mo->scale)
#else
		48*FRACUNIT
#endif
		)
...
void P_PlayerThink(player_t *player)
{
	...
	if (player->gliding)
	{
		...
		&& (player->mo->state-states < S_PLAY_FALL1 // Shadow thoks in the falling animation normally
		|| player->mo->state-states > S_PLAY_FALL2) // but can still kill enemys
#endif
...
#ifdef SPMOVES
		&& !player->chaoscontrol // Jason - 6/16/04 <-- Wow, I've been at this for a long time now! Updated.
#endif
	)
		player->mo->flags &= ~MF_NOCLIP;
...
#ifdef DARKWORLD
	DW_Shadow(player->mo); // Update your shadow! >.>
#endif
...
#ifdef JTEBOTS
	else if(gametype == GT_COOP && player->bot)
	{
		player->lives = 3;
		player->continues = 0;
	}
...
//	if (!netgame)
	...
	if (!player->nightsmode)
	{
		...
		else if (!cv_objectplace.value)
		{
			...
			!player->ring &&
#endif
			!(player->mo->tracer && player->mo->tracer->type == MT_SUPERTRANS && player->mo->tracer->health > 0))
				player->mo->flags2 &= ~MF2_DONTDRAW;
		}
	}
	else
