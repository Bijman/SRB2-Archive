...
//
// $Log: p_user.c,v $
// Revision 1.0  2004/06/16 4:41:48  Jason
// w00t, Chaos Control rox!
//
// DESCRIPTION:
...
void P_AddPlayerScore(player_t* player, int amount)
{
	...
	player->score += amount;
	#ifdef SPMOVES
	JTE_PlayerLevelUp(player, amount);
	#endif
...
void P_DoPlayerExit(player_t* player)
{
	...
	#endif
	) // If in Race or Circuit Mode, allow
	{
...
// End various movement calculations Tails
	...
	else
	{
		...
		if(player->powers[pw_tailsfly])
		{
			#ifdef SPMOVES
			if(player->charability == 9)
				topspeed = player->normalspeed * 4 / 3 > 30 ? 30 : player->normalspeed * 4 / 3;
			else
			#endif
...
//////////////////////////////////////
	if(player->climbing)
	{
		if(player->charability == 2)
			player->mo->momz = (cmd->forwardmove/10)*FRACUNIT;

        #ifdef SPMOVES
		if(player->charability == 6)
			player->mo->momz = (cmd->forwardmove/8)*FRACUNIT;

		if(player->charability == 8)
			player->mo->momz = (cmd->forwardmove/5)*FRACUNIT;

		if(player->charability == 15)
			player->mo->momz = -FRACUNIT/2;
		#endif
	}

	if (cmd->sidemove
		&& !(player->climbing || player->gliding
		|| player->exiting
...
void P_3dMovement(player_t* player)
{
	...
	if(!netgame && ((player == &players[consoleplayer] && cv_analog.value && cv_chasecam.value) || (cv_splitscreen.value && player == &players[secondarydisplayplayer] && cv_analog2.value && cv_chasecam2.value)))
	{
...
//////////////////////////////////////
	if ((netgame
		|| (player == &players[consoleplayer] && (!cv_analog.value || !cv_chasecam.value))
		|| (cv_splitscreen.value && player == &players[secondarydisplayplayer] && (!cv_analog2.value || !cv_chasecam2.value)))
		&& cmd->forwardmove
		&& !(player->exiting
		|| (player->mo->state == &states[S_PLAY_PAIN] && player->powers[pw_flashing]
		&& !onground)))
...
// Analog movement control Tails
	if(!netgame && ((player == &players[consoleplayer] && cv_analog.value && cv_chasecam.value) || (cv_splitscreen.value && player == &players[secondarydisplayplayer] && cv_analog2.value && cv_chasecam2.value)) && thiscam->chase)
	{
		if (!(player->exiting || (player->mo->state == &states[S_PLAY_PAIN] && player->powers[pw_flashing])))
		{
			...
			if(player->climbing)
			{
                ...
                #ifdef SPMOVES
				if(player->charability == 6)
					P_InstaThrust(player->mo, player->mo->angle-ANG90, (cmd->sidemove/8)*FRACUNIT);
	...
	else if (netgame || player->gliding
	|| (player == &players[consoleplayer] && !(cv_analog.value || !cv_chasecam.value)) || (cv_splitscreen.value && player == &players[secondarydisplayplayer] && !(cv_analog2.value || cv_chasecam2.value)))
	{
		if(player->climbing)
		{
		    if(player->charability == 2)
		...
		else if (cmd->sidemove && !player->exiting && !player->climbing && !(player->mo->state == &states[S_PLAY_PAIN] && player->powers[pw_flashing])) // Tails 04-12-2001
		{
			boolean mright;
			boolean mleft;
			angle_t sideangle;
...
void P_MovePlayer (player_t* player)
{
	...
	if(player->capsule && player->capsule->reactiontime)
	{
		...
		{
		    #ifdef SPMOVES
		    if((player->psomoney > 0 && mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO) || !(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO))
		    {
				...
				if(player->capsule && player->capsule->health <= 0 && player->capsule->type == MT_EGGCAPSULE)
				{
				...
				#ifdef SPMOVES
				else if(player->capsule && player->capsule->health <= 0 && player->capsule->type == MT_PAYMACHINE)
				{
					player->capsule->target = player->mo;
					if(player->capsule->fuse == 1)
						A_BlueShield(player->capsule);
					else if(player->capsule->fuse == 2)
						A_GreenShield(player->capsule);
					else if(player->capsule->fuse == 3)
						A_RedShield(player->capsule);
					else if(player->capsule->fuse == 4)
						A_BlackShield(player->capsule);
					else if(player->capsule->fuse == 5)
						A_YellowShield(player->capsule);
					P_SetMobjState(player->capsule, S_DISS);
					player->capsule->reactiontime = 0;
					player->capsule = NULL;
				}
		    }
...
#define MAXNORMALSPEED 6000
	...
	if(player->nightsmode)
	{
		...
		if(player->powers[pw_extralife] == 1 && player==&players[consoleplayer]) // Extra Life!
		{
			...
			#ifdef SPMOVES
				if(player->tunes)
					S_ChangeMusic(player->tunes, true);
				else
			#endif
	...
	if(cv_objectplace.value)
	{
		...
		if(cmd->buttons & BT_CAMLEFT && !player->usedown)
		{
			do
			{
				...
				#ifdef SPMOVES
				|| player->currentthing == MT_PSOBOSSDROP
				|| player->currentthing == MT_RANDOMAMBIENT
				#endif
...
*/
	...
	#ifdef SPMOVES
	JTE_PlayerGameplayStuff(player);
	#endif
...
// AKA my own gravity. =)
	if(player->gliding)
	{
		...
		if(player->gliding && (player->charability == 2
		#ifdef SPMOVES
		|| (player->charability == 6 && player->thokked)
		#endif
		))
		{
		...
		#ifdef SPMOVES
		else if(player->gliding && player->charability == 16)
		{
			P_InstaThrust(player->mo, player->mo->angle, 30*FRACUNIT/NEWTICRATERATIO);
			if(player->mo->momz >= -FRACUNIT/NEWTICRATERATIO && player->mo->momz <= FRACUNIT/NEWTICRATERATIO)
			   player->mo->momz = 0;
			else if(player->mo->momz < -FRACUNIT/NEWTICRATERATIO)
			   player->mo->momz += FRACUNIT*3/4/NEWTICRATERATIO;
		}

		else if(player->gliding && player->charability == 6 && !player->thokked)
			P_InstaThrust(player->mo, player->mo->angle, 40*FRACUNIT/NEWTICRATERATIO);
		#endif
		...
		if(!player->jumpdown)    // If not holding the jump button
		{
			...
			#ifdef SPMOVES
			if(player->charability == 6)
			   player->thokked = true; // Set up to use the normal glide.
			#endif
			P_SetMobjState(player->mo, S_PLAY_ATK1);
		}
		#ifdef SPMOVES
		else if(player->mo->z <= player->mo->floorz)
			player->gliding = 0; // down, stop gliding.
		#endif
	}
	...
	if(player->charability != 2
	#ifdef SPMOVES
	&& player->charability != 6
	&& player->charability != 16
	#endif
	) // If you can't glide, then why
	{				    // the heck would you be gliding?
		player->gliding = 0;
		player->glidetime = 0;
		#ifdef SPMOVES
		if(player->charability != 8 && player->charability != 15)
		#endif
			player->climbing = 0;
	}
...
//////////////////////////
	...
	if(!(gamemap >= sstage_start && gamemap <= sstage_end)
		&& (cv_gametype.value == GT_COOP || cv_gametype.value == GT_RACE)) // Don't do it in special stages. (or circuit mode Graue 12-22-2003)
	{
	...
	if(player->score >= 50000+player->xtralife2 && (cv_gametype.value == GT_COOP || cv_gametype.value == GT_RACE))
	{
		player->lives++;
...
// Does player have all emeralds? If so, flag the "Ready For Super!" Tails 04-08-2000
	if(emeralds & EMERALD1 && emeralds & EMERALD2 && emeralds & EMERALD3 && emeralds & EMERALD4 && emeralds & EMERALD5 && emeralds & EMERALD6 && emeralds & EMERALD7 && player->health > 50 && cv_gametype.value == GT_COOP)
	...
	if(player->powers[pw_super])
	{
		// If you're super and not Sonic OR Shadow, de-superize!
		if(!(player->skin == 0) && !(player->skin == 5 && !M_CheckParm("-noshadow") && !M_CheckParm("-nocharacters")))
		{
...
//			P_SetMobjState(player->mo, S_PLAY); // Return to normal
		...
		if((player->powers[pw_super]) && (player->mo->z < player->mo->watertop+10*FRACUNIT) && (player->mo->z > player->mo->watertop-10*FRACUNIT) && (cmd->forwardmove) && (player->rmomx) && (player->rmomy) && (player->mo->momz < 0) && (player->speed > 10))
		{
...
/////////////////////////
	...
	if(player->powers[pw_extralife] == 1 && player==&players[consoleplayer]) // Extra Life!
	{
		...
		else
		{
			...
			#endif
				S_ChangeMusic(mapheaderinfo[gamemap-1].musicslot, true); // Tails 03-14-2000
		}
	}
...
////////////////
	...
	if(player->charability == 1 || player->charability == 9)
	{
	...
	/*if(leveltime > 20999 && cv_gametype.value == GT_COOP) // one tic off so the time doesn't display 10:00
		P_DamageMobj(player->mo, NULL, NULL, 10000);*/
	...
	else
	{
		...
		{
			// Fancy effect already exists, but who cares?
			if(leveltime % 5 == 1)
				P_SpawnMobj (player->mo->x, player->mo->y, player->mo->z, MT_IVSP);
			if((cmd->forwardmove || cmd->sidemove) && (leveltime % 10 == 1) && (player->mo->momx || player->mo->momy) && player->speed > 10)
				P_SpawnMobj (player->mo->x, player->mo->y, player->mo->z + player->mo->height / 2, MT_SUPERSPARK);
			player->mo->flags =  (player->mo->flags & ~MF_TRANSLATION) | ((leveltime % 13)<<MF_TRANSSHIFT);
		}*/
		else if (player->powers[pw_super])
		{
			if((cmd->forwardmove || cmd->sidemove) && (leveltime % 10 == 1) && (player->mo->momx || player->mo->momy) && player->speed > 20)
				P_SpawnMobj (player->mo->x, player->mo->y, player->mo->z + player->mo->height / 2, MT_SUPERSPARK);
			switch(leveltime % 3)
			{
				// Silver
				case 0:
				player->mo->flags =  (player->mo->flags & ~MF_TRANSLATION) | ((4)<<MF_TRANSSHIFT);
				break;

				// Peach
				case 1:
				case 3:
				player->mo->flags =  (player->mo->flags & ~MF_TRANSLATION) | ((2)<<MF_TRANSSHIFT);
				break;

				// Yellow
				case 2:
				player->mo->flags =  (player->mo->flags & ~MF_TRANSLATION) | ((15)<<MF_TRANSSHIFT);
				break;
			}
		}
	}
	...
	if ((player->powers[pw_invulnerability] == 1) && (player->powers[pw_super] == false))
	{
		...
		if(player == &players[consoleplayer] && !(player->powers[pw_extralife] > 1) && !player->powers[pw_sneakers] && !(player->powers[pw_invulnerability] > 1) && !player->powers[pw_spacetime] && ((player->powers[pw_underwater] > 12*TICRATE + 1) || (!player->powers[pw_underwater])))
		{
		    #ifdef SPMOVES
			if(player->tunes)
				S_ChangeMusic(player->tunes, true);
	...
	#ifdef SPMOVES
	|| (player->gliding && player->charability == 6 && !player->thokked)
	|| (player->gliding && player->charability == 12 && player->thokked)
	#endif
	)
	{
		if(player->skincolor != 0)
		{
			...
			#ifdef SPMOVES
			if(player->gliding && (player->charability == 6 || player->charability == 12) && (leveltime & 1 && leveltime & 2 && leveltime & 3 && leveltime & 4 && leveltime & 5 && leveltime & 6 && leveltime & 7))
			   S_StartSound(player->mo, sfx_drill2);
            #endif
		}
	}
...
//			else
            #ifdef SPMOVES
            if(player->helditem)
				P_SetMobjState (player->mo, S_PLAY_PLG1); // If you're holding something, you can't jump normally!
            else
            #endif
				P_SetMobjState (player->mo, S_PLAY_ATK1);
		}
		else
		{
			...
			(
			#endif
			cv_gametype.value != GT_CIRCUIT || cv_circuit_specmoves.value == 1)
			#ifdef SPMOVES
			&& !player->helditem)
			#endif
			#endif
			#if defined(SPMOVES) && !defined(CIRCUITMODE)
			if(!player->helditem)
			#endif
			// Graue 12-08-2003
			{
				switch(player->charability)
				{
					case 0:
						...
						if (player->mfjumped)
						{
							if(player->superready && !player->powers[pw_super] && (player->skin == 0 || (player->skin == 5 && !M_CheckParm("-noshadow")))) // If you can turn into Super
							{					  // and aren't, do it!
								if(!(player->powers[pw_yellowshield] || player->powers[pw_blackshield] || player->powers[pw_greenshield] || player->powers[pw_redshield] || player->powers[pw_blueshield])) // But don't turn super with a sheild!
								{
									...
									player->mo->flags2 |= MF2_DONTDRAW;
									player->mo->flags = (player->mo->flags & ~MF_TRANSLATION) | ((15)<<MF_TRANSSHIFT); // Tails 08-20-2002
									player->mo->momx = player->mo->momy = player->mo->momz = 0;
									player->mfjumped = 0;
									P_SetMobjState(player->mo, S_PLAY_RUN1);
								}
...
/////////////////////
	...
	if(!netgame && ((player == &players[consoleplayer] && cv_analog.value && cv_chasecam.value) || (cv_splitscreen.value && player == &players[secondarydisplayplayer] && cv_analog2.value && cv_chasecam2.value)) 
		&& (cmd->forwardmove || cmd->sidemove) && !player->climbing && !player->gliding)
	{
		// If travelling slow enough, face the way the controls
		// point and not your direction of movement.
		if(player->speed < 5 || player->mo->z > player->mo->floorz)
		{
			tempx = tempy = 0;
	...
	else if(!netgame && ((player == &players[consoleplayer] && cv_analog.value && !cv_chasecam.value) || (cv_splitscreen.value && player == &players[secondarydisplayplayer] && cv_analog2.value && !cv_chasecam2.value)) 
		&& (cmd->forwardmove || cmd->sidemove))
	{
		if (player==&players[consoleplayer])
			player->mo->angle = localangle;
		else if(cv_splitscreen.value && player==&players[secondarydisplayplayer])
			player->mo->angle = localangle2;
	}

	else if(!netgame && ((player == &players[consoleplayer] && cv_analog.value && cv_chasecam.value) || (cv_splitscreen.value && player == &players[secondarydisplayplayer] && cv_analog2.value && cv_chasecam2.value)) 
		&& cmd->sidemove && (player->gliding))
	{
		if (player==&players[consoleplayer])
			player->mo->angle = localangle;
		else if(cv_splitscreen.value && player==&players[secondarydisplayplayer])
...
///////////////////////////////////////
	...
	{
		if(player->powers[pw_super] && player->mo->momz > -5)
			player->mo->momz -= 1;
		...
		#ifdef SPMOVES
		else if(player->rpLevel >= 9 && !player->usedown && (player->powers[pw_blueshield] || player->powers[pw_greenshield] || player->powers[pw_redshield] || player->powers[pw_yellowshield]))
		{
			// Don't let Super Sonic or invincibility use it
			if(!(player->powers[pw_super] || player->powers[pw_invulnerability]))
			{
				player->powers[pw_blueshield] = player->powers[pw_greenshield] = player->powers[pw_redshield] = player->powers[pw_yellowshield] = false;
				player->blackow = 1;
			}
		}
		#endif
	...
	if(cv_homing.value
	#ifdef SPMOVES
	|| player->charability == 4 || (player->charability == 10 && player->rpLevel >= 6 && player->rpLevel < 8 && cmd->buttons & BT_USE)
	#endif
	) // Jason - 6/17/04
	{
...
#define EXPLOSION player->powers[pw_explosionring]
	...
	if (cmd->buttons & BT_ATTACK) // Tails 12-03-2000
	{
		...
		if(mariomode)
		{
			if(!player->attackdown && (player->powers[pw_fireflower] ||
			    (cv_gametype.value == GT_MATCH || cv_gametype.value == GT_CTF
				|| (cv_gametype.value == GT_TAG && player->tagit)
		...
		#ifdef SPMOVES
		else if(JTE_PlayerFire(player))
		    return;
		#endif
		else if(((cv_gametype.value == GT_MATCH && !(mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE))
			|| cv_gametype.value == GT_CTF
			|| (cv_gametype.value == GT_TAG && player->tagit)
			#ifdef CIRCUITMODE
			|| (cv_circuit_ringthrow.value && cv_gametype.value == GT_CIRCUIT)
			#endif
			|| cv_ringslinger.value)
			&& player->mo->health > 1
			&& !player->attackdown
			&& (!player->weapondelay || (AUTOMATIC && leveltime & 1))
	...
	{
	    player->mo->height = 63*FRACUNIT;
	    player->mo->radius = 48*FRACUNIT;
	}
	else
	{
	  player->mo->radius = player->mo->info->radius;
	#endif
	if((player->mfspinning) || (player->mfjumped) || (player->powers[pw_tailsfly]) || (player->gliding)
    || (player->charability == 1 && (player->mo->state == &states[S_PLAY_SPC1] || player->mo->state == &states[S_PLAY_SPC2] || player->mo->state == &states[S_PLAY_SPC3] || player->mo->state == &states[S_PLAY_SPC4])))
	...
	if(player->powers[pw_super] && rendermode != render_soft && player == &players[displayplayer])
	...
	if(mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE)
	{
		...
		{
			if(!(player->mfjumped || player->mfspinning || player->powers[pw_tailsfly] || player->gliding
			    || player->climbing
	...
	if(P_RingNearby(player)
		...
		#ifdef SPMOVES
		|| (player->charability >= 3 && player->charability <= 8) || player->charability == 10
		#endif
		))
	{
	...
	#ifdef SPMOVES
	|| player->charability == 7 || (player->gliding && player->charability == 6 && !player->thokked)
	|| player->charability == 16
	#endif
	)
	{
...
void P_DeathThink (player_t* player)
{
	...
	if ((cmd->buttons & BT_JUMP) && (cv_gametype.value == GT_MATCH || cv_gametype.value == GT_CHAOS || cv_gametype.value == GT_TAG || cv_gametype.value == GT_CTF
	    #ifdef CIRCUITMODE
	    || cv_gametype.value == GT_CIRCUIT
	    #endif
	    )) // Tails 05-06-2001
	...
	|| cv_gametype.value == GT_CIRCUIT
	#endif
	) && !(countdown2 && !countdown) && !player->exiting)
	{
		player->minutes = (leveltime/(60*TICRATE));
		player->seconds = (leveltime/TICRATE) % 60;
		player->realtime = leveltime;
...
//    int                 waterz;
    ...
    else if ((!cv_objectplace.value && !player->nightsmode) && !((player->mo->flags2 & MF2_DONTDRAW) && player->charability == 3)) // Jason - 6/16/04
        player->mo->flags &= ~MF_NOCLIP;
...
// If so, begin the level end process. Tails 12-15-2000
	...
	if(!player->nightsmode)
	{
		if(player->powers[pw_flashing] && (leveltime & 1))
			player->mo->flags2 |= MF2_DONTDRAW;
		else if(!cv_objectplace.value && !(player->charability == 3 && player->thokked == 1 && !player->powers[pw_flashing]))
		{
