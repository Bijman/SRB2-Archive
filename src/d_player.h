...
#ifdef RIDERS
	SF_EXGEAR           =   16, // Second set of ability for Riders mode
	SF_THOKCOLOR        =   32,// 0 for Normal, 1 for thok color + prefcolor, 2 for thok color +/ prefcolor
	SF_PREFTHOKCOLOR    =   64,// Same as prefcolor, but for the thok color
	SF_RUNFRAME         =  128,// Running frames + special frames = 8 or 10 frames.
#endif
} skinflags_t;
...
#ifdef RIDERS
extern INT32 playerstatetics[MAXPLAYERS][S_PLAY_FLY9B+1];
#else
...
typedef enum
{
	...
	pw_starshroom, // 1 = Star, 2 = Mushroom
	pw_mushroom,
	pw_shell,
	pw_redshell,
	pw_banana,
...
#ifdef RIDERS
	tic_t checkpointtimes[256]; // Checkpoints for race mode, etc
