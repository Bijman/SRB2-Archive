...
typedef struct player_s
{
	...
	#ifdef SPMOVES
	// Glide-Blast created 6/26/04 - Reworked to use the glide variable 12/12/04

	int rpLevel; // For the RP level-up ability
	int rpScore; // Jason - 8/12/04
    ...
    // PSO Items. Weapons, shields, etc. The number is the inventory of them.
	...
	mobj_t* chao; // Player has a Chao!
	int customchao; // Custom Chao frames to use (Oooo, skin-able!)

	mobj_t* ride; // Player is riding something!
	mobj_t* pickupitem; // Player can pick up something!
	mobj_t* helditem; // Player is holding something!

	boolean lightdashdown; // Player has the lightdash button pressed

	mobj_t* psoteller;
	boolean psotellerfound;
	#endif
