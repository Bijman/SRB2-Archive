...
#ifdef SPMOVES
	SF_CHAOSCONTROL     = 0x00400, // Shadow's Chaos Control
	SF_METAL            = 0x00800, // Metal Sonic!
	SF_AMYDASH          = 0x01000, // Amy's Dash
	SF_HMS123           = 0x02000, // Everything
	SF_FLAME            = 0x04000, // Fire abilities
	SF_NODROWN          = 0x08000, // Can't drown
	SF_GRABOBJ          = 0x10000 // Can pick up other objects *NOT SILVER'S ABILITY*
#endif
} skinflags_t;	
...
typedef struct player_s
{
...
#ifdef SPMOVES
	byte thokked; // Needs to be a byte for Amy's triple jump
#else
...
#ifdef SPMOVES
	boolean chaoscontrol; // You are chaos controlling.
	...
	unsigned short sumaxhp;
	unsigned short sumaxtp;

	unsigned short sutp;
	unsigned short sumoney;
	unsigned char  suitems[NUMSUITEMS];
...
#ifdef SUPERVIRUS1
	unsigned short infected;
	unsigned char confusion;
	unsigned char confusiontime;
#endif
#ifdef SUPERVIRUS
	unsigned char fizz;
#endif
	...
	int bombtics; // How long you have the bomb bag for in Match.
