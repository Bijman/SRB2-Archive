...
#ifdef HOLIDAYS
lighttable_t *holidaycmaps[NUMHOLIDAYS];
#endif
#ifdef CCPAL
lighttable_t *realcolormaps;
lighttable_t *neoncolormaps;
#endif
...
void R_LoadTextures(void)
{
	...
	if(holidaynicks[holiday])
		holidaystr = va("%sPNAM",holidaynicks[holiday]);
	if(!holidaystr || W_CheckNumForName(holidaystr) == -1)
		holidaystr = "PNAMES";
	pnames = W_CacheLumpName(holidaystr, PU_STATIC);
#else
	...
	holidaystr = NULL;
	if(holidaynicks[holiday])
		holidaystr = va("%sTEX1",holidaynicks[holiday]);
	if(!holidaystr || W_CheckNumForName(holidaystr) == -1)
		holidaystr = "TEXTURE1";
	maptex = maptex1 = W_CacheLumpName(holidaystr, PU_STATIC);
	numtextures1 = LONG(*maptex);
	maxoff = W_LumpLength(W_GetNumForName(holidaystr));
#else
...
static void R_InitColormaps(void)
{
	...
	{
		if(holidaynicks[i])
			holidaystr = va("%sCMAP",holidaynicks[i]);
		else
			holidaystr = "COLORMAP";
		...
		lump = W_GetNumForName(holidaystr);
		holidaycmaps[i] = Z_MallocAlign(W_LumpLength(lump), PU_STATIC, NULL, 16);
		W_ReadLump(lump, holidaycmaps[i]);
		if(!colormaps || holiday == i)
			colormaps = holidaycmaps[i];
	}
#else
...
#ifdef CCPAL
	realcolormaps = colormaps;

	// The colormap for the neon sign palette.
	// Will be swapped with realcolormaps when the palettes change, with colormaps being set by the currently active palette.
	lump = W_GetNumForName("NEONMAP");
	neoncolormaps = Z_MallocAlign(W_LumpLength(lump), PU_STATIC, NULL, 16);
	W_ReadLump(lump, neoncolormaps);
#endif
