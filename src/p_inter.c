...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher, boolean heightcheck)
{
	...
	switch (special->type)
	{
		...
		case MT_REDFLAG:
#ifdef RABBIT // Fixme for dual netcode
			return; // LXShadow added this (red flag special disable).
#endif
		...
		case MT_BLUEFLAG:
#ifdef RABBIT // Fixme for dual netcode
			return; // LXShadow added this (blue flag special disable).
#endif
			if (player->powers[pw_flashing] || player->tossdelay)
				return;
			if (!special->spawnpoint)
...
#else
			if ((UINT32)cv_pointlimit.value <= score)
#endif
			{
				if (server)
					SendNetXCmd(XD_EXITLEVEL, NULL, 0);
...
boolean P_CheckRacers(void)
{
    ...
    for (i = 0; i < MAXPLAYERS; i ++)
    {
        if ((i != self_id && virtualplayers[i].in_game && ! virtualplayers[i].permadead && virtualplayers[i].laps < (unsigned) cv_numlaps.value) ||
            (i == self_id && players[0].lives > 0 && players[0].laps < (unsigned) cv_numlaps.value))
            break;
    }
#endif
...
#endif
#ifndef RABBIT // If rabbit, no deathmessages. Those should be done in ExecuteHitEvent.
...
boolean P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, INT32 damage)
{
...
#ifdef RABBIT
    if (target->type == MT_VIRTUALPLAYER) // If Eggman can hack, I can hack.
        return false;
#endif
	...
	if (player) // Player is the target
	{
...
#ifdef RABBIT // CSTODO: Improve the rest of the weapons
		// LXShadow - CS player hits player
		int i;
		VirtualPlayer* csplayer = NULL;
		boolean willidie = (player->mo->health <= 1 && ! HasShield(player->powers) && player->powers[pw_flashing] <= 0 && player->powers[pw_invulnerability] <= 0 && player->powers[pw_super] <= 0);

        if (damage == 10000) willidie = 1; // Yeah, you're dead mate.

        if (inflictor && inflictor->target)
        {
            for (i = 0; i < MAXPLAYERS; i ++)
            {
                if (virtualplayers[i].in_game && virtualplayers[i].mo == inflictor->target)
                    csplayer = &virtualplayers[i];
            }
        }

        if (csplayer && (gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value)))
        {
            if (!cv_friendlyfire.value && csplayer->ctfteam == player->ctfteam)
            {
                if (!(inflictor->flags & MF_FIRE))
                    P_GivePlayerRings(target->player, 1, false);
        ...
        // Client-side player hits player
        if (csplayer)
        {
            INT8 hit_type = -1;
            ...
            switch (inflictor->type)
            {
                ...
                case MT_SPINFIRE:
                    ...
                    break;
                default:
                    hit_type = 12; // Perform an "unknown hit" event if necessary.
                    break;
            }

            CS_DoHitEvent(inflictor, hit_type, willidie);
            source = inflictor->target; // To be carried on to P_KillMobj in case of death (so as to avoid the -50 points penalty).
        }

        // Nuked!
        if (inflictor && HACK_YOUAREBEINGNUKED)
            CS_DoHitEvent(inflictor, 2, willidie);

        // Client-side player died a dumb death?
        if (! inflictor && ! source)
        {
            INT8 hit_type = -1;
            ...
            if (P_PlayerTouchingSectorSpecial(target->player, 1, 1))
                ...
                hit_type = 12; // Unknown hit.

            /*if (hit_type == 3 || hit_type == 7 || hit_type == 8) // Instant death.
                willidie = 1;*/

            CS_DoHitEvent(NULL, hit_type, willidie);
        }

        // More environment hits.
        if (source && (! inflictor || source == inflictor))
        {
            INT8 hit_type = -1;

            if (source->type == MT_DISS)
            {
                if (source->threshold == 42)
                    hit_type = 9;
            ...
            if (hit_type == 9 || hit_type == 11) // Instant death.
                willidie = 1;
        ...
        // RVZ flares, etc?
        if (! source && inflictor && ! inflictor->target) // "Unknown hit", shared in above code.
            CS_DoHitEvent(NULL, 12, willidie);

        if (player->powers[pw_super])
            CS_DoHitEvent(NULL, 13, 0);
#endif
...
#ifdef RABBIT
	return; // Better sorry than anything (LXShadow PlayerRingBurst).
#endif
...
void P_PlayerFlagBurst(player_t *player, boolean toss)
{
    ...
    if (CS_host)
    {
        PlayerDropFlag(self_id);

        if (toss)
            players[0].tossdelay = 2 * TICRATE;
    }
    else if (CS_client)
        CS_AddEvent(EVENT_DROPMYFLAG | (toss << 5));
