...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher, boolean heightcheck)
{
	...
	if (special->health <= 0)
		return;
#ifdef RIDERS
	if (toucher->player && toucher->player->exiting)
	sound = sfx_None;
	else
#endif
	sound = sfx_itemup;
...
#ifdef RIDERS
		             || toucher->state == &states[S_PLAY_FLY1]
		             || toucher->state == &states[S_PLAY_FLY2]
		             || toucher->state == &states[S_PLAY_FLY3]
		             || toucher->state == &states[S_PLAY_FLY4])) // Tails can shred stuff with his propeller.
#else
	...
	switch (special->type)
	{
...
#ifdef RIDERS
			if (player->bumpertime < TICRATE/4 && player->charexgear == 1 && riderslevel)
			{
				if (!player->exiting)
#else
...
#ifdef RIDERS
				else if (player->charexgear == 1) // What? You're not a fly type? No fly for you!
#else
...
#ifdef RIDERS
					const fixed_t speed = FixedDiv((special->info->speed+142839)*FRACUNIT,75*FRACUNIT);
#else
...
#ifdef RIDERS
			if (player->bumpertime < TICRATE/4 && !riderslevel)
			{
				S_StartSound(player->mo, special->info->seesound);
				if (player->pflags & PF_NIGHTSMODE)
				{
		...
		case MT_EXTRALARGEBUBBLE:
			...
			else
			{
				...
				{
#ifdef RIDERS
					P_SetPlayerMobjState(player->mo, S_PLAY_RPLG1);
#else
...
boolean P_CheckRacers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		if (playeringame[i] && !(players[i].exiting || players[i].spectator) && players[i].lives > 0)
			break;
	}
...
#ifdef RIDERS
	if ((mariomode || kartmode) && inflictor && (inflictor->type == MT_SHELL || inflictor->type == MT_FIREBALL))
#else
...
#ifdef RIDERS
		if (gametype == GT_COOP) // Coop only Draykon 10-08-2007
#else
...
static inline boolean P_TagDamage(mobj_t *target, mobj_t *inflictor, mobj_t *source, INT32 damage)
{
	...
	// Check for a shield
	if (target->player->powers[pw_forceshield] || target->player->powers[pw_jumpshield] || target->player->powers[pw_zringshield] || target->player->powers[pw_bombshield] || target->player->powers[pw_watershield])
	{
		...
		target->player->powers[pw_zringshield] = false;
...
static void P_KillPlayer(player_t *player, mobj_t *source, INT32 damage)
{
	...
	player->powers[pw_zjumpshield] = false;
	player->powers[pw_zringshield] = false;
	player->powers[pw_zbombshield] = false;
	player->powers[pw_zwatershield] = false;
	player->mo->momx = player->mo->momy = player->mo->momz = 0;
...
static inline void P_SuperDamage(player_t *player, mobj_t *inflictor, mobj_t *source, INT32 damage)
{
	...
	P_InstaThrust(player->mo, ang, fallbackspeed);//fallbackspeed);
...
static inline void P_ShieldDamage(player_t *player, mobj_t *inflictor, mobj_t *source, INT32 damage)
{
	...
	if (player->powers[pw_forceshield] == 2) // Multi-hit and reflection effect
	{
		mobj_t *mo;
	...
	//else if (player->powers[pw_forceshield] == 1)
	//	player->powers[pw_forceshield] = false;
	...
	if (player) // Player is the target
	{
		...
		if (damage == 65 && player->position == 1)
		{
			...
			if (!player->powers[pw_invulnerability])
			{
				...
				// Start shrinking!
				player->mo->destscale = 35;
				player->powers[pw_shrink] = 70;
		...
		// Special code for being hit by Nuke Barrier in Mario Kart mode.
		if (damage == 66 && !player->blackow)
		{
			// Don't flip out while super!
			if (!player->powers[pw_invulnerability] && player->powers[pw_shrink] >= 0)
			{
			...
			P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_BOOMERSHIELD);
			return true;
		} else if (damage == 66 && player->blackow) return false;
#endif
		// Sudden-Death mode
		if (source && source->type == MT_PLAYER)
		{
			if ((gametype == GT_MATCH || gametype == GT_TAG || gametype == GT_CTF) && cv_suddendeath.value
				&& player->powers[pw_shrink] >= 0 && !player->powers[pw_invulnerability])
				damage = 10000;
		}
		...
		if (damage == 10000)
		{
			...
			&& (player->powers[pw_invulnerability] || player->powers[pw_shrink] < 0 || player->powers[pw_flashing]
		...
		else if (damage < 10000 && !player->powers[pw_super] && (player->powers[pw_forceshield] || player->powers[pw_jumpshield] || player->powers[pw_zringshield] || player->powers[pw_bombshield] || player->powers[pw_watershield]))  //If One-Hit Shield
		{
			P_ShieldDamage(player, inflictor, source, damage);
			return true;
		}
#ifdef RIDERS
		// No shield but have rings.
		else if ((player->mo->health > 1 && !riderslevel && !kartmode) // Sonic R
		|| (player->mo->health < 102 && riderslevel && !kartmode)      // Sonic Riders
		|| (player->mo->health < 1002 && !riderslevel && kartmode))    // Mario Kart
#else
		...
		if (inflictor && ((inflictor->flags & MF_MISSILE) || inflictor->player) && player->powers[pw_super] && ALL7EMERALDS(player->powers[pw_emeralds]))
		{
			if (player->powers[pw_forceshield] || player->powers[pw_jumpshield] || player->powers[pw_zringshield] || player->powers[pw_bombshield] || player->powers[pw_watershield])
			{
				player->powers[pw_jumpshield] = false; // Get rid of shield
				player->powers[pw_zringshield] = false;
				player->powers[pw_watershield] = false;
