...
{                  
	player_t*   player;
	int         sound;
	int emercount = 0; // Tails 12-12-2001
	int i; // Tails 12-12-2001
	mobj_t*   dummymo;

	if(cv_objectplace.value && toucher->player)
		return;
	...
	if(!player && !(toucher->flags2 & MF2_CHAOATTACK)) // Only players can touch stuff!
		return;
	...
	{
		  if(!player && toucher->flags2 & MF2_CHAOATTACK)
		...
		&& !(special->flags & MF_MISSILE))
	{
	  ...
	  ////////////////////////////////////////////////////////
		  ...
		  if((toucher->z <= special->z + special->height && toucher->z + toucher->height >= special->z) // Are you touching the side of it?
			&& ((toucher->player->nightsmode && toucher->player->drilling) || toucher->player->mfjumped || toucher->player->mfspinning
			|| toucher->player->powers[pw_invulnerability] || toucher->player->powers[pw_super] || toucher->player->powers[pw_shortsword])) // Do you possess the ability to subdue the object?
		  {
			if(toucher->momz < 0)
				toucher->momz = -toucher->momz;
			P_DamageMobj(special, toucher, toucher, 1);
	...
	case MT_NIGHTSHELPER:
		...
		return;
	case MT_NIGHTSWING:
		if (M_CheckParm ("-botdebug") && !player)
			CONS_Printf("Bot has collected a nights wing!\n");
		P_SpawnMobj(special->x, special->y, special->z, MT_SPARK);
		if(player)
		{
			player->linkcount++;
	...
	case MT_GOOP:
		if (M_CheckParm ("-botdebug") && !player)
			CONS_Printf("Bot hurt itself on a spikeball/flame/goop!\n");
		P_DamageMobj(toucher, special, special, 1);
		return;
	...
	case MT_KOOPAFLAME:
		if (M_CheckParm ("-botdebug") && !player)
			CONS_Printf("Bot hurt itself on a Puma/Hammer/Koopa/Koopa Flame!\n");
		P_DamageMobj(toucher, special, special, 1);
		return;
	case MT_SHELL:
	...
	case MT_FLINGCOIN:
		if (M_CheckParm ("-botdebug") && !player)
			CONS_Printf("Bot has collected a coin!\n");
	  ...
	  case MT_EMMY:
		...
		if(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO)
		{
			if(player == &players[consoleplayer])
				CONS_Printf("* Collected an %s!\n",psoitems[0].name); // Yeah, you can name the emeralds "Fried Steak!" for all I care!
			player->psoitems[0]++;
			break;
		}
		#endif
		...
		if(emeralds == 127
		  || (gameskill <= sk_easy && emeralds & EMERALD1
		  && emeralds & EMERALD2 && emeralds & EMERALD3)) //Got em all!
		{
			if(player)
			{
				player->mo->health += 50;
...
// end bubble grab Tails 03-07-2000
      ...
      default:
		...
		#ifdef SPMOVES
		else if(JTE_SpecialItems(toucher, special))
			return;
		#endif
        break;
    }
    ...
    if (special->flags & MF_COUNTITEM && player)
        player->itemcount++;
...
//    if (player == &players[displayplayer] || (cv_splitscreen.value && player==&players[secondarydisplayplayer]))
	if(sound != sfx_None && player)
		S_StartSound (player->mo, sound); // was NULL, but changed to player so you could hear others pick up rings Tails 01-11-2001
	if(sound != sfx_None && !player)
		S_StartSound (toucher, sound); // was NULL, but changed to player so you could hear others pick up rings Tails 01-11-2001
}
			...
			if(cv_gametype.value == GT_CHAOS)
			{
				...
				switch(target->type)
				{
					...
					case MT_CRAWLACOMMANDER:
						...
						break;
					#ifdef SPMOVES
					case MT_DARKFAIRYCHAO:
						score = 450*source->player->scoreadd;
						break;
					#endif
					default:
						score = 100*source->player->scoreadd;
...
//    target->tics -= P_Random()&3;
    ...
    #ifdef SPMOVES
    if(target->type == MT_DARKFAIRYCHAO)
    {
        junk.tag = 651;
	...
	if(target->flags & MF_ENEMY)
	{
		...
		// JTE: No no, it's better this way, since there are flying enemys and such.
		P_SpawnMobj (target->x,target->y,target->z + (target->height / 2), item);
	}
}
                       ...
                       int       damage )
{
	...
	if(target->flags & MF_MONITOR)
	{
		if(!source || !(source->player
		#ifdef SPMOVES
		|| (source->type == MT_FOXBULLET && source->target->player)
		#endif
		) || (inflictor && !inflictor->player)) // Graue 12-06-2003
			return false;
	}
...
//            return false;
    ...
    if(player) // Player is the target
	{
		#ifdef SPMOVES
		if(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO)
			return JTE_PSODamage(target,inflictor,source,damage);
		else
		{
		#endif
...
void P_PlayerRingBurst(player_t* player, int num_rings)
{
	...
	if(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO)
		if(num_rings)
			if(P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_PSOMONEY)->health = num_rings)
			{
				player->nightsfall = false;
				return;
			}
