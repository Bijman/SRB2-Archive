...
#include "z_zone.h" //for some patches and shit
...
void P_CheckTimeLimit(void)
{
	...
	if (cv_overtime.value && cv_timeup.value == 0)
	{
		...
		if ((D_NumPlayers() - spectators) > 1)
		{
			...
			if (gamestate == GS_LEVEL && (leveltime == (timelimitintics+5)))
				S_StartSound(NULL, sfx_sudend);
		...
		if (server && cv_theovertime.value == 1)
			//SendNetXCmd(XD_EXITLEVEL, NULL, 0);
		    CV_SetValue(&cv_victory, 1);
	}

	if (server && cv_theovertime.value == 0)
		//SendNetXCmd(XD_EXITLEVEL, NULL, 0);
	    CV_SetValue(&cv_timeup, 1);
}
