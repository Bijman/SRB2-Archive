...
{
	(void)weapon; //unused
#ifdef SRB2PLUS // no pickup until 3/4 seconds of flashing, no pickup while in painstate to match old flashing
    // todo: clean
    if (player->mo && player->mo->state == &states[S_PLAY_PAIN] && !(player->pflags & PF_SLIDING))
        return false;

    if (!player->startflashingtics && player->powers[pw_flashing])
        return false;

    if (player->startflashingtics && player->startflashingtics >= TICRATE && // so that zoom tubes and whatnot don't screw us up
        player->startflashingtics - player->powers[pw_flashing] < TICRATE*3/4)
        return false;
#else
...
#ifdef SRB2PLUS // new messages for team and self deaths
    boolean selfdamage = false;
    boolean teamdamage = false;
// SRB2PLUS todo: actually use these variables
#endif
...
static void P_HitMessages(mobj_t *target, mobj_t *inflictor, mobj_t *source)
{
	...
	boolean reflected = false;
#ifdef SRB2PLUS // new messages for team and self hits
    boolean selfdamage = false;
    boolean teamdamage = false;
#endif
	if (!target || !target->player)
	...
	if (!source || !source->player)
		return;
//#ifdef SRB2PLUS // tell the world that your teammate hit you or your hit yourself :(
//    if (((gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value)) &&
//		source->player->ctfteam == target->player->ctfteam) || (gametype == GT_TAG && (source->player->pflags & PF_TAGIT) == (target->player->pflags & PF_TAGIT)))
//		return;
//    if (source->player = target->player)
//        return;
// SRB2PLUS todo: actually use these variables
...
#ifndef SRB2PLUS // no more classic match_scoring
			&& cv_match_scoring.value == 0) // Suicide penalty
#else
            )
#endif
		{
#ifdef SRB2PLUS // instead of penalizing suicide, reward the last player and/or team to land a recent hit
            if (target->player->lastinflictor != -1 && playeringame[target->player->lastinflictor])
            {
                // todo: going to need to account for tag...
                if ((((gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value == 1))
                    && target->player->ctfteam != players[target->player->lastinflictor].ctfteam))
                    || (gametype == GT_MATCH && !cv_matchtype.value))
                {
                    P_AddPlayerScore(&players[target->player->lastinflictor], 100);
                    target->player->lastinflictor = -1;
                    target->player->lastinflictortime = 0;
                }
            }
#else
...
static inline boolean P_TagDamage(mobj_t *target, mobj_t *inflictor, mobj_t *source, INT32 damage)
{
	...
	if (player->powers[pw_flashing] || player->powers[pw_invulnerability])
		return false;
#ifndef SRB2PLUS // no friendlyfire option for you
...
{
	player_t *player = target->player;
#ifndef SRB2PLUS // I beg to differ...
...
#ifndef SRB2PLUS // burst weapons and emeralds even if no source
	if (source && (gametype == GT_MATCH || gametype == GT_CTF))
#else
   	if (gametype == GT_MATCH || gametype == GT_CTF) // todo: tag? race?
#endif
	{
...
static inline void P_ShieldDamage(player_t *player, mobj_t *inflictor, mobj_t *source, INT32 damage)
{
	...
	if (source && source->player && !player->powers[pw_super]) //don't score points against super players
#ifndef SRB2PLUS // no more classic match_scoring, todo: need to fix other scoring in this area later anyway
...
boolean P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, INT32 damage)
{
	...
	{
#ifndef SRB2PLUS // grenade self damage
...
// SRB2PLUS todo
		...
		{
#ifndef SRB2PLUS // Do not sacrifice gameplay depth for the sake of griefers
		...
		if (damage < 10000)
		{
			if (!(inflictor && ((inflictor->flags & MF_MISSILE) || inflictor->player) && player->powers[pw_super] && ALL7EMERALDS(player->powers[pw_emeralds])))
			{
#ifdef SRB2PLUS // change post hit flashing mechanics
                // Base post hit invincibility based on the number of rings carried instead of a fixed time
                if (damage < 5)
                    ...
                    player->powers[pw_flashing] = 4*TICRATE;
                else // 50 or greater
                    player->powers[pw_flashing] = 5*TICRATE;

                // todo: mirror for super damage later?
                // set last inflictor to award points to if the target suicides later
                if (source && source->player)
                {
                    UINT8 i;
                    for (i = 0; i < MAXPLAYERS; i++)
                        ...
                        {
                            player->lastinflictor = i;
                            player->lastinflictortime = 15*TICRATE;
                        }
                }
#else
...
#ifdef SRB2PLUS // fuse time for dropped objects
static void P_SetDroppedObjectFuse(mobj_t *dropped, player_t *player, INT32 basetime, INT32 mintime)
{
    if (player)
    {
        INT32 fusetime = basetime - player->losscount;

        if (fusetime < mintime)
            dropped->fuse = mintime * TICRATE;
        else
            dropped->fuse = fusetime * TICRATE;
    }
    else
        dropped->fuse = basetime * TICRATE;
}
#endif
...
void P_PlayerRingBurst(player_t *player, INT32 num_rings)
{
	...
	if (num_rings > 32 && !(player->pflags & PF_NIGHTSFALL))
		num_rings = 32;
#ifdef SRB2PLUS // do not spill weapon panels or emeralds on hit
    if (player->mo->health <= 1)
    {
#endif
	...
	for (i = 0; i < num_rings; i++)
	{
		...
		if (G_IsSpecialStage(gamemap))
		{
			mo = P_SpawnMobj(player->mo->x,
			                 ...
			                 player->mo->z,
			                 MT_FLINGBALL);
#ifdef SRB2PLUS // dropped blue sphere fuse
			P_SetDroppedObjectFuse(mo, player, 8, 1);
#else
		...
		if (mariomode)
		{
			mo = P_SpawnMobj(player->mo->x,
			                 ...
			                 player->mo->z,
			                 MT_FLINGCOIN);
#ifdef SRB2PLUS // dropped mario coin fuse
			P_SetDroppedObjectFuse(mo, player, 8, 1);
#else
			mo->fuse = (8-player->losscount)*TICRATE;
#endif
		...
		else
		{
			mo = P_SpawnMobj(player->mo->x,
			                 ...
			                 player->mo->z,
			                 MT_FLINGRING);
#ifdef SRB2PLUS // dropped ring fuse
			P_SetDroppedObjectFuse(mo, player, 8, 1);
#else
			mo->fuse = (8-player->losscount)*TICRATE;
#endif
...
#ifdef SRB2PLUS // losscount increment
    if (!player->losscount)
        player->losscount = 4;
    else
#endif
...
#ifdef SRB2PLUS // losscount cap
    if (player->losscount > 10) // Don't go over 10.
        player->losscount = 10;
#else
