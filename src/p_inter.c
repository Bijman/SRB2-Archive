...
{
	player_t *player;
	int i, sound = sfx_None, pemercount = 0;

	if (cv_objectplace.value)
		return;

#ifdef SPMOVES
	// Really shouldn't let ANY players with MF_NOCLIP touch enemies or rings, but meh. Not my problem.
	if(toucher->player && toucher->player->chaoscontrol)
		return;
#endif
	...
	if (special->flags & MF_BOSS)
	{
		if ((toucher->z <= special->z + special->height && toucher->z + toucher->height
			...
			|| (toucher->state-states == S_PLAY_FALL1 || toucher->state-states == S_PLAY_FALL2))
#endif
			) || toucher->player->powers[pw_invulnerability]
#ifdef SPMOVES
			|| toucher->player->powers[pw_amydash]
#endif
#ifdef UNIVERSE
			|| toucher->player->powers[pw_sword]
#endif
	...
	else if ((special->flags & MF_ENEMY) && !(special->flags & MF_MISSILE))
	{
		...
		////////////////////////////////////////////////////////
		  if ((toucher->z <= special->z + special->height && toucher->z + toucher->height >= special->z) // Are you touching the side of it?
			...
			|| toucher->player->powers[pw_sword]
#endif
			|| toucher->player->powers[pw_super])) // Do you possess the ability to subdue the object?
		  {
			if (toucher->momz < 0)
				toucher->momz = -toucher->momz;
			P_DamageMobj(special, toucher, toucher, 1);
...
#ifdef RUSSIA
			if(russia
			&& !toucher->player->powers[pw_flashing]
			&& !toucher->player->powers[pw_invulnerability])
			{
				SR_Collect(toucher, special);
				return;
			}
			else if(russia)
				SR_RingSpill(special);
#endif
	...
	// end bubble grab Tails 03-07-2000
		...
		default:
#ifdef UNIVERSE
			if(SU_SpecialItems(toucher, special))
				return;
#endif
			break;
		}
...
#ifdef SUPERVIRUS
	&& (source || !inflictor || inflictor != target)
#endif
	)
		return;
#endif
...
#ifdef SUPERVIRUS1
		if(source->player && source->player->infected)
		{
			CONS_Printf("%s%s%s was made an unholy sacrifice to the supervirus in %s%s%s.\n",
...
#ifndef BUGFIXES // THIS WAS ASSERTED ABOVE TO ALWAYS BE TRUE, DUMMY.
				if (inflictor)
#endif
			{
...
#ifndef BUGFIXES // THIS WAS ASSERTED ABOVE TO NEVER BE TRUE, DUMMY.
			else
			{
				str = "%s%s%s was killed by %s%s%s\n";
...
void P_KillMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source)
{
	...
	if (source && source->player)
	{
		...
		// Award Score Tails
		{
			...
			{
#ifdef UNIVERSE
				// SU mode does EXP differently then SRB2 does scores :P
				if(universe)
				{
					source->player->scoreadd = 0;
					if(sumobjinfo[target->type])
						score = sumobjinfo[target->type]->level*(sumobjinfo[target->type]->damage*20);
					else
						score = target->info->spawnhealth*100;
				}
				else
#endif
...
boolean P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
...
#ifdef SPMOVES
	if(target->player && target->player->chaoscontrol && damage != 10000)
		return false;
#endif

#ifdef SUPERVIRUS1
	if(target->player && target->player->infected)
		return false;
#endif

#ifdef RUSSIA
	if(russia && target->player && inflictor == NULL)
		return false;
#endif
...
#ifdef JTEBOTS
		// Co-op bots dun pop boxes!
		|| (gametype == GT_COOP && source->player->bot)
		|| (gametype == GT_COOP && inflictor && inflictor->player->bot)
#endif
	))
	{
		return false;
...
#ifdef RUSSIA
	if(russia && target->flags & MF_MONITOR
		&& source && source->player)
	{
		SR_PlayerPop(source->player);
		return false;
	}
#endif
	...
	{
		if(damage != 10000)
			return SU_DamageMobj(target, inflictor, source, damage);
		else
			return SU_DamageMobj(target, inflictor, source, -1);
	}
#endif

#ifdef MULTIHPENEMIES
	if(target->flags & MF_ENEMY && target->type != MT_CRAWLACOMMANDER)
	{
...
#ifdef SPMOVES
				if(source->player->powers[pw_amydash])
				{
					source->player->powers[pw_amydash] = 0;
...
#ifdef SPMOVES
			if(source && source->player && source->player->powers[pw_amydash])
			{
				source->player->powers[pw_amydash] = 0;
				source->player->powers[pw_flashing] = TICRATE;
				source->z++;
				if(source->eflags & MF_UNDERWATER)
					...
					source->momz = (69*FRACUNIT)/10;
				P_InstaThrust(source, source->angle-ANG180, 4*FRACUNIT);
				P_SetPlayerMobjState(source, S_PLAY_FALL1);
			}
#endif
		}
...
#ifdef SPMOVES
			if(source->player && source->player->powers[pw_amydash])
			{
				source->player->powers[pw_amydash] = 0;
				source->player->powers[pw_flashing] = TICRATE;
				source->z++;
...
#ifdef SCALEMARIO
				|| target->player->powers[pw_fireflower]
#endif
				)
				{
#ifdef SCALEMARIO
					...
					target->player->powers[pw_fireflower] = false;
#endif
