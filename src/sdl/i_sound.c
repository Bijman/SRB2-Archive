...
#if SDL_MIXER_VERSION_ATLEAST(1,2,4)
...
static boolean LoadSong(void *data, int lumplength, size_t selectpos)
{
	...
	music[selectpos] = Mix_LoadMUS_RW(musicRW->RW_mem); // new Mix_Chuck from RWops
...
boolean I_StartDigSong(const char *musicname, int looping)
{
	...
	if (!looping && loopingDig)
	{
		...
		if (loopstart > 0)
		{
			loopstartDig = (8L+loopstart) / 44100.0L; //8 PCM chucks off and PCM to secs
#ifdef PARANOIA
			//CONS_Printf("looping start at %g\n",(double)loopstartDig);
#endif
		}
		else
		{
