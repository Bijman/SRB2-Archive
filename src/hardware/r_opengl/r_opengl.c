...
#ifdef USE_PALETTED_TEXTURE
		if (glColorTableEXT &&
			(pTexInfo->glInfo.format == GR_TEXFMT_P_8) &&
			!(pTexInfo->flags & TF_CHROMAKEYED))
		{
			// Do nothing here.
			// Not a problem with MiniGL since it doesn't use paletted textures
		}
		else
#endif
#ifdef KOS_GL_COMPATIBILITY
				...
				(pTexInfo->glInfo.format == GR_TEXFMT_AP_88))
			{
				...
				for (j = 0; j < h; j++)
				{
					for (i = 0; i < w; i++)
					{
						...
						{
							if (pTexInfo->glInfo.format == GR_TEXFMT_AP_88 && !(pTexInfo->flags & TF_CHROMAKEYED))
								tex[w*j+i] = 0;
							else
			...
			else if (pTexInfo->glInfo.format == GR_RGBA)
			{
				...
				const RGBA_t *pImgData = (const RGBA_t *)pTexInfo->glInfo.data;
				int i, j;
			...
			else if (pTexInfo->glInfo.format == GR_TEXFMT_ALPHA_INTENSITY_88)
			{
				...
				CONS_LogPrintf("OpenGL Error: GL_SetTexture(bad format) %d\n", pTexInfo->glInfo.format);
#else
		if ((pTexInfo->glInfo.format == GR_TEXFMT_P_8) ||
			(pTexInfo->glInfo.format == GR_TEXFMT_AP_88))
		{
...
#else
#ifdef USE_PALETTED_TEXTURE // SRB2CBTODO: REMOVE THE PALETTE LIMIT!!!!!!!
		if (glColorTableEXT &&
			(pTexInfo->glInfo.format == GR_TEXFMT_P_8) &&
			!(pTexInfo->flags & TF_CHROMAKEYED))
		{
			glColorTableEXT(GL_TEXTURE_2D, GL_RGB8, 256, GL_RGB, GL_UNSIGNED_BYTE, palette_tex);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_COLOR_INDEX8_EXT, w, h, 0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, pTexInfo->glInfo.data);
		}
		else
#endif
