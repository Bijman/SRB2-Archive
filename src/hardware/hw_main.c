...
consvar_t cv_granisotropicmode = {"gr_anisotropicmode", "8", CV_CALL|CV_SAVE, granisotropicmode_cons_t,
...
static ULONG HWR_Lighting(int light, ULONG color, boolean lighter, boolean fogcolor)
{
	...
	if (RealColor.rgba == NORMALFOG) // Default fog
	{
		...
		GL_SetSpecialState(HWD_SET_FOG_MODE, 0); // SRB2CBTODO: Fade in and out lighting like software mode?
		GL_SetSpecialState(HWD_SET_FOG_DENSITY, 0); // Need fog for light distancing when light is less than 192 
		GL_SetSpecialState(HWD_SET_FOG_COLOR, NORMALFOG); // Default fog color SRB2CBTODO: Time of day + SEASONS!
...
#ifdef R_FAKEFLOORS // Render floors of FOFs
	if (gr_frontsector->ffloors)
	{
	...
	switch(spr->mobj->type)
	{
		...
		case MT_NIGHTSPARKLE:
			...
			HWR_DrawCorona(FIXED_TO_FLOAT(spr->mobj->x), FIXED_TO_FLOAT(spr->mobj->y),
						   ...
						   HWR_Lighting(LightLevelToLum(200), NORMALFOG, false), 8.0f); // 0xffffefff
			break;
		default:
			break;
	}
...
{	
	...
	switch(spr->mobj->type)
	{
		...
		HWR_DrawCorona(FIXED_TO_FLOAT(spr->mobj->x), FIXED_TO_FLOAT(spr->mobj->y),
					   FIXED_TO_FLOAT(spr->mobj->z)+FIXED_TO_FLOAT(spr->mobj->height>>1),
					   HWR_Lighting(LightLevelToLum(200), NORMALFOG, false, true), 8.0f); // 0xffffefff
			break;
		default:
			break;
	}
...
static void HWR_AddSprites(sector_t *sec)
{
	...
	{
		// Special rendering clipping for precipitation
