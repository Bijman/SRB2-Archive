...
#include "../JTE/JTEMisc.h" // SPRITESHADOWS SPRITEOFFS
...
#ifdef HARDWAREFIX2
#define FOGFACTOR 600
#define NORMALFOG 0x19000000
#define CALCFOGALPHA(x,y) ((float)(x)/((0x19 - (y))/12.0f+1.0f))
#define CALCLIGHT(x,y) ((float)(x)/((0xFF - (y))/127.0f+1.0f))
static ULONG HWR_Lighting(int light, ULONG color)
{
	RGBA_t realcolor, surfcolor;
	static int lastlight = -1;
	static ULONG lastcolor = 0;
	...
	if(cv_grfog.value)
	{
		surfcolor.s.red = 0xFF-(realcolor.s.green+realcolor.s.blue)/2;
		surfcolor.s.green = 0xFF-(realcolor.s.red+realcolor.s.blue)/2;
		surfcolor.s.blue = 0xFF-(realcolor.s.red+realcolor.s.green)/2;
		surfcolor.s.alpha = 0xFF;

		// Modulate the colors by alpha.
		realcolor.s.red = (byte)CALCFOGALPHA(realcolor.s.red,realcolor.s.alpha);
		realcolor.s.green = (byte)CALCFOGALPHA(realcolor.s.green,realcolor.s.alpha);
		realcolor.s.blue = (byte)CALCFOGALPHA(realcolor.s.blue,realcolor.s.alpha);

		// Set the fog options.
		if(lastlight == -1 || lastlight != light)
		{
			lastlight = light;
			light = CALCFOGALPHA(light,realcolor.s.alpha);
			HWD.pfnSetSpecialState(HWD_SET_FOG_DENSITY, (int)(FOGFACTOR-(FOGFACTOR*(float)light/255.0f)));
		}
			 ...
			 FBITFIELD PolyFlags, int lightlevel, int lumpnum, sector_t *FOFsector)
{
	...
	if (gr_frontsector)
	{
		...
		{
#ifdef HARDWAREFIX2
			Surf.FlatColor.rgba = HWR_Lighting(LightLevelToLum(lightlevel),sector->extra_colormap->rgba);
#else
			RGBA_t temp;
			int alpha;
...
static void HWR_DrawSprite(gr_vissprite_t *spr)
{
...
#ifdef SUPERFIX
	&& (spr->mobj->sprite != SPR_PLAY || (spr->mobj->player && spr->mobj->player->powers[pw_super]))
#endif
	)
	...
	{
#ifdef SPRITEOFFS
		if(spr->mobj && spriteoffs[spr->mobj->sprite])
		{
			const spriteoffs_t *offs = spriteoffs[spr->mobj->sprite];
#ifdef MOBJSCALE
			const float scale = (float)spr->mobj->scale;
			wv->x += FLOATSCALE(offs->x[i],scale);
			wv->y += FLOATSCALE(offs->y[i],scale);
			wv->z += FLOATSCALE(offs->z[i],scale);
#else
			wv->x += offs->x[i];
			wv->y += offs->y[i];
			wv->z += offs->z[i];
	...
	/// \todo coloured
#ifdef HARDWAREFIX2
	Surf.FlatColor.rgba = HWR_Lighting(spr->sectorlight,NORMALFOG);
#else
...
#endif
	{
		...
		{
#ifdef HARDWAREFIX2
			Surf.FlatColor.rgba = HWR_Lighting(spr->sectorlight,sector->extra_colormap->rgba);
#else
			RGBA_t temp;
			int alpha;
	...
	&& !((spr->mobj->frame & FF_TRANSMASK) == tr_transfir<<FF_TRANSSHIFT) // transfir have no shadow
	&& !((spr->mobj->frame & FF_TRANSMASK) == tr_transfx1<<FF_TRANSSHIFT) // transfx1 have no shadow
		...
		&& (!spr->mobj->player || spr->mobj->player->powers[pw_super]))) // Except for non-super players.
	{
		...
		mobjfloor = JTE_OpaqueFloorAtPos(
		...
		wallVerts[0].y = wallVerts[1].y = wallVerts[2].y = wallVerts[3].y =
		...
		wallVerts[0].z = wallVerts[1].z = spr->tz - (gpatch->height-gpatch->topoffset) + 3;
		wallVerts[2].z = wallVerts[3].z = spr->tz + gpatch->height - (gpatch->height-gpatch->topoffset) + 3;
		...
		{
			const float radius = (spr->x2-spr->x1)/2;
			wallVerts[0].x = wallVerts[3].x += FLOATSCALE(radius,(float)(100-spr->mobj->scale));
			wallVerts[2].x = wallVerts[1].x -= FLOATSCALE(radius,(float)(100-spr->mobj->scale));
			wallVerts[0].z = wallVerts[1].z += FLOATSCALE(abs(gpatch->topoffset-gpatch->height),(float)(100-spr->mobj->scale));
			wallVerts[2].z = wallVerts[3].z += FLOATSCALE(abs(gpatch->topoffset-gpatch->height),(float)(100-spr->mobj->scale));
			wallVerts[2].z = wallVerts[3].z -= FLOATSCALE(gpatch->height,(float)(100-spr->mobj->scale));
		}
#endif
		...
		if(spr->mobj->frame & FF_TRANSMASK || spr->mobj->frame & FF_SMOKESHADE || spr->mobj->flags2 & MF2_SHADOW)
		{
			// sprite lighting by modulating the RGB components
#ifdef HARDWAREFIX2
			Surf.FlatColor.rgba = HWR_Lighting(spr->sectorlight/2,NORMALFOG);
#else
			Surf.FlatColor.s.red = Surf.FlatColor.s.green = Surf.FlatColor.s.blue = spr->sectorlight/2;
#endif
		...
		if (!cv_grmd2.value || (md2_models[spr->mobj->sprite].scale < 0.0f))
		{
			...
			{
				Surf.FlatColor.s.alpha = (byte)(0x80 - floorheight/4);
				HWD.pfnDrawPolygon(&Surf, wallVerts, 4, PF_Translucent|PF_Modulated|PF_Clip);
			}
		}
	}
	noshadow:
...
static void HWR_ProjectSprite(mobj_t *thing)
{
	...
	{
		CONS_Printf("Warning: Mobj of type %i with invalid sprite data detected and removed.\n", thing->type);
		if(!thing->player)
	...
	if (thing->flags & MF_TRANSLATION)
	{
		...
		if (vis->mobj->skin) // This thing is a player!
			vis->colormap = (byte *)translationtables[(skin_t*)vis->mobj->skin-skins] - 256 + ((thing->flags & MF_TRANSLATION) >> (MF_TRANSSHIFT-8));
#else
	...
	if (thing->flags & MF_HIRES)
		vis->ty = FIXED_TO_FLOAT(thing->z + spritetopoffset[lump]/2) - gr_viewz;
	else
...
#ifdef HARDWAREFIX
	{
		...
		if(sector)
		{
			if(sector->ffloors)
			{
				...
				caster = sector->lightlist[R_GetPlaneLight(sector, thing->z+thing->height/2, false)].caster;
				sector = caster ? &sectors[caster->secnum] : sector;
			}
...
void HWR_RenderPlayerView(int viewnumber, player_t *player)
{
	...
	if(virus)
	{
		HWD.pfnSetSpecialState(HWD_SET_FOG_COLOR, 0x000000);
		HWD.pfnSetSpecialState(HWD_SET_FOG_DENSITY, 200);
		HWD.pfnSetSpecialState(HWD_SET_FOG_MODE, 1);
	}
	else
#endif
