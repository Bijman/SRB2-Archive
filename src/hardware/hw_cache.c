...
static void HWR_PatchPNG(int texnum, GLTexture_t *grtex)
{


    //if (!grtex->mipmap.downloaded && !grtex->mipmap.glInfo.data)
	{
	    // Determine the lump this texture has, and read it directly from the wad file

	    //textures[texnum].name''
...
GLTexture_t *HWR_GetTexture(int tex, boolean anisotropic)
{
	...
	{
	    if (textures[tex]->tx == true && textures[tex]->wadlocation != 0) // Only works for TX_START textures
	    {
            ...
            if ((lumpnum != LUMPERROR) && (W_LumpIsPng(lumpnum) == true))
            {
               HWR_PatchPNG(tex, grtex);
            }
            else
...
// Download a SRB2 'flat' to the hardware cache and make it ready for use
