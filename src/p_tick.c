...
#ifdef JTEFUNCODE
#include "st_stuff.h" // For objectplace mode's objects on screen counter thing.
#endif

#include "JTE/JB_Bot.h" // JTBOTS
...
void P_Ticker(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
		if (playeringame[i])
		{
			...
			{
				JB_BotThink(i);
				// Possible for JB_BotThink to put bots in PST_REBORN.
			...
			if (ssrings >= totalrings && totalrings > 0)
			{
				...
				if (!(emeralds & EMERALD1))
				{
					...
					for (i = 0; i < MAXPLAYERS; i++)
						if (playeringame[i])
#ifdef SCALESIZE
							P_SpawnMobj(players[i].mo->x, players[i].mo->y, players[i].mo->z + players[i].mo->height, MT_GREENEMERALD);
#else
				...
				else if ((emeralds & EMERALD1) && !(emeralds & EMERALD2))
				{
					...
					for (i = 0; i < MAXPLAYERS; i++)
						if (playeringame[i])
#ifdef SCALESIZE
							P_SpawnMobj(players[i].mo->x, players[i].mo->y, players[i].mo->z + players[i].mo->height, MT_ORANGEEMERALD);
#else
				...
				else if ((emeralds & EMERALD2) && !(emeralds & EMERALD3))
				{
					...
					for (i = 0; i < MAXPLAYERS; i++)
						if (playeringame[i])
#ifdef SCALESIZE
							P_SpawnMobj(players[i].mo->x, players[i].mo->y, players[i].mo->z + players[i].mo->height, MT_PINKEMERALD);
#else
				...
				else if ((emeralds & EMERALD3) && !(emeralds & EMERALD4))
				{
					...
					for (i = 0; i < MAXPLAYERS; i++)
						if (playeringame[i])
#ifdef SCALESIZE
							P_SpawnMobj(players[i].mo->x, players[i].mo->y, players[i].mo->z + players[i].mo->height, MT_BLUEEMERALD);
#else
				...
				else if ((emeralds & EMERALD4) && !(emeralds & EMERALD5))
				{
					...
					for (i = 0; i < MAXPLAYERS; i++)
						if (playeringame[i])
#ifdef SCALESIZE
							P_SpawnMobj(players[i].mo->x, players[i].mo->y, players[i].mo->z + players[i].mo->height, MT_REDEMERALD);
#else
				...
				else if ((emeralds & EMERALD5) && !(emeralds & EMERALD6))
				{
					...
					for (i = 0; i < MAXPLAYERS; i++)
						if (playeringame[i])
#ifdef SCALEMISC
							P_SpawnMobj(players[i].mo->x, players[i].mo->y, players[i].mo->z + players[i].mo->height, MT_LIGHTBLUEEMERALD);
#else
				...
				else if ((emeralds & EMERALD6) && !(emeralds & EMERALD7))
				{
					...
					for (i = 0; i < MAXPLAYERS; i++)
						if (playeringame[i])
#ifdef SCALESIZE
							P_SpawnMobj(players[i].mo->x, players[i].mo->y, players[i].mo->z + players[i].mo->height, MT_GREYEMERALD);
#else
