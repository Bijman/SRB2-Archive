...
void CONS_Printf(const char *fmt, ...)
{
	...
	{
#if 0
		// Only software mode can draw a picture here, because the OpenGL surface didn't setup yet
#if (defined (_WINDOWS)) || (defined (__OS2__) && !defined (SDL))
		CON_DrawBackpic(con_backpic, 0, vid.width); // Draw a startup picture
		I_LoadingScreen(txt); // Print the startup text
#else
		// Draw a startup picture(software only) and, 
		// for both renderers, the startup text
		CON_Drawer();
		I_FinishUpdate(); // Update the screen with a CONS_Print // SRB2CBTODO: NO VSYNC, also make a REAL function for no vsync
#endif
#endif
#if (defined (_WINDOWS)) || (defined (__OS2__) && !defined (SDL))
