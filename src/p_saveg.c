...
#ifdef RIDERS
	WRITEUINT8(save_p, player->charexgear);
	WRITEUINT8(save_p, player->thokcolor);
	WRITEUINT8(save_p, player->prefthokcolor);
	WRITEUINT8(save_p, player->runframe);
	WRITEUINT8(save_p, player->cheese);
	WRITEUINT8(save_p, player->chip);
	WRITEUINT8(save_p, player->birdie);
	WRITEUINT8(save_p, player->starpostwp);
...
#ifdef RIDERS
	savedata.charexgear = READUINT8(save_p);
	savedata.thokcolor = READUINT8(save_p);
	savedata.prefthokcolor = READUINT8(save_p);
	savedata.runframe = READUINT8(save_p);
	savedata.cheese = READUINT8(save_p);
	savedata.chip = READUINT8(save_p);
	savedata.birdie = READUINT8(save_p);
	savedata.starpostwp = READUINT8(save_p);
...
#ifdef RIDERS
		WRITEINT32(save_p, players[i].cheese);
		WRITEINT32(save_p, players[i].chip);
		WRITEINT32(save_p, players[i].birdie);
		WRITEINT32(save_p, players[i].cutawayz);
		WRITEINT32(save_p, players[i].position);
#endif
	}
}
...
#ifdef RIDERS
		players[i].cheese = READINT32(save_p);
		players[i].chip = READINT32(save_p);
		players[i].birdie = READINT32(save_p);
		players[i].cutawayz = READINT32(save_p);
