...
//        PADSAVEP();
		...
		#ifdef SPMOVES
		for(j=0;j<NUMPSOITEMS;j++)
			WRITELONG(save_p, players[i].psoitems[j]);

                WRITELONG(save_p, players[i].equipweapon);
                WRITELONG(save_p, players[i].equipbarrier);
                WRITELONG(save_p, players[i].equiparmor);
                WRITELONG(save_p, players[i].psomoney);
                WRITELONG(save_p, players[i].psomagic);
                WRITELONG(save_p, players[i].psolevel);
                WRITELONG(save_p, players[i].psoscore);
        #endif
	}
}
...
//        PADSAVEP();
		...
		#ifdef SPMOVES
        for(j=0;j<NUMPSOITEMS;j++)
            players[i].psoitems[j] = READLONG(save_p);

            players[i].equipweapon = READLONG(save_p);
            players[i].equipbarrier = READLONG(save_p);
            players[i].equiparmor = READLONG(save_p);
            players[i].psomoney = READLONG(save_p);
            players[i].psomagic = READLONG(save_p);
            players[i].psolevel = READLONG(save_p);
            players[i].psoscore = READLONG(save_p);
        #endif
    }
}
...
void P_NetArchivePlayers (void)
{
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
		...
		if(players[i].equipmag)
		{
			flags |= EQUIPMAG;
			WRITEULONG(save_p, players[i].equipmag->mobjnum);
		}

	if(players[i].chao)
	{
		flags |= CHAO;
		WRITEULONG(save_p, players[i].chao->mobjnum);
	}
	if(players[i].ride)
	{
		flags |= RIDE;
		WRITEULONG(save_p, players[i].ride->mobjnum);
	}
        #endif
		...
		#ifdef SPMOVES
		WRITEBYTE(save_p, players[i].rpLevel);
		WRITEBYTE(save_p, players[i].rpScore);
		#endif
...
void P_NetUnArchivePlayers (void)
{
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
		...
		if(flags & EQUIPMAG)
			players[i].equipmag = (mobj_t*) READULONG(save_p);

		if(flags & CHAO)
			players[i].chao = (mobj_t*) READULONG(save_p);

		if(flags & RIDE)
			players[i].ride = (mobj_t*) READULONG(save_p);
		#endif
		...
		#ifdef SPMOVES
		players[i].rpLevel = READBYTE(save_p);
		players[i].rpScore = READBYTE(save_p);
		#endif
              ...
              else
              {
                  for (i=0 ; i< NUMMOBJTYPES ; i++)
                       if (mobj->spawnpoint->type == mobjinfo[i].doomednum)
                           break;
                  if ( i == NUMMOBJTYPES )
                      I_Error("The savegame is corrupt\n");
                  mobj->type = i;
              }
...
boolean P_LoadGame (void)
{
    ...
    for(i=0;i<MAXPLAYERS;i++)
	if(playeringame[i])
		SetSavedSkin(i, players[i].skin, players[i].prefcolor);
