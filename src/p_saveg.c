...
#include "JTE/JTEMisc.h" // HOLIDAYS
...
#ifdef JTEBOTS
		WRITEBYTE(save_p, (boolean)(players[i].bot));
...
#ifdef ZELDA
		WRITEBYTE(save_p, players[i].maxhearts);
		WRITEBYTE(save_p, players[i].equipped);
		WRITEBYTE(save_p, players[i].bombsout);
		WRITEBYTE(save_p, players[i].swording);
		WRITELONG(save_p, players[i].bombtics);
		WRITEBYTE(save_p, players[i].bowmode);
		WRITEBYTE(save_p, players[i].swimming);
		WRITEBYTE(save_p, players[i].gettingiteminfo);
		WRITEBYTE(save_p, players[i].fairy);
#endif
#if defined(JTEFLYCHAINS) || defined(JTEBOTS)
		WRITEBYTE(save_p, players[i].carrying);
#endif
#ifdef SPMOVES
		WRITEBYTE(save_p, players[i].chaoscontrol);
		if(players[i].jet)
			flags |= JET;
#endif
#ifdef UNIVERSE
		WRITEBYTE(save_p, players[i].sulevel);
		WRITEUSHORT(save_p, players[i].suexp);
		WRITEUSHORT(save_p, players[i].sumaxhp);
		WRITEUSHORT(save_p, players[i].sumaxtp);
		WRITEUSHORT(save_p, players[i].sutp);
		WRITEUSHORT(save_p, players[i].sumoney);
		for(j = 0; j < NUMSUITEMS; j++)
			WRITEBYTE(save_p, players[i].suitems[j]);
		WRITEULONG(save_p, players[i].equipweapon);
		WRITEULONG(save_p, players[i].equipbarrier);
		WRITEULONG(save_p, players[i].equiparmor);
		WRITEULONG(save_p, players[i].equipmag);
		for(j = 0; j < 4; j++)
			WRITEULONG(save_p, players[i].equipslot[j]);
#endif
#ifdef SUPERVIRUS1
		WRITEUSHORT(save_p, players[i].infected);
		WRITEBYTE(save_p, players[i].confusion);
		WRITEBYTE(save_p, players[i].confusiontime);
#endif
#ifdef SUPERVIRUS
		WRITEBYTE(save_p, players[i].fizz);
#endif
#ifdef JTEBOTS
		WRITEBYTE(save_p, bots[i].ownernum);
		WRITEBYTE(save_p, bots[i].springmove);
		WRITEBYTE(save_p, bots[i].targettimer);
		bots[i].targetlist = NULL; // Todo: Send target list rather then destroying it.
#endif
#ifdef RUSSIA
		if(players[i].ring)
			flags |= RING;
...
#ifdef SPMOVES
		if(flags & JET)
			WRITEULONG(save_p, players[i].jet->mobjnum);
#endif

#ifdef RUSSIA
		if(flags & RING)
			WRITEULONG(save_p, players[i].ring->mobjnum);
#endif
...
#ifdef ZELDA
		players[i].maxhearts = READBYTE(save_p);
		players[i].equipped = READBYTE(save_p);
		players[i].bombsout = READBYTE(save_p);
		players[i].swording = READBYTE(save_p);
		players[i].bombtics = READLONG(save_p);
		players[i].bowmode = READBYTE(save_p);
		players[i].swimming = READBYTE(save_p);
		players[i].gettingiteminfo = READBYTE(save_p);
		players[i].fairy = READBYTE(save_p);
#endif

#if defined(JTEFLYCHAINS) || defined(JTEBOTS)
		players[i].carrying = READBYTE(save_p);
#endif
#ifdef SPMOVES
		players[i].chaoscontrol = READBYTE(save_p);
#endif
#ifdef UNIVERSE
		players[i].sulevel = READBYTE(save_p);
		players[i].suexp = READUSHORT(save_p);
		players[i].sumaxhp = READUSHORT(save_p);
		players[i].sumaxtp = READUSHORT(save_p);
		players[i].sutp = READUSHORT(save_p);
		players[i].sumoney = READUSHORT(save_p);
		for(j = 0; j < NUMSUITEMS; j++)
			players[i].suitems[j] = READBYTE(save_p);
		players[i].equipweapon = READULONG(save_p);
		players[i].equipbarrier = READULONG(save_p);
		players[i].equiparmor = READULONG(save_p);
		players[i].equipmag = READULONG(save_p);
		for(j = 0; j < 4; j++)
			players[i].equipslot[j] = READULONG(save_p);
#endif
#ifdef SUPERVIRUS1
		players[i].infected = READUSHORT(save_p);
		players[i].confusion = READBYTE(save_p);
		players[i].confusiontime = READBYTE(save_p);
#endif
#ifdef SUPERVIRUS
		players[i].fizz = READBYTE(save_p);
#endif
#ifdef JTEBOTS
		bots[i].ownernum = READBYTE(save_p);
		bots[i].springmove = READBYTE(save_p);
		bots[i].targettimer = READBYTE(save_p);
		bots[i].targetlist = NULL; // Todo: Read target list rather then assuming NULL.
#endif
...
#ifdef SPMOVES
		if(flags & JET)
			players[i].jet = (mobj_t*)(size_t)READULONG(save_p);
#endif

#ifdef RUSSIA
		if(flags & RING)
			players[i].ring = (mobj_t*)(size_t)READULONG(save_p);
#endif
...
#ifdef DARKWORLD
	MD_DARKSHADOW  = 0x8000000,
#endif
#ifdef JTEBOTS
	MD_BOT         = 0x10000000,
	MD_BOTTARGET   = 0x20000000,
#endif
#ifdef MOBJSCALE
	MD_SCALE       = 0x40000000,
	MD_DSCALE      = 0x80000000,
#endif
} mobj_diff_t;
...
#ifdef JTEBOTS
                // All bot struct stuff goes here.
				if(diff & MD_BOT)
				{
					if(diff & MD_BOTTARGET) // Welcome to sharp pointy pointer land!
						WRITEULONG(save_p, mobj->player->bot->target->mobjnum); // Ouch! X_x
				}
#endif
			}
			if (diff & MD_MOVEDIR)
...
#ifdef DARKWORLD
			if(diff & MD_DARKSHADOW)
				WRITEULONG(save_p, mobj->darkshadow->mobjnum);
#endif
#ifdef MOBJSCALE
			if(diff & MD_SCALE)
...
#ifdef DARKWORLD
				if(diff & MD_DARKSHADOW)
					mobj->darkshadow = (mobj_t *)(size_t)READULONG(save_p);
#endif
#ifdef MOBJSCALE
				if(diff & MD_SCALE)
...
#ifdef SPMOVES
			if(mobj->player && mobj->player->jet)
			{
				mobj->player->jet = FindNewPosition((size_t)mobj->player->jet);
				if(!mobj->player->jet)
					CONS_Printf("jet not found on %d\n", mobj->type);
			}
#endif
#ifdef RUSSIA
			if(mobj->player && mobj->player->ring)
			{
				mobj->player->ring = FindNewPosition((size_t)mobj->player->ring);
				if(!mobj->player->ring)
					CONS_Printf("ring not found on %d\n", mobj->type);
			}
#endif
		}
	}
...
#ifdef UNIVERSE
	WRITEBYTE(save_p, universe);
#endif
#ifdef RUSSIA
	WRITEBYTE(save_p, russia);
#endif
}
...
#ifdef UNIVERSE
	universe = READBYTE(save_p);
#endif
#ifdef RUSSIA
	russia = READBYTE(save_p);
#endif
...
#ifdef HOLIDAYS
	WRITEBYTE(save_p, serverholiday);
	WRITEBYTE(save_p, holiday);
#else
...
#endif
#ifdef ZELDA
	WRITEBYTE(save_p, zelda);
#endif
#ifdef SUPERVIRUS
	WRITEBYTE(save_p, virus);
#endif
...
#ifdef HOLIDAYS
	serverholiday = READBYTE(save_p);
	holiday = READBYTE(save_p);
	JTE_LoadHolidayCfg();
#else
...
#endif
#ifdef ZELDA
	zelda = READBYTE(save_p);
#endif
#ifdef SUPERVIRUS
	virus = READBYTE(save_p);
#endif
