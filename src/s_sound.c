...
#ifndef BUGFIXES // Don't check twice. :E
	// check for bogus sound #
...
#ifdef HOLIDAYS
	for(replace = holidayreplace; replace; replace = replace->next)
	{
		if(replace->type == 5 && !strcmp(sfx->name,replace->original))
		{
			sfx_id = atoi(replace->replace);
			sfx = &S_sfx[sfx_id];
			break;
		}
	}
...
/*#ifdef MOBJSCALE
	if(origin)
		pitch = (int)FLOATSCALE((float)NORM_PITCH,origin->scale);
	else
#endif*/
	pitch = NORM_PITCH;
...
#if !defined(SUPERVIRUS3) && !defined(JTEMISC) // Keep cleaning up sounds outside of levels.
...
#if defined(SUPERVIRUS3) || defined(JTEMISC)
	if(gamestate != GS_LEVEL)
		; // No listeners outside of levels.
	else
#endif
#ifndef JTEMISC
	if (cv_chasecam.value)
...
static boolean S_MIDIMusic(musicinfo_t *music, int looping)
{
	...
	{
#ifdef HOLIDAYS
		holidaystr = music->name;
		for(replace = holidayreplace; replace; replace = replace->next)
			if(replace->type == 1 && !stricmp(holidaystr, replace->original))
			{
				holidaystr = replace->replace;
				break;
			}
		if (W_CheckNumForName(va("d_%s", holidaystr)) == -1)
			return false;
		music->lumpnum = W_GetNumForName(va("d_%s", holidaystr));
#else
	...
	if (msg_id != -1)
	{
		...
		if (W_CheckNumForName(va("d_%s", holidaystr)) == -1)
			return false;
		sprintf(msg_buffer.msg_text, "d_%s", holidaystr);
#else
...
static boolean S_DigMusic(musicinfo_t *music, int looping)
{
	...
	if (!I_StartDigSong(holidaystr, looping))
#else
