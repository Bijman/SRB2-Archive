...
static patch_t *pinghud = NULL;    // kek

void SCR_DisplayPingRate(void)
{
	if (cv_playersping.value >= 70 && cv_playersping.value < 140 && netgame && gamestate != GS_TITLESCREEN) pinghud = W_CachePatchName("NETYELOW", PU_STATIC);
	else if (cv_playersping.value < 70 && netgame && gamestate != GS_TITLESCREEN) pinghud = W_CachePatchName("NETGREEN", PU_STATIC);
	else if (cv_playersping.value >= 140 && cv_playersping.value < 210 && netgame && gamestate != GS_TITLESCREEN) pinghud = W_CachePatchName("NETORG", PU_STATIC);
	else if (cv_playersping.value >= 210 && netgame && gamestate != GS_TITLESCREEN) pinghud = W_CachePatchName("NETRED", PU_STATIC);
	
	
	if (netgame && gamestate != GS_TITLESCREEN) V_DrawScaledPatch(0, 0, 0, pinghud);
