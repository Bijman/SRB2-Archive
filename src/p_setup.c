...
#define MAXLEVELFLATS 256 // SRB2CBTODO: Needs more?
...
boolean P_AddWadFile(const char *wadfilename, char **firstmapname)
{
	...
	boolean txaddonly = false; // Does a wad have TX_ textures and no old skool style textures? Then just add to the total texture list!
	...
	for (i = 0; i < numlumps; i++, lumpinfo++)
	{
		...
		if (name[0] == 'D')
		{
			...
			txaddonly = true;
#if 1 // SRB2CBTODO: texturechange check
	...
	if (texturechange) // initialized in the sound check
		R_LoadTextures(wadnum, false); // numtexture changes
	else if (!texturechange && txaddonly) // initialized in the sound check
		R_LoadTextures(wadnum, true); // numtexture changes
	else
...
boolean P_DelWadFile(void) // SRB2CBTODO: full support
{
	size_t i;
	...
	W_UnloadWadFile(wadnum); // SRB2CBTODO: full support
	R_LoadTextures(wadnum, false);
	return false;
}
