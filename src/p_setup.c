...
#ifdef DARKWORLD
	mapheaderinfo[i-1].darkworld = false;
#endif
}
...
#ifdef HOLIDAYS
		// Check for holiday flat replacements
		for(hflat = holidayreplace; hflat; hflat = hflat->next)
			if(hflat->type == 0 && *(int *)hflat->original == v1 && *(int *)&hflat->original[4] == v2)
			{
				flatname = hflat->replace;
				break;
			}
#endif
...
#ifdef SACITY
	if(gamemap-1 == SCM_WaitingZone
	|| gamemap-1 == SCM_SACity)
	{
		gametype = GT_COOP;
		if(server)
		{
			CV_SetValue(&cv_itemrespawntime, 10);
			CV_Set(&cv_tailspickup, "On");
...
noscript:
	...
	// xmas mode
#ifdef HOLIDAYS
	if (maptol & TOL_XMAS && holiday != H_XMAS)
	{
		holiday = H_XMAS;
		JTE_LoadHolidayCfg();
	}
	else if(holiday != serverholiday)
	{
		holiday = serverholiday;
		JTE_LoadHolidayCfg();
	}
#else
	if (maptol & TOL_XMAS)
