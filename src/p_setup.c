...
void I_FinishUpdate();
static void P_ClearSingleMapHeaderInfo(int i);
...
void P_ClearMapHeaderInfo(void)
{
	...
	for(i=1; i<NUMMAPS+1; i++)
		P_ClearSingleMapHeaderInfo(i); // JTE: Why was there a need for a double list in the first place?
}
...
static void P_ClearSingleMapHeaderInfo(int i)
{
	...
	mapheaderinfo[i-1].sky2num = 1;
	mapheaderinfo[i-1].sky3num = 1;
	mapheaderinfo[i-1].sky4num = 1;
	mapheaderinfo[i-1].sky5num = 1;
	mapheaderinfo[i-1].sky6num = 1;
	mapheaderinfo[i-1].sky7num = 1;
	mapheaderinfo[i-1].sky8num = 1;
	mapheaderinfo[i-1].sky9num = 1;
...
void P_InitMapHeaders(void)
{
	...
	for(mapnum = 1; mapnum < NUMMAPS+1; mapnum++)
	{
		...
		if(!mapheaderinfo[moremapnumbers].sky2num)
			mapheaderinfo[moremapnumbers].sky2num = mapheaderinfo[moremapnumbers].skynum;

		if(!mapheaderinfo[moremapnumbers].sky3num)
			mapheaderinfo[moremapnumbers].sky3num = mapheaderinfo[moremapnumbers].skynum;

		if(!mapheaderinfo[moremapnumbers].sky4num)
			mapheaderinfo[moremapnumbers].sky4num = mapheaderinfo[moremapnumbers].skynum;

		if(!mapheaderinfo[moremapnumbers].sky5num)
			mapheaderinfo[moremapnumbers].sky5num = mapheaderinfo[moremapnumbers].skynum;

		if(!mapheaderinfo[moremapnumbers].sky6num)
			mapheaderinfo[moremapnumbers].sky6num = mapheaderinfo[moremapnumbers].skynum;

		if(!mapheaderinfo[moremapnumbers].sky7num)
			mapheaderinfo[moremapnumbers].sky7num = mapheaderinfo[moremapnumbers].skynum;

		if(!mapheaderinfo[moremapnumbers].sky8num)
			mapheaderinfo[moremapnumbers].sky8num = mapheaderinfo[moremapnumbers].skynum;

		if(!mapheaderinfo[moremapnumbers].sky9num)
			mapheaderinfo[moremapnumbers].sky9num = mapheaderinfo[moremapnumbers].skynum;
	}
}
...
/*        if (ss->tag < 0)
		...
		if(ss->tag == 424) // Mario
		{
			if(!(grade & 8 || nomonsters))
				I_Error("You need to unlock this level first!\n");
		}
		else if(ss->tag == 425) // NiGHTS
		{
			if(!(grade & 16 || nomonsters))
				I_Error("You need to unlock this level first!\n");
		}
		else if(ss->tag == 426) // Christmas Hunt
		{
			if(!(grade & 32 || nomonsters))
				I_Error("You need to unlock this level first!\n");
		}
		else if(ss->tag == 427) // Adventure
		{
			if(!(grade & 64 || nomonsters))
				I_Error("You need to unlock this level first!\n");
		}
		else if(ss->tag == 428) // Sonic Golf Graue 12-13-2003 // SRB2 Universe in SPMoves
		{
			if(!(grade & 1024 || nomonsters))
				I_Error("You need to unlock this level first!\n");
		}
		#ifdef SPMOVES
		else if(ss->tag == 429) // Chao World SPM
		{
			if(!(grade & 2048 || nomonsters))
				I_Error("Sorry, but Chao World SPM can only be accessed if you have found at least %d Multiplayer Eggs. :(\n",NUMMULTIEGGS);
		}
		#endif
    }
    ...
    // set the sky flat nums
    skyflatnum = P_AddLevelFlat (SKYFLATNAME,foundflats);
    sky2flatnum = P_AddLevelFlat (SKY2FLATNAME,foundflats);
    sky3flatnum = P_AddLevelFlat (SKY3FLATNAME,foundflats);
    sky4flatnum = P_AddLevelFlat (SKY4FLATNAME,foundflats);
    sky5flatnum = P_AddLevelFlat (SKY5FLATNAME,foundflats);
    sky6flatnum = P_AddLevelFlat (SKY6FLATNAME,foundflats);
    sky7flatnum = P_AddLevelFlat (SKY7FLATNAME,foundflats);
    sky8flatnum = P_AddLevelFlat (SKY8FLATNAME,foundflats);
    sky9flatnum = P_AddLevelFlat (SKY9FLATNAME,foundflats);
...
void P_SetupLevelSky2 (void)
{
    char       sky2texname[12];

    sprintf (sky2texname,"SKY%d",mapheaderinfo[gamemap-1].sky2num);
    sky2texture = R_TextureNumForName (sky2texname);

    // scale up the old skies, if needed
    R_SetupSkyDraw ();
}

void P_SetupLevelSky3 (void)
{
    char       sky3texname[12];

    sprintf (sky3texname,"SKY%d",mapheaderinfo[gamemap-1].sky3num);
    sky3texture = R_TextureNumForName (sky3texname);

    // scale up the old skies, if needed
    R_SetupSkyDraw ();
}

void P_SetupLevelSky4 (void)
{
    char       sky4texname[12];

    sprintf (sky4texname,"SKY%d",mapheaderinfo[gamemap-1].sky4num);
    sky4texture = R_TextureNumForName (sky4texname);

    // scale up the old skies, if needed
    R_SetupSkyDraw ();
}

void P_SetupLevelSky5 (void)
{
    char       sky5texname[12];

    sprintf (sky5texname,"SKY%d",mapheaderinfo[gamemap-1].sky5num);
    sky5texture = R_TextureNumForName (sky5texname);

    // scale up the old skies, if needed
    R_SetupSkyDraw ();
}

void P_SetupLevelSky6 (void)
{
    char       sky6texname[12];

    sprintf (sky6texname,"SKY%d",mapheaderinfo[gamemap-1].sky6num);
    sky6texture = R_TextureNumForName (sky6texname);

    // scale up the old skies, if needed
    R_SetupSkyDraw ();
}

void P_SetupLevelSky7 (void)
{
    char       sky7texname[12];

    sprintf (sky7texname,"SKY%d",mapheaderinfo[gamemap-1].sky7num);
    sky7texture = R_TextureNumForName (sky7texname);

    // scale up the old skies, if needed
    R_SetupSkyDraw ();
}

void P_SetupLevelSky8 (void)
{
    char       sky8texname[12];

    sprintf (sky8texname,"SKY%d",mapheaderinfo[gamemap-1].sky8num);
    sky8texture = R_TextureNumForName (sky8texname);

    // scale up the old skies, if needed
    R_SetupSkyDraw ();
}

void P_SetupLevelSky9 (void)
{
    char       sky9texname[12];

    sprintf (sky9texname,"SKY%d",mapheaderinfo[gamemap-1].sky9num);
    sky9texture = R_TextureNumForName (sky9texname);
...
noscript:
	...
	if(mapheaderinfo[gamemap-1].typeoflevel & TOL_CHATROOM)
	{
		...
		{
			if(cv_splitscreen.value)
				players[secondarydisplayplayer].charability = 14;

			players[consoleplayer].charability = 14;
		}
		else
		{
		    for(i=0;i<MAXPLAYERS;i++)
			if(playeringame[i])
		          players[i].charability = 14;
		}
	}
	else
	{
		...
		{
			sprintf(skincmd, "skin2 %s\n", skins[0].name);
			CV_Set(&cv_skin2, skins[0].name);
			sprintf(skincmd, "skin2 %s\n", skins[players[secondarydisplayplayer].skin].name);
			CV_Set(&cv_skin2, skins[players[secondarydisplayplayer].skin].name);
		}
		...
		{
			if(cv_splitscreen.value)
				SetPlayerSkinByNum(secondarydisplayplayer, players[secondarydisplayplayer].skin);

			SetPlayerSkinByNum(consoleplayer, players[consoleplayer].skin);
		}
		#ifdef SPMOVES
		players[consoleplayer].chao = NULL;
		if(cv_splitscreen.value)
			players[secondarydisplayplayer].chao = NULL;
	}
	#endif
