...
static CV_PossibleValue_t playersping_cons_t[] = {{0, "MIN"}, {100000, "MAX"}, {0, NULL}}; //no limit for that ok?
consvar_t cv_playersping =      {"dontuseitpls",      "0", CV_NETVAR, playersping_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL};
...
static void Got_AddPlayer(UINT8 **p, INT32 playernum)
{
	...
	if (netgame)
		CONS_Printf(M_GetText("A new challenger (%d) has joined the brawl (node %d)\n"), newplayernum+1, node);
...
static inline void PingUpdate(void)
{
	...
	//pingmeasurecount = cv_realping.value;
	...
	if (server)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
			if (playeringame[i]) 
			    ...
			    CV_SetValue(&cv_playersping, (realpingtable[i]));
		pingmeasurecount++;
	}
#endif
