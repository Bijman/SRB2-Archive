...
#include "JTE/JTEMisc.h" // MOBJSCALE SCALEVAR
...
static void Command_Kick(void)
{
	...
	{
#ifdef JTEBOTS
		CONS_Printf("kick <playername> or <playernum> or ALLBOTS: kick a player or bot\n");
#else
	...
	|| (i != (char)-1
		&& playeringame[i]
		&& players[i].bot
		&& players[i].bot->ownernum == consoleplayer)
		|| !stricmp(COM_Argv(1),"allbots")
#endif
	)
	{
#ifdef JTEBOTS
...
#ifdef JTEBOTS
			if(players[pnum].bot)
				CONS_Printf("says goodbye (Bot removed)\n");
			else
#endif
...
#ifdef SCALEVAR
	// Reset the scale and variables.
	if(cv_myscale.value != atoi(cv_myscale.defaultvalue))
	{
		CV_Set(&cv_myscale,cv_myscale.defaultvalue);
#endif
#ifdef SCALESIZE
		cv_viewheight.value = atoi(cv_viewheight.defaultvalue);
...
#ifdef HOLIDAYS
	// Reset the holiday (xmas maps, server holiday different from local holiday, etc.)
	if(holiday != localholiday)
	{
		holiday = serverholiday = localholiday;
		JTE_LoadHolidayCfg();
	}
#endif
