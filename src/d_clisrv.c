...
#ifdef SRB2PLUS // cons packet save of new variables
    con->lastinflictor = SHORT(players[i].lastinflictor);
    con->lastinflictortime = (tic_t)LONG(players[i].lastinflictortime);
    con->startflashingtics = (tic_t)LONG(players[i].startflashingtics);
    con->jumpangle = (angle_t)LONG(players[i].jumpangle);
#endif
	con->hasmo = false;
...
#ifdef SRB2PLUS // cons packet read of new player vars
    players[i].lastinflictor = SHORT(con->lastinflictor);
    players[i].lastinflictortime = (tic_t)LONG(con->lastinflictortime);
    players[i].startflashingtics = (tic_t)LONG(con->startflashingtics);
    players[i].jumpangle = (angle_t)LONG(con->jumpangle);
#endif
