...
#ifdef RABBIT
char server_ips[MAXSERVERLIST][25]; // Until I can find a way to get a server's IP from the serverlist. =/
#endif
...
static void SV_SendServerInfo(INT32 node, tic_t servertime)
{
    ...
    if (CS_host)
    {
        netbuffer->u.serverinfo.netmode = NETMODE_CS;
        netbuffer->u.serverinfo.numberofplayer = CS_PlayerCount();
    }
    else
        netbuffer->u.serverinfo.netmode = NETMODE_STANDARD; // CSTODO: Cross compatability with regular netcode system?
#endif
...
static void SV_SendPlayerInfo(INT32 node)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
...
#else
        // LXShadow: Client-side playerinfo.
        // Tag is not yet supported.
        // Jointime isn't supported.
        // IPs aren't sent out (I see no good reason to do this, anyway).
        UINT8 ctfteam;
        boolean spectator;
        ...
        ctfteam = i == self_id ? players[0].ctfteam : virtualplayers[i].ctfteam;
        spectator = i == self_id ? players[0].spectator : virtualplayers[i].spectator;

		netbuffer->u.playerinfo[i].node = i; // In CS, we have no nodes.
                                             // Using i, because using one node for all seems to make the MS show only one player.
		if (i == self_id)
            strncpy(netbuffer->u.playerinfo[i].name, (const char *) &player_names[0], MAXPLAYERNAME+1);
        else
            strncpy(netbuffer->u.playerinfo[i].name, (const char*) virtualplayers[i].name, MAXPLAYERNAME+1);
        ...
        if (spectator)
            ...
            netbuffer->u.playerinfo[i].team = ctfteam;
        else
            netbuffer->u.playerinfo[i].team = 0;

		netbuffer->u.playerinfo[i].score = i == self_id ? players[0].score : virtualplayers[i].score;
		netbuffer->u.playerinfo[i].timeinserver = 0; // No jointimes in CS mode. What are they for, anyways?
		netbuffer->u.playerinfo[i].skin = i == self_id ? players[0].skin : virtualplayers[i].skin;

		netbuffer->u.playerinfo[i].data = i == self_id ? (UINT8)players[0].skincolor : virtualplayers[i].colour;

//		if (players[i].pflags & PF_TAGIT) // Tag not currently supported in CS mode.
//			netbuffer->u.playerinfo[i].data |= 32;

		if (i == flagholder[1] || i == flagholder[2])
			netbuffer->u.playerinfo[i].data |= 64;

		if ((i == self_id && players[0].powers[pw_super]) ||
            (i != self_id && virtualplayers[i].powers[pw_super]))
			netbuffer->u.playerinfo[i].data |= 128;
#endif
	}
...
#ifdef RABBIT
                    strcpy(server_ips[node], server_list[i].ip);
#endif
				}
			}
		}
...
SINT8 nametonum(const char *name)
{
	...
	if (playernum)
	{
		if (playernum == self_id || virtualplayers[playernum].in_game)
			return (SINT8)playernum;
		else
			return -1;
	}
	...
	for (i = 0; i < MAXPLAYERS; i++)
		if ((i == self_id || virtualplayers[i].in_game) && !stricmp(i == self_id ? player_names[0] : virtualplayers[i].name, name))
			return (SINT8)i;
...
#else
    COM_AddCommand("kick", Command_CSKick_f);
    COM_AddCommand("ban", Command_CSBan_f);
    COM_AddCommand("clearbans", Command_CSClearbans_f);
    COM_AddCommand("nodes", Command_CSNodes_f);
	COM_AddCommand("playerlinks", Command_CSPlayerlinks);
    COM_AddCommand("ping", Command_CSPing_f);
	COM_AddCommand("ringstats", Command_CSStats);
#endif
#endif

#ifdef RABBIT
	COM_AddCommand("csconnect", Command_CSConnect_f);
	COM_AddCommand("cshost", Command_CSHost_f);
#ifndef BUNNIES
	COM_AddCommand("csdelay", Command_CSDelay_f);
	COM_AddCommand("csping", Command_CSPing_f);
	COM_AddCommand("cskick", Command_CSKick_f);
	COM_AddCommand("csban", Command_CSBan_f);
	COM_AddCommand("csclearbans", Command_CSClearbans_f);
	COM_AddCommand("csnodes", Command_CSNodes_f);
#endif

    CV_RegisterVar(&cv_clienttoclient);
    CV_RegisterVar(&cv_safenodes);
    CV_RegisterVar(&cv_ringmove);
    CV_RegisterVar(&cv_csdelay);
    CV_RegisterVar(&cv_cssync);
#endif
...
#ifdef RABBIT
void HandleStandardPacket()
{
    SINT8 node = (SINT8) doomcom->remotenode;
    ...
    if (netbuffer->packettype == PT_ASKINFO)
    {
...
FILESTAMP
	while (HGetPacket())
	{
		...
		{
#ifdef RABBIT // Servers connect to themselves? ARGH.
		    if (CS_mode && D_NumPlayers() >= 1) // Let the server connect to themselves first. (ARGH!)
                SV_SendRefuse(node, M_GetText("The server is in CS mode.\nStandard joins are not possible."));
#endif
...
#if 0 // LXShadow: Disabled for now; also yay no more reset-view bug.
// #ifdef RABBIT
	// Time for some oh-so-hacky code! (LXShadow - run CPU_Tick())
	static tic_t lastupdated;
	if (lastupdated == I_GetTime())
		return;
	lastupdated = I_GetTime();

	//localangle += 8 << FRACBITS;
	//netcmds[maketic%BACKUPTICS][consoleplayer].angleturn += 32;
	int i;
	memcpy(&players[consoleplayer].cmd, &netcmds[maketic%BACKUPTICS][consoleplayer], sizeof (ticcmd_t));

	for (i = 0; i < MAXPLAYERS; i ++)
	{
		if (i == consoleplayer && (players[consoleplayer].cmd.buttons & BT_USE))
			continue;

		if (players[i].cpu_player)
			CPU_Tick(&players[i]);

		memcpy(&netcmds[maketic%BACKUPTICS][i], &players[i].cmd, sizeof (ticcmd_t));

		if (i == consoleplayer)
		{
			memcpy(&netcmds[maketic%BACKUPTICS][consoleplayer], &players[consoleplayer].cmd, sizeof (ticcmd_t));

			localangle = players[consoleplayer].cmd.angleturn << 16;
			localaiming = players[consoleplayer].cmd.aiming << 16;
		}
	}
#endif
}
