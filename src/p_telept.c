...
boolean P_Teleport(mobj_t *thing, fixed_t x, fixed_t y, angle_t angle)
{
    ...
    S_StartSound (P_SpawnMobj(oldx, oldy, oldz + fogDelta, MT_TFOG), sfx_telept);
    an = angle >> ANGLETOFINESHIFT;

    //S_StartSound (P_SpawnMobj (x+20*finecosine[an], y+20*finesine[an], thing->z + fogDelta, MT_TFOG), sfx_telept);
