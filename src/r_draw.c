...
char *Color_Names[MAXSKINCOLORS]={
   ...
   //"Super", // I want em both!
};

CV_PossibleValue_t Color_cons_t[]={{0,NULL},{1,NULL},{2,NULL},{3,NULL},
                                   {4,NULL},{5,NULL},{6,NULL},{7,NULL},
{8,NULL},{9,NULL},{10,NULL},{11,NULL},{12, NULL},{13, NULL},{14, NULL},{15, NULL},{16, NULL}, {0,NULL}};
...
void R_InitSkinTranslationTables(int starttranscolor, int endtranscolor, int skinnum)
{
    ...
    for (i=0 ; i<256 ; i++)
    {
        if (i >= starttranscolor && i <= endtranscolor)
        {
			...
			// Yellow
			if ((i&0xf) <8)
				translationtables[skinnum] [i+15*256] = 0xe0 + (i&0xf);
			else
				translationtables[skinnum] [i+15*256] = 0xa0-8 + (i&0xf);
