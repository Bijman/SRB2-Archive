...
void R_InitTranslationTables(void)
{
	...
	for (i = 0; i < 256; i++)
	{
		if (i >= 160 && i <= 175)
		{
			...
			{ // Why not use (113 + (bi-8)/2)? Why the strangely specific color assignments?
				case 8:
				case 9:
					defaulttranslationtables[i+14*256] = 113;   // yellow
					break;
				case 10:
					defaulttranslationtables[i+14*256] = 114;   // yellow
					break;
				case 11:
				case 12:
				case 13:
					defaulttranslationtables[i+14*256] = 115;   // yellow
					break;
				default:
					defaulttranslationtables[i+14*256] = 116;   // yellow
					break;
				case 15:
					defaulttranslationtables[i+14*256] = 117; // yellow
					break;
			}

			// 1.09 colors
			defaulttranslationtables[i+15*256] = (byte)(0x10 + bi); // Black
			defaulttranslationtables[i+16*256] = (byte)(0x98 + bi/2); // Dark Red
			defaulttranslationtables[i+17*256] = (byte)(0xe5 + bi); // Dark blue

			// MFE colors
			defaulttranslationtables[i+18*256] = (byte)(0xb8 + bi/4); // Neon Green
			if (bi <= 5)
				defaulttranslationtables[i+19*256] = 0x71; // HMS123 Yellow
			else
				defaulttranslationtables[i+19*256] = 0x71+(bi-5)/4; // HMS123 Yellow
			defaulttranslationtables[i+20*256] = (byte)(0x30 + bi); // Brown
			// Random eyesore.
			switch (bi) {
				case 0:
				case 11:
					defaulttranslationtables[i+21*256] = 0x80; // Red
					break;
				case 1:
				case 15:
					defaulttranslationtables[i+21*256] = 0x57; // Orange
					break;
				case 2:
				case 8:
					defaulttranslationtables[i+21*256] = 0xC1; // Pink
					break;
				case 3:
				case 12:
					defaulttranslationtables[i+21*256] = 0xB8; // Green
					break;
				case 4:
				case 9:
					defaulttranslationtables[i+21*256] = 0x00; // White
					break;
				case 5:
				case 13:
					defaulttranslationtables[i+21*256] = 0x68; // Yellow
					break;
				case 6:
				case 10:
					defaulttranslationtables[i+21*256] = 0xE8; // Blue
					break;
				case 7:
				case 14:
					defaulttranslationtables[i+21*256] = 0xD7; // Light blue
					break;
			}
			defaulttranslationtables[i+22*256] = (byte)(0x78 + bi/2); // Hot Pink
			defaulttranslationtables[i+23*256] = (byte)(0xE0 + bi/2); // Light Blue
			defaulttranslationtables[i+24*256] = (byte)(0x50 + bi/2); // Light Orange
			defaulttranslationtables[i+25*256] = (byte)(0x60 + bi/4); // White Yellow
			defaulttranslationtables[i+26*256] = (byte)(0x18 + bi/2); // Pitch
			defaulttranslationtables[i+26*256] = (byte)(0x08 + bi); // Grey
		}
		else // Keep other colors as is.
		{
			// Far more effecient and correct.
...
void R_InitSkinTranslationTables(int starttranscolor, int skinnum)
{
	...
	for (i = 0; i < 256; i++)
	{
		if (i >= starttranscolor && i < starttranscolor+16)
		{
			...
			translationtables[skinnum][i+7*256] = (byte)(0xc8 + bi/2); // Steel blue
			...
			// 1.09 colors
			translationtables[skinnum][i+15*256] = (byte)(0x10 + bi); // Black
			translationtables[skinnum][i+16*256] = (byte)(0x98 + bi/2); // Dark Red
			translationtables[skinnum][i+17*256] = (byte)(0xe5 + bi); // Dark blue

			// MFE colors
			translationtables[skinnum][i+18*256] = (byte)(0xb8 + bi/4); // Neon Green
			if (bi <= 5)
				translationtables[skinnum][i+19*256] = 0xBB; // HMS123 Green
			else
				translationtables[skinnum][i+19*256] = 0x71+(bi-5)/4; // HMS123 Yellow
			translationtables[skinnum][i+20*256] = (byte)(0x30 + bi); // Brown
			// Random eyesore.
			switch (bi) {
				case 0:
				case 11:
					translationtables[skinnum][i+21*256] = 0x80; // Red
					break;
				case 1:
				case 15:
					translationtables[skinnum][i+21*256] = 0x57; // Orange
					break;
				case 2:
				case 8:
					translationtables[skinnum][i+21*256] = 0xC1; // Pink
					break;
				case 3:
				case 12:
					translationtables[skinnum][i+21*256] = 0xB8; // Green
					break;
				case 4:
				case 9:
					translationtables[skinnum][i+21*256] = 0x00; // White
					break;
				case 5:
				case 13:
					translationtables[skinnum][i+21*256] = 0x68; // Yellow
					break;
				case 6:
				case 10:
					translationtables[skinnum][i+21*256] = 0xE8; // Blue
					break;
				case 7:
				case 14:
					translationtables[skinnum][i+21*256] = 0xD7; // Light blue
					break;
			}
			translationtables[skinnum][i+22*256] = (byte)(0x78 + bi/2); // Hot pink
			translationtables[skinnum][i+23*256] = (byte)(0xE0 + bi/2); // Light Blue
			translationtables[skinnum][i+24*256] = (byte)(0x50 + bi/2); // Light Orange
			translationtables[skinnum][i+25*256] = (byte)(0x60 + bi/4); // White yellow
			translationtables[skinnum][i+26*256] = (byte)(0x18 + bi/2); // Pitch
			translationtables[skinnum][i+27*256] = (byte)(0x08 + bi); // Grey
		}
		else // Keep other colors as-is.
		{
			// Far more effecient and correct.
			...
			switch(i) {
			case 31:
				translationtables[skinnum][i+19*256] = 0x80; // HMS123 Red Eyes
				break;
			case 82:
				translationtables[skinnum][i+19*256] = 115; // HMS123 skintone
				break;
			default:
				if (i >= 64 && i <= 79)
					translationtables[skinnum][i+19*256] = 112+(i-64)/2; // HMS123 skintone
				else if (i >= 125 && i < 130)
					translationtables[skinnum][i+19*256] = 130; // HMS123 red shoes
				else
					translationtables[skinnum][i+19*256] = (byte)i;
				break;
			}
		}
	}
