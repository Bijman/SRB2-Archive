...
#else
	// To add more colors, edit MAXSKINCOLORS in Doomdef.h
	"None",       //  0
	"Black",      //  1
	"Grey",       //  2
	"Silver",     //  3
...
#else
			// Every color in the palette CAN become a player color.
			// You just haven't looked hard enough :3
			defaulttranslationtables[i       ] = (UINT8)(0x10 + bi);   // Black
			defaulttranslationtables[i+   256] = (UINT8)(0x08 + bi);   // Grey
			defaulttranslationtables[i+ 2*256] = (UINT8)(0x00 + bi);   // Silver
			defaulttranslationtables[i+ 3*256] = (UINT8)(0x00 + bi/2); // White
			defaulttranslationtables[i+ 4*256] = (UINT8)(0x90 + bi/2); // Pink
			defaulttranslationtables[i+ 5*256] = (UINT8)(0x78 + bi);   // Light Red
			defaulttranslationtables[i+ 6*256] = (UINT8)(0x7d + bi);   // Red
			defaulttranslationtables[i+ 7*256] = (UINT8)(0x88 + bi/2); // Dark Red
			defaulttranslationtables[i+ 8*256] = (UINT8)(0xc0 + bi/2); // Purple
			defaulttranslationtables[i+ 9*256] = (UINT8)(0xf8 + bi/2); // Lavender
			defaulttranslationtables[i+10*256] = (UINT8)(0xc8 + bi/2); // Steel Blue
			defaulttranslationtables[i+11*256] = (UINT8)(0xe7 + bi);   // Dark blue
			defaulttranslationtables[i+12*256] = (UINT8)(0xe0 + bi);   // Blue
			defaulttranslationtables[i+13*256] = (UINT8)(0xe0 + bi/2); // Light Blue
			defaulttranslationtables[i+14*256] = (UINT8)(0xd0 + bi);   // Cyan
			defaulttranslationtables[i+15*256] = (UINT8)(0xb8 + bi/4); // Neon Green
			defaulttranslationtables[i+16*256] = (UINT8)(0xa0 + bi);   // Green
			defaulttranslationtables[i+17*256] = (UINT8)(0xb0 + bi/2); // Army
			switch (bi)
			{
				case 0:
					defaulttranslationtables[i+18*256] = 97;
					break;
				case 1:
					defaulttranslationtables[i+18*256] = 98;           // Yellow
					break;
				case 2:
					defaulttranslationtables[i+18*256] = 99;           // Yellow
					break;
				case 3:
					defaulttranslationtables[i+18*256] = 100;          // Yellow
					break;
				case 4:
					defaulttranslationtables[i+18*256] = 101;          // Yellow
					break;
				case 5:
					defaulttranslationtables[i+18*256] = 102;          // Yellow
					break;
				case 6:
					defaulttranslationtables[i+18*256] = 103;          // Yellow
					break;
				case 7:
					defaulttranslationtables[i+18*256] = 104;          // Yellow
					break;
				case 8:
				case 9:
					defaulttranslationtables[i+18*256] = 113;          // Yellow
					break;
				case 10:
					defaulttranslationtables[i+18*256] = 114;          // Yellow
					break;
				case 11:
				case 12:
				case 13:
					defaulttranslationtables[i+18*256] = 115;          // Yellow
					break;
				case 15:
					defaulttranslationtables[i+18*256] = 117;          // Yellow
					break;
				default:
					defaulttranslationtables[i+18*256] = 116;          // Yellow
					break;
			}
			defaulttranslationtables[i+19*256] = (UINT8)(0x70 + bi/2); // Gold
			defaulttranslationtables[i+20*256] = (UINT8)(0x50 + bi);   // Orange
			defaulttranslationtables[i+21*256] = (UINT8)(0x40 + bi);   // Peach
			defaulttranslationtables[i+22*256] = (UINT8)(0x20 + bi);   // Beige
			defaulttranslationtables[i+23*256] = (UINT8)(0x30 + bi);   // Brown
			defaulttranslationtables[i+24*256] = (UINT8)(0x98 + bi/2); // Rosewood
#endif
		}
...
void R_InitSkinTranslationTables(INT32 starttranscolor, INT32 skinnum)
{
	...
	{
//#ifdef COLORS
//		if (i >= starttranscolor && i < starttranscolor+26)
//#else
...
#else
			translationtables[skinnum][i       ] = (UINT8)(0x10 + bi);   // Black
			translationtables[skinnum][i+   256] = (UINT8)(0x08 + bi);   // Grey
			translationtables[skinnum][i+ 2*256] = (UINT8)(0x00 + bi);   // Silver
			translationtables[skinnum][i+ 3*256] = (UINT8)(0x00 + bi/2); // White
			translationtables[skinnum][i+ 4*256] = (UINT8)(0x90 + bi/2); // Pink
			translationtables[skinnum][i+ 5*256] = (UINT8)(0x78 + bi);   // Light Red
			translationtables[skinnum][i+ 6*256] = (UINT8)(0x7d + bi);   // Red
			translationtables[skinnum][i+ 7*256] = (UINT8)(0x88 + bi/2); // Dark Red
			translationtables[skinnum][i+ 8*256] = (UINT8)(0xc0 + bi/2); // Purple
			translationtables[skinnum][i+ 9*256] = (UINT8)(0xf8 + bi/2); // Lavender
			translationtables[skinnum][i+10*256] = (UINT8)(0xc8 + bi/2); // Steel Blue
			translationtables[skinnum][i+11*256] = (UINT8)(0xe7 + bi);   // Dark blue
			translationtables[skinnum][i+12*256] = (UINT8)(0xe0 + bi);   // Blue
			translationtables[skinnum][i+13*256] = (UINT8)(0xe0 + bi/2); // Light Blue
			translationtables[skinnum][i+14*256] = (UINT8)(0xd0 + bi);   // Cyan
			translationtables[skinnum][i+15*256] = (UINT8)(0xb8 + bi/4); // Neon Green
			translationtables[skinnum][i+16*256] = (UINT8)(0xa0 + bi);   // Green
			translationtables[skinnum][i+17*256] = (UINT8)(0xb0 + bi/2); // Army
			switch (bi)
			{
				case 0:
					translationtables[skinnum][i+18*256] = 97;
					break;
				case 1:
					translationtables[skinnum][i+18*256] = 98;           // Yellow
					break;
				case 2:
					translationtables[skinnum][i+18*256] = 99;           // Yellow
					break;
				case 3:
					translationtables[skinnum][i+18*256] = 100;          // Yellow
					break;
				case 4:
					translationtables[skinnum][i+18*256] = 101;          // Yellow
					break;
				case 5:
					translationtables[skinnum][i+18*256] = 102;          // Yellow
					break;
				case 6:
					translationtables[skinnum][i+18*256] = 103;          // Yellow
					break;
				case 7:
					translationtables[skinnum][i+18*256] = 104;          // Yellow
					break;
				case 8:
				case 9:
					translationtables[skinnum][i+18*256] = 113;          // Yellow
					break;
				case 10:
					translationtables[skinnum][i+18*256] = 114;          // Yellow
					break;
				case 11:
				case 12:
				case 13:
					translationtables[skinnum][i+18*256] = 115;          // Yellow
					break;
				case 15:
					translationtables[skinnum][i+18*256] = 117;          // Yellow
					break;
				default:
					translationtables[skinnum][i+18*256] = 116;          // Yellow
					break;
			}
			translationtables[skinnum][i+19*256] = (UINT8)(0x70 + bi/2); // Gold
			translationtables[skinnum][i+20*256] = (UINT8)(0x50 + bi);   // Orange
			translationtables[skinnum][i+21*256] = (UINT8)(0x40 + bi);   // Peach
			translationtables[skinnum][i+22*256] = (UINT8)(0x20 + bi);   // Beige
			translationtables[skinnum][i+23*256] = (UINT8)(0x30 + bi);   // Brown
			translationtables[skinnum][i+24*256] = (UINT8)(0x98 + bi/2); // Rosewood
#endif
		}
		else // Keep other colors as-is.
		{
