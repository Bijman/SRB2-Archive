...
void T_CameraScanner(elevator_t *elevator)
{
	...
	if (players[displayplayer].mo)
	{
		if (players[displayplayer].mo->subsector->sector == elevator->actionsector)
		{
			...
			CV_SetValue(&cv_cam_height, elevator->sector->floorheight/FRACUNIT);
			CV_SetValue(&cv_cam_dist, elevator->sector->ceilingheight/FRACUNIT);
	...
	if (cv_splitscreen.value && players[secondarydisplayplayer].mo)
	{
		if (players[secondarydisplayplayer].mo->subsector->sector == elevator->actionsector)
		{
			...
			CV_SetValue(&cv_cam2_height, elevator->sector->floorheight/FRACUNIT);
			CV_SetValue(&cv_cam2_dist, elevator->sector->ceilingheight/FRACUNIT);
