...
signed int eracerank[4]; // Tails 05-01-2001
	...
	if(gamemap >= sstage_start && gamemap <= sstage_end)
	{
		...
		else
		{
			...
			{
				V_DrawScaledPatch(70, 26, FB, gotemall);
				V_DrawCenteredString(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, V_WHITEMAP, "Or have you? Play on Normal or\n higher to collect them all!");
			}
			else
				V_DrawScaledPatch(48, 26, FB, cemerald);
		}
...
void WI_Ticker(void)
{
    ...
    if (bcnt == 1)
    {
        // intermission music
      #ifdef SPMOVES
      if(!(grade & 4096) && gamemap-1 == 10 && !modifiedgame && !(netgame || multiplayer) && gameskill == sk_easy)
            grade |= 4096;
	  if(mapheaderinfo[gamemap-1].typeoflevel & TOL_PSO)
		S_ChangeMusic(mus_psowin, false);
	  else
	  #endif
    ...
    switch (state)
    {
      case StatCount:
        ...
        #endif
        ) WI_updateDeathmatchStats(); // Deathmatch intermission Tails 02-20-2002
...
static void WI_loadData(void)
{
	...
	//chaos8 = W_CachePatchName("CHAOS8", PU_STATIC); // Tails 08-12-2001
...
static void WI_unloadData(void)
{
    ...
    if (rendermode==render_soft)
    {
		...
		//Z_ChangeTag(chaos8, PU_CACHE); // Tails 08-12-2001
