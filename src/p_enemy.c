...
#include "r_things.h"
#ifdef JTEMISC
#include "i_sound.h" // I_ChangeSongSpeed
#endif

#include "hardware/hw3sound.h"

#include "JTE/JB_Bot.h" // JTEBOTS
#include "JTE/JB_Command.h" // JTEBOTS
...
static boolean P_Move(mobj_t *actor)
{
	...
	if (movedir == DI_NODIR
#ifdef MOVEFIX
	|| !actor->health // Don't move if you're dead, duh.
#endif
	)
		return false;
...
static boolean P_LookForShield(mobj_t *actor)
{
...
#ifdef RUSSIA
	// In Soviet Russia, ring doesn't collect you if it's already carrying someone!!
	if(russia)
	{
		for(c = 0; c < MAXPLAYERS; c++)
		{
			if(playeringame[c] && players[c].ring == actor)
				return false;
		}
		c = 0;
	}
	...
	for (; ; actor->lastlook = (actor->lastlook + 1) & PLAYERSMASK)
	{
		...
		if ((player->powers[pw_ringshield]
			...
			|| (russia && !player->ring && !player->powers[pw_invulnerability] && !player->powers[pw_flashing])
#endif
#ifdef SPMOVES
...
#ifdef RUSSIA
	if(russia && actor->z <= actor->floorz)
	{
		if(!(leveltime%(TICRATE/4))
			&& P_AproxDistance(P_AproxDistance(
			actor->x - actor->target->x,
			actor->y - actor->target->y),
			actor->z - actor->target->z) / FRACUNIT
			< 256
			&& (P_Random() < 64 || actor->target->z > actor->target->floorz))
		{
			// In Soviet Russia, Crawla jumps on YOU!
...
void A_SkullAttack(mobj_t *actor)
{
	...
	if(russia)
	{
		dest->z++;
		dest->target = actor;
		dest->flags |= MF_NOGRAVITY;
		dest->flags2 |= MF2_SKULLFLY;
		if (actor->info->attacksound)
			S_StartAttackSound(dest, actor->info->attacksound);
		dest->angle = R_PointToAngle2(dest->x, dest->y, actor->x, actor->y);
		...
		if(dest->player && dest->player-players == consoleplayer)
			localangle = dest->angle;
		else if(dest->player && dest->player-players == secondarydisplayplayer)
			localangle2 = dest->angle;
		an = dest->angle >> ANGLETOFINESHIFT;

		dest->momx = FixedMul(speed, finecosine[an]);
		dest->momy = FixedMul(speed, finesine[an]);
		...
		dest->momz = (actor->z + (actor->height>>1) - dest->z) / dist;
...
void A_BossZoom(mobj_t *actor)
{
	...
	if(russia)
	{
		...
		if(dest->player && dest->player-players == consoleplayer)
			...
			localangle2 = dest->angle;
		an = dest->angle >> ANGLETOFINESHIFT;
		dest->momx = FixedMul(actor->info->speed*5*FRACUNIT, finecosine[an]);
		dest->momy = FixedMul(actor->info->speed*5*FRACUNIT, finesine[an]);
		dist = P_AproxDistance(actor->x - dest->x, actor->y - dest->y);
...
#ifdef JTEMISC // This function is retarded. Plain and simple.
	if(!(actor->flags & MF_ENEMY) && !(actor->flags & MF_MISSILE))
		return;
#endif
...
#ifdef UNIVERSE
			// Also scan for SU boss drops and activate them!
			// Why scan twice, eh? :P
			else if(mo2->type == MT_SUBOSSDROP)
				mo2->fuse = 3*TICRATE;
#endif
		}
...
#ifdef MUSICFIX
	|| mapheaderinfo[gamemap-1].nossmusic
#endif
	))
	{
#ifndef MUSICFIX
...
void A_AttractChase(mobj_t *actor)
{
#ifdef RUSSIA
	// In Soviet Russia, ring collects YOU!
	if(russia && actor->target && actor->target->player
	&& actor->target->player->ring == actor)
	{
		// You've got a player? Coo'.
	...
	if (actor->target && actor->target->player
#ifdef RUSSIA
		&& (!russia || (actor->target->player->ring && actor->target->player->ring != actor)
		|| actor->target->player->powers[pw_invulnerability] || actor->target->player->powers[pw_flashing])
#endif
#ifdef SPMOVES
		&& !(actor->target->player->charflags & SF_HMS123)
#endif
		&& !actor->target->player->powers[pw_ringshield] && actor->type != (mobjtype_t)actor->info->reactiontime)
	{
		mobj_t *newring;
#ifdef RUSSIA
		// Shouldn't occur if the ring has players, but it's possible...
		// So drop them now rather then leaving them trapped.
		SR_RingSpill(actor);
#endif
		...
		P_SetMobjState(actor, S_DISS);
#ifdef JTEMISC
		return; // Actor is no longer gonna be there next tic anyway. :/
#endif
	}
...
void A_DropMine(mobj_t *actor)
{
	...
	if(russia)
	{
		mobj_t *mo = P_SpawnMobj(actor->x, actor->y, actor->z - 12*FRACUNIT + (locvar1*FRACUNIT), actor->info->raisestate);
		mo->momx = actor->momx;
		mo->momy = actor->momy;
		mo->momz = actor->momz;
		mo->pmomz = actor->pmomz;
		mo->flags = actor->flags;
		mo->fuse = 5*TICRATE;

		actor->momx = 0;
		actor->momy = 0;
		actor->flags = mo->info->flags;
...
#ifdef RUSSIA
	// In Soviet Russia, Detons run from YOU!
	if(russia)
		exact = R_PointToAngle2(actor->x, actor->y, actor->tracer->x, actor->tracer->y)+ANG180;
	else
#endif
...
void A_CrawlaCommanderThink(mobj_t *actor)
{
	...
	if (actor->target->player && actor->health > 1)
	{
			...
			&& (actor->target->player->mfjumped || actor->target->player->mfspinning))
		{
			...
			{
#ifdef JTEMISC // An actual strafing manuver rather then going backwards. :P
				if(P_Random()&1)
					P_InstaThrust(actor, actor->angle-(ANG90+ANG45), 20*FRACUNIT);
				else
					P_InstaThrust(actor, actor->angle+(ANG90+ANG45), 20*FRACUNIT);
#else
...
#ifdef UNIVERSE
				if(universe)
					P_InstaThrust(actor, actor->angle, 30*FRACUNIT);
				else
#endif
...
void A_Boss2Chase(mobj_t *actor)
{
	...
	if (leveltime % (speedvar*15/10)-1 == 0)
	{
		...
		goop->flags = (goop->flags & ~MF_TRANSLATION) | ((11)<<MF_TRANSSHIFT);
#endif
...
void A_SparkFollow(mobj_t *actor)
{
	...
	P_UnsetThingPosition(actor);
	{
		...
		if(actor->scale != 100)
		{
			actor->x = actor->target->x + FIXEDSCALE(FixedMul(finecosine[fa],actor->info->speed),actor->scale);
			actor->y = actor->target->y + FIXEDSCALE(FixedMul(finesine[fa],actor->info->speed),actor->scale);
		}
		else
		{
#endif
...
#ifdef SPRITEOFFS
		filler = P_SpawnMobj(actor->x, actor->y, actor->z, MT_EGGMOBILE_FG);
		filler->target = actor;

		filler = P_SpawnMobj(actor->x, actor->y, actor->z, MT_EGGMOBILE_BG);
		filler->target = actor;
#endif
...
#ifdef SUPERVIRUS2
		if(!modifiedgame && !virus && !netgame
			&& JTERAND(P_Random(),8) < 1 && gamemap == 3)
		{
			Vir_SpawnVirusShield(actor);
			actor->fuse = 40*TICRATE;
		}
#endif
