...
player_t* plyr; // Tails
#ifdef ACBOT
consvar_t cv_predictingmonsters = {"predictingmonsters","0",CV_NETVAR | CV_SAVE,CV_OnOff};
#endif
...
boolean P_Look4Rings(mobj_t *actor)
{
        mobj_t *mo;
        thinker_t *think;
	boolean found = false;
		...
		switch(mo->type)
		{
			...
			case MT_AUTOMATICRING:
				break;
			default: // Not a ring/coin/wing, so it's useless.
				continue;
		}

                if(P_AproxDistance(P_AproxDistance(actor->x-mo->x, actor->y-mo->y), actor->z - mo->z) > 512*FRACUNIT  // Out of range
			|| actor->z + actor->info->height + 90 < mo->z) // And too high to jump to.
                {
                        continue;
                }
		...
		if(actor->target && P_AproxDistance(P_AproxDistance(actor->x-mo->x, actor->y-mo->y), actor->z-mo->z) > P_AproxDistance(P_AproxDistance(actor->x-actor->target->x, actor->y-actor->target->y), actor->z-actor->target->z))
			continue;
	...
	if(found)
	{
		...
		//P_LightDash(actor, actor->target);
		return true;
	}
	actor->momx /= 2;
	actor->momy /= 2;
...
void A_Chase (mobj_t*   actor)
{
    ...
    if(!actor->movecount
        ...
        && !actor->info->missilestate // If you have your own missiles, don't use mine!
        && actor->target // Make sure you have a target!
        && actor->target->player // Make sure they're a player!
        && actor->target->player->skin == spm_fox // Don't shoot them if they're not Fox! Save your missiles!
        && actor->target->health > 1 // Don't shoot them if they're helpless! (This is good for the start of levels or something. :S)
        && (actor->target->momx || actor->target->momy || actor->target->momz)) // Dun hit non-moving players! (Set an example for netgames! XD)
        {
	mobj_t* missile;

	A_FaceTarget(actor);
	/*missile =*/ P_SpawnMissile (actor, actor->target, MT_DARKFAIRYCHAOSHOT);

	/*if(gameskill == sk_easy) // Angle up to 90 degrees of offset.
	{
		missile->angle += (P_Random()*P_Random())%ANG90;
		missile->angle -= (P_Random()*P_Random())%ANG90;
	}
	else if(gameskill == sk_medium) // Angle up to 60 degrees of offset.
	{
		missile->angle += (P_Random()*P_Random())%(ANG90/3);
		missile->angle -= (P_Random()*P_Random())%(ANG90/3);
	}
	else if(gameskill == sk_hard) // Angle up to 20 degrees of offset.
	{
		missile->angle += (P_Random()*P_Random())%(ANG90/7);
		missile->angle -= (P_Random()*P_Random())%(ANG90/7);
	}*/
	// Everything else is perfect aim.

	if(gameskill <= sk_medium)
		actor->reactiontime = actor->info->reactiontime*2;
	else
...
void A_BossDeath (mobj_t* mo)
{
	...
	for (th = thinkercap.next ; th != &thinkercap ; th=th->next)
	{
		...
		if (mo2 != mo
			&& mo2->flags & MF_BOSS
			&& !(mo2->flags2 & MF2_BOSSFLEE))
		{
			// other bosses are not dead
			return;
		}
	}
	...
	else
	{
		...
		{
        		// Scan the remaining thinkers to activate boss items!
			for (th = thinkercap.next ; th != &thinkercap ; th=th->next)
			{
				if (th->function.acp1 != (actionf_p1)P_MobjThinker)
					continue;
...
//		P_RemoveMobj(actor);
	...
	else if(actor->target->player->powers[pw_blueshield])
	{
		...
		actor->z = actor->target->z - (actor->target->info->height - actor->target->height)/3; // Adjust height for height changes
		P_SetThingPosition (actor);
		if(actor->target->player->powers[pw_super] == true || actor->target->player->powers[pw_invulnerability] > 1 || (actor->target->player->mo->flags2 & MF2_DONTDRAW && actor->target->player->mo->flags & MF_NOCLIP && !actor->target->player->powers[pw_flashing]))
			P_SetMobjState(actor, S_DISS);
	}
	return true;
}
...
void A_SignPlayer(mobj_t* actor)
{
	if (plyr->skin == 1
	#ifdef SPMOVES
	|| plyr->skin == spm_hikaru
	#endif
	) // Tails & Hikaru
		actor->state->nextstate = S_SIGN53;
	else if (plyr->skin == 2
	#ifdef SPMOVES
	|| plyr->skin == spm_jte || plyr->skin == spm_jte2|| plyr->skin == spm_mechaknux  || plyr->skin == spm_cyan
	#endif
	)
		actor->state->nextstate = S_SIGN55;
	#ifdef SPMOVES
	else if (plyr->skin == spm_zim) // Zim!
	    actor->state->nextstate = S_SIGN56;
	else if (plyr->skin == spm_eggman) // Eggman!
	    actor->state->nextstate = S_SIGN52;
    #endif
	else
		actor->state->nextstate = S_SIGN54;
	#ifdef SPMOVES
	actor->flags = (actor->flags & ~MF_TRANSLATION) | ((plyr->skincolor)<<MF_TRANSSHIFT);
	#endif
}
...
void A_JetChase (mobj_t*   actor)
{
    ...
    if (actor->type == MT_JETTBOMBER && (actor->z > (actor->floorz + (32<<FRACBITS)))
		...
		#ifdef SPMOVES
		&& !(mapheaderinfo[gamemap-1].typeoflevel & TOL_CHATROOM)
		#endif
		)
    {
        if (actor->info->attacksound)
...
void A_CrawlaCommanderThink(mobj_t* actor)
{
	...
	if(actor->target && actor->health > 1)
	{
		if(dist < 128*FRACUNIT && actor->target->player
			&& (actor->target->player->mfjumped || actor->target->player->mfspinning
			#ifdef SPMOVES
			|| actor->target->player->mfstartdash || actor->target->player->gliding
			#endif
			))
		{
			...
			P_LookForPlayers(actor,true); // You'll want to try and find a new target...
			#endif
			return;
		}
		#ifdef SPMOVES
		if(dist < 128*FRACUNIT && actor->target && !actor->target->player) // Don't hurt your own teammates! Run from em if they're insane, stupid!
		{
			// Auugh! He's trying to kill you! Strafe! STRAAAAFFEEE!!
			if(actor->target->momx || actor->target->momy)
			{
	...
	if(actor->reactiontime && actor->reactiontime <= actor->info->reactiontime && dist > actor->target->radius - FRACUNIT)
	{
		actor->threshold = 0;
		...
		if(actor->health > 1 && !(dist < 512*FRACUNIT && actor->target->player->mfstartdash))
	...
	else if (!actor->reactiontime)
	{
		if(actor->health > 1) // Hover Mode
		{
			...
			#ifdef SPMOVES
			&& !actor->target->player->mfstartdash
			#endif
			)
			{
				actor->angle = R_PointToAngle2(actor->x, actor->y, actor->target->x, actor->target->y);
		...
		actor->reactiontime = actor->info->reactiontime + P_Random()/2;
	}
	...
	{
		if(dist < 256*FRACUNIT && actor->target && actor->target->player && !actor->target->player->mfstartdash && !actor->target->player->mfjumped)
		{
		...
		else
		{
			...
			actor->momz = 15*FRACUNIT; // Bounce up in air
        		P_LookForPlayers(actor,true); // If someone's closer, get them instead!
		}
	}
