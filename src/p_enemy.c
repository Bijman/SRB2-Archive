...
void A_RecyclePowersW(mobj_t *actor);
...
static boolean P_LookForShield(mobj_t *actor)
{
	...
	for (; ; actor->lastlook = ((actor->lastlook + 1) & PLAYERSMASK))
	{
		...
		if (player->powers[pw_ringshield]
			&& (P_AproxDistance(P_AproxDistance(actor->x-player->mo->x, actor->y-player->mo->y), actor->z-player->mo->z) < RING_DIST/2))
		{
			P_SetTarget(&actor->tracer, player->mo);
			return true;
		}
...
void A_CustomPower(mobj_t *actor)
{
	...
	if ((locvar1 == pw_jumpshield || locvar1 == pw_forceshield //outliers!
		|| ((locvar1 >= pw_ringshield || locvar1 >= pw_zringshield) && locvar1 <= pw_flameshield))
...
void A_RingShield(mobj_t *actor)
{
	...
	if (riderslevel)
	{
		player->powers[pw_ringshield] = 300;
		P_SpawnShieldOrb(player);
	}
	if (!(player->powers[pw_ringshield] && riderslevel))
#else
	if (!(player->powers[pw_ringshield]))
#endif
	{
...
#ifdef RIDERS
	if (mariomode || kartmode) // Just in case...
#else
	if (mariomode)
#endif
...
#ifdef RIDERS
		if ((!actor->tracer->health) || (actor->tracer->player && ((actor->tracer->player->powers[pw_ringshield] && !kartmode) || kartmode)
...
#ifdef RIDERS
			&& ((actor->tracer->player->powers[pw_ringshield] && !kartmode) || kartmode))
#else
	...
	for (; ; actor->lastlook = (actor->lastlook + 1) & PLAYERSMASK)
	{
		...
		if (actor->target && actor->target->player)
		{
			...
			if (gametype == GT_CTF
				...
				continue;
			//                 USER               TARGET
			if (actor->target->player->position < player->position) // Red Shells only go after people ahead of you  ~Zarro
				continue;
								//zposu, zpost
			//zposu = (actor->target->player->position+1)*10;
			//zpost = (player->position+1);

			//ZTDIST = (96*(zposu/zpost));
			ZTDIST = 2048;

			//if (ZTDIST > 2048) ZTDIST = 2048;
			//if (ZTDIST <  256) ZTDIST =  256;

			ZTDIST = ZTDIST*FRACUNIT;
		}
		...
		if (!P_CheckSight(actor, player->mo))
			continue; // out of sight
#ifdef RIDERS
		if ((((player->powers[pw_ringshield] == true && !kartmode) || kartmode)
		    && P_AproxDistance(P_AproxDistance(player->mo->x-actor->x,
		    player->mo->y-actor->y), player->mo->z-actor->z) < ZTDIST))
#else
...
static inline boolean PIT_GrenadeRing(mobj_t *thing)
{
...
#ifdef RIDERS
	if (!kartmode && thing->type != MT_PLAYER)
#else
	...
	// Explode!
#ifdef RIDERS
	if (!kartmode || (kartmode && (grenade->health <= 0 || (grenade->health > 0
    && grenade->health < 70 && thing->type == MT_PLAYER))))
#endif
...
void A_RotateSpikeBall(mobj_t *actor)
{
	const fixed_t radius = 56*FRACUNIT;
	...
	if (actor->target->player && actor->target->eflags & MFE_VERTICALFLIP)
	{
		actor->eflags |= MFE_VERTICALFLIP;
		HEIGHT = actor->target->height / 2;
	}
	else
	{
		actor->eflags &= ~MFE_VERTICALFLIP;
		HEIGHT = actor->target->height / 5;
	}

	actor->angle += FixedAngle(0);
	P_UnsetThingPosition(actor);
	{
...
void A_RecyclePowers(mobj_t *actor)
{
	...
	else if (numplayers == 2) //simple swap is all that's needed
	{
		...
		{
			if (!(i == pw_shell	       || i == pw_redshell
			||    i == pw_banana       || i == pw_fakeitem
			||    i == pw_kitchensink  || i == pw_bomb
			||    i == pw_starshroom   || i == pw_initboo     
			||    i == pw_thunder      || i == pw_megamushroom
			||    i == pw_blackshell   || i == pw_itemslot
			//||    i == pw_triples     
			/*||    i == pw_ijumpshield  || i == pw_iforceshield 
			||    i == pw_iwatershield || i == pw_ibombshield 
			||    i == pw_iringshield  */))
				continue;
			if ((i == pw_shell || i == pw_redshell || i == pw_banana || i == pw_bomb)
			&& players[one].powers[i] != 1 && players[one].powers[i] != 11)
				temp[i] = players[one].powers[i];
			else if (!(i == pw_shell || i == pw_redshell || i == pw_banana || i == pw_bomb))
				temp[i] = players[one].powers[i];
			else continue;
			if ((i == pw_shell || i == pw_redshell || i == pw_banana || i == pw_bomb)
			&& players[two].powers[i] != 1 && players[two].powers[i] != 11)
	...
	else
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
				...
				&& !((netgame || multiplayer) && players[i].spectator))
			{
				...
				for (j = 0; j < NUMPOWERS; j++)
				{
					if (!(j == pw_shell	       || j == pw_redshell
					||    j == pw_banana       || j == pw_fakeitem
					||    j == pw_kitchensink  || j == pw_bomb
					||    j == pw_starshroom   || j == pw_initboo     
					||    j == pw_thunder      || j == pw_megamushroom
					||    j == pw_blackshell   || j == pw_itemslot
					//||    j == pw_triples     
					/*||    j == pw_ijumpshield  || j == pw_iforceshield 
					||    j == pw_iwatershield || j == pw_ibombshield 
					||    j == pw_iringshield  */))
						continue;

					if ((j == pw_shell || j == pw_redshell || j == pw_banana || j == pw_bomb)
					&& players[i].powers[j] != 1 && players[i].powers[j] != 11)
						players[i].powers[j] = temp[recyclefrom][j];
					else if (!(j == pw_shell || j == pw_redshell || j == pw_banana || j == pw_bomb))
						players[i].powers[j] = temp[recyclefrom][j];
					else continue;
				}
...
// Function: A_RecyclePowersW
//
// Description: Take two player's powers, and swap 'em.
//
// var1 = unused
// var2 = unused
//
void A_RecyclePowersW(mobj_t *actor)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		if (players[i].powers[pw_whiteboo] > 0 && players[i].powers[pw_whiteboo] < 100)
			one = i;
		else if (players[i].powers[pw_whiteboo] >= 100)
			two = i;
	}
	for (i = 0; i < NUMPOWERS; i++)
	{
	...
	//S_StartSound(players[one].mo, sfx_gravch);
	//S_StartSound(players[two].mo, sfx_gravch);
}
...
// Function: A_AirBox
//
// Description: Awards the player 10 air points.
//
// var1 = unused
// var2 = unused
//
...
void A_ItemPop(mobj_t *actor)
{
	...
	switch (actor->type)
	{
		...
		{
			if (actor->target && actor->target->player
				&& !((actor->target->player->powers[pw_shell]      == 2 || actor->target->player->powers[pw_shell]        == 12)
				||   (actor->target->player->powers[pw_redshell]   == 2 || actor->target->player->powers[pw_redshell]     == 12)
				||   (actor->target->player->powers[pw_banana]     == 2 || actor->target->player->powers[pw_banana]       == 12)
				||    actor->target->player->powers[pw_fakeitem]    > 1 || actor->target->player->powers[pw_kitchensink]
				||   (actor->target->player->powers[pw_bomb]       == 2 || actor->target->player->powers[pw_bomb]         == 12) 
				||    actor->target->player->powers[pw_starshroom]  > 0 || actor->target->player->powers[pw_goldshroom]
				...
				||   (actor->target->player->powers[pw_blackshell] == 2 || actor->target->player->powers[pw_blackshell]   == 12)
				||    actor->target->player->powers[pw_initboo]         || actor->target->player->powers[pw_whiteboo]
				||    actor->target->player->powers[pw_pinkboo]         || actor->target->player->powers[pw_greenboo])
			   )
...
// Function: A_CapsulePop
//
// Description: Used by Riders capsules when they explode.
//
// var1 = unused
// var2 = unused
//
void A_CapsulePop(mobj_t *actor)
{
	...
	if (item != 0)
	{
		...
		CONS_Printf("Powerup item not defined in 'damage' field for A_CapsulePop\n");
