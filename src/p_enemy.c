...
#ifdef SRB2PLUS // speed boost buff hack for certain ring types, bypassing MAXMOVE
    if ((actor->type == MT_REDRING && !(actor->flags2 & MF2_RAILRING))
        || actor->flags2 & MF2_AUTOMATIC || actor->flags2 & MF2_BOUNCERING || actor->flags2 & MF2_SCATTER)
    {
        fixed_t oldmomx = actor->momx;
        fixed_t oldmomy = actor->momy;
        fixed_t oldmomz = actor->momz;

        // Grenade speed variable based on throw strength
        // Explosion at normal speed (60 FRACUNIT/tic)
        if (actor->flags2 & MF2_AUTOMATIC || actor->flags2 & MF2_SCATTER) // 25% speed boost (75 FRACUNIT/tic)
        {
            actor->momx /= 4;
            actor->momy /= 4;
            actor->momz /= 4;
        }
        else if (actor->type == MT_REDRING && !(actor->flags2 & MF2_RAILRING)) // 33% speed boost (80 FRACUNIT/tic)
        {
            actor->momx /= 3;
            actor->momy /= 3;
            actor->momz /= 3;
        }
        else if (actor->flags2 & MF2_BOUNCERING) // 50% speed boost (90 FRACUNIT/tic)
        {
            actor->momx /= 2;
            actor->momy /= 2;
            actor->momz /= 2;
        ...
        P_RailThinker(actor); // apply boost, then reset
...
static inline boolean PIT_GrenadeRing(mobj_t *thing)
{
	...
	if (thing->type != MT_PLAYER) // Don't explode for anything but an actual player.
		return true;
#ifdef SRB2PLUS // after two seconds, grenades can blow up their owners
	if (thing == grenade->target && grenade->fuse > grenade->info->mass - 2*TICRATE)
		return true;
#else
