...
static boolean P_LookForShield(mobj_t *actor)
{
	...
	for (; ; actor->lastlook = ((actor->lastlook + 1) & PLAYERSMASK))
	{
...
#ifdef RABBIT
		if (actor->lastlook != self_id && ! virtualplayers[actor->lastlook].in_game)
			continue;
#else
		if (!playeringame[actor->lastlook])
			continue;
        ...
        {
            VirtualPlayer* player = &virtualplayers[actor->lastlook];
...
#ifdef RABBIT
		if (item == MT_1UPICO && newmobj->target) // LXShadow changed a bunch of things here. Evil crash bug...
		{
		    if (newmobj->target->player)
		    {
            ...
            for (i = 0; i < MAXPLAYERS; i ++)
            {
                if (virtualplayers[i].in_game && virtualplayers[i].mo == newmobj->target)
                    player = &virtualplayers[i];
            }

		    if (player)
		    {
		        newmobj->skin = &skins[player->skin];
		        P_SetMobjState(newmobj, newmobj->info->spawnstate);
		    }
		}
#else
...
void A_AttractChase(mobj_t *actor)
{
	...
	if (!actor->tracer
#ifndef RABBIT // LXShadow modified A_AttractChase
		|| !actor->tracer->player
		|| !actor->tracer->health
#endif
...
#ifdef RABBIT
	if ((gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value)) && !cv_friendlyfire.value)
    {
        if (grenade->target->player && thing->player && grenade->target->player->ctfteam == thing->player->ctfteam) // Don't blow up at your teammates, unless friendlyfire is on
            return true;

        // LXShadow added grenade friendliness.
        int i;
        VirtualPlayer* player = NULL;
        for (i = 0; i < MAXPLAYERS; i ++)
        {
            if (virtualplayers[i].in_game && virtualplayers[i].mo == grenade->target)
                player = &virtualplayers[i];
        }

        if (player && thing->player)
        {
            if (player->ctfteam == thing->player->ctfteam)
                return true;
        }
    }
#else
