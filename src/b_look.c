...
void B_LookForThings (player_t* p)
{
    ...
    while (currentthinker != &thinkercap)	//search through the list of all thinkers
    {
        if (currentthinker->function.acp1 == (actionf_p1)P_MobjThinker)
		{
			...
			if (((mo->flags & MF_COUNTKILL)) && (mo->flags & MF_SOLID))//its a monster thats not dead
				enemyFound = true;
			else if (mo->player)
			{
...
//  HERETIC??? --> ///////// bonuses/powerups ////////////////////////
					case MT_ARTIINVULNERABILITY:	//invulnrability always run to get it
						if (cv_gametype.value == GT_MATCH || !p->powers[pw_invulnerability])
							itemWeight = 10;
						break;
					case MT_ARTIINVISIBILITY:	//invisability
						if (cv_gametype.value == GT_MATCH || !p->powers[pw_invisibility])
							itemWeight = 9;
						break;
					case MT_ARTISUPERHEAL:	//soul sphere
						if (cv_gametype.value == GT_MATCH || p->health < maxsoul)
							itemWeight = 8;
						break;
	...
	/////////////// weapons ////////////////////////////				
					case SPR_SHOT:
						...
						else if (((cv_gametype.value == GT_MATCH == 2) || (mo->flags & MF_DROPPED)) && (p->ammo[am_shell] < p->maxammo[am_shell]))
							itemWeight = 3;
						break;
					case SPR_MGUN:
						...
						else if (((cv_gametype.value == GT_MATCH == 2) || (mo->flags & MF_DROPPED)) && (p->ammo[am_clip] < p->maxammo[am_clip]))
							itemWeight = 3;
						break;
					case SPR_LAUN:
						...
						else if (((cv_gametype.value == GT_MATCH == 2) || (mo->flags & MF_DROPPED)) && (p->ammo[am_misl] < p->maxammo[am_misl]))
							itemWeight = 3;
						break;
					case SPR_PLAS:
						...
						else if (((cv_gametype.value == GT_MATCH == 2) || (mo->flags & MF_DROPPED)) && (p->ammo[am_cell] < p->maxammo[am_cell]))
							itemWeight = 3;
						break;
					case SPR_BFUG:
...
//NON-HERETIC???////////// bonuses/powerups now checks for skill level
				case MT_INV:	//invulnrability always run to get it
				...
				case MT_BLUETV:	//blue shield, don't bother if you have a shield already.
					if (gameskill == sk_baby)
					{
						if (!p->powers[pw_blueshield] && !p->powers[pw_greenshield] && !p->powers[pw_redshield] && !p->powers[pw_yellowshield] && !p->powers[pw_blackshield])
						itemWeight = 1;
					}
					else if (gameskill == sk_easy)
					{
				...
				case MT_GREENTV:	//green shield, don't bother if you have a lower level shield already.
					if (gameskill == sk_baby)
					{
						if (!p->powers[pw_greenshield] && !p->powers[pw_redshield] && !p->powers[pw_yellowshield] && !p->powers[pw_blackshield])
						itemWeight = 1;
					}
					else if (gameskill == sk_easy)
					{
				...
				case MT_RING: case MT_FLINGRING: case MT_COIN: case MT_FLINGCOIN: //medication  MEDIKIT or STIMPACK
					if (gameskill == sk_baby)
					{
