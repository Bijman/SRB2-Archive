...
void P_ClearSingleMapHeaderInfo(int i)
{
	...
	mapheaderinfo[i-1].scriptislump = false;
	#ifdef MULTISKY
	mapheaderinfo[i-1].sky2num = i;
	mapheaderinfo[i-1].sky3num = i;
	mapheaderinfo[i-1].sky4num = i;
	mapheaderinfo[i-1].sky5num = i;
	mapheaderinfo[i-1].sky6num = i;
	mapheaderinfo[i-1].sky7num = i;
	mapheaderinfo[i-1].sky8num = i;
	mapheaderinfo[i-1].sky9num = i;
	#endif
}
...
/*        if (ss->tag < 0)
    ...
    #ifdef MULTISKY
    sky2flatnum = P_AddLevelFlat ("F_SKY2",foundflats);
    sky3flatnum = P_AddLevelFlat ("F_SKY3",foundflats);
    sky4flatnum = P_AddLevelFlat ("F_SKY4",foundflats);
    sky5flatnum = P_AddLevelFlat ("F_SKY5",foundflats);
    sky6flatnum = P_AddLevelFlat ("F_SKY6",foundflats);
    sky7flatnum = P_AddLevelFlat ("F_SKY7",foundflats);
    sky8flatnum = P_AddLevelFlat ("F_SKY8",foundflats);
    sky9flatnum = P_AddLevelFlat ("F_SKY9",foundflats);
    #endif
...
void P_LoadThings (int lump)
{
    ...
    for (i=0 ; i<nummapthings ; i++, mt++)
    {
		...
		thisx = SHORT(READSHORT(data));
		thisy = SHORT(READSHORT(data));
		thisangle = SHORT(READSHORT(data));
		thistype = SHORT(READSHORT(data));
		thisoptions = SHORT(READSHORT(data));
...
noscript:
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
        ...
        #ifndef RP
        players[i].timebonus = players[i].ringbonus =
        #endif
        countdown = countdown2 = 0; // Reset the bonus counters Tails 03-10-2000
...
//    P_InitLava ();
    ...
    //P_LoadBlockMap (lastloadedmaplumpnum+ML_BLOCKMAP); //Alam: new Blockmap code
