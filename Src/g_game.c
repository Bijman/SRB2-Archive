...
void G_BuildTiccmd (ticcmd_t* cmd, int realtics)
{
    ...
    #ifndef RP
    || players[consoleplayer].nightsmode
    #endif
    ) // Analog Test Tails 06-10-2001
    {
        if (turnright)
		{
            side += sidemove[speed];
    ...
    #else
    if (gamekeydown[gamecontrol[gc_rp1][0]] ||
        gamekeydown[gamecontrol[gc_rp1][1]])
        cmd->buttons |= BT_RP1;

    if (gamekeydown[gamecontrol[gc_rp2][0]] ||
        gamekeydown[gamecontrol[gc_rp2][1]])
        cmd->buttons |= BT_RP2;

    if (gamekeydown[gamecontrol[gc_rp3][0]] ||
        gamekeydown[gamecontrol[gc_rp3][1]])
        cmd->buttons |= BT_RP3;

    if (gamekeydown[gamecontrol[gc_rp4][0]] ||
        gamekeydown[gamecontrol[gc_rp4][1]])
        cmd->buttons |= BT_RP4;

    if (gamekeydown[gamecontrol[gc_rp5][0]] ||
        gamekeydown[gamecontrol[gc_rp5][1]])
        cmd->buttons |= BT_RP5;

    if (gamekeydown[gamecontrol[gc_rp6][0]] ||
        gamekeydown[gamecontrol[gc_rp6][1]])
        cmd->buttons |= BT_RP6;

    if (gamekeydown[gamecontrol[gc_rp7][0]] ||
        gamekeydown[gamecontrol[gc_rp7][1]])
        cmd->buttons |= BT_RP7;

    /*if (gamekeydown[gamecontrol[gc_rplock][0]] ||
        gamekeydown[gamecontrol[gc_rplock][1]])
        cmd->buttons |= BT_RPLOCK;

    if (gamekeydown[gamecontrol[gc_rpunlock][0]] ||
        gamekeydown[gamecontrol[gc_rpunlock][1]])
        cmd->buttons |= BT_RPUNLOCK;*/

    #if NUMABILITYS > 7
    #error REMEMBER TO ADD NEW GAME CONTROL BUTTON TOGGLE THINGIES FOR TEH NEW RP ABILITYS1!!!11!!!!!11!!
    #endif
...
void G_BuildTiccmd2 (ticcmd_t* cmd, int realtics)
{
    ...
    #ifndef RP
    || players[secondarydisplayplayer].nightsmode
    #endif
    ) // Analog Test Tails 06-10-2001
    {
        if (turnright)
...
boolean G_Responder (event_t* ev)
{
        ...
        && ev->data1 == KEY_F12) // Tails 03-13-2001
    {
		...
		#ifdef RP
		CONS_Printf("Camera change is disabled (for now)...\n");
		#else
        else
		{
			// spy mode
...
void G_PlayerReborn (int player)
{
	...
	#ifdef RP
	byte aframes[NUMASTATES-1];
	byte elements[NUMELEMENTS-1];
	byte abilitys[NUMABILITYS-1];
	int i;
	...
	#ifdef RP
	for(i=0;i<NUMASTATES;i++)
		aframes[i] = players[player].aframes[i];
	for(i=0;i<NUMELEMENTS;i++)
		elements[i] = players[player].elements[i];
	for(i=0;i<NUMABILITYS;i++)
		abilitys[i] = players[player].abilitys[i];
	#endif
	...
	#ifdef RP
	for(i=0;i<NUMASTATES;i++)
		players[player].aframes[i] = aframes[i];
	for(i=0;i<NUMELEMENTS;i++)
		players[player].elements[i] = elements[i];
	for(i=0;i<NUMABILITYS;i++)
		players[player].abilitys[i] = abilitys[i];
	#endif
