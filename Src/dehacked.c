...
char* strup1(char* n) //Alam_GBC: *inx doesn't have MSCRT's strupr()
{
	int i;
	char* upr = n;
	for(i = 0; n[i]; i++)
...
/*        else if(!strcmp(word,"Text"))
        ...
        else if(!strcmp(word,"SRB2"))
             {
               ...
               if( ver!=11)
                  deh_error("Warning : patch from a different SRB2 version (%d), only version 1.1 is supported\n",ver);
               #else
               int ver = searchvalue(strtok(NULL,"\n"));
               if(ver > VERSION)
                  deh_error("Warning : SOC patch from a different version of SRB2 (%d), only RP Beta %d and below are supported\n",ver,VERSION);
               #endif
             }
/*        else if(!strcmp(word,"Patch"))
  ...
  {
      #ifdef RP
      CONS_Printf("SOC edit contains warnings\n");
      #else
