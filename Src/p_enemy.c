...
static boolean P_CheckMeleeRange (mobj_t*      actor)
{
    ...
    if (! P_CheckSight (actor, actor->target)
		#ifndef RP
		&& !(actor->type == MT_JETTBOMBER || actor->type == MT_SKIM)
		#endif
		)
        return false;
	}
...
void A_BossScream (mobj_t* actor)
{
	...
	#ifdef RP
    S_StartSound(P_SpawnMobj(x,y,z, MT_BIGEXPLODE), actor->info->deathsound);
	#else
...
void A_BubbleSpawn (mobj_t*   actor)
{
 ...
 if (P_Random () > 128)
   ...
   {
       #ifdef RP
        P_SpawnMobj (actor->x,actor->y,actor->z + (actor->height / 2), MT_HUGEBUBBLE);
       #else
...
{
    #ifdef RP
	if(actor->type == MT_HUGEBUBBLE)
		actor->momz = actor->info->speed; // make bubbles rise!
    #else
