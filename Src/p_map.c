	...
	switch(spring->type)
	{
		...
		case MT_REDDIAG: // Red diagonal spring (pointing up from ground)
			...
			if(object->player)
			{
				...
				#ifdef RP
			    P_SetMobjState (object, S_PLAY_SPRN1);
				#else
					#ifdef MARIO
			    P_SetMobjState (object, S_PLAY_ATK1);
					#else
...
// Snowballs can hit other things Tails 12-12-2001
    ...
    #ifndef RP
    || tmthing->type == MT_SHELL || tmthing->type == MT_FIREBALL
    #endif
    ) // Tails 12-12-2001
    {
        // see if it went over / under
			...
			switch(tmthing->type)
			{
				case MT_FAN: // fan
					if(thing->z <= tmthing->z + (tmthing->health << FRACBITS))
					{
						...
						P_ResetPlayer(thing->player);
						#ifdef RP
						if(thing->player->astate != AS_FALL)
							P_SetMobjState (thing, S_PLAY_FALL1);
						#else
							#ifdef MARIO
						if(thing->state != &states[S_PLAY_ATK1])
							P_SetMobjState (thing, S_PLAY_ATK1);
							#else
		...
		if(tmthing->z >= thing->z)
		{
			switch(thing->type)
			{
				case MT_FAN: // fan
					...
					#ifdef RP
					if(tmthing->player->astate != AS_FALL)
						P_SetMobjState (tmthing, S_PLAY_FALL1);
					#else
						#ifdef MARIO
					if(tmthing->state != &states[S_PLAY_ATK1])
						P_SetMobjState (tmthing, S_PLAY_ATK1);
						#else
...
*/
    ...
    if(newsubsec->sector->ffloors)
    {
      ...
      for(rover = newsubsec->sector->ffloors; rover; rover = rover->next)
      {
        ...
        #ifndef RP
        && !(thing->player && !thing->player->nightsmode && thing->player->skin == 1 && !thing->player->mfspinning && thing->player->speed > 28 && thing->ceilingz - *rover->topheight >= thing->info->height && thing->z < *rover->topheight + 30*FRACUNIT && thing->z > *rover->topheight - 30*FRACUNIT)
        #endif
        ) || !(rover->flags & FF_EXISTS)) continue;
                    ...
                    boolean       allowdropoff)
{
...
#endif
    {
		...
		#ifdef RP
		if(thing->player && thing->state == &states[S_PLAY_SPRN1])
		#else
			#ifdef MARIO
		if(thing->player) // NEVER step up in Mario mode! No ramps here!
			#else
		...
		if(slidemo->player) // Whole hella climbin' stuff goin' on here, sucka! Tails 04-12-2001
		{
			if(slidemo->player->gliding || slidemo->player->climbing)
			{
				...
				if(P_PointOnLineSide(slidemo->x, slidemo->y, li) == 0)
				{
					...
					&& ((linequadrant == 1 && (playerquadrant == 2 || playerquadrant == 3
					|| playerquadrant == 1))
					|| (linequadrant == 2 && (playerquadrant == 3 || playerquadrant == 2
					|| playerquadrant == 4))
					|| (linequadrant == 3 && (playerquadrant == 4 || playerquadrant == 1
					|| playerquadrant == 3))
					|| (linequadrant == 4 && (playerquadrant == 1 || playerquadrant == 2
					|| playerquadrant == 4)))))
					{
						int i;
...
//
        ...
        #ifndef RP
        && mo->target && mo->type != MT_EGGMOBILE
											&& mo->type != MT_EGGMOBILE2
											#endif
											&& mo->type != MT_RING
											#ifndef RP
           && mo->type != MT_COIN // Tails
           #endif
           )
