...
//
// DESCRIPTION:
//    Macros to read/write from/to a char*, used for packet creation and such
//
//-----------------------------------------------------------------------------
...
//
#define WRITEBYTE(p,b)      do { byte* p_tmp = (byte*)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
#define WRITECHAR(p,b)      do { char* p_tmp = (char*)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
#define WRITESHORT(p,b)     do { short* p_tmp = (short*)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
#define WRITEUSHORT(p,b)    do { USHORT* p_tmp = (USHORT*)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
#define WRITELONG(p,b)      do { long* p_tmp = (long*)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
#define WRITEULONG(p,b)     do { ULONG* p_tmp = (ULONG*)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
#define WRITEFIXED(p,b)     do { fixed_t* p_tmp = (fixed_t*)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
#define WRITEANGLE(p,b)     do { angle_t* p_tmp = (angle_t*)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
...
#define WRITEDOUBLE(p,b)    do { double *p_tmp = (double *)p; *p_tmp = b; p_tmp++; p = (void*)p_tmp; } while(0)
...
static inline void writedouble(void * prr, int val)
{
  char * cp = ptr;
  cp[0] = val ;  val >>= 8;
  cp[1] = val ;  val >>= 8;
...
#define WRITEMEM(p,s,n)     memcpy(p, s, n);p+=n
// New "functions" Tails
#define WRITEDOUBLE(p,b)      writedouble(((double   *)p)++, (b)) //Alam_GBC: Test Me!
...
static inline double readouble(void * prt)
{
        unsigned int hi, lo;
        long long *val = ptr;
        ...
        val >>= 32;
        hi = (unsigned int)(val&0xFFFFFFFF);
        val = Swapui(lo);
        val <<= 32;
        val |= Swapui(hi);
        return (double)val;
