                         ...
                         statenum_t    state )
{
    ...
    #ifdef DARKWORLD
    DarkWorldShadows(mobj); // Update your shadow! >.>
    #endif
	...
	if(player)
	{
		if (player->rmomx > -STOPSPEED
					...
					&& player->cmd.sidemove == 0 && !player->mfspinning))
			{
				...
				if (
				#ifdef RP
				(player && player->astate == AS_WALK)
				#else
				(player && player->walking)
				#endif
				&& (mo->type!=MT_SPIRIT)) // use my new walking variable! Tails 10-30-2000
				{
				#ifdef RP
					if(player->powers[pw_super])
						P_SetMobjState (player->mo, S_PLAY_SSTN1); // Super Sonic Stuff Tails 04-18-2000
					else
						P_SetMobjState (player->mo, S_PLAY_STND1);
				#else
		/*          if(player->powers[pw_super])
				   {
...
void P_XYMovement (mobj_t* mo)
{
    ...
    do
    {
        ...
        if (!P_TryMove (mo, ptryx, ptryy, true)) //SoM: 4/10/2000
        {
			...
			else if (mo->flags & MF_MISSILE)
			{
				...
				if (blockingline
				#ifndef RP
				&& mo->type != MT_REDRING && mo->type != MT_LASER
    ...
    if (mo->z > mo->floorz
    #ifndef RP
    && mo->type != MT_CRAWLACOMMANDER && mo->type != MT_EGGMOBILE && mo->type != MT_EGGMOBILE2
    #endif
    )
		return;         // no friction when airborne
...
// Intercept the stupid 'fall through 3dfloors' bug Tails 03-17-2002
    ...
    if ( mo->flags & MF_FLOAT
			&& mo->target && mo->health
			#ifndef RP
			&& !(mo->type == MT_DETON || mo->type == MT_JETTBOMBER || mo->type == MT_JETTGUNNER || mo->type == MT_CRAWLACOMMANDER || mo->type == MT_EGGMOBILE2)
			#endif
			) // Tails 07-21-2001
    {
...
// Intercept the stupid 'fall through 3dfloors' bug Tails 03-17-2002
    if(mo->subsector->sector->ffloors)
    {
      ...
      for(rover = mo->subsector->sector->ffloors; rover; rover = rover->next)
      {
        ...
        #ifndef RP
        && !(mo->player && !mo->player->nightsmode && mo->player->skin == 1 && !mo->player->mfspinning && mo->player->speed > 28 && mo->ceilingz - *rover->topheight >= mo->info->height && mo->z < *rover->topheight + 30*FRACUNIT && mo->z > *rover->topheight - 30*FRACUNIT)
        #endif
        )) continue;
...
/*
    ...
    if (mo->z <= mo->floorz)
    {
		...
		#ifdef RP
		if(mo->player && mo->player->astate == AS_PAIN && mo->player->playerstate != PST_DEAD)
				...
				if(mo->player)
				{
					if(mo->health)
					{
						...
						if(mo->player->powers[pw_super])
						{
							...
							{
								if(mo->player->speed > runspeed && mo->player->astate != AS_SUPER_RUN)
									P_SetMobjState (mo, S_PLAY_SRUN1);
								else if ((mo->player->rmomx > STOPSPEED || mo->player->rmomy > STOPSPEED) && mo->player->astate != AS_WALK)
									P_SetMobjState (mo, S_PLAY_WALK1);
								else if ((mo->player->rmomx < -STOPSPEED || mo->player->rmomy < -STOPSPEED) && mo->player->astate != AS_WALK)
									P_SetMobjState (mo, S_PLAY_WALK1);
								else if ((mo->player->rmomx < FRACUNIT && mo->player->rmomx > -FRACUNIT && mo->player->rmomy < FRACUNIT && mo->player->rmomy > -FRACUNIT) && mo->player->astate != AS_SUPER_STAND)
									P_SetMobjState (mo, S_PLAY_SSTN1);
							}
							else
							{
								if(mo->player->speed > runspeed && mo->player->astate != AS_SUPER_RUN)
									P_SetMobjState (mo, S_PLAY_SRUN1);
								else if ((mo->momx || mo->momy) && mo->player->astate != AS_WALK)
									P_SetMobjState (mo, S_PLAY_WALK1);
								else if (!mo->momx && !mo->momy && mo->player->astate != AS_SUPER_STAND)
									P_SetMobjState (mo, S_PLAY_SSTN1);
							}
						}
					    #else
						...
						#ifdef RP
						{
							...
							if(mo->player->cmomx || mo->player->cmomy)
							{
								if(mo->player->speed > runspeed && mo->player->astate != AS_DASH)
									P_SetMobjState (mo, S_PLAY_DASH1);
								else if ((mo->player->rmomx > STOPSPEED || mo->player->rmomy > STOPSPEED) && mo->player->astate != AS_WALK)
									P_SetMobjState (mo, S_PLAY_WALK1);
								else if ((mo->player->rmomx < -STOPSPEED || mo->player->rmomy < -STOPSPEED) && mo->player->astate != AS_WALK)
									P_SetMobjState (mo, S_PLAY_WALK1);
								else if ((mo->player->rmomx < FRACUNIT && mo->player->rmomx > -FRACUNIT && mo->player->rmomy < FRACUNIT && mo->player->rmomy > -FRACUNIT) && mo->player->astate != AS_STAND)
									P_SetMobjState (mo, S_PLAY_STND1);
							}
							else
							{
								if(mo->player->speed > runspeed && !mo->player->astate != AS_DASH)
									...
									P_SetMobjState (mo, S_PLAY_WALK1);
								else if (!mo->momx && !mo->momy && mo->player->astate != AS_STAND)
									P_SetMobjState (mo, S_PLAY_STND1);
							}
						}
						#else
						{
							...
							{
								#ifdef MARIO
								if(false); // Yes, I'm that lazy today.
								#else
...
playergravity:
		...
		if(mo->player->playerstate == PST_DEAD) // Added crash check Tails 11-16-2001)
		{
            ...
            #ifdef RP
            if(mo->z <= mo->floorz && mo->player->astate != AS_PAIN)
			{
				mo->momz = 0;
				mo->z = mo->floorz;
				P_SetMobjState(mo, S_PLAY_PAIN1);
			}
            #endif
			return;
		}
					...
					if (node->m_sector->ffloors)
					{
						...
						for(rover = node->m_sector->ffloors; rover; rover = rover->next)
						{
								...
								) && *rover->bottomheight == mo->ceilingz) // The player's head hit the bottom!
							{
								// DO THE MARIO!
								#ifdef MARIO
								EV_MarioBlock(rover->master->frontsector, node->m_sector, *rover->topheight, rover->master, mo, rover->flags & FF_BUSTUP);
								#else
...
/*	fixed_t zatpoint = 0;
    ...
    if(mobj->health <= 0)
	{
		...
		#ifdef RP
		P_SetMobjState (mobj, S_PLAY_DETH1);
		#else
...
void P_MobjThinker (mobj_t* mobj)
{
    ...
    #ifdef DARKWORLD
    if(mobj->type == MT_DARKSHADOW)
    {
        if(!mobj->darkshadow || !mobj->darkshadow->darkshadow
        || mobj->darkshadow->state == &states[S_DISS])
            P_SetMobjState(mobj, S_DISS);
        return;
    }
	DarkWorldShadows(mobj);
    #endif
	...
	if(mobj->flags & MF_SCENERY)
	{
		...
		switch(mobj->type)
		{
			...
			case MT_MEDIUMBUBBLE:
			#ifdef RP
			case MT_HUGEBUBBLE:	// start bubble dissipate Tails
			#else
                      ...
                      mobjtype_t    type )
{
	...
	|| mobj->type == MT_COIN
	    ...
	    #ifndef RP
		|| mobj->type == MT_DETON || mobj->type == MT_JETTBOMBER || mobj->type == MT_JETTGUNNER
		#endif
		) // Special flag for rings Tails 06-03-2001
    ...
    //SOM: Fuse for bunnies, squirls, and flingrings
      if(mobj->type == MT_BIRD || mobj->type == MT_SQRL || mobj->type == MT_MOUSE || (mobj->flags & MF_ENEMY && eastermode))
		...
		else
		{
			...
			for (y=yl ; y<=yh ; y += FRACUNIT*64)
				for (x=xl ; x<=xh ; x += FRACUNIT*64)
				{
					...
					|| R_PointInSubsector(x,y)->sector->ceilingpic == sky9flatnum
					#endif
					)  // Found the outdoors!
					{
...
//        P_AddAmbientSfx(mthing->type-1200);
	...
	#endif
	{
		...
		#ifdef RP
		else if (i >= MT_INTERCOM1 && i <= MT_INTERCOM9)
		{
			if(mthing->options & MTF_AMBUSH && mobjinfo[i].flags & MF_NOGRAVITY)
				z = ss->sector->floorheight + 128*FRACUNIT + (mthing->options >> 4);
		}
		#else
