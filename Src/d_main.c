...
void IdentifyVersion (void)
{
   ...
   I_LocateWad();
	...
	{
		#ifdef RP
		if (!access ("Srb2RP.wad",R_OK))
			D_AddFile ("Srb2RP.wad");
		else
			I_Error ("Srb2RP.wad not found!\n");
		#else
		#ifdef MARIO
		if (!access ("SMB1.wad",R_OK))
			D_AddFile ("SMB1.wad");
		else
			I_Error ("SMB1.wad not found!\n");
		#else
...
/*    p = M_CheckParm ("-wart");
  ...
  //--------------------------------------------------------- CONFIG.CFG
    ...
    #ifdef RP
	if (M_CheckParm ("-willtherealeggmanpleasestandup")) grade = MAXINT; // Tails 10-23-2002
	#else
