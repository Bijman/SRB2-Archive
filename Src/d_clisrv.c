...
static void CL_RemovePlayer(int playernum)
{
    ...
    if( players[playernum].mo )
    {
        ...
        #ifdef DARKWORLD
        if(players[playernum].mo->darkshadow)
            P_RemoveMobj(players[playernum].mo->darkshadow);
        #endif
    }
...
void Command_Kick(void)
{
    ...
    if (COM_Argc() != 2)
    {
        CONS_Printf ("kick <playername>/<playernum>" /*<message num>*/" : kick a player\n");
        return;
    }
   ...
   if(server || admin)
    {
        ...
        if(buf[0]==-1)
           return;
        /*buf[1]=COM_Argv(2);
        if(buf[1] < KICK_MSG_GO_AWAY || buf[1] > KICK_MSG_TIMEOUT)*/
                buf[1]=KICK_MSG_GO_AWAY;
        SendNetXCmd(XD_KICK,&buf,2);
    }
    else
        CONS_Printf("You are not the server or an admin.\n");
}
...
void Got_KickCmd(char **p,int playernum)
{
    ...
    switch(msg)
    {
       case KICK_MSG_GO_AWAY:
               CONS_Printf("has been kicked (DIE!!!)\n");
               break;
