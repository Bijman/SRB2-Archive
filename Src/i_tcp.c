...
{
    int           i,j,c;
#if defined (WATTCP) || defined(__WIN32__)
	int fromlen;
#else
	size_t fromlen;
#endif
...
#ifdef __WIN32__
	{
		unsigned long falseval = false; // Alam_GBC: disable the new UDP connection reset behavior for Win2000 and WinXP
...
#endif
	int i;
#if defined(__APPLE_CC__) || defined(WATTCP) || defined(__WIN32__)
	int j;
#else
	size_t j;
#endif
