...
static void R_ProjectSprite (mobj_t* thing)
{
    ...
    #ifdef RP
    if(thing->player)
        RP_SetAstate(thing->player);
    #endif

    //Fab:02-08-98: 'skin' override spritedef currently used for skin
    if (thing->skin)
        #ifdef RP
        sprdef = RP_Skin(thing);
        #else
...
void Sk_SetDefaultValue(skin_t *skin)
{
    ...
    for(i = 0; i < NUMELEMENTS; i++)
        strcpy (skin->element[i], "0");
    for(i = 0; i < NUMABILITYS; i++)
        strcpy (skin->power[i], "0");
	strcpy (skin->aframes[AS_STAND], "1");
	strcpy (skin->aframes[AS_WAIT], "2");
	strcpy (skin->aframes[AS_WALK], "8");
	strcpy (skin->aframes[AS_DASH], "4");
	strcpy (skin->aframes[AS_JUMP], "4");
	strcpy (skin->aframes[AS_EDGE], "2");
	strcpy (skin->aframes[AS_PAIN], "1");
	strcpy (skin->aframes[AS_DEATH], "1");
	strcpy (skin->aframes[AS_SPRING], "1");
	strcpy (skin->aframes[AS_FALL], "2");
	strcpy (skin->aframes[AS_HANG], "1");
	strcpy (skin->aframes[AS_GASP], "1");
	strcpy (skin->aframes[AS_FLY], "0");
	strcpy (skin->aframes[AS_TIRED], "0");
	strcpy (skin->aframes[AS_GLIDE], "0");
	strcpy (skin->aframes[AS_CLING], "0");
	strcpy (skin->aframes[AS_CLIMB], "0");
	strcpy (skin->aframes[AS_LIGHTORB], "0");
	strcpy (skin->aframes[AS_DARKORB], "0");
	strcpy (skin->aframes[AS_SUPER_STAND], "1");
	strcpy (skin->aframes[AS_SUPER_RUN], "1");
    #else
	...
	#ifdef RP
	strcpy (skin->prefcolor, "10");
	strcpy (skin->normalspeed, "25");
	strcpy (skin->thrustfactor, "4");
	strcpy (skin->accelstart, "100");
	strcpy (skin->acceleration, "35");
	strcpy (skin->jumpfactor, "80");
	#else
    ...
    #ifdef RP
    memcpy(&skins[0].sprstnd, &sprites[SPR_STND], sizeof(spritedef_t));
    memcpy(&skins[0].sprwait, &sprites[SPR_WAIT], sizeof(spritedef_t));
    memcpy(&skins[0].sprwalk, &sprites[SPR_WALK], sizeof(spritedef_t));
    memcpy(&skins[0].sprdash, &sprites[SPR_DASH], sizeof(spritedef_t));
    memcpy(&skins[0].sprjump, &sprites[SPR_JUMP], sizeof(spritedef_t));
    memcpy(&skins[0].spredge, &sprites[SPR_EDGE], sizeof(spritedef_t));
    memcpy(&skins[0].sprpain, &sprites[SPR_PAIN], sizeof(spritedef_t));
    memcpy(&skins[0].sprdeth, &sprites[SPR_DETH], sizeof(spritedef_t));
    memcpy(&skins[0].sprsprn, &sprites[SPR_SPRN], sizeof(spritedef_t));
    memcpy(&skins[0].sprfall, &sprites[SPR_FALL], sizeof(spritedef_t));
    memcpy(&skins[0].sprhang, &sprites[SPR_HANG], sizeof(spritedef_t));
    memcpy(&skins[0].sprgasp, &sprites[SPR_GASP], sizeof(spritedef_t));
    memcpy(&skins[0].sprflyy, &sprites[SPR_FLYY], sizeof(spritedef_t));
    memcpy(&skins[0].sprtire, &sprites[SPR_TIRE], sizeof(spritedef_t));
    memcpy(&skins[0].sprglid, &sprites[SPR_GLID], sizeof(spritedef_t));
    memcpy(&skins[0].sprclng, &sprites[SPR_CLNG], sizeof(spritedef_t));
    memcpy(&skins[0].sprclim, &sprites[SPR_CLIM], sizeof(spritedef_t));
    memcpy(&skins[0].sprlorb, &sprites[SPR_LORB], sizeof(spritedef_t));
    memcpy(&skins[0].sprdorb, &sprites[SPR_DORB], sizeof(spritedef_t));
    memcpy(&skins[0].sprsstn, &sprites[SPR_SSTN], sizeof(spritedef_t));
    memcpy(&skins[0].sprsrun, &sprites[SPR_SRUN], sizeof(spritedef_t));
    #else
    memcpy(&skins[0].spritedef, &sprites[SPR_PLAY], sizeof(spritedef_t));
    #endif

    #ifdef MARIO
    MGB_DefaultSkinValues(skin);
    #endif
}
...
void SetPlayerSkin (int playernum, char *skinname)
{
    ...
    for(i=0;i<numskins;i++)
    {
        ...
        if (stricmp(skins[i].name,skinname)==0)
        {
			...
			#ifdef RP
			for(j = 0; j < NUMASTATES; j++)
				players[playernum].aframes[j] = atoi(skins[i].aframes[j]);
			for(j = 0; j < NUMELEMENTS; j++)
				players[playernum].elements[j] = atoi(skins[i].element[j]);
			for(j = 0; j < NUMABILITYS; j++)
				players[playernum].abilitys[j] = atoi(skins[i].power[j]);
			#else
			...
			#ifdef RP
			players[playernum].normalspeed = atoi(skins[i].normalspeed); // Tails
			players[playernum].thrustfactor = atoi(skins[i].thrustfactor); // Tails
			players[playernum].accelstart = atoi(skins[i].accelstart); // Tails
			players[playernum].acceleration = atoi(skins[i].acceleration); // Tails
			#else
			...
			#ifdef RP
			if(players[playernum].jumpfactor > 300)
				players[playernum].jumpfactor = 300;
			#else
...
void SetPlayerSkinByNum (int playernum, int skinnum)
{
	...
	if(skinnum < numskins) // Make sure it exists!
	{
		...
		#ifdef RP
		for(j = 0; j < NUMASTATES; j++)
			players[playernum].aframes[j] = atoi(skins[skinnum].aframes[j]);
		for(j = 0; j < NUMELEMENTS; j++)
			players[playernum].elements[j] = atoi(skins[skinnum].element[j]);
		for(j = 0; j < NUMABILITYS; j++)
			players[playernum].abilitys[j] = atoi(skins[skinnum].power[j]);
		#else
		...
		#ifdef RP
		players[playernum].normalspeed = atoi(skins[skinnum].normalspeed); // Tails
		players[playernum].thrustfactor = atoi(skins[skinnum].thrustfactor); // Tails
		players[playernum].accelstart = atoi(skins[skinnum].accelstart); // Tails
		players[playernum].acceleration = atoi(skins[skinnum].acceleration); // Tails
		#else
		// Set the "old" versions directly Graue 12-22-2003
...
// End Custom translation table Tails 06-07-2002
			...
			if (!strcmp(token, "numstnd"))
			{
                strncpy (skins[numskins].aframes[AS_STAND], value, 2);
                strupr (skins[numskins].aframes[AS_STAND]);
			}
			else
			if (!strcmp(token, "numwait"))
			{
                strncpy (skins[numskins].aframes[AS_WAIT], value, 2);
                strupr (skins[numskins].aframes[AS_WAIT]);
			}
			else
			if (!strcmp(token, "numwalk"))
			{
                strncpy (skins[numskins].aframes[AS_WALK], value, 2);
                strupr (skins[numskins].aframes[AS_WALK]);
			}
			else
			if (!strcmp(token, "numdash"))
			{
                strncpy (skins[numskins].aframes[AS_DASH], value, 2);
                strupr (skins[numskins].aframes[AS_DASH]);
			}
			else
			if (!strcmp(token, "numjump"))
			{
                strncpy (skins[numskins].aframes[AS_JUMP], value, 2);
                strupr (skins[numskins].aframes[AS_JUMP]);
			}
			else
			if (!strcmp(token, "numedge"))
			{
                strncpy (skins[numskins].aframes[AS_EDGE], value, 2);
                strupr (skins[numskins].aframes[AS_EDGE]);
			}
			else
			if (!strcmp(token, "numpain"))
			{
                strncpy (skins[numskins].aframes[AS_PAIN], value, 2);
                strupr (skins[numskins].aframes[AS_PAIN]);
			}
			else
			if (!strcmp(token, "numdeth"))
			{
                strncpy (skins[numskins].aframes[AS_DEATH], value, 2);
                strupr (skins[numskins].aframes[AS_DEATH]);
			}
			else
			if (!strcmp(token, "numsprn"))
			{
                strncpy (skins[numskins].aframes[AS_SPRING], value, 2);
                strupr (skins[numskins].aframes[AS_SPRING]);
			}
			else
			if (!strcmp(token, "numfall"))
			{
                strncpy (skins[numskins].aframes[AS_FALL], value, 2);
                strupr (skins[numskins].aframes[AS_FALL]);
			}
			else
			if (!strcmp(token, "numhang"))
			{
                strncpy (skins[numskins].aframes[AS_HANG], value, 2);
                strupr (skins[numskins].aframes[AS_HANG]);
			}
			else
			if (!strcmp(token, "numgasp"))
			{
                strncpy (skins[numskins].aframes[AS_GASP], value, 2);
                strupr (skins[numskins].aframes[AS_GASP]);
			}
			else
			if (!strcmp(token, "numflyy"))
			{
                strncpy (skins[numskins].aframes[AS_FLY], value, 2);
                strupr (skins[numskins].aframes[AS_FLY]);
			}
			else
			if (!strcmp(token, "numtire"))
			{
                strncpy (skins[numskins].aframes[AS_TIRED], value, 2);
                strupr (skins[numskins].aframes[AS_TIRED]);
			}
			else
			if (!strcmp(token, "numglid"))
			{
                strncpy (skins[numskins].aframes[AS_GLIDE], value, 2);
                strupr (skins[numskins].aframes[AS_GLIDE]);
			}
			else
			if (!strcmp(token, "numclng"))
			{
                strncpy (skins[numskins].aframes[AS_CLING], value, 2);
                strupr (skins[numskins].aframes[AS_CLING]);
			}
			else
			if (!strcmp(token, "numclim"))
			{
                strncpy (skins[numskins].aframes[AS_CLIMB], value, 2);
                strupr (skins[numskins].aframes[AS_CLIMB]);
			}
			else
			if (!strcmp(token, "numlorb"))
			{
                strncpy (skins[numskins].aframes[AS_LIGHTORB], value, 2);
                strupr (skins[numskins].aframes[AS_LIGHTORB]);
			}
			else
			if (!strcmp(token, "numdorb"))
			{
                strncpy (skins[numskins].aframes[AS_DARKORB], value, 2);
                strupr (skins[numskins].aframes[AS_DARKORB]);
			}
			else
			if (!strcmp(token, "numsstn"))
			{
                strncpy (skins[numskins].aframes[AS_SUPER_STAND], value, 2);
                strupr (skins[numskins].aframes[AS_SUPER_STAND]);
			}
			else
			if (!strcmp(token, "numsrun"))
			{
                strncpy (skins[numskins].aframes[AS_SUPER_RUN], value, 2);
                strupr (skins[numskins].aframes[AS_SUPER_RUN]);
			}
			#else
            else
            ...
            else
            {
                ...
                for(i=0;i<NUMABILITYS;i++)
                {
                    sprintf(word,"power%d",i+1);
                    if (word && !stricmp(token,word))
                    {
                        strncpy (skins[numskins].power[i], value, 1);
                        strupr (skins[numskins].power[i]);
                        found=true;
                    }
                }
                // Elements
                for(i=0;i<NUMELEMENTS;i++)
                {
                    sprintf(word,"element%d",i+1);
                    if (word && !stricmp(token,word))
                    {
                        strncpy (skins[numskins].element[i], value, 1);
                        strupr (skins[numskins].element[i]);
                        found=true;
                    }
                }
                #endif
...
next_token:
        ...
        if( !sprname )
        {
            ...
            #ifdef RP
            int loop;
            for(loop = AS_STAND; loop < NUMASTATES; loop++)
            {
                // Get the name
                ...
                {
                    case AS_STAND:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprstnd, wadnum, lumpnum, lastlump);
                        break;
                    case AS_WAIT:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprwalk, wadnum, lumpnum, lastlump);
                        break;
                    case AS_WALK:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprwalk, wadnum, lumpnum, lastlump);
                        break;
                    case AS_DASH:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprdash, wadnum, lumpnum, lastlump);
                        break;
                    case AS_JUMP:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprjump, wadnum, lumpnum, lastlump);
                        break;
                    case AS_EDGE:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].spredge, wadnum, lumpnum, lastlump);
                        break;
                    case AS_PAIN:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprpain, wadnum, lumpnum, lastlump);
                        break;
                    case AS_DEATH:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprdeth, wadnum, lumpnum, lastlump);
                        break;
                    case AS_SPRING:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprsprn, wadnum, lumpnum, lastlump);
                        break;
                    case AS_FALL:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprfall, wadnum, lumpnum, lastlump);
                        break;
                    case AS_HANG:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprhang, wadnum, lumpnum, lastlump);
                        break;
                    case AS_GASP:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprgasp, wadnum, lumpnum, lastlump);
                        break;
                    case AS_FLY:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprflyy, wadnum, lumpnum, lastlump);
                        break;
                    case AS_TIRED:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprtire, wadnum, lumpnum, lastlump);
                        break;
                    case AS_GLIDE:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprglid, wadnum, lumpnum, lastlump);
                        break;
                    case AS_CLING:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprclng, wadnum, lumpnum, lastlump);
                        break;
                    case AS_CLIMB:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprclim, wadnum, lumpnum, lastlump);
                        break;
                    case AS_LIGHTORB:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprlorb, wadnum, lumpnum, lastlump);
                        break;
                    case AS_DARKORB:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprdorb, wadnum, lumpnum, lastlump);
                        break;
                    case AS_SUPER_STAND:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprsstn, wadnum, lumpnum, lastlump);
                        break;
                    case AS_SUPER_RUN:
                        R_AddSingleSpriteDef (sprname, &skins[numskins].sprsrun, wadnum, lumpnum, lastlump);
                        break;
                    default:
                        break;
                }
