...
//////////////////////////////////////
	...
	if (cmd->sidemove
		...
		#endif
		&& player->powers[pw_flashing] && !onground)))
	{
...
{
    ticcmd_t*           cmd;
	angle_t				movepushangle; // Test Tails 06-10-2001
...
// Analog movement control Tails
	...
	{
		if (!(player->gliding || player->exiting ||
		#ifdef RP
		(player->astate == AS_PAIN
		#else
		(player->mo->state == &states[S_PLAY_PAIN]
	...
	else if (netgame || (player == &players[consoleplayer] && !cv_analog.value) || (cv_splitscreen.value && player == &players[secondarydisplayplayer] && !cv_analog2.value))
	{
		...
		else if (cmd->sidemove && !player->gliding && !player->exiting && !player->climbing &&
		#ifdef RP
		!(player->astate == AS_PAIN
		#else
		!(player->mo->state == &states[S_PLAY_PAIN]
		#endif
		&& player->powers[pw_flashing])) // Tails 04-12-2001
		{
			boolean mright;
			boolean mleft;
			angle_t sideangle;
...
// what state the player is in. Tails 08-19-2000
    #ifndef RP
	#ifdef MARIO
	if (player->mo->state == &states[S_PLAY_RUN1] || player->mo->state == &states[S_PLAY_RUN2] || player->mo->state == &states[S_PLAY_RUN3])
	#else
	...
	// break into a run!
	    #ifdef RP
		if((player->speed > runspeed) && player->astate == AS_WALK && (onground))
	...
	// Otherwise, just walk.
	    #ifdef RP
		else if((player->rmomx || player->rmomy) && (player->astate == AS_STAND || player->astate == AS_WAIT || player->astate == AS_EDGE) && !(player->powers[pw_super]))
			P_SetMobjState (player->mo, S_PLAY_WALK1);
	    #else
		    #ifdef MARIO
...
// match their velocity.
	if(onground) // Only if on the ground.
	{
		...
		#ifdef RP
		else if(player->astate == AS_DASH || player->astate == AS_SUPER_RUN)
		#else
...
/*	if  (player->powers[pw_super] && (cmd->forwardmove || cmd->sidemove)
	...
	#ifdef RP
	if(player->astate == AS_DASH && !(player->speed > runspeed))
		P_SetMobjState (player->mo, S_PLAY_WALK1);
	#else
	...
	#ifdef RP
	if(player->astate == AS_SPRING && player->mo->momz < 0)
		P_SetMobjState (player->mo, S_PLAY_FALL1);
	#else
	#ifndef MARIO
	...
	#ifdef RP
	else if((player->astate == AS_SPRING || player->astate == AS_FALL || player->astate == AS_HANG) && player->mo->z == player->mo->floorz && !player->mo->momz)
	...
	#ifdef RP
	if(!player->mo->momx && !player->mo->momy && !player->mo->momz && player->astate == AS_WALK)
		P_SetMobjState(player->mo, S_PLAY_STND1);
	#else
...
// AKA my own gravity. =)
	if(player->gliding)
	{
		...
		#ifdef RP
		P_InstaThrust(player->mo, player->mo->angle, (20*FRACUNIT + player->gliding*1000)/NEWTICRATERATIO);
		player->gliding++;
		if (player->gliding < 0) player->gliding--;
		#else
...
////////////////
	...
	/*if(leveltime > 20999 && !netgame) // one tic off so the time doesn't display 10:00
		P_DamageMobj(player->mo, NULL, NULL, 10000);*/
...
////////////////////////////
	...
	#endif
	{
		...
		//Spinning and Spindashing
		#ifdef RP
		if(player->charspin && cmd->buttons & BT_USE && !player->exiting && !(player->astate == AS_PAIN && player->powers[pw_flashing])) // subsequent revs
		#else
		...
		#endif
		{
			if(player->speed < 5 && !player->mo->momz && onground && !player->usedown && !player->mfspinning)
			{
				...
				#ifdef RP
				P_SetMobjState (player->mo, S_PLAY_JUMP1);
				#else
				P_SetMobjState (player->mo, S_PLAY_ATK1);
				#endif
			    player->usedown = true;
	...
	if (
	#ifndef RP
	!(mapheaderinfo[gamemap-1].typeoflevel & TOL_GOLF) &&
	#endif
	cmd->buttons & BT_JUMP && !player->jumpdown && !player->exiting &&
	#ifdef RP
	!(player->astate == AS_PAIN
	#else
	!(player->mo->state == &states[S_PLAY_PAIN]
...
//////////////////
		...
		)
	{
		...
		// Otherwise, face the direction you're travelling.
		  ...
		  #ifdef RP
		  player->astate == AS_WALK || player->astate == AS_DASH || player->astate == AS_SUPER_RUN || player->astate == AS_JUMP
		  #else
		  player->walking || player->running || player->spinning || ((player->mo->state == &states[S_PLAY_ABL1] || player->mo->state == &states[S_PLAY_ABL1] || player->mo->state == &states[S_PLAY_ABL2] || player->mo->state == &states[S_PLAY_SPC1] || player->mo->state == &states[S_PLAY_SPC2] || player->mo->state == &states[S_PLAY_SPC3] || player->mo->state == &states[S_PLAY_SPC4]) && player->charability == 1)
		  #endif
  		  )
...
////////////////////////////
	...
	if(cmd->buttons & BT_USE && (player->mfjumped || player->powers[pw_tailsfly])) // Let Tails use it while he's flying...can be useful!
	{
		...
		else if(player->powers[pw_blackshield] && !player->usedown)
		{
			...
			{
			   player->blackow = true; // This signals for the BOOM to take effect, as seen below.
	...
	if(player->climbing == 1)
	{
		...
		if(glidesector->sector != player->mo->subsector->sector)
		{
			...
			if(floorclimb == false && glidesector->sector->floorheight < player->mo->z + 16*FRACUNIT && (glidesector->sector->ceilingpic == skyflatnum
			#ifdef MULTISKY
			|| glidesector->sector->ceilingpic == sky2flatnum
			|| glidesector->sector->ceilingpic == sky3flatnum
			|| glidesector->sector->ceilingpic == sky4flatnum
			|| glidesector->sector->ceilingpic == sky5flatnum
			|| glidesector->sector->ceilingpic == sky6flatnum
			|| glidesector->sector->ceilingpic == sky7flatnum
			|| glidesector->sector->ceilingpic == sky8flatnum
			|| glidesector->sector->ceilingpic == sky9flatnum
			#endif
			|| glidesector->sector->ceilingheight > (player->mo->z + player->mo->height + 8*FRACUNIT)))
			{
				thrust = true;
				boostup = true;
...
//			P_Thrust(player->mo, player->mo->angle, FRACUNIT);
			...
			if(player->climbing && climb && (player->mo->momx || player->mo->momy || player->mo->momz)
				...
				P_SetMobjState(player->mo, S_PLAY_CLIM1);
			else if ((!(player->mo->momx || player->mo->momy || player->mo->momz) || !climb) && player->astate != AS_CLING)
				P_SetMobjState(player->mo, S_PLAY_CLNG1);
		    #else
	...
	if(!player->mo->momz)
	{
		...
		for (node = player->mo->touching_sectorlist; node; node = node->m_snext)
		{
			...
			#else
			&& (player->mo->state == &states[S_PLAY_STND]))
			#endif
			{
	    ...
	    #ifdef RP
		if(teeter)
			P_SetMobjState(player->mo, S_PLAY_EDGE1);
		else if(checkedforteeter == true && player->astate == AS_EDGE)
			P_SetMobjState(player->mo, S_PLAY_STND1);
		#else
		if(teeter)
...
#define EXPLOSION player->powers[pw_explosionring]
	...
	if((player->mfspinning) || (player->mfjumped)
	#ifndef RP
	|| (player->powers[pw_tailsfly]) || (player->gliding) || (player->charability == 1 && (player->mo->state == &states[S_PLAY_SPC1] || player->mo->state == &states[S_PLAY_SPC2] || player->mo->state == &states[S_PLAY_SPC3] || player->mo->state == &states[S_PLAY_SPC4]))
	#endif
    )
	...
	if(player->carried && !(cmd->buttons & BT_JUMP))
	{
		...
		#ifdef RP
		P_SetMobjState(player->mo, S_PLAY_HANG1);
		#else
			#ifdef MARIO
		P_SetMobjState(player->mo, S_PLAY_ATK1);
			#else
	...
	if(mapheaderinfo[gamemap-1].typeoflevel & TOL_ADVENTURE)
	{
		...
		{
			if(!(player->mfjumped || player->mfspinning || player->gliding || player->climbing
				#ifndef RP
	...
	#ifdef MARIO
	if(player->mfjumped && player->powers[pw_blueshield])
	#else
	if(player->mfspinning
	#ifndef RP
	...
	#endif
	{
		...
		for (node = player->mo->touching_sectorlist; node; node = node->m_snext)
		{
			...
			if(node->m_sector->ffloors)
			{
				...
				{
				    #ifdef MARIO
				    if(player->mo->z + player->mo->height < rover->master->frontsector->ceilingheight)
				        continue;
				    else if(player->mo->z >= rover->master->frontsector->floorheight)
				        continue;
				    #endif
...
void P_FindEmerald (player_t* player)
{
    ...
    for (th = thinkercap.next ; th != &thinkercap ; th=th->next)
    {
        ...
        #ifdef RP
        else if (!hunt2 && mo2->type == MT_HUNT2)
        #else
        ...
        #ifdef RP
        else if (!hunt3 && mo2->type == MT_HUNT3)
        #else
...
void P_DeathThink (player_t* player)
{
	...
	if(player->mo->z <= player->mo->subsector->sector->floorheight)
	{
		...
		if(player->astate != AS_PAIN)
			P_SetMobjState(player->mo, S_PLAY_PAIN1);
	}
	#endif
...
/*    P_PathTraverse ( mo->x, mo->y, x, y, PT_ADDLINES, PTR_UseTraverse );*/
    ...
    //#endif
    (thiscam == &camera ? cv_cam_speed.value : cv_cam2_speed.value) != 0 && (abs(thiscam->x - mo->x) > (thiscam == &camera ? cv_cam_dist.value : cv_cam2_dist.value)*3 || 
...
// added : now angle turn is a absolute value not relative
	...
	// Use the custom number of frames of animation using the astate.
    if(player->aframes[player->astate] == 0 && player->mo->state != &states[S_PLAY_STND1])
        P_SetMobjState(player->mo, S_PLAY_STND1);
    else
    {
        int firstframe = S_PLAY_STND1+(player->astate*MAXPLAYFRAMES);
        int maxframe = player->aframes[player->astate];

        if(player->mo->state->nextstate >= firstframe+maxframe)
            player->mo->state->nextstate = states[firstframe+MAXPLAYFRAMES-1].nextstate;
    }
    #endif

    #ifdef DARKWORLD
    DarkWorldShadows(player->mo);
    #endif
