...
#ifdef RP
consvar_t cv_playername           = {"name"                ,"stranger"       ,CV_CALL | CV_NOINIT,NULL,SendNameAndColor};
consvar_t cv_playercolor          = {"color"               ,"10"        ,CV_CALL | CV_NOINIT,Color_cons_t,SendNameAndColor};
consvar_t cv_skin                 = {"skin"                ,DEFAULTSKIN,CV_CALL | CV_NOINIT,NULL /*skin_cons_t*/,SendNameAndColor};
consvar_t cv_playername2          = {"name2"               ,"traveler"    ,CV_CALL | CV_NOINIT,NULL,SendNameAndColor2};
consvar_t cv_playercolor2         = {"color2"              ,"10"        ,CV_CALL | CV_NOINIT,Color_cons_t,SendNameAndColor2};
consvar_t cv_skin2                = {"skin2"               ,DEFAULTSKIN,CV_CALL | CV_NOINIT,NULL /*skin_cons_t*/,SendNameAndColor2};
#else
...
#if defined(SDL) || defined(LJOYSTICK)
extern void I_JoyScale(void);
...
// Tails 08-13-2002
#ifdef RP
consvar_t cv_playerspeed    = {"playerspeed"    ,"1"     ,CV_NETVAR|CV_FLOAT|CV_NOSHOWHELP, 0};
#else
...
#ifdef RP
consvar_t cv_objectplace = {"objectplace","0",CV_NETVAR,CV_OnOff}; // Tails 06-17-2001
#else
...
*/
    ...
    {
        #ifdef RP
        CONS_Printf ("map <mapnumber>: change the map\n");
        #else
...
/*
    ...
    #ifdef RP
	if(!netgame && !multiplayer && !cv_splitscreen.value)
	{
		CONS_Printf("You must be in a netgame to change the map!\n");
		return;
	}
    #else
...
void Command_Pause(void)
{
	...
	if(cv_pause.value != 1 && (server || admin))
		...
		CONS_Printf("Only the server or an admin can pause the game.\n");
}
...
void Command_ServerTeamChange_f(void)
{
	...
	if(!(server || admin))
	{
		CONS_Printf("You're not the server or an admin. You can't change players' teams.\n");
		return;
	}
	...
	if(buf == players[(int)playernum].ctfteam)
	{
		CONS_Printf("The player is already on that team! %d\n %d\n", players[(int)playernum].ctfteam, buf);
		return;
	}
...
void Command_Changepassword_f(void)
{
	if(!(server || admin))
	{
		CONS_Printf("You're not the server or an admin. You can't change this.\n");
		return;
	}
...
//		return; // You don't need this anyway
	...
	{
		CONS_Printf("%s is now a remote admin.\n",player_names[num]);
		return;
	}
...
void Command_ExitLevel_f(void)
{
    ...
    if(!(server || admin))
    {
        CONS_Printf("Only the server or an admin can exit the level\n");
        return;
    }
...
void Command_Load_f(void)
{
    ...
    if(!(server || admin))
    {
        CONS_Printf("Only server or an admin can do a load game\n");
        return;
    }
...
void Command_Save_f(void)
{
    ...
    if(!(server || admin))
    {
        CONS_Printf("Only server or an admin can do a save game\n");
        return;
    }
