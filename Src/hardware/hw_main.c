...
*/
    ...
    if (gr_backsector)
    {
        ...
        if ((gr_frontsector->ceilingpic == skyflatnum
            #ifdef MULTISKY
            || gr_frontsector->ceilingpic == sky2flatnum
            || gr_frontsector->ceilingpic == sky3flatnum
            || gr_frontsector->ceilingpic == sky4flatnum
            || gr_frontsector->ceilingpic == sky5flatnum
            || gr_frontsector->ceilingpic == sky6flatnum
            || gr_frontsector->ceilingpic == sky7flatnum
            || gr_frontsector->ceilingpic == sky8flatnum
            || gr_frontsector->ceilingpic == sky9flatnum
            #endif
            ) && (gr_backsector->ceilingpic  == skyflatnum
            #ifdef MULTISKY
            || gr_backsector->ceilingpic == sky2flatnum
            || gr_backsector->ceilingpic == sky3flatnum
            || gr_backsector->ceilingpic == sky4flatnum
            || gr_backsector->ceilingpic == sky5flatnum
            || gr_backsector->ceilingpic == sky6flatnum
            || gr_backsector->ceilingpic == sky7flatnum
            || gr_backsector->ceilingpic == sky8flatnum
            || gr_backsector->ceilingpic == sky9flatnum
            #endif
            ))
        {
            worldtop = worldhigh;
...
// ----- end special tricks -----
    ...
    if (locFloorHeight < dup_viewz) {
        ...
        #ifdef MULTISKY
        && gr_frontsector->floorpic != sky2flatnum
        && gr_frontsector->floorpic != sky3flatnum
        && gr_frontsector->floorpic != sky4flatnum
        && gr_frontsector->floorpic != sky5flatnum
        && gr_frontsector->floorpic != sky6flatnum
        && gr_frontsector->floorpic != sky7flatnum
        && gr_frontsector->floorpic != sky8flatnum
        && gr_frontsector->floorpic != sky9flatnum
        #endif
        ) {
...
#define WRAPANGLE (ANGLE_MAX/4)
    ...
    #ifdef MULTISKY
    if(player->mo->subsector->sector->ceilingpic == sky2flatnum
        || player->mo->subsector->sector->floorpic == sky2flatnum)
    {
        HWR_GetTexture( sky2texture );
        f=40+(textures[sky2texture]->height/2)*FIXED_TO_FLOAT(finetangent[(2048-((int)aimingangle>>(ANGLETOFINESHIFT+1))) & FINEMASK]);
        if(f<0) f=0;
        if(f>240-127) f=240-127;

        v[3].tow = v[2].tow = f/(textures[sky2texture]->height/2);
        v[0].tow = v[1].tow = (f+127)/(textures[sky2texture]->height/2);
    }
    else if(player->mo->subsector->sector->ceilingpic == sky3flatnum
        || player->mo->subsector->sector->floorpic == sky3flatnum)
    {
        HWR_GetTexture( sky3texture );
        f=40+(textures[sky3texture]->height/2)*FIXED_TO_FLOAT(finetangent[(2048-((int)aimingangle>>(ANGLETOFINESHIFT+1))) & FINEMASK]);
        if(f<0) f=0;
        if(f>240-127) f=240-127;

        v[3].tow = v[2].tow = f/(textures[sky3texture]->height/2);
        v[0].tow = v[1].tow = (f+127)/(textures[sky3texture]->height/2);
    }
    else if(player->mo->subsector->sector->ceilingpic == sky4flatnum
        || player->mo->subsector->sector->floorpic == sky4flatnum)
    {
        HWR_GetTexture( sky4texture );
        f=40+(textures[sky4texture]->height/2)*FIXED_TO_FLOAT(finetangent[(2048-((int)aimingangle>>(ANGLETOFINESHIFT+1))) & FINEMASK]);
        if(f<0) f=0;
        if(f>240-127) f=240-127;

        v[3].tow = v[2].tow = f/(textures[sky4texture]->height/2);
        v[0].tow = v[1].tow = (f+127)/(textures[sky4texture]->height/2);
    }
    else if(player->mo->subsector->sector->ceilingpic == sky5flatnum
        || player->mo->subsector->sector->floorpic == sky5flatnum)
    {
        HWR_GetTexture( sky5texture );
        f=40+(textures[sky5texture]->height/2)*FIXED_TO_FLOAT(finetangent[(2048-((int)aimingangle>>(ANGLETOFINESHIFT+1))) & FINEMASK]);
        if(f<0) f=0;
        if(f>240-127) f=240-127;

        v[3].tow = v[2].tow = f/(textures[sky5texture]->height/2);
        v[0].tow = v[1].tow = (f+127)/(textures[sky5texture]->height/2);
    }
    else if(player->mo->subsector->sector->ceilingpic == sky6flatnum
        || player->mo->subsector->sector->floorpic == sky6flatnum)
    {
        HWR_GetTexture( sky6texture );
        f=40+(textures[sky6texture]->height/2)*FIXED_TO_FLOAT(finetangent[(2048-((int)aimingangle>>(ANGLETOFINESHIFT+1))) & FINEMASK]);
        if(f<0) f=0;
        if(f>240-127) f=240-127;

        v[3].tow = v[2].tow = f/(textures[sky6texture]->height/2);
        v[0].tow = v[1].tow = (f+127)/(textures[sky6texture]->height/2);
    }
    else if(player->mo->subsector->sector->ceilingpic == sky7flatnum
        || player->mo->subsector->sector->floorpic == sky7flatnum)
    {
        HWR_GetTexture( sky7texture );
        f=40+(textures[sky7texture]->height/2)*FIXED_TO_FLOAT(finetangent[(2048-((int)aimingangle>>(ANGLETOFINESHIFT+1))) & FINEMASK]);
        if(f<0) f=0;
        if(f>240-127) f=240-127;

        v[3].tow = v[2].tow = f/(textures[sky7texture]->height/2);
        v[0].tow = v[1].tow = (f+127)/(textures[sky7texture]->height/2);
    }
    else if(player->mo->subsector->sector->ceilingpic == sky8flatnum
        || player->mo->subsector->sector->floorpic == sky8flatnum)
    {
        HWR_GetTexture( sky8texture );
        f=40+(textures[sky8texture]->height/2)*FIXED_TO_FLOAT(finetangent[(2048-((int)aimingangle>>(ANGLETOFINESHIFT+1))) & FINEMASK]);
        if(f<0) f=0;
        if(f>240-127) f=240-127;

        v[3].tow = v[2].tow = f/(textures[sky8texture]->height/2);
        v[0].tow = v[1].tow = (f+127)/(textures[sky8texture]->height/2);
    }
    else if(player->mo->subsector->sector->ceilingpic == sky9flatnum
        || player->mo->subsector->sector->floorpic == sky9flatnum)
    {
        HWR_GetTexture( sky9texture );
        f=40+(textures[sky9texture]->height/2)*FIXED_TO_FLOAT(finetangent[(2048-((int)aimingangle>>(ANGLETOFINESHIFT+1))) & FINEMASK]);
        if(f<0) f=0;
        if(f>240-127) f=240-127;

        v[3].tow = v[2].tow = f/(textures[sky9texture]->height/2);
        v[0].tow = v[1].tow = (f+127)/(textures[sky9texture]->height/2);
    }
    else
    {
        HWR_GetTexture( skytexture );
...
#ifdef MULTISKY
void HWR_DrawSky2Background( player_t* player )
{
    FOutVector      v[4];
    angle_t         angle;
    float f;
