...
*/
	...
	case MT_SPECIALSPIKEBALL:
		...
		#ifdef RP
		P_SetMobjState(toucher, S_PLAY_PAIN1);
		#else
...
// start bubble grab Tails 03-07-2000
	  ...
	  if(player->powers[pw_underwater] < underwatertics + 1)
        ...
        P_SpawnMobj (special->x,special->y,special->z, MT_POP);
           ...
           sound = sfx_gasp;
        #ifdef RP
		P_SetMobjState(player->mo, S_PLAY_GASP1);
        #else
...
*/
      ...
      default:
          #ifdef RP
          if(RP_TouchSpecialThing(special,toucher)) return;
          #endif
                              ...
                              mobj_t*       source )
{
	...
	else if(inflictor && (inflictor->flags & MF_PUSHABLE) && source && source->player)
	{
		...
		#endif
		{
			...
			#endif
			{
				if(target->player==&players[consoleplayer])
				{
					...
					#else
					S_ChangeMusic(mus_contsc, true); // Yousa dead now, Okieday? Tails 03-14-2000
