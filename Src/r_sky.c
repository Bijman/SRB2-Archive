...
//    }
    ...
    #ifdef MULTISKY
    count   = textures[sky2texture]->patchcount;
    patches = &textures[sky2texture]->patches[0];
    for (height=0,i=0;i<count;i++,patches++)
    {
        W_ReadLumpHeader (patches->patch, &wpatch, sizeof(patch_t));
        wpatch.height = SHORT(wpatch.height);
    ...
    sky2texturemid = (textures[sky2texture]->height/2)<<FRACBITS;

    count   = textures[sky3texture]->patchcount;
    patches = &textures[sky3texture]->patches[0];
    for (height=0,i=0;i<count;i++,patches++)
    {
        W_ReadLumpHeader (patches->patch, &wpatch, sizeof(patch_t));
        wpatch.height = SHORT(wpatch.height);
    ...
    sky3texturemid = (textures[sky3texture]->height/2)<<FRACBITS;

    count   = textures[sky4texture]->patchcount;
    patches = &textures[sky4texture]->patches[0];
    for (height=0,i=0;i<count;i++,patches++)
    {
        W_ReadLumpHeader (patches->patch, &wpatch, sizeof(patch_t));
        wpatch.height = SHORT(wpatch.height);
    ...
    sky4texturemid = (textures[sky4texture]->height/2)<<FRACBITS;

    count   = textures[sky5texture]->patchcount;
    patches = &textures[sky5texture]->patches[0];
    for (height=0,i=0;i<count;i++,patches++)
    {
        W_ReadLumpHeader (patches->patch, &wpatch, sizeof(patch_t));
        wpatch.height = SHORT(wpatch.height);
    ...
    sky5texturemid = (textures[sky5texture]->height/2)<<FRACBITS;

    count   = textures[sky6texture]->patchcount;
    patches = &textures[sky6texture]->patches[0];
    for (height=0,i=0;i<count;i++,patches++)
    {
        W_ReadLumpHeader (patches->patch, &wpatch, sizeof(patch_t));
        wpatch.height = SHORT(wpatch.height);
    ...
    sky6texturemid = (textures[sky6texture]->height/2)<<FRACBITS;

    count   = textures[sky7texture]->patchcount;
    patches = &textures[sky7texture]->patches[0];
    for (height=0,i=0;i<count;i++,patches++)
    {
        W_ReadLumpHeader (patches->patch, &wpatch, sizeof(patch_t));
        wpatch.height = SHORT(wpatch.height);
    ...
    sky7texturemid = (textures[sky7texture]->height/2)<<FRACBITS;

    count   = textures[sky8texture]->patchcount;
    patches = &textures[sky8texture]->patches[0];
    for (height=0,i=0;i<count;i++,patches++)
    {
        W_ReadLumpHeader (patches->patch, &wpatch, sizeof(patch_t));
        wpatch.height = SHORT(wpatch.height);
    ...
    sky8texturemid = (textures[sky8texture]->height/2)<<FRACBITS;

    count   = textures[sky9texture]->patchcount;
    patches = &textures[sky9texture]->patches[0];
    for (height=0,i=0;i<count;i++,patches++)
    {
        W_ReadLumpHeader (patches->patch, &wpatch, sizeof(patch_t));
        wpatch.height = SHORT(wpatch.height);
    ...
    sky9texturemid = (textures[sky9texture]->height/2)<<FRACBITS;
    #endif
