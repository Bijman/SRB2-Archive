...
void P_NetArchivePlayers (void)
{
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
		...
		#ifdef RP
		WRITEBYTE(save_p, players[i].mfjumped);
		WRITEBYTE(save_p, players[i].mfspinning);
		WRITEBYTE(save_p, players[i].mfstartdash);
		WRITESHORT(save_p, players[i].gliding); 
		WRITEBYTE(save_p, players[i].climbing); 
		#else
		...
		WRITEBYTE(save_p, players[i].blackow); 
		#ifndef RP
        ...
        for(j = 0; j < NUMELEMENTS; j++)
            WRITEBYTE(save_p, players[i].elements[j]);
        for(j = 0; j < NUMABILITYS; j++)
            WRITEBYTE(save_p, players[i].abilitys[j]);
        WRITESHORT(save_p, players[i].trigger);
        WRITEBYTE(save_p, players[i].intercom);
        WRITEBYTE(save_p, players[i].astate);
        for(j = 0; j < NUMASTATES; j++)
            WRITEBYTE(save_p, players[i].aframes[j]);
		#else
		if(flags & LASTAXIS)
...
void P_NetUnArchivePlayers (void)
{
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
		...
		#ifdef RP
		players[i].mfjumped = READBYTE(save_p);
		players[i].mfspinning = READBYTE(save_p);
		players[i].mfstartdash = READBYTE(save_p);
		#else
		...
		#ifdef RP
		players[i].gliding = READSHORT(save_p); // Are you gliding? Tails 11-15-2000
		players[i].climbing = READBYTE(save_p); // Climbing on the wall Tails 11-18-2000
		#else
		...
		players[i].blackow = READBYTE(save_p); // Tails 01-11-2001
		...
		#ifdef RP
        for(j = 0; j < NUMELEMENTS; j++)
            players[i].elements[j] = READBYTE(save_p);
        for(j = 0; j < NUMABILITYS; j++)
            players[i].abilitys[j] = READBYTE(save_p);
        players[i].trigger = READSHORT(save_p);
        players[i].intercom = READBYTE(save_p);
        players[i].astate = READBYTE(save_p);
        for(j = 0; j < NUMASTATES; j++)
            players[i].aframes[j] = READBYTE(save_p);
		#else
		if(flags & LASTAXIS)
...
*/
            ...
            #ifdef DARKWORLD
            if( mobj->darkshadow    )                                      diff |= MD_DARKSHADOW;
            #endif
            ...
            #ifdef DARKWORLD
            if( diff & MD_DARKSHADOW )   WRITEULONG(save_p, mobj->darkshadow->mobjnum  );
            #endif
    ...
    while (1)
    {
        ...
        switch (tclass)
        {
          case tc_mobj :
              ...
              #ifdef DARKWORLD
              if( diff & MD_DARKSHADOW) mobj->darkshadow  = (mobj_t *)READULONG(save_p);
              #endif
